...
from zalo_bot.error import PassportDecryptionError
...
def decrypt(secret, hash, data):
    ...
    if not CRYPTO_INSTALLED:
        raise RuntimeError(
            "To use Zalo Bot Passports, PTB must be installed via `pip install "
            '"python-zalo-bot[passport]"`.'
        )
    ...
    # If the newly calculated hash did not match the one Zalo Bot gave us
    if data_hash != hash:
        # Raise a error that is caught inside zalo_bot.PassportData and transformed into a warning
...
class EncryptedCredentials(ZaloObject):
    """Contains data required for decrypting and authenticating EncryptedPassportElement. See the
    Zalo Bot Passport Documentation for a complete description of the data decryption and
    ...
    Note:
        ...
        :attr:`zalo_bot.PassportData.decrypted_credentials`.

    Args:
        data (:class:`zalo_bot.Credentials` | :obj:`str`): Decrypted data with unique user's
    ...
    def decrypted_secret(self) -> bytes:
        ...
        Raises:
            zalo_bot.error.PassportDecryptionError: Decryption failed. Usually due to bad
                private/public key but can also suggest malformed/tampered data.
        """
        if self._decrypted_secret is None:
            if not CRYPTO_INSTALLED:
                raise RuntimeError(
                    "To use Zalo Passports, PTB must be installed via `pip install "
                    '"python-zalo-bot[passport]"`.'
                )
            # Try decrypting according to step 1
            # We make sure to base64 decode the secret first.
            # Zalo Bot says to use OAEP padding so we do that. The Mask Generation Function
            # is the default for OAEP, the algorithm is the default for PHP which is what
            # Zalo Bot's backend servers run.
            try:
    ...
    def decrypted_data(self) -> "Credentials":
        """
        :class:`zalo_bot.Credentials`: Lazily decrypt and return credentials data. This object
...
class Credentials(ZaloObject):
    """
    Attributes:
        secure_data (:class:`zalo_bot.SecureData`): Credentials for encrypted data
...
class SecureData(ZaloObject):
    ...
    Args:
        personal_details (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            personal details.
        passport (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted passport.
        internal_passport (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            internal passport.
        driver_license (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            driver license.
        identity_card (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted ID card
        address (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            residential address.
        utility_bill (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            utility bill.
        bank_statement (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            bank statement.
        rental_agreement (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            rental agreement.
        passport_registration (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
            registration from internal passport.
        temporary_registration (:class:`zalo_bot.SecureValue`, optional): Credentials for encrypted
...
class SecureValue(ZaloObject):
    ...
    Attributes:
        ...
        selfie (:class:`zalo_bot.FileCredentials`): Optional. Credentials for encrypted selfie
            of the user with a document. Can be available for "passport", "driver_license",
            "identity_card" and "internal_passport".
        translation (Tuple[:class:`zalo_bot.FileCredentials`]): Optional. Credentials for an
            encrypted translation of the document. Available for "passport", "driver_license",
            "identity_card", "internal_passport", "utility_bill", "bank_statement",
            "rental_agreement", "passport_registration" and "temporary_registration".
        ...
        files (Tuple[:class:`zalo_bot.FileCredentials`]): Optional. Credentials for encrypted
            files. Available for "utility_bill", "bank_statement", "rental_agreement",
            "passport_registration" and "temporary_registration" types.
...
class _CredentialsBase(ZaloObject):
