# 📚 rubiapi Bot Python Library Documentation

## 🧠 Introduction
`rubiapi` is a Python library to interact with the [Rubika Bot API](https://rubika.ir/). This library helps you create Telegram-like bots with support for messages, inline buttons, chat keypads, and callback handling.
...
```python
from rubiapi import Robot
from rubiapi.context import Message
...
```python
from rubiapi.keypad import ChatKeypadBuilder
...
```python
from rubiapi.button import InlineBuilder
...
# 📘 rubiapi Bot Method Reference

مستندات مربوط به متدهای اصلی کلاس `Robot` در کتابخانه rubiapi.
...
# 📦 rubiapi `Message` Class & Media Reply API Documentation

## 🧾 معرفی کلاس `Message`

کلاس `Message` در کتابخانه rubiapi ابزاری کلیدی برای مدیریت پیام‌های دریافتی در ربات است. این کلاس، قابلیت‌هایی همچون پاسخ به پیام، ارسال مدیا، حذف یا ویرایش پیام، و استفاده از صفحه‌کلید و دکمه‌های اینلاین را فراهم می‌کند.
...
این پروژه یک ربات بر پایه کتابخانه‌ی `rubiapi` است که به کاربر امکان می‌دهد با استفاده از کی‌پد، یک تایمر تنظیم کرده و پس از پایان تایمر، پیامی برای او ارسال شود. تمرکز اصلی این مستند، بر روی کلاس `Job` است که برای زمان‌بندی اجرای دستورات استفاده شده است.

## ساختار کلی پروژه
- استفاده از کتابخانه `rubiapi` برای ارتباط با Rubika Bot API
...
## کلاس `Job` چیست؟
کلاس `Job` در فایل `rubiapi.jobs` تعریف شده و هدف آن اجرای یک تابع خاص پس از گذشت یک بازه زمانی مشخص است.

### نحوه استفاده:
```python
from rubiapi.jobs import Job
...
bot.run()
```
# ✅ Force Join (اجبار به عضویت در کانال) — rubiapi Bot

این مستند نحوه استفاده از قابلیت **اجبار به عضویت در یک کانال (Force Join)** در ربات‌های ساخته‌شده با کتابخانه rubiapi را توضیح می‌دهد.
...
- نصب و راه‌اندازی کتابخانه `rubiapi`
...
def handle_force_join(bot: Robot, message: Message):
    ...
    if bot.check_join(CHANNEL_GUID, message.chat_id):
        ...
        join_link = "https://rubika.ir/rubiapi_library"
        message.reply(
