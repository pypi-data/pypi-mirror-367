...
class Sessions:
    ...
    def cheackSessionExists(self):
        return exists(f"{self.client.session}.rubiapi")
    
    def loadSessionData(self):
        return loads(open(f"{self.client.session}.rubiapi", encoding="UTF-8").read())
    ...
    def createSession(self):
        ...
        while True:
            ...
            open(f"{self.client.session}.rubiapi", "w", encoding="UTF-8").write(dumps(sessionData, indent=4))
            ...
            Methods(
                ...
                timeOut=self.client.timeOut,
                showProgressBar=True
            ).registerDevice(deviceModel=f"rubiapi-Api-{self.client.session}")
            print(f"\nSign successful")
