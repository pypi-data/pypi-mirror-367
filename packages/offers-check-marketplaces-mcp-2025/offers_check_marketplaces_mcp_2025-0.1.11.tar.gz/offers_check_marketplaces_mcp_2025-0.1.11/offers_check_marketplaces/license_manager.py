...
class LicenseManager:
    ...
    def _get_cache_file_path(self) -> Path:
        """
        Определяет путь к файлу кеша лицензий.
        Использует системную временную директорию или скрытую папку в домашней директории.
        
        Returns:
            Путь к файлу кеша лицензий
        """
        import tempfile
        import sys
        ...
        try:
            ...
            cache_dir = temp_dir / "offers-check-marketplaces"
            cache_dir.mkdir(exist_ok=True)
            return cache_dir / ".license_cache.json"
        except (OSError, PermissionError):
            logger.debug("Не удалось использовать системную временную директорию для кеша")
        
        # Fallback: используем скрытую папку в домашней директории
        try:
            home_cache_dir = Path.home() / ".offers-check-cache"
            home_cache_dir.mkdir(exist_ok=True)
            return home_cache_dir / ".license_cache.json"
        except (OSError, PermissionError):
            logger.debug("Не удалось использовать домашнюю директорию для кеша")
        
        # Последний fallback: текущая директория (скрытый файл)
        return Path(".license_cache.json")
    
    def _get_config_file_path(self) -> Path:
        """
        Определяет путь к файлу конфигурации лицензий.
        Использует тот же подход, что и для кеша.
        
        Returns:
            Путь к файлу конфигурации лицензий
        """
        import tempfile
        ...
        try:
            ...
            cache_dir = temp_dir / "offers-check-marketplaces"
            cache_dir.mkdir(exist_ok=True)
            return cache_dir / ".license_config.json"
        except (OSError, PermissionError):
            logger.debug("Не удалось использовать системную временную директорию для конфигурации")
        
        # Fallback: используем скрытую папку в домашней директории
        try:
            home_cache_dir = Path.home() / ".offers-check-cache"
            home_cache_dir.mkdir(exist_ok=True)
            return home_cache_dir / ".license_config.json"
        except (OSError, PermissionError):
            logger.debug("Не удалось использовать домашнюю директорию для конфигурации")
        
        # Последний fallback: текущая директория (скрытый файл)
        return Path(".license_config.json")
