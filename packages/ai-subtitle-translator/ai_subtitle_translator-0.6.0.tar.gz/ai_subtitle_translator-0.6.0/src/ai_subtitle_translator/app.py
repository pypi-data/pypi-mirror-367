...
async def translate_subtitle(
    ...
    provider: str = Form(
        default=lambda: os.getenv("DEFAULT_PROVIDER", "deepseek"),
        description="AI provider (openai, gemini, deepseek)",
    ),
    fallback_provider: str | None = Form(
        default=lambda: os.getenv("FALLBACK_PROVIDER"),
    ...
    batch_size: int = Form(default=60, description="Batch size for API calls"),
):
    """
