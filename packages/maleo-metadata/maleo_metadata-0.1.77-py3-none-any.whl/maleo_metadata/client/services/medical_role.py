...
from maleo_metadata.schemas.parameter.client.medical_role import ReadMultipleParameter
...
class MaleoMetadataMedicalRoleClientService(MaleoClientService):
    RESOURCE = Resource(
        identifier=ResourceIdentifier(
            key="medical_roles", name="MedicalRole", url_slug="medical-roles"
        ),
        details=None,
    )
    ...
    async def get_medical_roles(
    ...
    ) -> ReadMultipleResourceOperationResult[MedicalRoleDataSchema, None]:
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[MedicalRoleDataSchema, None](
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_medical_roles(
                authentication=authentication,
    ...
    async def get_medical_role(
        ...
        # Retrieve medical roles using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_medical_role(
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
                headers=headers,
