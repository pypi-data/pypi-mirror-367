...
class MaleoMetadataGenderClientService(MaleoClientService):
    RESOURCE = Resource(
        identifier=ResourceIdentifier(key="genders", name="Gender", url_slug="genders"),
        details=None,
    )
    ...
    def __init__(
        ...
        self._namespace = self._redis_namespaces.create(
            self._key, "gender", origin=self._CACHE_ORIGIN, layer=self._CACHE_LAYER
        )
    ...
    async def get_genders(
    ...
    ) -> ReadMultipleResourceOperationResult[GenderDataSchema, None]:
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[GenderDataSchema, None](
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_genders(
                authentication=authentication,
            ...
            if response.is_success:
                ...
                result = ReadMultipleResourceOperationResult[GenderDataSchema, None](
                    data=data,
    ...
    async def get_gender(
        ...
        # Retrieve genders using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_gender(
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
                headers=headers,
