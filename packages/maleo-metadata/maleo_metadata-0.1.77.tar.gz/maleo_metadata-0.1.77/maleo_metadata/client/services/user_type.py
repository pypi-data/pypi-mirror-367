...
class MaleoMetadataUserTypeClientService(MaleoClientService):
    RESOURCE = Resource(
        identifier=ResourceIdentifier(
            key="user_types", name="UserType", url_slug="user-types"
        ),
        details=None,
    )
    ...
    def __init__(
        ...
        self._namespace = self._redis_namespaces.create(
            self._key, "user_type", origin=self._CACHE_ORIGIN, layer=self._CACHE_LAYER
        )
    ...
    async def get_user_types(
    ...
    ) -> ReadMultipleResourceOperationResult[UserTypeDataSchema, None]:
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[UserTypeDataSchema, None](
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_user_types(
                authentication=authentication,
            ...
            if response.is_success:
                ...
                result = ReadMultipleResourceOperationResult[UserTypeDataSchema, None](
                    data=data,
    ...
    async def get_user_type(
        ...
        # Retrieve user types using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_user_type(
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
                headers=headers,
