...
class MaleoMetadataSystemRoleClientService(MaleoClientService):
    RESOURCE = Resource(
        identifier=ResourceIdentifier(
            key="system_roles", name="SystemRole", url_slug="system-roles"
        ),
        details=None,
    )
    ...
    def __init__(
        ...
        self._namespace = self._redis_namespaces.create(
            self._key, "system_role", origin=self._CACHE_ORIGIN, layer=self._CACHE_LAYER
        )
    ...
    async def get_system_roles(
    ...
    ) -> ReadMultipleResourceOperationResult[SystemRoleDataSchema, None]:
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[SystemRoleDataSchema, None](
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_system_roles(
                authentication=authentication,
    ...
    async def get_system_role(
        ...
        # Retrieve system roles using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_system_role(
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
                headers=headers,
