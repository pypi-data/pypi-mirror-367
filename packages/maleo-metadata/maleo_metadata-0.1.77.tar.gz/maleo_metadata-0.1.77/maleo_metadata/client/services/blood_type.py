...
class MaleoMetadataBloodTypeClientService(MaleoClientService):
    RESOURCE = Resource(
        identifier=ResourceIdentifier(
            key="blood_types", name="BloodType", url_slug="blood-types"
        ),
        details=None,
    )
    ...
    def __init__(
        ...
        self._namespace = self._redis_namespaces.create(
            self._key, "blood_type", origin=self._CACHE_ORIGIN, layer=self._CACHE_LAYER
        )
    ...
    async def get_blood_types(
    ...
    ) -> ReadMultipleResourceOperationResult[BloodTypeDataSchema, None]:
        ...
        result_str = await self._redis.get(key)
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[BloodTypeDataSchema, None](
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_blood_types(
                authentication=authentication,
            ...
            if response.is_success:
                ...
                result = ReadMultipleResourceOperationResult[BloodTypeDataSchema, None](
                    data=data,
    ...
    async def get_blood_type(
        ...
        # Retrieve blood types using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.get_blood_type(
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
                headers=headers,
