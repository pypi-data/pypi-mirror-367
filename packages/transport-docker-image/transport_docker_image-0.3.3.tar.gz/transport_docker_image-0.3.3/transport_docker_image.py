...
def main(args):
    ...
    if existing_layers:
        ...
        if manifest_content:
            ...
            for item in manifest_obj:
                ...
                for layer in layers:
                    ...
                    # empty layer, size = 0
                    if layer_hash in existing_layers and layer_hash != "sha256:5f70bf18a086007016e948b04aed3b82103a36bea41755b6cddfaf10ace3c6ef":
        ...
        if layers_to_remove:
            exec_command('cd %s && rm -rf %s' % (
                shlex.quote(os.path.join(tmp_dir, quoted_source_image_name)),
                ' '.join(['./' + shlex.quote(x) for x in layers_to_remove]),
