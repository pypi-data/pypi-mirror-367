...
class GradScaler:
    ...
    def __init__(self, 
        ...
        self._min_scale = 1.0 / (2**24)  # Very small minimum like PyTorch
    ...
    def unscale_(self, optimizer) -> None:
        ...
        for param_name, param in optimizer.params:
            if param.requires_grad and param.grad is not None:
                ...
                # PyTorch-style inf/nan detection
                ...
                # Gradient clipping like PyTorch (optional but recommended)
                # Clip gradients with norm > 10.0 to prevent future overflow
                grad_norm = float(xp.linalg.norm(param.grad).item() if hasattr(xp.linalg.norm(param.grad), 'item') else xp.linalg.norm(param.grad))
                if grad_norm > 10.0:
                    clip_coef = 10.0 / (grad_norm + 1e-6)
    ...
    def update(self) -> None:
        ...
        if self._found_inf_this_step:
            ...
            # Prevent scale from going below minimum (PyTorch uses very small minimum)
            ...
            if self._growth_tracker >= self._growth_interval:
                self._scale = min(self._scale * self._growth_factor, 2**24)  # Cap maximum scale
                self._growth_tracker = 0
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        self._min_scale = state_dict.get('min_scale', 1.0 / (2**24))