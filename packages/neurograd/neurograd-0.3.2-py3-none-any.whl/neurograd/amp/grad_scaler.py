...
class GradScaler:
    ...
    def __init__(self, 
        ...
        """
        self._device = device
        self._enabled = enabled and (device == "cuda")
    ...
    def unscale_(self, optimizer) -> None:
        ...
        # Check all parameters with gradients
        for param_name, param in optimizer.params:
            if param.requires_grad and param.grad is not None:
                # Debug: Print gradient info if there's an issue
                try:
                    # Ensure param.grad is an xp.ndarray
                    if not isinstance(param.grad, xp.ndarray):
                        # Convert to proper array type
                        param.grad = xp.asarray(param.grad)
                    
                    # Check for inf/nan only on float types
                    if hasattr(param.grad, 'dtype') and param.grad.dtype.kind in 'fc':
                        if not xp.isfinite(param.grad).all():
                            found_inf = True
                            break
                            
                except (AttributeError, TypeError, ValueError) as e:
                    # Debug info for troubleshooting
                    print(f"Warning: Could not check gradient for {param_name}")
                    print(f"  param.grad type: {type(param.grad)}")
                    print(f"  param.grad dtype: {getattr(param.grad, 'dtype', 'NO_DTYPE')}")
                    print(f"  Error: {e}")
                    # Assume it's safe if we can't check
                    pass
                
                # Unscale gradient in-place
                if self._scale > 0:
                    try:
                        param.grad = param.grad / self._scale
                    except Exception as e:
                        print(f"Warning: Could not unscale gradient for {param_name}: {e}")
                        pass
