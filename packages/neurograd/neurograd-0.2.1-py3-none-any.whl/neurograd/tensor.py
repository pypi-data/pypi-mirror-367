...
class Tensor:
    ...
    def cast(self, dtype):
        try: 
            # For mixed precision, ensure gradients maintain precision compatibility
            new_data = xp.asarray(self.data, dtype=dtype)
            
            # Create new tensor to avoid in-place modification issues with autograd
            new_tensor = Tensor(
                data=new_data,
            ...
            # Copy gradient if it exists, casting to match new dtype
            if self.grad is not None:
                new_tensor.grad = xp.asarray(self.grad, dtype=dtype)
