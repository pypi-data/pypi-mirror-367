...
class Function(ABC):
    ...
    def __call__(self, *inputs) -> Tensor:
        ...
        # Apply autocast if enabled - optimized for speed
        try:
            from neurograd.amp.autocast import is_autocast_enabled, autocast_tensor
            if is_autocast_enabled():
                op_name = getattr(self, 'name', None) or self.__class__.__name__
                if op_name != 'Cast':  # Avoid recursion with Cast operations
                    processed_inputs = [autocast_tensor(inp, op_name=op_name) for inp in processed_inputs]
        except ImportError:
