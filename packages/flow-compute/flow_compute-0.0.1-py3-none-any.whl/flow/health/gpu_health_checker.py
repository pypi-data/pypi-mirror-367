...
class GPUdStatusDiagnoser:
    ...
    def diagnose(self, api_url: str, task_age_hours: Optional[float]) -> GPUdDiagnosis:
        ...
        if self._is_legacy_task(task_age_hours):
            ...
            return GPUdDiagnosis(
                GPUdStatus.FAILED,
                "Unable to connect to GPUd service"
            )
    ...
    def check_gpud_marker(self, ssh_command_prefix: list[str]) -> Optional[str]:
        ...
        try:
            ...
            cmd = ssh_command_prefix + [f"cat {self.GPUD_STATUS_FILE} 2>/dev/null || echo 'not_found'"]
    ...
    def handle_diagnosis(self, diagnosis: GPUdDiagnosis, task_id: str) -> None:
        ...
        if diagnosis.status == GPUdStatus.LEGACY:
            ...
            self.warnings.append({
                ...
                "suggestion": "Use 'flow run' to start tasks with monitoring, or install GPUd manually"
            })
...
class SSHConnectionHandler:
    ...
    def analyze_ssh_error(error: Exception, task_age_hours: Optional[float]) -> GPUdDiagnosis:
        ...
        if "connection refused" in error_str and task_age_hours and task_age_hours > 24:
            ...
            return GPUdDiagnosis(
                GPUdStatus.UNREACHABLE,
                "Node is not reachable"
            )
        else:
            return GPUdDiagnosis(
                GPUdStatus.UNREACHABLE,
                f"SSH connection failed: {error_str}"
            )
