"""Allocation monitoring alternative view.

Provides real-time visualization of resources
across Flow-provisioned infrastructure.
"""
...
class BeautifulTaskRenderer:
    ...
    def _render_task_list_interactive(self, tasks: List[Task]) -> Panel:
        ...
        for idx, task in enumerate(tasks[:15]):  # Display limit for terminal height
            ...
            gpu_info = self._format_gpu_elegant(task.instance_type, getattr(task, 'num_instances', 1))
        ...
        help_text = Text(
            "\n↑↓/jk: navigate  Enter: details  q: quit  r: refresh",
            style="dim",
            justify="center"
        )
...
class AllocCommand(BaseCommand):
    ...
    @property
    def help(self) -> str:
        return "Monitor and manage GPU resource allocations"
    ...
    def get_command(self) -> click.Command:
        ...
        @click.option("--refresh-rate", default=2.0, type=float, help="Update interval in seconds (default: 2.0)")
        def alloc(watch: bool, gpus: Optional[int], gpu_type: Optional[str], refresh_rate: float):
            """Monitor GPU resource allocations.
            
            Pro tip: Use `flow status` for comprehensive task monitoring.
            
            \b
            Examples:
                flow alloc              # Display current allocations
                flow alloc --watch      # Continuous monitoring mode
