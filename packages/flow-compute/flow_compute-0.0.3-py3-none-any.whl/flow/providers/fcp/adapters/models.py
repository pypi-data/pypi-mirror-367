"""Adapter between FCP and domain models.

This module handles all the impedance mismatch between FCP's API
...
from ..core.models import FCPBid, FCPInstance, FCPVolume
...
class FCPAdapter:
    ...
    def bid_to_task(
        cls,
        bid: FCPBid,
        instances: Optional[List[FCPInstance]] = None,
        instance_type_name: Optional[str] = None,
    ) -> Task:
        """Convert FCP bid to domain Task model.
        ...
        Note:
            ...
            - FCP uses "bid" terminology, we use "task"
            - FCP has "Allocated" status, we use "RUNNING"
            - FCP uses dollar strings, we use floats
        """
    ...
    # Instance status mapping from FCP to domain
    INSTANCE_STATUS_MAP = {
    ...
    @classmethod
    def fcp_instance_to_instance(cls, fcp_instance: FCPInstance, bid: FCPBid) -> Instance:
        """Convert FCP instance to domain Instance model.

        Args:
            fcp_instance: FCP instance model
        ...
        # Determine SSH host
        ssh_host = fcp_instance.ssh_host or fcp_instance.public_ip

        # Map instance status
        instance_status = cls.INSTANCE_STATUS_MAP.get(
            fcp_instance.status.lower(), InstanceStatus.PENDING
        )

        return Instance(
            instance_id=fcp_instance.fid,
            task_id=bid.fid,
            status=instance_status,
            ssh_host=ssh_host,
            private_ip=fcp_instance.private_ip,
            created_at=fcp_instance.created_at,
            terminated_at=fcp_instance.terminated_at,
        )

    @classmethod
    def fcp_volume_to_volume(cls, fcp_volume: FCPVolume) -> Volume:
        """Convert FCP volume to domain Volume model.

        Args:
            fcp_volume: FCP volume model

        Returns:
            Domain Volume model
        """
        # Determine storage interface from FCP data
        interface = FCPStorageMapper.determine_interface(
            mount_path=fcp_volume.mount_path,
            # Note: FCP volume metadata handling - when additional metadata
            # we can pass additional fields here
        )

        return Volume(
            volume_id=fcp_volume.fid,
            name=fcp_volume.name,
            size_gb=fcp_volume.size_gb,
            region=fcp_volume.region,
            interface=interface,
            created_at=fcp_volume.created_at,
            attached_to=fcp_volume.attached_to,
        )

    @classmethod
    def _build_status_message(cls, bid: FCPBid, instances: Optional[List[FCPInstance]]) -> str:
    ...
    def _parse_price(cls, price_str: str) -> float:
        """Parse FCP price string to float.
