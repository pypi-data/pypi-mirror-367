...
Examples:
    ...
    $ flow dev -c 'pip install torch'  # Install directly on VM
    $ flow dev -c 'python train.py'    # Run with installed packages

    # Named environments (isolated containers):
    $ flow dev -e ml -c 'pip install tensorflow'  # Isolated env
    $ flow dev -e web -c 'npm install express'    # Another env
    $ flow dev -e ml -c 'python model.py'         # Has TF, not express
...
class DevVMManager:
    ...
    def create_dev_vm(self, instance_type: str = None, ssh_keys: list = None, max_price_per_hour: float = None, no_unique: bool = False) -> Task:
        """Create a new dev VM.
...
class DevContainerExecutor:
    ...
    def execute_command(self, command: str, image: str = None, interactive: bool = False, env_name: str = "default") -> int:
...
class DevCommand(BaseCommand):
    ...
    @property
    def help(self) -> str:
        return """Persistent dev VM - default runs directly, named envs use containers
        
        Example: flow dev my-env"""
    ...
    def get_command(self) -> click.Command:
        ...
        def dev(
            ...
            DEFAULT MODE (no containers, fast):
                flow dev                          # SSH to VM
                flow dev -c 'pip install numpy'   # Install on VM
                flow dev -c 'python train.py'     # Direct execution
            
            \b
            ISOLATED ENVIRONMENTS (containers):
                flow dev -e ml -c 'pip install tensorflow'
                flow dev -e web -c 'npm install express'
                flow dev -e ml -c 'python app.py'  # Has TF, not express
            ...
            if verbose:
                ...
                console.print("flow dev -c 'pip install numpy'    # Install on VM")
                console.print("flow dev -c 'python train.py'      # Uses numpy\n")
                
                console.print("# Named environments (containers):")
                console.print("flow dev -e ml -c 'pip install tensorflow'")
                console.print("flow dev -e web -c 'npm install express'")
                console.print("flow dev -e ml -c 'python app.py'  # Has TF, not express\n")
                
                console.print("# Management:")
                console.print("flow dev --status                  # Check VM & environments")
            ...
            self._execute(
                command, env, instance_type, image, ssh_keys, reset, stop, 
                status, force_new, max_price_per_hour, upload, upload_path, no_unique
            )
        ...
        try:
            ...
            if not vm:
                ...
                try:
                    ...
                    final_status = wait_for_task(flow_client, vm.task_id, watch=False, task_name=vm.name)
                ...
                if progress:
                    ...
                    time.sleep(0.5)
                    # Stop the progress animation before continuing
                    progress.__exit__(None, None, None)
                    progress = None
                ...
                try:
                    if env_name == "default":
                        ...
                        provider.upload_code_to_task(
                            ...
                            timeout=600,
                            console=console  # Pass our console to avoid duplicate output
                        )
                        console.print(f"[green]✓[/green] Code uploaded to /root")
                    else:
                        ...
                        console.print(f"[green]✓[/green] Code uploaded to environment '{env_name}'")
            ...
            if command:
                ...
                if env_name != "default":
                    console.print(
                        f"[dim]Connecting to environment '{env_name}'[/dim]"
                    )
                else:
                    console.print(
                ...
                # Create a new progress animation for the connection
                from ..utils.animated_progress import AnimatedEllipsisProgress
                from flow.api.ssh_utils import DEFAULT_PROVISION_MINUTES
                
                # Use progress bar with estimate if VM might need SSH wait
                progress = AnimatedEllipsisProgress(
                    console, 
                ...
                # TODO: Would be nice to start in the environment directory
            ...
            if not command or command in interactive_commands:
                if env_name == "default":
                    self.show_next_actions(
                        [
                            "Run command on VM: [cyan]flow dev -c 'python script.py'[/cyan]",
                            "Create isolated env: [cyan]flow dev -e project1 -c 'pip install deps'[/cyan]",
                    ...
                    self.show_next_actions(
                        [
                            f"Work in {env_name}: [cyan]flow dev -e {env_name} -c 'python app.py'[/cyan]",
                            "Switch to default: [cyan]flow dev -c 'python script.py'[/cyan]",
    ...
    def _wait_for_vm_ready(self, flow_client: Flow, vm: Task) -> Task:
        ...
        if age_message and "unexpected" in age_message:
            ...
            console.print(f"[dim]Ctrl+C to exit (provisioning continues) • Check: flow dev • Status: flow status {vm.name}[/dim]\n")
