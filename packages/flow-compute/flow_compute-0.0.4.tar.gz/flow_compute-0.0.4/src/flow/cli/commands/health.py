...
from rich.progress import Progress, SpinnerColumn, BarColumn, TextColumn, MofNCompleteColumn, TaskProgressColumn
...
class HealthChecker:
            ...
            try:
                ...
                try:
                    ...
                    if diagnosis:
                        ...
                        return self.snapshot_factory.create_unreachable_snapshot(task, "SSH tunnel creation failed")
    ...
    def _check_gpud_via_ssh(self, task: Any, task_age_hours: float | None, marker_status: str | None) -> Any:
        ...
        try:
            ...
            check_cmd = ssh_cmd + ["curl", "-s", "-f", "-m", "2", "http://localhost:15132/health"]
            ...
            if result.returncode == 0:
                ...
                ps_result = subprocess.run(ssh_cmd + ["sh", "-c", "ps aux | grep gpud | grep -v grep"], 
                ...
                if ps_result.returncode == 0 and "gpud" in ps_result.stdout:
                    ...
                    if marker_status in ["install_failed", "start_failed"]:
                        ...
                        return GPUdDiagnosis(
                            ...
                            "GPUd not installed or not running"
                        )
            ...
            return GPUdDiagnosis(
                GPUdStatus.FAILED,
                f"Failed to check GPUd via SSH: {str(e)}"
            )
    ...
    def _create_live_display_table(self, tasks: list, snapshots: list) -> Table:
        ...
        table.add_column("Node", style="cyan", no_wrap=True, width=18)
        table.add_column("Monitor", justify="center", width=10)
        table.add_column("GPUs", justify="center", width=8)
        table.add_column("Temp", justify="center", width=8)
        table.add_column("Usage", justify="center", width=8)
    ...
    def _add_live_table_row(self, table: Table, snapshot: NodeHealthSnapshot) -> None:
        ...
        if snapshot.gpu_metrics:
            avg_temp = sum(g.temperature_c for g in snapshot.gpu_metrics) / len(snapshot.gpu_metrics)
        ...
        # Utilization
        if snapshot.gpu_metrics:
            avg_usage = sum(g.gpu_utilization_pct for g in snapshot.gpu_metrics) / len(snapshot.gpu_metrics)
        ...
        # Memory
        if snapshot.gpu_metrics:
            avg_mem = sum(g.memory_utilization_pct for g in snapshot.gpu_metrics) / len(snapshot.gpu_metrics)
        ...
        if not snapshot.gpud_healthy:
            if "legacy" in str(snapshot.machine_info.get("note", "")).lower():
                status = "[dim]Pre-monitoring era[/dim]"
            elif "not installed" in str(snapshot.machine_info.get("note", "")).lower():
                status = "[yellow]Install GPUd →[/yellow]"
            else:
    ...
    def _create_fleet_summary_panel(self, snapshots: list, total_tasks: int, current_node: str = None, animation_frame: int = 0) -> Panel:
        ...
        if current_node and len(snapshots) < total_tasks:
            ...
            content_lines.append(f"[yellow]{spinner}[/yellow] Analyzing node: [{pulse_color}]{current_node}[/{pulse_color}]{dots}")
        ...
        if snapshots:
            ...
            if with_monitoring:
                ...
                degraded = sum(1 for s in with_monitoring if s.health_status == HealthStatus.DEGRADED)
                critical = sum(1 for s in with_monitoring if s.health_status == HealthStatus.CRITICAL)
            ...
            if without_monitoring:
                content_lines.append("")
                content_lines.append(f"[bold yellow]Without Monitoring:[/bold yellow] {len(without_monitoring)} nodes")
    ...
    def check_fleet_gpu_health(self, show_all: bool = False, json_mode: bool = False) -> FleetHealthSummary:
        ...
        if json_mode:
            ...
            with AnimatedEllipsisProgress(
                ...
                time.sleep(0.5)  # Give time to see the shimmer effect
            ...
            with Live(console=console, refresh_per_second=20, vertical_overflow="visible") as live:
                while not check_complete.is_set() or animation_frame < 10:
                    ...
                    layout = Layout()
                    layout.split_column(
                        Layout(self._create_fleet_summary_panel(
                ...
                layout = Layout()
                layout.split_column(
                    Layout(self._create_fleet_summary_panel(snapshots, len(tasks), None, 0), size=13),
                    Layout(self._create_live_display_table(tasks, snapshots))
                )
                live.update(layout)
...
• Current tasks: SSH and run:
  [cyan]curl -fsSL https://pkg.gpud.dev/install.sh | bash[/cyan]
"""
                info_panel = Panel(
                    info_content,
