...
class TaskFormatter:
    ...
    def format_status_with_color(status: str) -> str:
        ...
        # Create unified status display with symbol and text as single unit
        # Pad to length of longest status ("preempting" = 10) for consistent alignment
        padded_status = status.ljust(10)
        return f"[{config['color']}]{config['symbol']} {padded_status}[/{config['color']}]"
