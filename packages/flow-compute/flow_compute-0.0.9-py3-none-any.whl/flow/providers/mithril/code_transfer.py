...
class CodeTransferManager:
    ...
    def _wait_for_ssh(self, task: Task, config: CodeTransferConfig) -> SSHConnectionInfo:
        ...
        if task.ssh_host:
            logger.info(f"Verifying SSH connection for task {task.task_id}")
            
            # Build connection info directly without waiting
            # The ssh_waiter will still test the connection but won't show "waiting" messages
            try:
                # Don't use progress reporter or callbacks for existing SSH
                # This will do a quick connectivity test without the "waiting" animation
                connection = self.ssh_waiter.wait_for_ssh(
                    task, timeout=30, progress_callback=None  # Short timeout, no progress
                )
                logger.info("SSH connection verified")
                return connection
            except Exception as e:
                # If verification fails, fall back to normal wait behavior
                logger.warning(f"SSH verification failed, retrying: {e}")
