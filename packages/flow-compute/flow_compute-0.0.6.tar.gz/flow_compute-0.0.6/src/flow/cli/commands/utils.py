...
def wait_for_task(
    flow_client: Flow, task_id: str, watch: bool = False, json_output: bool = False, task_name: Optional[str] = None, show_submission_message: bool = True
) -> str:
    ...
    if watch:
        ...
        with AnimatedEllipsisProgress(
            console, "Waiting for instance allocation", transient=True, show_progress_bar=True, estimated_seconds=120
        ) as progress:
            while True:
                status = flow_client.status(task_id)
