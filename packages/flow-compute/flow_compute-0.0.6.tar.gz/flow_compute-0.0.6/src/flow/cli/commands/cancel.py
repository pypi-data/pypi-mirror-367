...
class CancelCommand(BaseCommand, TaskOperationCommand):
    ...
    @property
    def help(self) -> str:
        return "Cancel GPU tasks - pattern matching uses regex by default: flow cancel -n 'dev-'"
    ...
    def get_command(self) -> click.Command:
        ...
        @click.option("--wildcard", "-wc", is_flag=True, help="Treat pattern as wildcard instead of regex")
        @click.option("--verbose", "-v", is_flag=True, help="Show detailed examples and patterns")
        def cancel(
            task_identifier: str | None, yes: bool, all: bool, name_pattern: str | None, wildcard: bool, verbose: bool
        ):
            """Cancel a running task.
            ...
            if verbose:
                ...
                console.print("  flow cancel -n 'dev-'            # Matches any task containing 'dev-'")
                console.print("  flow cancel -n '^dev-'           # Matches tasks starting with 'dev-'")
                console.print("  flow cancel -n 'dev-$'           # Matches tasks ending with 'dev-'")
                console.print("  flow cancel -n '.*-v[0-9]+'      # Version pattern (e.g., app-v1, test-v23)")
                ...
                console.print("  flow cancel -n 'dev-*' --wildcard       # Cancel all starting with dev-")
                console.print("  flow cancel -n '*-gpu-8x*' --wildcard   # Match GPU type")
                console.print("  flow cancel -n 'train-v?' --wildcard    # Single character wildcard\n")
                
                console.print("[yellow]Note: When using wildcards with --wildcard flag, quote them to prevent shell expansion:[/yellow]")
                console.print("  [green]✓ CORRECT:[/green]  flow cancel -n 'gpu-test-*' --wildcard")
                console.print("  [red]✗ WRONG:[/red]    flow cancel -n gpu-test-* --wildcard   # Shell expands *\n")
    ...
    def _show_cancel_confirmation(self, task: Task) -> None:
        ...
        gpu_display = GPUFormatter.format_ultra_compact(task.instance_type, getattr(task, 'num_instances', 1))
