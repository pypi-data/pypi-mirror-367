...
def get_biomass_dict(logger, universe, dbexp):
    ...
    biomass_dict = {
        ...
        # Cofactors:
        # Note: universal ('cofactors_uni') and conditional ('cofactors_con') cofactors have been defined in Xavier2017 .
...
def get_precursors_to_pathway():
    ...
    precursors_to_pathway = {  # listing alternative biosynthetic pathways
        ...
        'labeps_c': ['gr_epsLAB'],  # EPS of lactic acid bacteria
...
def introduce_universal_biomass(logger, dbexp, universe): 
    ...
    rstring = rstring + f' + {get_uni_biom_coeff()} {(" + " + str(get_uni_biom_coeff()) + " ").join(biomass_dict["energy_stock"])}'
    
    
    # Cofactors:
    rstring = rstring + f' + {get_uni_biom_coeff()} {(" + " + str(get_uni_biom_coeff()) + " ").join(biomass_dict["cofactors_uni"])}'
    rstring = rstring + f' + {get_uni_biom_coeff()} {(" + " + str(get_uni_biom_coeff()) + " ").join(biomass_dict["cofactors_con"])}'
    rstring = rstring + f' + {get_uni_biom_coeff()} {(" + " + str(get_uni_biom_coeff()) + " ").join(biomass_dict["cofactors_add"])}'
