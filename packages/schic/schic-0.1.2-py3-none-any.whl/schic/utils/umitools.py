...
def whitelist_run(bc_pattern, stdin, set_cell_number, stdout, tool, tool_command, parms):
    """
    extract UMI with umitools.
    Args:
        bc-pattern: Barcode pattern for paired reads.
    ...
    params_list = parms.split()
    command = [
        tool, tool_command, f"--stdin={stdin}", f"--bc-pattern={bc_pattern}", *params_list, f"--set-cell-number={set_cell_number}",
        f"--stdout={stdout}"
        ]
    try:
        print(f"Running command: {' '.join(command)}")
