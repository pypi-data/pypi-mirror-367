...
def whitelist(
    ...
    set_cell_number: Annotated[str, typer.Option("--set-cell-number", "-S", help="setting cell number.")],
    ...
    parms: Annotated[str, typer.Option("--parms", help="umi_tools parameters for whitelist.")]="--method=reads --ed-above-threshold=correct --extract-method=regex --plot-prefix=whitelist --knee-method=density --random-seed=123",
    ):
    """
    ...
    with Progress(
        ...
        try:
            ...
            start=time.time()
            whitelist_run(bc_pattern, stdin, set_cell_number, stdout, tool, tool_command, parms)
            end=time.time()
