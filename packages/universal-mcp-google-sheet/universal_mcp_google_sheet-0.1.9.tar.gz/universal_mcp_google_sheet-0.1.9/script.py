...
integration = AgentRIntegration(name="google-sheet", client=AgentrClient(api_key="sk_416e4f88-3beb-4a79-a0ef-fb1d2c095aee", base_url="https://api.agentr.dev"))
app_instance = GoogleSheetApp(integration=integration)
...
tool_manager.add_tool(app_instance.create_spreadsheet)
...
async def main():
    ...
    tool=tool_manager.get_tool("create_spreadsheet")
    ...
    result = await tool_manager.call_tool(name="create_spreadsheet", arguments={"title": "test_spreadsheet"})
    print(result)
    print(type(result))
