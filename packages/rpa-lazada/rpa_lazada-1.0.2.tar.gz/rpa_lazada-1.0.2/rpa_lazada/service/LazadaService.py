...
class LazadaService():
    ...
    def account_login(self, driver, data, options):
        ...
        try:
            ...
            if is_local in [1, '1']:
                # 本土登录URL
                driver.get(f"https://sellercenter.lazada{self.site_map(options['site'])}/apps/seller/login")
        except Exception as e:
            # 显式等待目标内容
            # 可以预防 HTTPConnectionPool(host='localhost', port=56652): Read timed out. (read timeout=120)
        ...
        # 保存店铺缓存
        self.save_storage(driver,data, shop_global_id)

        return common.back(1, '登录成功')

    def save_storage(self, driver,data, shop_global_id):
        '''
        @Desc    : 保存店铺缓存
        @Author  : 洪润涛
        ...
        try:
            ...
            if is_local in [1, '1']:
                driver.get(f'https://acs-m.lazada{self.site_map(data["site"])}/h5/mtop.global.merchant.subaccount.otp.userinfo/1.0/')
        except Exception as e:
            # 显式等待目标内容
            WebDriverWait(driver, 15).until(
                EC.visibility_of_element_located((By.TAG_NAME, "body"))
