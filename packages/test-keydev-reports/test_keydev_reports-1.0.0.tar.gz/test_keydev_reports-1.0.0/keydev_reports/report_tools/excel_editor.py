from typing import Iterable, NoReturn, Dict
...
class ExcelEditor(BaseEditor):
    ...
    def replace_cell_value(self, search_text: str, replace_text: str) -> NoReturn:
        """
    ...
    def replace_data(self, data: Dict[str, str]) -> NoReturn:
        """
    ...
    def fill_data_from_startpoint_merged(self, data: dict):
        """
    ...
    def copy_cell_style(self, sheet: Worksheet, source_cell: Cell, target_cell: Cell) -> Cell | MergedCell:
        ...
        # Если ячейка объединенная, то копируем объединение в следующую ячейку
            ...
            sheet.merge_cells(start_row=target_cell.row,
    ...
    def fill_data_to_template(self, data: dict, max_col: int = 0) -> NoReturn:
        """
            ...
            for search_symbol, values in data.items():
                ...
                if target_cell:
                    ...
                    for row_index, _ in enumerate(values, start=target_cell.row):
                        self.copy_row(sheet, target_cell.row, target_cell.row, target_cell.column, max_column,
                                      row_index)
                    # Заполняем данные
                    self.populate_data_in_merge_range(sheet, values, target_cell.row, target_cell.column)

    def populate_data_in_merge_range(self, sheet, data: list, start_row: int, start_col: int) -> NoReturn:
        """
