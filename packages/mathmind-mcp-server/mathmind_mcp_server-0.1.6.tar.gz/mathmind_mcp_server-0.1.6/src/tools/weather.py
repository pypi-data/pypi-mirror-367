...
def get_city_info(
    ...
    """
    base_url = 'https://kk36x76f2x.re.qweatherapi.com/geo/v2/city/lookup'
    params = {
        'key': '40c90f2bda144a73bfd6d018b71fc757',
        'location': location,
    }
    if adm:
        params['adm'] = adm
    ...
    if response.status_code != 200:
        raise Exception(f"Failed to get city info: {response.status_code} {response.text}")
    resp_data = response.json()
    # 根据状态码处理不同情况
    if resp_data.get('code') != '200':
        raise Exception(f"Failed to get city info: {resp_data.get('code')}")
    data = resp_data.get('location')
    return data


def get_24h_weather(location_id: str, place: str):
    """
    获取指定城市24小时天气预报
    ...
    Returns:
        str: 格式化后的天气预报信息,包括:
            ...
            - 降水概率: 百分比
    """
    url = "https://kk36x76f2x.re.qweatherapi.com/v7/weather/24h"
    params = {
        'key': '40c90f2bda144a73bfd6d018b71fc757',
...
def format_weather_data(data, place):
    ...
    formatted_data = f"# 这是查询到的关于{place}未来24小时的天气信息: \n\n"
...
def get_24h_weather_by_location(location: str, api_key: str) -> str:
    """
    获取指定城市24小时天气预报
    ...
    # TODO 验证 api_key 是否有效

    locations = get_city_info(location)
    if not locations:
        raise Exception(f"未找到城市: {location}")
    return get_24h_weather(locations[0]['id'], locations[0]['name']) + f"\n\n{SUFFIX}"


if __name__ == '__main__':
    location = "北京丰台区"
    print(get_24h_weather_by_location(location))
