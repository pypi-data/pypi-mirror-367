...
def inspect_command(args):
    ...
    try:
        ...
        FileNotFoundError,
        ValueError,
    ) as e:  # Added ValueError for invalid section
    ...
    try:
        ...
        if args.export_type == "cover":
            ...
            if cover_art.startswith(b"\x89PNG"):
                extension = ".png"
            default_filename = f"{base_name}_cover{extension}"
        ...
        # --- Logic for exporting metadata ---
            ...
            metadata = (
                inspector.inspect()
            )  # No section argument needed here as it's always full metadata
            default_filename = f"{base_name}_meta.json"
