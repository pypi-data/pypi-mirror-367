...
app = Flask(__name__)
initial_block_lock_path = "chordauth_startup.lock"
register_block_lock_path = "chordauth_register.lock"
write_database_block_lock_path = "chordauth_write_database.lock"
get_database_block_lock_path = "chordauth_get_database.lock"
remove_from_database_block_lock_path = "remove_from_database_block_lock_path.lock"
...
try:
    with FileLock(initial_block_lock_path):
        if os.path.exists("used_chordauth_already.chordauth_config"):
            try:
                ...
                with open("chordauth_public_key.chordauth_config") as i:
                ...
                try:
                    with open(str(user_private_key)) as r:        
                        raw_private_key = bytes.fromhex(r.read())
            ...
            try:
                ...
                if keys[0] is True:
                    ...
                    with open(private_key_path, "w") as file:
                        file.write(private_key_contents.hex())
                    with open(public_key_path, "w") as file:
                        file.write(public_key_contents)
                    with open("used_chordauth_already.chordauth_config", 'w') as f:
                        f.write("Already used.")
                    
                    with open("chordauth_public_key.chordauth_config", "w") as file:
                        file.write(public_key_contents)
...
def register():
    try:
        with FileLock(register_block_lock_path):
            ...
            if not os.path.exists("chord_auth_api_keys.json"):
                keys_dict = {}
            else:
                with open('chord_auth_api_keys.json', 'r') as file:
                    keys_dict = json.load(file)
            
            keys_dict[hashlib.sha256(api_key.encode()).hexdigest()] = unique_id
            
            with open('chord_auth_api_keys.json', 'w') as f:
...
def get_database():
    try:
        with FileLock(get_database_block_lock_path):
            ...
            if validity is True:
                if not os.path.exists("chord_auth_database.json"):
                    return "Storage database not initialized.", 400
                else:
                    with open('chord_auth_database.json', 'r') as file:
            ...
            if api_key:
                ...
                if validity is True:
                    ...
                    for item in chord_db:
                        ...
                        if item["group_name"] == keys_dict[hashlib.sha256(api_key.encode()).hexdigest()]:
                            del chord_db[i]
                            with open('chord_auth_database.json', 'w') as file:
                                json.dump(chord_db, file)
                            del keys_dict[hashlib.sha256(api_key.encode()).hexdigest()]
                            with open('chord_auth_api_keys.json', 'w') as file:
