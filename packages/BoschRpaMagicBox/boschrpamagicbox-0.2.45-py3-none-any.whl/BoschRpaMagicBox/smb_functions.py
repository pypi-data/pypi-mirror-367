...
def smb_copy_file_local_to_remote(username, password, server_name, share_name, local_file_path, remote_file_path, port=445):
    ...
    connection_cache = {}
    try:
        smbclient.register_session(server_name, username=username, password=password, port=port, encrypt=False, connection_cache=connection_cache, auth_protocol='ntlm')
    except:
        smbclient.register_session(server_name, username=username, password=password, port=port, encrypt=True, connection_cache=connection_cache, auth_protocol='ntlm')
    ...
    try:
        ...
        smbclient.delete_session(server_name, port=port, connection_cache=connection_cache)
