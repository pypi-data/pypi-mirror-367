...
def _format_cells(
    cells: Optional[list[CellContext]],
) -> str:
    if not cells:
        return ""

    cell_title = "\n\n## Available code from other cells:\n"
    cell_info = ""
    for cell in cells:
        current_cell_info = ""
        if cell.code:
            current_cell_info += _tag("python_code", cell.code) + "\n"
        if cell.output:
            current_cell_info += _tag("console_ouput", cell.output)+ "\n"
        if cell.result:
            current_cell_info += _tag("cell_result", cell.result)+ "\n"
        current_cell_info += _tag("has_error", str(cell.has_error))
        cell_info += _tag("cell", current_cell_info)

    return cell_title + cell_info
...
plt.gca()  # Return the current axes to display the plot
    ...
    # Overwrite prompt to only send context
    system_prompt = ""
    ...
    if context:
        system_prompt += _format_variables(context.variables)
        system_prompt += _format_schema_info(context.schema)
        system_prompt += _format_cells(context.cells)
