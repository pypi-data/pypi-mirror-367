    ...
    if isinstance(message, ChatMessage):
        ...
        if message.tool_calls or message.invalid_tool_calls:
            message_dict["tool_calls"] = [
                _lc_tool_call_to_qualcomm_inference_suite_tool_call(tc) for tc in message.tool_calls
            ] + [
...
class ChatQIS(BaseChatModel, BaseLangChainMixin):
    ...
    Setup:
        Install ``langchain-qualcomm-inference-suite`` and set environment variables ``IMAGINE_API_KEY`` and ``IMAGINE_API_ENDPOINT``.
    ...
    Key init args — client params:
        ...
        api_key: Optional[str]
            QIS API key. If not passed in will be read from env var IMAGINE_API_KEY.
    ...
    Invoke:
        ...
        .. code-block:: python
            ...
            AIMessage(content='The translation of "I love programming" to French is:\n\n"J\'adore le programmation."', additional_kwargs={}, response_metadata={'token_usage': {'prompt_tokens': 33, 'total_tokens': 55, 'completion_tokens': 22}, 'model_name': 'Llama-3.1-8B', 'system_fingerprint': '', 'finish_reason': <FinishReason.stop: 'stop'>}, id='run-c182fb49-12dc-4790-841c-d781418c4364-0', usage_metadata={'input_tokens': 33, 'output_tokens': 22, 'total_tokens': 55})
    ...
    Stream:
        ...
        .. code-block:: python
            ...
            AIMessageChunk(content='The translation of "I love programming" to French is:\n\n"J\'adore le programmation."', additional_kwargs={}, response_metadata={'finish_reason': <FinishReason.stop: 'stop'>}, id='run-eaf1a058-4aaf-48e0-bdcd-2a822eb8da46')
    ...
    Structured output:
        ...
        See ``ChatQIS.with_structured_output()`` for more.
    ...
    Response metadata
        ...
        .. code-block:: python
             ...
             {'token_usage': {'prompt_tokens': 33, 'total_tokens': 55, 'completion_tokens': 22}, 'model_name': 'Llama-3.1-8B', 'system_fingerprint': '', 'finish_reason': <FinishReason.stop: 'stop'>}
    ...
    def _stream(
        ...
        params = {**params, **kwargs}
        params.pop("stream")  # We are already calling a stream method, so no need to pass this
    ...
    async def _astream(
        ...
        default_chunk_class = AIMessageChunk
        async for chunk in self.async_client.chat_stream(messages=message_dicts, **params):
            if not isinstance(chunk, dict):
                chunk = chunk.model_dump()
            if len(chunk["choices"]) == 0:
                if token_usage := chunk.get("usage"):
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        should_stream = kwargs["stream"] if kwargs.get("stream") is not None else self.streaming
        if should_stream:
    ...
    #         if (
    #                 is_pydantic_schema and issubclass(schema, BaseModelV1)
