...
class TaientClient:
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        # 检查username和password必传，否则直接抛出异常
        if not config or 'username' not in config or 'password' not in config:
            raise ValueError("配置必须包含 'username' 和 'password'")

        # 配置初始化
        self.HRP_HOST = config.get("server")
        self.HRP_PORT = config.get("port")
        self.XA_USERNAME = config.get('username')
        self.XA_PASSWORD = config.get('password')
    ...
    def hrp_url(self, api: str) -> str:
        return f"http://{self.HRP_HOST}:{self.HRP_PORT}/api/v1{api}"
    ...
    def _login_and_get_cookie(self) -> str:
        ...
        payload = {"username": self.XA_USERNAME, "password": self.XA_PASSWORD}
