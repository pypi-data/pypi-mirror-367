...
def init(interactive: bool, step: str, jenkins: str, fs_name: str, delete_temp_files: bool, **kwargs):
    ...
    if not profile.current().namespace and not fs_name:
        fail("Usage: hcs dev fs init <YOUR_FEATURE_STACK_ID>")

    if not fs_name:
        fs_name = profile.current().namespace
...
def _prepare_profile(fs_name: str):
    ...
    if not profile.exists(fs_name):
        ...
        profile_data["namespace"] = fs_name
        ...
        updated = False
        if profile_data["namespace"] != fs_name:
            log.warn(
                f"Current profile namespace ({profile_data['namespace']}) does not match provided namespace ({fs_name}). Updating profile to use the specified namespace."
            )
            profile_data["namespace"] = fs_name
            updated = True
...
def _create_infra_azure():
    ...
    ret = _hcs("plan apply -f " + azure_plan_path, raise_on_error=False)
    if ret.returncode != 0:
...
def _create_infra_azsim():
    ...
    ret = _hcs("plan apply -f " + azusim_plan_path, raise_on_error=False)
    if ret.returncode == 0:
...
def _create_infra_akka():
    ...
    ret = _hcs("plan apply -f " + infra_plan_path, raise_on_error=False)
    if ret.returncode == 0:
...
def _create_infra_lcm_zerocloud():
    _hcs("lcm template create -p zero-floating", output_json=True)
    _hcs("lcm template create -p zero-dedicated", output_json=True)
    _hcs("lcm template create -p zero-multisession", output_json=True)
...
def _organize_env_file(env_file):
    group_rules = OrderedDict(
        [
            ...
            (
                "Subnet",
                ["DMZ_SUBNET", "DMZ_SUBNET_CIDR", "DESKTOP_SUBNET", "DESKTOP_SUBNET_CIDR", "MANAGEMENT_SUBNET", "MANAGEMENT_SUBNET_CIDR"],
            ),
...
def _touch_fs_to_avoid_recycle():
    namespace = profile.current().namespace
    ...
    try:
        if not k8s_util.validate_kubectl_config(fs_name, raise_on_error=False):
            ...
            if feature_stack_service:
                print(f"kubectl config validation failed.")
        ...
        fail(
            ...
            f"  2. Or download and copy feature stack kubeconfig to ~/.kube/config\n"
        )
...
def _hcs(cmd: str, output_json=False, raise_on_error=True):
    cmd = f"hcs {cmd}"
    if output_json:
...
@step
def _show_mqtt_info():
    ret = kubectl(f"exec -t mqtt-server-0 -- vmq-admin session show", ignore_error=True)
    print(ret.stdout)
    if ret.stderr:
...
def _update_mqtt():
    with_template_file(
        ...
        substitutions={r"(namespace:\s*)\S+": rf"\1{profile.current().namespace}"},
        fn=lambda temp_file_path: kubectl(f"apply -f {temp_file_path}"),
        base_path=__file__,
        delete_after=_delete_temp_files,
    )
    ...
    with_template_file(
        ...
        substitutions={
            r"(namespace:\s*)\S+": rf"\1{profile.current().namespace}",
...
def _create_org_details():
    payload2 = {
        "customerName": f"{profile.current().namespace}-dev",
...
def _map_fqdn_to_lb(fs_name):
    ...
    print(f'echo "<YOUR_UAG_LB_IP_ADDRESS> {os.getenv("UAG_FQDN")}" | sudo tee -a /etc/hosts')
    log.info(f"After that, you can view your desktops at https://{fs_name}.fs.devframe.cp.horizon.omnissa.com/appblast/endpoint/appblast")
