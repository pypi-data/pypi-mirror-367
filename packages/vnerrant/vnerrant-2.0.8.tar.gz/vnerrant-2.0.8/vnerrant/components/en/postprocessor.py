...
AUXES = frozenset({
    ...
    "won't", "wouldn't",
    "shan't", "shouldn't",
    "mayn't", "mightn't", "mustn't", "oughtn't",
})
...
class Postprocessor(BasePostprocessor):
    ...
    def __has_qmark_within(self, text: str, start: int, end: int, max_steps: int) -> bool:
    ...
    def __next_shape_is_pronoun_or_not_pronoun(self, text: str, start: int, end: int) -> bool:
        """
    ...
    def _postprocess_question_tag(self, edit_collection: EditCollection):
        ...
        for edit in edit_collection.edits:
            ...
            if parent_error == ParentErrorType.CONTRACTION:
                ...
                orig_prev = self.__safe_prev(orig_start, edit_collection.orig_doc.text, order=1)
                orig_next = self.__safe_next(orig_start, orig_end, edit_collection.orig_doc.text, order=1)
                corr_prev = self.__safe_prev(corr_start, edit_collection.cor_doc.text, order=1)
                corr_next = self.__safe_next(corr_start, corr_end, edit_collection.cor_doc.text, order=1)

                if not (orig_prev and orig_next and corr_prev and corr_next): continue
                ...
                if (
                    orig_has_aux_near and corr_has_aux_near
            ...
            if orig_text in AUXES or corr_text in AUXES:
                ...
                if (
                    self.__next_shape_is_pronoun_or_not_pronoun(edit_collection.orig_doc.text, orig_start, orig_end)
                    and self.__next_shape_is_pronoun_or_not_pronoun(edit_collection.cor_doc.text, corr_start, corr_end)
                    and self.__has_qmark_within(edit_collection.cor_doc.text, corr_start, corr_end, max_steps=3)
                ):
            ...
            if parent_error == ParentErrorType.PRONOUN:
                if (
                self.__prev_shape_is_aux_or_not_aux(edit_collection.orig_doc.text, orig_start)
                and self.__prev_shape_is_aux_or_not_aux(edit_collection.cor_doc.text, corr_start)
                ):
                    nxt = self.__safe_next(corr_start, corr_end, edit_collection.cor_doc.text, order=1)
                    if nxt:
                        _, _, tok = nxt
                        if "?" in tok:
                            edit.edit_type = edit.edit_type[:2] + ParentErrorType.QUESTION_TAG