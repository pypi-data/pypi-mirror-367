    ...
    for idx, edit in enumerate(collection.edits):
        ...
        if idx != 0 and (collection.edits[idx - 1].original.end_char >= orig_start_char):
            if collection.edits[idx - 1].edit_type[2:] == "ORTH" or (
                collection.edits[idx - 1].original.text.lower() in dependency_resources.aux_conts
                or collection.edits[idx - 1].original.text.lower() in dependency_resources.mapping_conts
                or collection.edits[idx - 1].corrected.text.lower() in dependency_resources.aux_conts
                or collection.edits[idx - 1].corrected.text.lower() in dependency_resources.mapping_conts
                ):
                edits_[-1] = edit_
            else:
                edits_.append(edit)
