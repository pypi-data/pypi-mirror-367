...
import grpc
import logging
from .proto import sdk_pb2
from .proto import sdk_pb2_grpc
import numpy as np
...
class HilbertClient:
    ...
    def search(self, name, queries, k=10, nprob=32, batch_size=300):
        nq = queries.shape[0]
        dim = queries.shape[1]
        ...
        for start_idx in range(0, nq, batch_size):
            ...
            logger.debug(f"Processing batch: queries {start_idx}-{end_idx-1} ({batch_nq} queries)")
            flat_queries = batch_queries.flatten().tolist()
        ...
        if nq > 0:
            logger.debug(f"First query result - distances: {all_distances[0]}, labels: {all_labels[0]}")
