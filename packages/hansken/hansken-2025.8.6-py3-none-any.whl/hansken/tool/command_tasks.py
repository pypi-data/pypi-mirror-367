...
tasks_parser = add_command(
    'tasks',
    run_tasks_command,
    optional_project=True,
...
tasks_parser.add_argument(
    ...
    choices=('open', 'closed'),
    default='open',
    help='select state of tasks to list, one of {} (defaults to open)'.format(', '.join(('open', 'closed'))),
)
tasks_parser.add_argument('--status', metavar='STATUS', help='status of the tasks to list, e.g. queued or running')
tasks_parser.add_argument(
    '--from',
    dest='start',
    type=parse_date,
...
tasks_parser.add_argument(
    ...
    default='simple',
    help='specify tabulation format, one of {} (see documentation of python module tabulate)'.format(
