...
class BSky(PluginDirective):
    ...
    def extend_domain(cls, domain):
        ...
        global get_entries_bskyposts
        def get_entries_bskyposts(domain, orgs=None, idents=None, cats=None, countries=None):
            """Get bskypost from the domain."""
            logger.debug(f"get_entries_bskyposts {cats} {orgs} {countries}")
        ...
        domain.get_entries_bskyposts = get_entries_bskyposts
        ...
        global process_doc_bskypost
        def process_doc_bskypost(domain, env: BuildEnvironment, docname: str,
            ...
            for bskypost in document.findall(bskypost_node):
                ...
                if env.config.osint_emit_warnings:
                    logger.warning(__("ANALYSE entry found: %s"), bskypost[0].astext(),
                                   location=bskypost)
                                   # ~ )
        domain.process_doc_bskypost = process_doc_bskypost

        global resolve_xref_bskypost
        """Resolve reference for index"""
        def resolve_xref_bskypost(domain, env, osinttyp, target):
            ...
            if osinttyp == 'bskypost':
                ...
                return match
            return []
        domain.resolve_xref_bskypost = resolve_xref_bskypost
    ...
    def extend_processor(cls, processor):
        ...
        global process_bskypost
        def process_bskypost(processor, doctree: nodes.document, docname: str, domain):
            '''Process the node'''
        ...
        processor.process_bskypost = process_bskypost
    ...
    @classmethod
    def cache_file(cls, env, bskypost_name):
        """
        """
        if cls._bskypost_cache is None:
            cls._bskypost_cache = env.config.osint_bskypost_cache
            os.makedirs(cls._bskypost_cache, exist_ok=True)
        return os.path.join(cls._bskypost_cache, f"{bskypost_name.replace(f'{cls.category}.', '')}.txt")

    @classmethod
    def store_file(cls, env, bskypost_name):
        """
        """
        if cls._bskypost_store is None:
            cls._bskypost_store = env.config.osint_bskypost_store
            os.makedirs(cls._bskypost_store, exist_ok=True)
        return os.path.join(cls._bskypost_store, f"{bskypost_name.replace(f'{cls.category}.', '')}.txt")
...
class IndexBSky(Index):
    ...
    def get_datas(self):
        # ~ datas = self.domain.get_entries_IndexBSkys()
        # ~ datas = self.domain.get_entries_bskyposts()
