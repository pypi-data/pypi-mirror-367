...
class WebSocketClient:
    ...
    def get_expiration(self, duration: int) -> tuple[int, int]:
        ...
        if (date_to_timestamp(exp_date + timedelta(minutes=1)) - server_timestamp) > 30:
        ...
        while index < max_15m_expirations:
            if exp_date.minute % 15 == 0 and (date_to_timestamp(exp_date) - server_timestamp) > 300:
                expirations.append(date_to_timestamp(exp_date))
                index += 1
            exp_date += timedelta(minutes=1)
