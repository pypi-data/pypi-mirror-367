...
class WebSocketClient:
    ...
    def get_expiration(self, duration: int) -> tuple[int, int]:
        ...
        now_date = datetime.fromtimestamp(local_timestamp)

        expirations = []

        exp_date = now_date.replace(second=0, microsecond=0)
        if (date_to_timestamp(exp_date + timedelta(minutes=1)) - local_timestamp) > 30:
            exp_date += timedelta(minutes=1)
        else:
            exp_date += timedelta(minutes=2)

        for _ in range(5):
            expirations.append(date_to_timestamp(exp_date))
            exp_date += timedelta(minutes=1)

        index = 0
        max_15m_expirations = 50
        exp_date = now_date.replace(second=0, microsecond=0)
        while index < max_15m_expirations:
            if exp_date.minute % 15 == 0 and (date_to_timestamp(exp_date) - local_timestamp) > 300:
                expirations.append(date_to_timestamp(exp_date))
                index += 1
            exp_date += timedelta(minutes=1)
        ...
        closest_expiration = min(
            safe_expirations,
            key=lambda exp: abs((exp - local_timestamp) - target_seconds)
        )
