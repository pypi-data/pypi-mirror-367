...
class ExecutionEngine:
    def __init__(self, crawler: "Crawler", spider_closed_callback: Callable) -> None:
        ...
        self.batch_count = self._get_batch_count(crawler.settings)
    ...
    def _get_batch_count(self, settings: BaseSettings) -> int:
        concurrent_requests = settings.getint('CONCURRENT_REQUESTS')
        rabbitmq_concurrent_requests_multiplier = settings.getint('CONCURRENT_REQUESTS_MULTIPLIER', 10)
        return  concurrent_requests * rabbitmq_concurrent_requests_multiplier
    ...
    def _next_request(self) -> None:
        ...
        while (
            ...
            pass
        current_count = 0
        while self.slot.start_requests is not None and not self._needs_backout():
            try:
                ...
                self.crawl(request)
            current_count += 1
            if current_count >= self.batch_count:
                break
