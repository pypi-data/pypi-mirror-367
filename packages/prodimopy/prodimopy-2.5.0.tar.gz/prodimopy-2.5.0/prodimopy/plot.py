...
class Plot(object):
  ...
  def _mapField2Label(self,fieldname,log=True):
    ...
    label_map={"nHtot" : r"$n_{\langle H \rangle}\,[cm^{-3}]$",
               ...
               "zetaX" : r"$\zeta_{X}\,per\,H\,[s^{-1}]$",
               "zetaCR" : r"$\zeta_{CR}\,per\,H_2\,[s^{-1}]$"
    }
  ...
  def plot_cont(self,model,values,label="value",zlog=True,grid=False,
                ...
                bgcolor=None,showcb=True,cb_format="%.1f",scalexy=[1,1],patches=None,
    ...
    acont : array_like(float,ndim=2)
      Should not be used any more. Default: `None`
      Use `oconts` instead.
    ...
    if type(values) is str:
      tmpvalues=getattr(model,values,None)
      if tmpvalues is None:
        raise ValueError("ERROR: plot_cont: field %s not found in model" % values)
      # if label is none also use the passed values string as label
      if label=="value": 
        label=values
      values=tmpvalues      
...
#    ax.plot(model.x[:,0],model.z[:,35],color="black")
  ...
  def plot_abuncont(self,model,species='O',rel2H=True,label=None,zlog=True,
                zlim=[None,None],zr=True,cmap=None,clevels=None,clabels=None,contour=True,
                extend="neither",oconts=None,acont=None,acontl=None,nbins=70,
                bgcolor=None,cb_format="%.1f",showcb=True,scalexy=[1,1],patches=None,
    ...
    return self.plot_cont(model,values,label=labelN,zlog=zlogN,
                ...
                bgcolor=bgcolor,cb_format=cb_format,showcb=showcb,scalexy=scalexy,patches=patches,
  ...
  def plot_reaccont(self,model,chemana,rtype,level=1,showAbun=False,values=None,
    ...
    for i in range(xstart,xend,1):
      for j in range(ystart,yend,1):
        if ax is None:
          ax= fig.axes[0]
        ax.text(i,j,reacs[0][i,j],fontsize=3.0,horizontalalignment="center",verticalalignment="bottom",color="white")

    return fig
  
  def plot_reac_form_dest(self,chemana: pread.Chemistry,level: int=1,**kwargs):
    '''
    ...
    chemana : The chemistry analysis object resulting from :func:`~prodimopy.read.analyse_chemistry`.

    level : The level of the reactions to be plotted (1 for most important, 2 for second most important etc.).
    ...
    .. todo:: 
      * Currently only level=1 is supported, i.e. the most important reactions are plotted.
    ...
    for reactype,ax in zip(["f","d"],axes): 
      ...
      if (nreac>10):
        print("WARN: plot_reac_form_dest: more than 10 reactions found, only the first 10 will be plotted, "
        "The rest will be in white.")
        for i in range(10,nreac):
          map[map==sreacsf[i]]=-1 # mark as invalid, will be shown in cmap.set_under color
        nreac=10
      ...
      for i in range(nreac):
        ...
        l=ax.scatter(0,0,marker="s",color=colors[i],label=reacToStr(reac))
        handles.append(l)
      ...
      fig.legend(loc="lower right",bbox_to_anchor=bbox_to_anchor,ncol=2,markerscale=0,handles=handles,
                labelcolor=colors,prop=font,columnspacing=-0.5,mode="expand")
        
    fig.subplots_adjust(top=0.93,bottom=0.31,left=0.07,right=.995)     

    return self._closefig(fig)
  
  def plot_reac_ixiz(self,ix: int,iz: int,rtype: str,
    ...
    chemana_steadystate :
      The chemsitry analysis of an equivalent steady-steate model (i.e. same grid, etc.) for the
    ...
    for i in range(len(reacidx)):
      ...
      ax.plot(ages,reactions[i],label=reacToStr(chemanas[0].model.chemnet.reactions[reacidx[i]-1]))
    ...
    if chemana_steadystate is not None:
      ...
      for i,(reacidx,rate) in enumerate(zip(reacidxsss,ratesss)):
        ax.scatter(ages[-1]*1.05,rate,label=reacToStr(chemana_steadystate.model.chemnet.reactions[reacidx-1]),marker="<",color=str(colors[i]),s=20/np.log(i+2))
  ...
  def plot_sed(self,model,plot_starSpec=True,incidx=0,unit="erg",sedObs=None,reddening=False,
    ...
    for iinc in incidx:
      ...
      if unit=="W":
        ...
        y=model.sed.fnuJy
        if unit=="mJy": y=y*1000
      else:
        y=model.sed.nu*model.sed.fnuErg

      ymin=np.min(y[model.sed.lam>1])

      if reddening==True or type(reddening)==str and sedObs is not None and sedObs.A_V is not None:
        if type(reddening)==str:
...
#    ax.yaxis.set_label_position("right")
  ...
  def plot_lineprofile(self,model: pread.Data_ProDiMo|None=None,wl: float|None=None,ident: str|None=None,lineObj: pread.DataLine|None=None,linetxt=None,lineObs=None,incidx=0,lw=None,
                       unit: str | None = None,normalized=False,convolved=False,removecont=True,style=None,color=None,ax=None,**kwargs):
    '''
    ...
    for i,iinc in enumerate(incidx):
      ...
      if unit != None and not normalized:
      ...
      if style=="step":
        ax.step(x,y,marker=None,label=incstr,where="mid",color=color,lw=lw)
      else:
        ax.plot(x,y,marker=None,label=incstr,color=color,lw=lw)
    ...
    if lineObs is not None:
      ...
      if lineO.profile is not None:
        ...
        if unit!= None: lineO.profile.flux_unit=unit
        ...
        if lineO.profileErr is not None:
          ax.fill_between(x,y-lineO.profileErr ,y+lineO.profileErr,color='0.8',zorder=0)
  ...
  def plot_contImage(self,model,wl,iinc=0,zlim=[None,None],cmap="inferno",rlim=[None,None],cb_show=True,cb_fraction=0.15,
                     ax=None,**kwargs):
    """
    ...
    tmpImg,wl= contImages.getImage(wl,iinc=iinc)
    ...
    if vmin is None: vmin=np.max([np.min(imagepl),vmax/1.e6])
    ...
    ax.set_title(r"$\lambda= {:6.2f}\, \mu m; \, i={:3.1f}^\circ$".format(wl, contImages._inclinations[iinc]),pad=0)
...
def spnToLatex(spname):
  ...
  if str(spname)=="PHOTON": return r"\gamma" 
  if str(spname)=="dust": return "\u26AB"
...
def reacToStr(reaction: pchem.Reaction) -> str: 
  ...
  out=fr"{reaction.id:>4d}$\,{reaction.type.strip()}\!:\,"  
