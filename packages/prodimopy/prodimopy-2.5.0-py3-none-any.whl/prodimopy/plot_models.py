...
class PlotModels(object):
  ...
  def plot_lines(self,models,lineIdents,useLineEstimate=True,jansky=False,
    ...
    peakFlux : boolean
      ...
      flux. Is sometimes useful to make predictions for observations, as in that
  ...
  def plot_tcdspec(self,models,species,relToH=False,facGrayBox=3.0,ice=False,linewidths=None,ax=None,**kwargs):
    '''
...
#     ax2.set_xticklabels(["{:.2f}".format(x) for x in nhver_to_zr(ix, ax.get_xticks(), model)])
  ...
  def plot_sed(self,models,plot_starSpec=True,sedObs=None,sedObsModels=False,
               unit="erg",incidx=0,reddening=False,ax=None,**kwargs):
    '''
    ...
    for model in models:
      ...
      if plot_starSpec:
        ...
        ax.plot(xStar,yStar,color=colsed,linewidth=0.5*lwsed,zorder=-1,linestyle="--")
        ...
        if plSedObs is not None:
          ...
          if plSedObs.specs is not None:
            for spec in plSedObs.specs:
              ...
              ax.plot(spec[:,0],f,linestyle=":",linewidth=0.5,color=sedcolor,zorder=2000)
...
#    ax.set_ylabel(r"$\mathrm{\nu F_{\nu}\,[erg\,cm^{-2}\,s^{-1}]}$")
  ...
  def plot_lineprofile(self,models,wl,ident=None,contsub=True,linetxt=None,lineObs=None,incidx=0,
                       unit: str | None = None,normalized=False,convolved=False,style=None,ax=None,**kwargs):
    '''
