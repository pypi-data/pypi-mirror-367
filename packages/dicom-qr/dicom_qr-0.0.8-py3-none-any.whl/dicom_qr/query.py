...
class QueryPacs:
    ...
    def get_series_for_study(self, study_uid: str | UID, modalities: Optional[Sequence[str]] = None) -> list[SeriesData]:
        ...
        for response, response_dataset in responses:
            ...
            if last_status in [DicomStatus.PENDING_0, DicomStatus.PENDING_1, DicomStatus.SUCCESS]:
                ...
                if modalities is not None and len(modalities) > 0:
                    if not set(modalities) & set(study.ModalitiesInStudy):
                        continue
