...
DEFAULT_SERIES_COLUMNS = [
    ...
    ColumnLayout(SeriesColumns.STUDYINSTANCEUID),
    ColumnLayout(SeriesColumns.SERIESINSTANCEUID),
...
def create_series_table(series: list[SeriesData], title: str = 'Series found',
                        columns: Optional[list[ColumnLayout]] = None) -> rich.table.Table:
    ...
    for index, study in enumerate(series, 1):
        data = []
        for col in columns:
            if col.name in (SeriesColumns.INDEX, SeriesColumns.SERIESNUMBER):
