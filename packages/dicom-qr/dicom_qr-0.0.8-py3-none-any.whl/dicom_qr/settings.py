...
class PacsConfig(BaseModel):
    ...
    def set_port(cls, value) -> int:  # type: ignore # pylint: disable=no-self-argument
        """ Validator to check the port is of type int """
        if not isinstance(value, int):
            raise ValueError('Port number is not an int.')
        return value

    @classmethod
    def load(cls, _filename: Union[Path, str]) -> PacsConfig:
        ...
        with Path(_filename).open('r', encoding='utf-8') as settings_file:
            setting = PacsConfig(**jsonpickle.decode(settings_file.read()))
...
def get_settings_filename(filename: Path) -> Path:
    ...
    """
    if 'PACS_RETRIEVE_SETTINGS' in os.environ:
        filename = Path(os.environ['PACS_RETRIEVE_SETTINGS'])
        if filename.exists():
            return filename
    ...
    filename = Path.home() / '.config/pacs_retrieve/settings.json'
    if filename.exists():
        return filename
...
def get_settings(filename: Path = Path('settings.json')) -> PacsConfig:
    """ Get the settings object, uses ['get_settings_filename'][pacs_retrieve.settings.get_settings_filename]
