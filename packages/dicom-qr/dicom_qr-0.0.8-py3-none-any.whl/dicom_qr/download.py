...
class RetrieveProgress:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, progress: RichProgressObject) -> None:
        ...
        self.task_id: Optional[rich.progress.TaskID] = None

    def progress(self, result: RetrieveResult) -> None:
        """ progress callback """
        assert result.completed is not None and result.remaining is not None
...
def from_accession_number(
        ...
        settings: PacsConfig,
        sop_class_uid: Optional[Iterable[str]],
...
def from_search(
        ...
        sop_class_uid: Optional[Iterable[str]] = None) -> list[SeriesData]:
