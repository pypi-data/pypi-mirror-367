...
app = typer.Typer(add_completion=False, no_args_is_help=True, help='PACS query and retrieve tool.')
...
def validate_accession_number(accession_number: str) -> str:
    """ Make sure the accession number has 16 characters. Padd or crop if neccesary. Required for LUMC Sectra PACS. """
...
def list_series_for_study(
        ...
        modalities: Optional[List[str]] = typer.Argument(None),
...
def list_studies_patient_id(
        ...
        modalities: Optional[List[str]] = typer.Argument(None),
        study_descriptions: Optional[str] =
...
def studyuid_from_acc_number_file(
        ...
        output: Optional[Path] = typer.Argument(None, file_okay=True, dir_okay=False, readable=True),
    ...
    with QueryPacs(get_settings()) as query:
        ...
        for accession_number in get_accession_numbers(filename):
            studies = query.get_studies(SearchTerms(accession_number=accession_number))
            for study in studies:
                subject_studies.append(study)
...
def download_from_name_date(
        ...
        modalities: Optional[List[str]] = typer.Option(None, '--modality', '-m',
                                                       help='Filter series based on modalities'),
        sop_class_uid: Optional[List[str]] = typer.Option(None, '--sop-class-uid', '-s',
                                                          help='Filter series based on SOPClassUID'),
        dumpdir: Optional[Path] = typer.Option(None, exists=False, file_okay=False, dir_okay=True,
                                               writable=True,
                                               help='Override output folder.'),
        verbose: bool = typer.Option(False, '--verbose', '-v', help='Print more output'),
        very_verbose: bool = typer.Option(False, '-vv', help='Print even more output')) -> List[SeriesData]:
...
def download_from_accession_number(
        ...
        modalities: Optional[List[str]] = typer.Option(None, '--modality', '-m', help='Filter series based on modalities'),
        sop_class_uid: Optional[List[str]] = typer.Option(None, '--sop-class-uid', '-s',
                                                          help='Filter series based on SOPClassUID'),
        dumpdir: Optional[Path] = typer.Option(None, exists=False, file_okay=False, dir_okay=True, writable=True,
                                               help='Override output folder.'),
        verbose: bool = typer.Option(False, '--verbose', '-v', help='Print more output'),
        very_verbose: bool = typer.Option(False, '-vv', help='Print even more output')) -> List[SeriesData]:
...
def download_from_studyuid_file(
        ...
        sop_class_uid: Optional[List[str]] = typer.Option(None, '--sop-class-uid', '-s',
                                                          help='Filter series based on SOPClassUID'),
        already_processed: Optional[Path] = typer.Option(None, exists=True, file_okay=True, dir_okay=False, writable=False,
                                                         help='File with already processed UIDs.'),
        dumpdir: Optional[Path] = typer.Option('dumpdir', exists=False, file_okay=False, dir_okay=True, writable=True,
                                               help='Override output folder.'),
        verbose: bool = typer.Option(False, '--verbose', '-v', help='Print more output'),
        very_verbose: bool = typer.Option(False, '-vv', help='Print even more output')) -> None:
...
def _get_uids(filename: Path, alreay_processed: Optional[Path]) -> List[str]:
...
def version_callback(value: bool) -> None:
    ...
    typer.echo(f'Version: {metadata.version("pacs_retrieve")}')
