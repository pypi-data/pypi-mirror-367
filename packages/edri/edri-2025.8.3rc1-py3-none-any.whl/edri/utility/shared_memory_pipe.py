...
class SharedMemoryPipe:
    ...
    def __init__(self,
        ...
        if self.is_writer:
            shm_kwargs.update({"create": True,
                               "size": self.slot_size * self.max_slots})
            ...
            self.items = Semaphore(self._items_sem_name,
                                   flags=O_CREX, initial_value=0)
            self.slots = Semaphore(self._slots_sem_name,
                                   flags=O_CREX, initial_value=self.max_slots)
        else:
    ...
    # pickling support for reader processes
