@echo off
setlocal
REM Windows Command Line Script (Batch) to launch "pyenv-virtualenv-prefix.py".
REM
REM Dependencies:
REM   * pyenv
...
call python "%~dp0pyenv-virtualenv-prefix.py" %*
set /a RC=%ERRORLEVEL%
if %RC% equ 130 goto cancel
if %RC% leq 5 goto finish
...
:error1
echo.
echo [91mERROR    Failed to determine the virtual environment prefix (RC = %RC%).[0m
...
endlocal
exit /b 130
REM Ignore I/O error, but return it
:finish
endlocal
exit /B 5
