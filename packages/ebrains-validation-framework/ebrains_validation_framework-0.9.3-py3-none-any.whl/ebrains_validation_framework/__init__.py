...
class BaseClient(object):
    ...
    def _get_attribute_options(self, param, valid_params):
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Error from '{url}': {response.text}")
...
class ModelCatalog(BaseClient):
    ...
    def download_model_instance(
        ...
        elif model_source.startswith("https://data-proxy.ebrains.eu/"):
            datastore = URI_SCHEME_MAP["http"]()
            fileList = datastore.download_data(f"https://data.kg.ebrains.eu/zip?container={model_source}",
                                               local_directory=local_directory, overwrite=overwrite)
        else:
