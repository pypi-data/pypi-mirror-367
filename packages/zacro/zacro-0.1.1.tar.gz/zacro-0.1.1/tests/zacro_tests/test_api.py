...
<robot xmlns:xacro="http://ros.org/wiki/xacro">
  <link name="test"/>
    ...
    def test_class_api_methods(self):
        ...
        self.assertTrue(hasattr(processor, 'set_remove_first_joint'))
...
<robot xmlns:xacro="http://ros.org/wiki/xacro">
  ...
  <xacro:test_macro name="test"/>
        ...
        # Test format_output only
        result1 = zacro.xacro_from_string(xml_str, format_output=True)
        self.assertTrue(result1.startswith('<?xml version="1.0" encoding="UTF-8"?>'))
        self.assertEqual(result1.count('<joint'), 1)

        # Test remove_first_joint only
        result2 = zacro.xacro_from_string(xml_str, remove_first_joint=True)
        self.assertEqual(result2.count('<joint'), 0)

        # Test both options
        result3 = zacro.xacro_from_string(xml_str, format_output=True, remove_first_joint=True)
...
<robot xmlns:xacro="http://ros.org/wiki/xacro">
  <link name="default_test"/>
        ...
        # Should not be formatted by default
        self.assertNotIn('\n  <', result)
