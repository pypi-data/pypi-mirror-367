...
def reload_devices():
    ...
    # 2. insert all Netpicker has to offer
    data = get_netpicker_devices()
    g_np_dev = (ND(ipaddress=ip, platform=platform, tenant=tenant, name=name) for ip, platform, tenant, name in data)
    ND.objects.bulk_create(g_np_dev)
    # 3. delete goers
    MD = models.MappedDevice
    on_join = ' and '.join(f"nd.{f}=d.{f}" for f in ['tenant', 'ipaddress'])
    qs_goers = f"""
        select d.id from netpicker_mappeddevice as nd left outer join netpicker_netpickerdevice d
        on {on_join}
