...
class Client(PmAgentClient):
    ...
    async def check_in(self, amount: Decimal | int | float, cur: str, tid: str | int = None) -> Decimal | None:
        history = self.api.history(type="incoming", append=tid)
        if tid:
        ...
        return (amount <= (am := Decimal(t["creditedAmount"])) <= ceil(amount) and t["creditedCurrency"] == cur) and am
...
async def main(uid: int):
    ...
    try:
        ...
        await pyr.check_in(4, "RUB")

        dest, amount, cur = "P1132998804", 2, "RUB"
