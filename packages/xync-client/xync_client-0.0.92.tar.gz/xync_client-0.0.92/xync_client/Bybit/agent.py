...
class AgentClient(BaseAgentClient):  # Bybit client
    ...
    def get_cad(self, ads: list[Ad], ceil: Decimal, k: Literal[-1, 1], place: int, cur_plc: int) -> Ad:
        if not ads:
            return None
    ...
    async def racing(
        ...
        pmexs: list[models.Pmex] = await models.Pmex.filter(
            pm_id__in=[pm.id for pm in race.road.ad.pms], ex=self.actor.ex
        )
        k = (-1) ** int(taker_side)  # on_buy=1, on_sell=-1
        sleep_sec = 2  # 1 if set(pms) & {"volet"} and coinex.coin_id == 1 else 5
        ...
        while self.actor.person.user.status > 0:
            ...
            self.overprice_filter(ads, race.ceil, k)
            if not ads:
            ...
            if Decimal(mad.price) == new_price:  # Если место уже нужное или нужная цена и так уже стоит
            ...
            try:
                print(
                    f"{coinex.coin.ticker}{'-' if taker_side else '+'}{req.price}{f'({req.premium})' if req.premium else ''}",
                    end="; ",
                    flush=True,
                )
                ...
                if ExcCode(e.status_code) == ExcCode.FixPriceLimit:
                    ...
                    asset = await models.Asset.get(addr__actor=self.actor, addr__coin_id=coinex.coin_id)
                    req.quantity = round(
                        asset.free - (asset.freeze or 0) - (asset.lock or 0), coinex.coin.scale or coinex.scale
                    )
                    _res = self.ad_upd(req)
                elif ExcCode(e.status_code) == ExcCode.RareLimit:
...
def step_is_need(mad, cad) -> bool:
    return cad.recentExecuteRate > mad.recentExecuteRate or (
        cad.recentExecuteRate == mad.recentExecuteRate and cad.recentOrderNum > mad.recentOrderNum
    )
...
async def main():
    ...
    async with FileClient(TOKEN) as b:
        ...
        tasks = [cl.racing(race) for race in races]
