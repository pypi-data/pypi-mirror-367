...
class SyncCallContextExecutor(BaseCallContextExecutor):
    def execute(self, ctx: CallContext, func: Callable[[CallContext], T]) -> T:
        logger.debug(f"SyncCallContextExecutor: Starting execution for user {ctx.user_id}, turn {ctx.turn_id}")
        self._run_before_hooks(ctx)

        try:
            logger.debug(f"SyncCallContextExecutor: Executing function for user {ctx.user_id}")
            result = func(ctx)
            self._run_completed_hooks(ctx)
            logger.debug(f"SyncCallContextExecutor: Successfully completed execution for user {ctx.user_id}")
            return result
        except Exception as e:
            logger.debug(f"SyncCallContextExecutor: Error occurred for user {ctx.user_id}: {e}")
            ctx.set_error(e)
...
class AsyncCallContextExecutor(BaseCallContextExecutor):
    ...
    async def async_execute(self, ctx: CallContext, func: Callable[[CallContext], Awaitable[T]]) -> T:
        logger.debug(f"AsyncCallContextExecutor: Starting execution for user {ctx.user_id}, turn {ctx.turn_id}")
        self._run_before_hooks(ctx)
        await self._run_before_async_hooks(ctx)

        try:
            logger.debug(f"AsyncCallContextExecutor: Executing async function for user {ctx.user_id}")
            result = await func(ctx)
            self._run_completed_hooks(ctx)
            await self._run_completed_async_hooks(ctx)
            logger.debug(f"AsyncCallContextExecutor: Successfully completed async execution for user {ctx.user_id}")
            return result
        except Exception as e:
            logger.debug(f"AsyncCallContextExecutor: Error occurred for user {ctx.user_id}: {e}")
            ctx.set_error(e)
            self._run_error_hooks(ctx, e)
...
class StreamCallContextExecutor(BaseCallContextExecutor):
    ...
    def stream_execute(
        ...
        logger.debug(f"StreamCallContextExecutor: Starting stream execution for user {ctx.user_id}, turn {ctx.turn_id}")
        self._run_before_hooks(ctx)

        try:
            logger.debug(f"StreamCallContextExecutor: Beginning stream generation for user {ctx.user_id}")
            for item in func(ctx):
                yield item
            self._run_completed_hooks(ctx)
            logger.debug(f"StreamCallContextExecutor: Successfully completed stream execution for user {ctx.user_id}")
        except Exception as e:
            logger.debug(f"StreamCallContextExecutor: Error occurred for user {ctx.user_id}: {e}")
            ctx.set_error(e)
            self._run_error_hooks(ctx, e)
            raise
        finally:
