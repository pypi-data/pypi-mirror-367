...
class Component(ABC):
    ...
    def __init__(self, install_dir: Optional[Path] = None, component_subdir: Path = Path(''), ai_type: str = 'claude'):
        """
        Initialize component with installation directory
        
        Args:
            install_dir: Target installation directory (defaults to ~/.claude or ~/.gemini)
        ...
        self.component_files = self._discover_component_files(ai_type)
    ...
    @abstractmethod
    def _get_source_dir(self, ai_type: str) -> Optional[Path]:
