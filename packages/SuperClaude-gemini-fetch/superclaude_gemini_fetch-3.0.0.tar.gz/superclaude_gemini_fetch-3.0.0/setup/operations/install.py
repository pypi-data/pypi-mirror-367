...
Examples:
  ...
  SuperClaude install --verbose --force        # Verbose with force mode
    ...
    parser.add_argument(
        ...
        default='claude',
        help="Specify the AI model type to install for (claude or gemini)"
    )
...
def perform_installation(components: List[str], args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        component_instances = registry.create_component_instances(components, args.install_dir, args.ai_type)
...
def run(args: argparse.Namespace) -> int:
    ...
    # Set install_dir based on ai_type
    if args.ai_type == 'gemini':
        args.install_dir = Path.home() / ".gemini"
    else: # default to claude
        args.install_dir = Path.home() / ".claude"
