...
class MCPComponent(Component):
    ...
    def __init__(self, install_dir: Optional[Path] = None, ai_type: str = 'claude'):
        """Initialize MCP component"""
        super().__init__(install_dir, ai_type=ai_type)
        self.cli_command = ai_type
    ...
    def validate_prerequisites(self, installSubPath: Optional[Path] = None) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if result.returncode != 0:
                errors.append(f"{self.cli_command.capitalize()} CLI not found - required for MCP server management")
            else:
                version = result.stdout.strip()
                self.logger.debug(f"Found {self.cli_command.capitalize()} CLI {version}")
        except (subprocess.TimeoutExpired, FileNotFoundError):
            errors.append(f"{self.cli_command.capitalize()} CLI not found - required for MCP server management")
    ...
    def _install_mcp_server(self, server_info: Dict[str, Any], config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Install using Claude/Gemini CLI
            if config.get("dry_run"):
                self.logger.info(f"Would install MCP server (user scope): {self.cli_command} mcp add -s user {server_name} {command} -y {npm_package}")
                return True
            
            self.logger.debug(f"Running: {self.cli_command} mcp add -s user {server_name} {command} -y {npm_package}")
            
            result = subprocess.run(
                [self.cli_command, "mcp", "add", "-s", "user", "--", server_name, command, "-y", npm_package],
                capture_output=True,
                text=True,
    ...
    def _uninstall_mcp_server(self, server_name: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Running: {self.cli_command} mcp remove {server_name} (auto-detect scope)")
            
            result = subprocess.run(
                [self.cli_command, "mcp", "remove", server_name],
                capture_output=True,
                text=True,
                timeout=60,
                shell=(sys.platform == "win32")
    ...
    def validate_installation(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if result.returncode != 0:
                errors.append(f"Could not communicate with {self.cli_command.capitalize()} CLI for MCP server verification")
            else:
