...
Nova Act supports streaming mode, which provides real-time updates during the execution of `act()` commands. When streaming is enabled, you can receive intermediate progress updates as the agent performs its actions.

To enable streaming, set the `stream=True` parameter when initializing `NovaAct`:

```python
from nova_act import NovaAct

# Enable streaming mode
with NovaAct(starting_page="https://www.amazon.com", stream=True) as nova:
    # When streaming is enabled, act() becomes a generator
    for update in nova.act("search for a coffee maker"):
        print(f"Stream update: {update}")
```

When streaming is enabled:
- The `act()` method becomes a generator that yields real-time updates
- You'll receive updates about observations (current URL) and model responses
- Each update provides insight into what the agent is currently doing
- The final result is still available through the generator's return value

**Streaming Response Formats:**

The format of streaming updates depends on which dispatcher is being used:

**Custom Dispatcher (default):**
```
observation: https://www.amazon.com/s?k=coffee+maker&crid=WQTBHJXV5JDC&sprefix=coffee+maker%2Caps%2C1219&ref=nb_sb_noss_1
model_response: think("I am now on the search results page for coffee maker, so my last action was successful. I have searched for a coffee maker. The task is complete, and I need to simply return because no information was asked of me.");
```

**Extension Dispatcher:**
```
model_response: think("The search field now contains 'coffee maker', but the Enter key wasn't pressed so my last action was unsuccessful. I see a search button, but it has not been selected. I need to click the search button to initiate the search.");
tool_call: agentClick(" <box>10,1049,47,1092</box> ");
```
...
with NovaAct(starting_page="https://www.amazon.com", stream=True) as nova:
    for update in nova.act("search for a coffee maker"):
        if update.startswith("observation:"):
            print(f"Current page: {update}")
        elif update.startswith("model_response:"):
            print(f"Agent thinking: {update}")
        elif update.startswith("tool_call:"):
            print(f"Agent action: {update}")
```

> **Note:** When streaming is disabled (default), `act()` returns an `ActResult` directly. When streaming is enabled, `act()` returns a generator that yields updates and the final result is obtained when the generator is exhausted.
...
* `stream (bool)`: Whether to enable streaming mode for `act()`. When `True`, `act()` returns a generator that yields updates and the final result is obtained when the generator is exhausted. Default is `False`.
