...
from vihi_nova_act.preview.custom_actuation.interface.actuator import ActuatorBase
from vihi_nova_act.preview.custom_actuation.interface.browser import BrowserActuatorBase, BrowserObservation
from vihi_nova_act.preview.custom_actuation.interface.types.agent_redirect_error import AgentRedirectError
from vihi_nova_act.preview.custom_actuation.routing.error_handler import handle_error
from vihi_nova_act.preview.custom_actuation.routing.interpreter import AWLInterpreter
from vihi_nova_act.preview.custom_actuation.routing.request import (
    construct_plan_request,
)
from vihi_nova_act.preview.custom_actuation.routing.routes import Routes
from vihi_nova_act.types.act_errors import ActError
from vihi_nova_act.types.act_result import ActResult
from vihi_nova_act.types.errors import ClientNotStarted, ValidationFailed
        ...
        with kb_cm as watcher:
            ...
            for i in range(1, act.max_steps + 1):
                ...
                try:
                    ...
                    # _TRACE_LOGGER.info(f"observation: {observation['activeURL']}")
                    ...
                    # print(f"plan_request: {plan_request}")
                    ...
                    _TRACE_LOGGER.info(f"program: {program}")

                    if self._stream:
                        yield f"observation: {observation['activeURL']}"
                        yield f"model_response: {program}"
                        # yield f"tool_call: {step_object}"
                    ...
                    try:
                        ...
                        # _TRACE_LOGGER.info(f"is_act_done: {is_act_done}")
                        # _TRACE_LOGGER.info(f"result: {result}")
                        # _TRACE_LOGGER.info(f"program_error: {program_error}")
