...
def get_col_bin_auto(
    *dfs: pl.DataFrame,
    ...
    if dtype.is_numeric():
        return get_col_bin_numeric(*dfs, col=col, num_n_bins=num_n_bins, num_sig_digits=num_sig_digits, verbose=verbose, traceable=traceable)
    elif dtype in (pl.Date, pl.Datetime):
        return get_col_bin_datetime(*dfs, col=col, dt_truncate_unit=dt_truncate_unit, verbose=verbose, traceable=traceable)
    elif dtype in (pl.Utf8, pl.Categorical):
        return get_col_bin_categorical(*dfs, col=col, verbose=verbose, traceable=traceable)
    else:
...
def get_col_bin_numeric(
    *dfs: pl.DataFrame,
    ...
    if traceable:
        trace = get_col_bin_trace(df_bin_detail_info=df_bin_detail_info)
...
def get_col_bin_categorical(
    *dfs: pl.DataFrame,
    ...
    # return (*dfs_bin, None)
    if traceable:
        trace = get_col_bin_trace(df_bin_detail_info=None)
        return (*dfs_bin, trace)
    return dfs_bin
