...
def rattle(moleculeseed, bridgelist, adjmatrix_ref, qdegamp=180.0):
    ...
    return moleculeout
#-------------------------------------------------------------------------------
def make_rotamers_check(moleculeseed, number, bridgelist, adjmatrix_ref):
    qdegamp=180
    ...
    return moleculeout
#-------------------------------------------------------------------------------
def mutants_rotamers(rotamerlist, bridgelist, adjmatrix_ref):
    qdegamp=180
...
def ejemplo():
    ...
    for i in range(len(bridgelist)):
        for qdeg in range(0, 360, 5):
            ...
            moleculeout.extend([tmp])
    xmol=make_rotamers_check(atoms, 4, bridgelist, adjmatrix)
    ...
    atoms_list_mut=mutants_rotamers(xopt_sort[:nmutants], bridgelist, adjmatrix)
