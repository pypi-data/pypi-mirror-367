...
The default canonical_id used by bazel is the list of URLs specified in bazel
concatenated together using a space character to separate each URL (see
...
The  ``canonical_ids`` key is a list, it is possible to associate multiple
canonical_ids with each source. When buildstream constructs the
repository_cache, it will add a canonical_id marker file for each of the
canonical_ids set. When downloading files, bazel checks if the canonical_id
that it calculates matches any of the canonical_id marker files in the
repository cache created by buildstream. Use multiple canonical IDs if there are
multiple bazel external repository which need to download the same file.
