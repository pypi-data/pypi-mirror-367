...
### `zanshin summary alerts_scans`

**Usage**:

```console
$ zanshin summary alerts_scans [OPTIONS] ORGANIZATION_ID
```
...
### `zanshin summary following_alerts_over_time`

**Usage**:

```console
$ zanshin summary following_alerts_over_time [OPTIONS] ORGANIZATION_ID
```
...
* `--following-ids UUID`: Only summarize alerts from the specified following ids
* `--severities [CRITICAL|HIGH|MEDIUM|LOW|INFO]`: Only summarize alerts with the specified severities
* `--dates TEXT`: Dates to gather the data for (YYYY-MM-DD format)
* `--help`: Show this message and exit.
