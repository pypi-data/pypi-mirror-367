...
def plot_dendrogram(df, save_file=False):
    ...
    plt.xlabel('Systems', fontsize=18, labelpad=20)  # X-axis label
...
def cluster_distributions(
    ...
    """
    Cluster simulation output distributions using entropic OT distances and
    agglomerative clustering, with optional barycenter computation.

    Parameters
    ----------
    dist_file : str
        JSON string containing empirical distributions.
    reg : float, optional
        Entropic regularization parameter for OT distance. Default is 0.5.
    n_clusters : int or None, optional
        Number of clusters. If None, selected via silhouette maximization.
    calculate_barycenter : bool, optional
        Whether to compute barycenters. Default False.
    stop_threshold : float, optional
        Convergence threshold for Sinkhorn iterations. Default is 1e-9.
    num_of_iterations : int, optional
        Maximum Sinkhorn iterations when filling the OT distance matrix.
        Default is 1000.
    plt_dendrogram : bool, optional
        If True, saves and shows a dendrogram plot. Default True.
    sup_barycenter : int, optional
        Number of support points to initialize barycenter computation.
    t0 : float, optional
        Base step size parameter for a-update in barycenter computation.
    theta : float, optional
        Relaxation parameter for X-update in barycenter computation.

    Returns
    -------
    tuple
        json_inputs : str
            JSON with systems and their assigned clusters.
        json_barycenters : str
            JSON containing cluster barycenters (supports and probabilities).
    """
    ...
    for i in range(1, len(df_clusters) + 1):
        ...
        bary_X, bary_a = find_barycenter(
            X, list_of_arrays, b_list, t0, theta, tol=1e-5 * 0.9, max_iter=400
        )
