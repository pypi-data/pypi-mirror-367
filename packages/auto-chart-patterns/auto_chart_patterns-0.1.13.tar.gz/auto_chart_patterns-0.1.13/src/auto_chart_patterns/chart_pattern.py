...
def is_same_height(pivot1: Pivot, pivot2: Pivot, ref_pivots: List[Pivot], flat_ratio: float) -> bool:
    ...
    logger.debug(f"Pivot {pivot1.point.index} ({pivot1.point.price:.6f}) "
                 f"and {pivot2.point.index} ({pivot2.point.price:.6f}), "
                 f"ref_prices: {ref_prices:.6f}, ratio: {ratio:.6f}, same_height: {same_height}")
    return same_height
