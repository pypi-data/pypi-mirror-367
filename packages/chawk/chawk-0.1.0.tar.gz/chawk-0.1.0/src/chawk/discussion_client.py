...
from bbpy.exceptions import BlackboardAPIError, CourseNotFoundError
...
class DiscussionClient:
    ...
    def __get_messages(self, course_id: str, forum_id: str) -> list:
        #get_list = f"{ORG_DOMAIN}/learn/api/public/v1/courses/courseId:{course_id}/discussions/{forum_id}/messages/"
    ...
    def clear_discussion_student_replies(self, course_id: str, role: str = "Student") -> None:
        ...
        for f in all_forums:
            for m in self.__get_messages(course_id, f):
                pass
               # if get_user_course_roles(course_id, self.parent.user.get_local_username_from_id(m.get("author"))) == role:
