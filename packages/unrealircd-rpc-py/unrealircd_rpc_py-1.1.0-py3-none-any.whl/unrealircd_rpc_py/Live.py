...
class Live:
    ...
    async def query(self, method: Union[Literal['log.subscribe', 'log.unsubscribe'], str],
        ...
        if self.req_method == 'unixsocket':
            ...
            self.Logs.error(f'The method ({self.req_method}) requested is not valid! please try unixsocket or websocket')
    ...
    async def __send_websocket(self):
        ...
        try:
            if not self.__check_url(self.url) and not self.url is None:
                ...
                self.EngineError.set_error(code=-1,
                                           message='You must provide the url in this format: https://your.rpcjson.link:port/api'
                                           )
                return None
    ...
    def dict_to_namespace(self, dictionary) -> Union[SimpleNamespace, Any]:
