...
class OnboardingFlow:
    ...
    def choose_ai_provider(self) -> str:
        ...
        provider_table.add_row("1", "🌐 OpenRouter", "Access to multiple models (GPT-4, Claude, etc.) - Requires API key")
        provider_table.add_row("2", "🖥️  Local (Ollama)", "Run models locally on your machine - Free, private")
        provider_table.add_row("3", "🏠 LM Studio", "Local models with LM Studio - Free, private")
        provider_table.add_row("4", "☁️  OpenAI Direct", "Direct OpenAI API - Requires OpenAI API key")
    ...
    def setup_ollama(self) -> Dict[str, Any]:
        ...
        info_panel = Panel(
            Text.assemble(
                ("🖥️  Ollama Setup", "bold bright_green"), "\n\n",
                ("Ollama runs AI models locally on your machine.", "bright_white"), "\n",
                ("Benefits: Free, private, no API keys needed", "green"), "\n\n",
                ("Make sure Ollama is installed and running:", "bright_white"), "\n",
                ("• Install from: ", "bright_white"), ("https://ollama.ai", "bright_blue underline"), "\n",
                ("• Run: ", "bright_white"), ("ollama serve", "bright_yellow"), "\n",
                ("• Pull a model: ", "bright_white"), ("ollama pull llama3", "bright_yellow")
            ),
            border_style="bright_green",
            padding=(1, 2)
        )
        ...
        base_url = Prompt.ask(
            "[bold bright_green]Ollama base URL[/bold bright_green]",
            default="http://localhost:11434"
        )
        
        model = Prompt.ask(
            "[bold bright_green]Model name[/bold bright_green]",
            default="llama3"
        )
    ...
    def run_onboarding(self) -> Dict[str, Any]:
        ...
        if provider == "openrouter":
            ...
            ai_config = self.setup_ollama()
        elif provider == "lmstudio":
            ai_config = self.setup_lmstudio()
        elif provider == "openai":
            ai_config = self.setup_openai()
    ...
    if active_config_file and active_config_file.exists():
        try:
            with open(active_config_file, 'r') as f:
                ...
                has_provider = any(k in config for k in ['openrouter_api_key', 'ollama_base_url', 'lmstudio_base_url', 'openai_api_key'])
