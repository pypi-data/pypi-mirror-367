    ...
    def __init__(self):
        ...
        try:
            ...
            if profile and profile != "default":
                ...
                logger.info(
                    f"Initialized FIS client with profile: {profile}, region: {region}"
                )
                ...
                logger.info(
                    f"Initialized FIS client with config file credentials, region: {region}"
                )
            else:
                # Use default credentials (environment variables, IAM role, etc.)
                self.client = boto3.client("fis", region_name=region)
                logger.info(
                    f"Initialized FIS client with default credentials, region: {region}"
                )
    ...
    def setup_tool_handlers(self):
        ...
        async def handle_list_tools() -> List[types.Tool]:
            ...
            for i, tool in enumerate(tools):
                logger.info(
                    f"Tool {i}: {type(tool)} - {getattr(tool, 'name', 'NO_NAME')}"
                )
        ...
        async def handle_call_tool(
            ...
            try:
                if name == "create_experiment_template":
                    ...
                    return await self.fis_service.get_experiment_template(
                        arguments["id"]
                    )
