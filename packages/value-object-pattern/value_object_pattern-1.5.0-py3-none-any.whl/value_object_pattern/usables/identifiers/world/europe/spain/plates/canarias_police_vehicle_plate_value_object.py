"""
CanariasPoliceVehiclePlateValueObject value object.
"""
...
class CanariasPoliceVehiclePlateValueObject(NotEmptyStringValueObject, TrimmedStringValueObject):
    """
    CanariasPoliceVehiclePlateValueObject value object ensures the provided value is a valid Spanish Canarias police plate.
    ...
    Example:
    ```python
    from value_object_pattern.usables.identifiers.world.europe.spain.plates import CanariasPoliceVehiclePlateValueObject

    plate = CanariasPoliceVehiclePlateValueObject(value='CGPC-1234')

    print(repr(plate))
    # >>> CanariasPoliceVehiclePlateValueObject(value=CGPC1234)
    ```
    """  # noqa: E501  # fmt: skip

    _IDENTIFICATION_REGEX: Pattern[str] = re_compile(pattern=r'([cC][gG][pP][cC])[\s-]?([0-9]{4})')  # noqa: E501  # fmt: skip
    ...
    def _raise_value_is_not_canarias_police_plate(self, value: str) -> NoReturn:
        ...
        """
        raise ValueError(f'CanariasPoliceVehiclePlateValueObject value <<<{value}>>> is not a valid Spanish Canarias police plate.')  # noqa: E501  # fmt: skip
