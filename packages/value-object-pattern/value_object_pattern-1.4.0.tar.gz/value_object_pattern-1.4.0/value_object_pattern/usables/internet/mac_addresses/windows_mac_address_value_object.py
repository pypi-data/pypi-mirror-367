"""
WindowsMacAddressValueObject value object.
"""
...
class WindowsMacAddressValueObject(NotEmptyStringValueObject, TrimmedStringValueObject):
    """
    WindowsMacAddressValueObject value object ensures the provided value is a valid MAC address in Windows format
    (D5-B9-EB-4D-C2-CC).
    ...
    mac = WindowsMacAddressValueObject(value='D5-B9-EB-4D-C2-CC')
    print(repr(mac))
    ...
    @validation(order=0)
    def _ensure_value_is_windows_mac_address(self, value: str) -> None:
        """
        Ensures the value object `value` is a valid Windows MAC address.

        Args:
            value (str): The provided value.

        Raises:
            ValueError: If the `value` is not a valid Windows MAC address.
        """
        if not self._IDENTIFICATION_REGEX.fullmatch(string=value):
            self._raise_value_is_not_windows_mac_address(value=value)

    def _raise_value_is_not_windows_mac_address(self, value: str) -> NoReturn:
        """
        Raises a ValueError if the value object `value` is not a valid Windows MAC address.
        ...
        Raises:
            ValueError: If the `value` is not a valid Windows MAC address.
        """
        raise ValueError(f'WindowsMacAddressValueObject value <<<{value}>>> is not a valid Windows MAC address.')

    def to_raw(self) -> RawMacAddressValueObject:
        """
        Converts the Windows MAC address to raw format (D5B9EB4DC2CC).
    ...
    def to_universal(self) -> UniversalMacAddressValueObject:
        """
        Converts the Windows MAC address to universal format (D5:B9:EB:4D:C2:CC).
    ...
    def to_cisco(self) -> CiscoMacAddressValueObject:
        ...
        raw_value = self.value.replace('-', '')
        cisco_value = f'{raw_value[:4]}.{raw_value[4:8]}.{raw_value[8:]}'

        return CiscoMacAddressValueObject(value=cisco_value)

    def to_space(self) -> SpaceMacAddressValueObject:
        """
        Converts the Windows MAC address to space format (D5 B9 EB 4D C2 CC).
        ...
        space_value = self.value.replace('-', ' ')
