"""
TemporalPrivateIndividualVehiclePlateValueObject value object.
"""
...
class TemporalPrivateIndividualVehiclePlateValueObject(NotEmptyStringValueObject, TrimmedStringValueObject):
    """
    TemporalPrivateIndividualVehiclePlateValueObject value object ensures the provided value is a valid Spanish temporal
    ...
    Example:
    ```python
    from value_object_pattern.usables.identifiers.world.europe.spain.plates import TemporalPrivateIndividualVehiclePlateValueObject

    plate = TemporalPrivateIndividualVehiclePlateValueObject(value='T-1234-BCD')

    print(repr(plate))
    # >>> TemporalPrivateIndividualVehiclePlateValueObject(value=T1234BCD)
    ```
    """  # noqa: E501  # fmt: skip

    _IDENTIFICATION_REGEX: Pattern[str] = re_compile(pattern=r'([tT])[-\s]?([0-9]{4})[-\s]?([bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ]{3})')  # noqa: E501  # fmt: skip
    ...
    def _raise_value_is_not_temporal_private_individual_plate(self, value: str) -> NoReturn:
        ...
        """
        raise ValueError(f'TemporalPrivateIndividualVehiclePlateValueObject value <<<{value}>>> is not a valid Spanish temporal private individual plate.')  # noqa: E501  # fmt: skip
