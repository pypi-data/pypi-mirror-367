"""
NationalPoliceVehiclePlateValueObject value object.
"""
...
class NationalPoliceVehiclePlateValueObject(NotEmptyStringValueObject, TrimmedStringValueObject):
    """
    NationalPoliceVehiclePlateValueObject value object ensures the provided value is a valid Spanish national police plate.
    ...
    plate = NationalPoliceVehiclePlateValueObject(value='CNP-1234-AA')

    print(repr(plate))
    # >>> NationalPoliceVehiclePlateValueObject(value=CNP1234AA)
    ```
    """  # noqa: E501  # fmt: skip

    _IDENTIFICATION_REGEX: Pattern[str] = re_compile(pattern=r'([cC][nN][pP])[\s-]?([0-9]{4})[\s-]?([a-zA-Z]{2})')  # noqa: E501  # fmt: skip
    ...
    def _raise_value_is_not_police_plate(self, value: str) -> NoReturn:
        ...
        """
        raise ValueError(
            f'NationalPoliceVehiclePlateValueObject value <<<{value}>>> is not a valid Spanish police plate.'
        )
