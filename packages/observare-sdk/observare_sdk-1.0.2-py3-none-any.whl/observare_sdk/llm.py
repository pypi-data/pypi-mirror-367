        ...
        **kwargs
        ...
        if OBSERVARE_MODULES_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to initialize safety engines: {e}")
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Log original and processed messages to show redaction (debug only)
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            # Log original and processed messages to show redaction (debug only)
            logger.debug("🔍 Original messages (async):")
            for i, msg in enumerate(messages):
                logger.debug(f"   {i+1}. [{type(msg).__name__}] {msg.content}")
            ...
            if hasattr(self.underlying_llm, '_stream'):
                ...
                if collected_content and self.hallucination_engine:
                    try:
                        ...
                        logger.warning(f"Hallucination detection failed in streaming: {e}")
            ...
            if hasattr(self.underlying_llm, '_astream'):
                ...
                if collected_content and self.hallucination_engine:
                    try:
                        ...
                        logger.warning(f"Async hallucination detection failed in streaming: {e}")
    ...
    def _preprocess_messages(
        ...
        for message in messages:
            try:
                ...
                # Log PII detection telemetry
                if self.telemetry_handler and redaction_result.total_redactions > 0:
                    self._log_pii_event(redaction_result, message.content, redaction_result.redacted_text, correlation_id)
                    
            except Exception as e:
                logger.warning(f"PII redaction failed for message: {e}")
                if getattr(self, 'fail_safe', True):
    ...
    async def _apostprocess_response(
        ...
        try:
            ...
            if chat_result.generations and len(chat_result.generations) > 0:
                ...
                # Log hallucination detection telemetry
                if self.telemetry_handler:
                    self._log_hallucination_event(detection_result)
    ...
    async def _detect_and_log_hallucination(self, prompt: str, response: str):
        ...
        try:
            ...
            if detection_result.hallucination_probability > 0.6:
                logger.warning(f"⚠️  HIGH hallucination risk detected! (probability: {detection_result.hallucination_probability:.3f})")
            elif detection_result.hallucination_probability > 0.2:
                logger.warning(f"⚠️  MEDIUM hallucination risk detected (probability: {detection_result.hallucination_probability:.3f})")
            else:
            ...
            logger.warning(f"Background hallucination detection failed: {e}")
...
Example: {{"hallucination_probability": 0.1, "confidence": "high", "reasoning": "Response is factual and appropriate", "risk_factors": []}}"""
            ...
            logger.warning(f"Content-based hallucination check failed: {e}")
            return None
    ...
    def _log_pii_event(self, pii_result: PIIRedactionResult, original_content: str = None, redacted_content: str = None, correlation_id: str = None):
        ...
        try:
            ...
            # Add content snippets for correlation (truncated for privacy)
            if original_content:
                event_data["original_content_preview"] = original_content[:100] + "..." if len(original_content) > 100 else original_content
            if redacted_content:
            ...
            logger.warning(f"Failed to log PII event: {e}")
    ...
    def _log_hallucination_event(self, detection_result: HallucinationDetection, prompt: str = None, response: str = None, correlation_id: str = None):
        ...
        try:
            ...
            logger.warning(f"Failed to log hallucination event: {e}")
    ...
    def _extract_and_emit_token_usage(self, chat_result: ChatResult, correlation_id: str, processing_time: float):
        ...
        try:
            ...
            logger.warning(f"Failed to extract and emit token usage: {e}")
    ...
    def _emit_llm_conversation_event(self, messages: List[BaseMessage], chat_result: ChatResult, correlation_id: str, processing_time: float):
        ...
        try:
            ...
            logger.warning(f"Failed to emit LLM conversation event: {e}")
    ...
    def _reinitialize_engines(self):
        ...
        try:
            ...
            logger.warning(f"Failed to reinitialize engines: {e}")
            if not self.fail_safe:
                raise
