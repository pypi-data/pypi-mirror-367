...
class ObservareChat(BaseChatModel):
    ...
    def _preprocess_messages(
        ...
        for message in messages:
            try:
                # Handle case where message might be a string representation of objects
                if isinstance(message, str):
                    # Skip processing string representations of message objects
                    if message.startswith('[') and ('MessageChunk' in message or 'Message(' in message):
                        logger.debug("Skipping string representation of message objects")
                        continue
                    message_content = message
                elif hasattr(message, 'content'):
                    message_content = message.content or ""
                else:
                    # Fallback - convert to string but avoid processing object representations
                    message_str = str(message)
                    if 'MessageChunk' in message_str or 'Message(' in message_str:
                        logger.debug("Skipping object representation in PII processing")
                        continue
                ...
                redaction_result = self.pii_engine.redact_text(message_content)
                pii_results.append(redaction_result)
                
                # Create new message with redacted content (only for actual message objects)
                if isinstance(message, str):
                    # For string messages, create a simple HumanMessage
                    processed_message = HumanMessage(content=redaction_result.redacted_text)
                    ...
                    try:
                        ...
                        # Final fallback - create HumanMessage
                        processed_message = HumanMessage(content=redaction_result.redacted_text)
...
Example: {{"hallucination_probability": 0.1, "confidence": "high", "reasoning": "Response is factual and appropriate", "risk_factors": []}}"""
    ...
    def _log_pii_event(self, pii_result: PIIRedactionResult, redacted_content: str = None, correlation_id: str = None):
        ...
        try:
            ...
            # Add full redacted content (NEVER original content for security)
            if redacted_content:
                event_data["redacted_content_full"] = redacted_content
    ...
    def _log_hallucination_event(self, detection_result: HallucinationDetection, prompt: str = None, response: str = None, correlation_id: str = None):
        ...
        try:
            ...
            # Add full content for analysis
            if prompt:
                event_data["prompt_full"] = prompt
            if response:
                event_data["response_full"] = response
