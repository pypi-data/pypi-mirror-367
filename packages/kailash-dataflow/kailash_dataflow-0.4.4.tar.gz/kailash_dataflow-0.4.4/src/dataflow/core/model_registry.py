...
class ModelRegistry:
    ...
    def discover_models(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not self._initialized:
            logger.debug("Registry not initialized for model discovery, initializing now...")
            if not self.initialize():
                logger.warning("Failed to initialize registry for model discovery, returning empty results")
                return {}
    ...
    def _model_exists_with_checksum(self, checksum: str) -> bool:
        ...
        try:
            ...
            if results.get("check_checksum", {}).get("error"):
                logger.warning(f"Error checking checksum: {results['check_checksum']['error']}")
                return False
