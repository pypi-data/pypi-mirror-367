            ...
            def run(self, **kwargs) -> Dict[str, Any]:
                ...
                if operation == "create":
                    ...
                    try:
                        ...
                        for field_name, field_info in model_fields.items():
                            ...
                            if field_name in kwargs:
                                ...
                                raise ValueError(
                                    ...
                                    f"Expected fields: {[f for f in model_fields.keys() if f not in ['id', 'created_at', 'updated_at']]}"
                                )
                            else:
                                # Optional field without default - use None
                                complete_params[field_name] = None

                        # Now parameters match SQL placeholders exactly
                        ...
                        if "could not determine data type of parameter" in error_msg:
                            import re
                            ...
                            error_msg = (
                                ...
                                f"SQL expects {len(expected_fields)} parameters but received data for {len(provided_fields)} fields.\n"
                                f"Expected fields: {expected_fields}\n"
                                f"Provided fields: {provided_fields}\n"
                                f"Missing fields: {list(set(expected_fields) - set(provided_fields))}\n"
                                f"Note: DataFlow now auto-completes fields with defaults, but this error suggests a deeper issue."
                            )
                    ...
                    updates = {
                        ...
                        if k not in ["record_id", "id", "database_url"]
