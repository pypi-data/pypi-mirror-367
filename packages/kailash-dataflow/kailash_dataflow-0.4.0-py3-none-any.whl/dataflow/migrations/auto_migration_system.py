...
class AutoMigrationSystem:
    ...
    async def _acquire_migration_lock(self) -> bool:
        """Acquire migration lock based on database type."""
        if self.dialect == "sqlite":
    ...
    async def _release_migration_lock(self) -> None:
        """Release migration lock based on database type."""
        if self.dialect == "sqlite":
    ...
    async def _acquire_sqlite_migration_lock(self) -> bool:
        ...
        while retry_count < max_retries:
            try:
                ...
                results, _ = self.runtime.execute(workflow.build())
                if "acquire_lock" not in results or results["acquire_lock"].get("error"):
                    ...
                    if "UNIQUE constraint failed" in error_msg:
                        if retry_count < max_retries - 1:
                            ...
                            logger.error("Failed to acquire SQLite lock after maximum retries")
                            return False
                    else:
                        logger.error(
                ...
                if retry_count < max_retries - 1:
                    logger.warning(f"Error acquiring SQLite lock, retrying... (attempt {retry_count + 1}/{max_retries}): {e}")
                    await asyncio.sleep(1)
                    retry_count += 1
                    continue
                logger.error(f"Failed to acquire SQLite migration lock after retries: {e}")
                raise
