...
class DataFlow:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._enable_model_persistence:
            try:
                if self._model_registry.initialize():
                    logger.debug(
                        "Model registry initialized successfully during DataFlow startup"
                    )
                else:
                    logger.warning(
                        "Model registry initialization failed during startup, will retry per model"
                    )
            except Exception as e:
                logger.warning(
                    f"Model registry initialization error during startup: {e}, will retry per model"
                )
        ...
        # Skip migration system initialization if:
        # 1. auto_migrate=False (user explicitly disabled auto-migration)
        # 2. OR (existing_schema_mode=True AND auto_migrate=False) - legacy compatibility
        # 3. OR migrations are disabled via environment variable
        if (
            migration_enabled
            and self._auto_migrate  # Only initialize if auto-migration is enabled
            and not (
                self._existing_schema_mode and not self._auto_migrate
            )  # Legacy: skip for existing schema without auto-migrate
    ...
    def _initialize_migration_system(self):
        ...
        try:
            ...
            # Initialize AutoMigrationSystem with async workflow pattern
