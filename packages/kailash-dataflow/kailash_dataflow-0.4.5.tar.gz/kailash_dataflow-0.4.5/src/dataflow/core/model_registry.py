...
class ModelRegistry:
    ...
    def register_model(
        ...
        try:
            ...
            if not self._initialized:
                logger.debug(
                    f"Registry not initialized for model {model_name}, initializing now..."
                )
                if not self.initialize():
                    logger.warning(
                        f"Failed to initialize registry for model {model_name}, skipping registration"
                    )
                    return (
                        True  # Return True to not block model registration in DataFlow
                    )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to check existing checksum for {model_name}: {checksum_error}"
                )
    ...
    def discover_models(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not self._initialized:
            logger.debug(
                "Registry not initialized for model discovery, initializing now..."
            )
            if not self.initialize():
                logger.warning(
                    "Failed to initialize registry for model discovery, returning empty results"
                )
                return {}
    ...
    def _model_exists_with_checksum(self, checksum: str) -> bool:
        ...
        try:
            ...
            if results.get("check_checksum", {}).get("error"):
                logger.warning(
                    f"Error checking checksum: {results['check_checksum']['error']}"
                )
                return False
            ...
            logger.warning(f"Exception during checksum check: {e}")
            return False
