...
from typing import Dict, Union, Optional, Any, Self, Type, TypeVar
...
class TruoraCheck(BaseModel):
    ...
    # These are optional
    name_score: Optional[int] = Field(default=0, description="Score for name matching")
    id_score: Optional[int] = Field(default=-1, description="Score for ID matching")
    score: Optional[int] = Field(default=-1, description="Overall check score")
...
class TruoraCustomSchema(BaseModel):
    type: str = Field(..., description="CustomTypeName")
    country: CountryCode = Field(
        ...,
        description="""
            Country code - 
    ...
    dataset_alert_in_media: float = Field(default=0, description="dataset_alert_in_media")
    ...
    dataset_document_validation: float = Field(default=0, description="dataset_document_validation")
...
class CreditScoreData(BaseModel):
    ...
    def calculate_totals(self) -> Self:
        ...
        self.saldo_total = self.saldo_rotativo + self.saldo_consumo
        self.productos_totales = self.productos_al_dia + self.productos_castigados
        return self
