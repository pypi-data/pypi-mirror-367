...
class JobRequestScheduling(JobRequestSchedulingBase):
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False) -> Self:
        """Anonymized copy."""
...
class JobBase(BaseSchema):
    ...
    @model_validator(mode="after")
    def _init_restart_info(self) -> Self:
    ...
    def replace_event_times_attrs(self, **kwargs) -> Self:
    ...
    def replace_request_resources_attrs(self, **kwargs) -> Self:
    ...
    def replace_request_scheduling_attrs(self, **kwargs) -> Self:
    ...
    def replace_request_extra_attrs(self, **kwargs) -> Self:
    ...
    def replace_state_scheduling(self, new_scheduling: JobStateScheduling) -> Self:
    ...
    def replace_state_scheduling_attrs(self, **kwargs) -> Self:
    ...
    def add_halt_info(self, halt_info: HaltInfo) -> Self:
        """Add halt info."""
        return self.replace_state_scheduling_attrs(halt_events=self.state.scheduling.halt_events + [halt_info])

    def replace_state_final_usage_statistics(self, final_usage_statistics: GPULabUsageStatistics) -> Self:
    ...
    def replace_state_waste_review(self, waste_review: WasteReview | None) -> Self:
    ...
    def replace_state_resources(self, new_resources: JobStateResources) -> Self:
    ...
    def replace_state_resources_fields(self, **kwargs) -> Self:
...
class Job(JobBase):
    ...
    def replace_owner_attrs(self, **kwargs) -> Self:
        """Update owner in job."""
        return self.model_copy(
            update={"owner": self.owner.model_copy(update=kwargs) if self.owner else JobOwner(**kwargs)}
        )

    def sanitized_copy(self, logged_in=True, same_project=False) -> Self:
        """Copy with removed confidential data.
