...
class MixedPrecisionTest:
        ...
        if int8_hardware_supported:
            ...
            if not int8_result.get("runtime_supported", False):
                ...
                if pytorch_int8_result.get("supported", False):
                    ...
                    int8_result["note"] = f"TensorRT INT8 not supported in this version, using PyTorch INT8"
                else:
    ...
    def _test_int8_with_tensorrt(self, size: int, duration: float) -> Dict[str, any]:
        ...
        try:
            ...
            input_a = network.add_input("input_a", trt.DataType.INT8, (1, size, size))
            input_b = network.add_input("input_b", trt.DataType.INT8, (1, size, size))
            ...
            # Simple INT8 test - just element-wise operations
            # Add element-wise multiplication (simpler than matrix multiply)
            mul = network.add_elementwise(
                input_a, input_b, trt.ElementWiseOperation.PROD
            )
            
            output = mul.get_output(0)
            try:
            ...
            elif "Matrix Multiply" in error_msg or "No supported formats" in error_msg:
                error_msg = "TensorRT INT8 matrix multiplication not supported in this version."
                solution = "Using PyTorch INT8 fallback instead"
