...
class MixedPrecisionTest:
    ...
    def run_test(self, duration: float = 10) -> Dict[str, any]:
        ...
        # INT8 test with PyTorch (more reliable than TensorRT)
        if int8_hardware_supported:
            int8_result = self._test_precision(torch.int8, size, duration / 4)
            int8_result["hardware_supported"] = True
            int8_result["runtime_supported"] = int8_result.get("supported", False)
            int8_result["method"] = "pytorch"
            
            if int8_result.get("supported", False):
                int8_result["note"] = "Using PyTorch INT8 (TensorRT too complex for this use case)"
            else:
                int8_result["note"] = "PyTorch INT8 not available"
