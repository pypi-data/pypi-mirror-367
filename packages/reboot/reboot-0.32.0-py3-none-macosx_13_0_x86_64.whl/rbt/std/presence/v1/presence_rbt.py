                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = PresenceBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class PresenceAlphaServicer(PresenceBaseServicer):
    ...
    _instances: dict[str, PresenceAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = PresenceAlphaServicer._instances
    ...
    def state(self) -> rbt.std.presence.v1.presence_pb2.Presence:
        state = PresenceAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = PresenceAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Subscribe(
        ...
        # `_state is None`.
        assert PresenceAlphaServicer._state.get() is None
        PresenceAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Subscribe(context, request)
        finally:
            PresenceAlphaServicer._state.set(None)
    ...
    async def _Watch(
        ...
        PresenceAlphaServicer._workflow.set(True)
        try:
            return await self._instance(
                context.state_id
            ).Watch(context, request)
        finally:
            PresenceAlphaServicer._workflow.set(False)
...
class PresenceAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = PresenceAlphaServicer
...
class Presence:
    ...
    alpha: IMPORT_typing.TypeAlias = PresenceAlpha
...
class PresenceServicerNodeAdaptor(Presence.Servicer):
