                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = MousePositionBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class MousePositionAlphaServicer(MousePositionBaseServicer):
    ...
    _instances: dict[str, MousePositionAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = MousePositionAlphaServicer._instances
    ...
    def state(self) -> rbt.std.presence.mouse_tracker.v1.mouse_position_pb2.MousePosition:
        state = MousePositionAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = MousePositionAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Update(
        ...
        # `_state is None`.
        assert MousePositionAlphaServicer._state.get() is None
        MousePositionAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Update(context, request)
        finally:
            MousePositionAlphaServicer._state.set(None)
...
class MousePositionAlpha:

    Servicer: IMPORT_typing.TypeAlias = MousePositionAlphaServicer
...
class MousePosition:
    ...
    alpha: IMPORT_typing.TypeAlias = MousePositionAlpha
...
class MousePositionServicerNodeAdaptor(MousePosition.Servicer):
