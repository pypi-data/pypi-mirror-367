                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = MessageBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class MessageAlphaServicer(MessageBaseServicer):
    ...
    _instances: dict[str, MessageAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = MessageAlphaServicer._instances
    ...
    def state(self) -> rbt.thirdparty.mailgun.v1.mailgun_pb2.Message:
        state = MessageAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = MessageAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Send(
        ...
        # `_state is None`.
        assert MessageAlphaServicer._state.get() is None
        MessageAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Send(context, request)
        finally:
            MessageAlphaServicer._state.set(None)
    ...
    async def _SendWorkflow(
        ...
        MessageAlphaServicer._workflow.set(True)
        try:
            return await self._instance(
                context.state_id
            ).SendWorkflow(context, request)
        finally:
            MessageAlphaServicer._workflow.set(False)
...
class MessageAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = MessageAlphaServicer


class Message:

    alpha: IMPORT_typing.TypeAlias = MessageAlpha
