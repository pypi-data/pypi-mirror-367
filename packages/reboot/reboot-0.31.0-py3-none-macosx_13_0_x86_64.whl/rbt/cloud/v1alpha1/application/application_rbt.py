...
IMPORT_reboot_versioning.check_generated_code_compatible("0.31.0")
                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = ApplicationBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class ApplicationAlphaServicer(ApplicationBaseServicer):
    ...
    _instances: dict[str, ApplicationAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = ApplicationAlphaServicer._instances
    ...
    def state(self) -> rbt.cloud.v1alpha1.application.application_pb2.Application:
        state = ApplicationAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = ApplicationAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Create(
        ...
        # `_state is None`.
        assert ApplicationAlphaServicer._state.get() is None
        ApplicationAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Create(context, request)
        finally:
            ApplicationAlphaServicer._state.set(None)
    ...
    async def _UpWorkflow(
        ...
        ApplicationAlphaServicer._workflow.set(True)
        try:
            return await self._instance(
                context.state_id
            ).UpWorkflow(context, request)
        finally:
            ApplicationAlphaServicer._workflow.set(False)
    ...
    async def _RevisionStatus(
        ...
        try:
            return await self._instance(
                context.state_id
            ).RevisionStatus(context, request)
        finally:
            ApplicationAlphaServicer._state.set(None)
    ...
    async def _ActiveRevisions(
        ...
        try:
            return await self._instance(
                context.state_id
            ).ActiveRevisions(context, request)
        finally:
            ApplicationAlphaServicer._state.set(None)
...
class ApplicationAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = ApplicationAlphaServicer


class Application:

    alpha: IMPORT_typing.TypeAlias = ApplicationAlpha
    ...
    class WeakReference(IMPORT_typing.Generic[Application_ScheduleTypeVar]):
        ...
        def spawn(
            ...
            *,
            ...
            if (
                ...
                isinstance(context, IMPORT_reboot_aio_contexts.WorkflowContext)
            ):
                return (
                    self.per_iteration() if context.within_loop()
                    else self.per_workflow()
        ...
        async def PushInfo(
            ...
            # calls are `.always().
            if (
                isinstance(__context__, IMPORT_reboot_aio_contexts.WorkflowContext) and
                (__options__ is None or __options__.idempotency is None)
            ):
                return await (
