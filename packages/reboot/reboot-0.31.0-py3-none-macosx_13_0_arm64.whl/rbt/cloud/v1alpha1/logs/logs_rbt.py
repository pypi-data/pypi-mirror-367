                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = ApplicationLogsBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class ApplicationLogsAlphaServicer(ApplicationLogsBaseServicer):
    ...
    _instances: dict[str, ApplicationLogsAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = ApplicationLogsAlphaServicer._instances
    ...
    def state(self) -> rbt.cloud.v1alpha1.logs.logs_pb2.ApplicationLogs:
        state = ApplicationLogsAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = ApplicationLogsAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Ensure(
        ...
        # `_state is None`.
        assert ApplicationLogsAlphaServicer._state.get() is None
        ApplicationLogsAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Ensure(context, request)
        finally:
            ApplicationLogsAlphaServicer._state.set(None)
    ...
    async def _RegisterConsensus(
        ...
        try:
            return await self._instance(
                context.state_id
            ).RegisterConsensus(context, request)
        finally:
            ApplicationLogsAlphaServicer._state.set(None)
...
class ApplicationLogsAlpha:

    Servicer: IMPORT_typing.TypeAlias = ApplicationLogsAlphaServicer


class ApplicationLogs:

    alpha: IMPORT_typing.TypeAlias = ApplicationLogsAlpha
