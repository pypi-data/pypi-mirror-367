                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = APIKeyBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class APIKeyAlphaServicer(APIKeyBaseServicer):
    ...
    _instances: dict[str, APIKeyAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = APIKeyAlphaServicer._instances
    ...
    def state(self) -> rbt.cloud.v1alpha1.auth.auth_pb2.APIKey:
        state = APIKeyAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = APIKeyAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Create(
        ...
        # `_state is None`.
        assert APIKeyAlphaServicer._state.get() is None
        APIKeyAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Create(context, request)
        finally:
            APIKeyAlphaServicer._state.set(None)
...
class APIKeyAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = APIKeyAlphaServicer


class APIKey:

    alpha: IMPORT_typing.TypeAlias = APIKeyAlpha
