                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = MemoizeBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class MemoizeAlphaServicer(MemoizeBaseServicer):
    ...
    _instances: dict[str, MemoizeAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = MemoizeAlphaServicer._instances
    ...
    def state(self) -> rebootdev.memoize.v1.memoize_pb2.Memoize:
        state = MemoizeAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = MemoizeAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Reset(
        ...
        # `_state is None`.
        assert MemoizeAlphaServicer._state.get() is None
        MemoizeAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Reset(context, request)
        finally:
            MemoizeAlphaServicer._state.set(None)
...
class MemoizeAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = MemoizeAlphaServicer


class Memoize:

    alpha: IMPORT_typing.TypeAlias = MemoizeAlpha
