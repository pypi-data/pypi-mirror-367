                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = SubscriberBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class SubscriberAlphaServicer(SubscriberBaseServicer):
    ...
    _instances: dict[str, SubscriberAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = SubscriberAlphaServicer._instances
    ...
    def state(self) -> rbt.std.presence.subscriber.v1.subscriber_pb2.Subscriber:
        state = SubscriberAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = SubscriberAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Create(
        ...
        # `_state is None`.
        assert SubscriberAlphaServicer._state.get() is None
        SubscriberAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Create(context, request)
        finally:
            SubscriberAlphaServicer._state.set(None)
    ...
    async def _WaitForDisconnect(
        ...
        SubscriberAlphaServicer._workflow.set(True)
        try:
            return await self._instance(
                context.state_id
            ).WaitForDisconnect(context, request)
        finally:
            SubscriberAlphaServicer._workflow.set(False)
...
class SubscriberAlpha:

    Servicer: IMPORT_typing.TypeAlias = SubscriberAlphaServicer
...
class Subscriber:
    ...
    alpha: IMPORT_typing.TypeAlias = SubscriberAlpha
...
class SubscriberServicerNodeAdaptor(Subscriber.Servicer):
