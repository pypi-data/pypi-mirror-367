                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = SecretBaseServicer.Effects(
                                    state=state,
...
class SecretAlphaServicer(SecretBaseServicer):
    ...
    _instances: dict[str, SecretAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = SecretAlphaServicer._instances
    ...
    def state(self) -> rbt.cloud.v1alpha1.secrets.secrets_pb2.Secret:
        state = SecretAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = SecretAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Write(
        ...
        SecretAlphaServicer._workflow.set(True)
        try:
            return await self._instance(
                context.state_id
            ).Write(context, request)
        finally:
            SecretAlphaServicer._workflow.set(False)
    ...
    async def _Read(
        ...
        # `_state is None`.
        assert SecretAlphaServicer._state.get() is None
        SecretAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Read(context, request)
        finally:
            SecretAlphaServicer._state.set(None)
...
class SecretAlpha:
    ...
    Servicer: IMPORT_typing.TypeAlias = SecretAlphaServicer


class Secret:

    alpha: IMPORT_typing.TypeAlias = SecretAlpha
