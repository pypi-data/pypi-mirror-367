                    ...
                    with servicer._middleware.use_context(
                        ...
                        async with servicer._middleware._state_manager.transactionally(
                            ...
                            async with servicer._middleware._state_manager.writer(
                                ...
                                effects = SortedMapBaseServicer.Effects(
                                    state=state,
                                    tasks=context._tasks,
                                    response=IMPORT_google_protobuf_wrappers_pb2.BytesValue(
...
class SortedMapAlphaServicer(SortedMapBaseServicer):
    ...
    _instances: dict[str, SortedMapAlphaServicer] = {}

    def _instance(self, state_id: str):
        instances = SortedMapAlphaServicer._instances
    ...
    def state(self) -> rbt.std.collections.v1.sorted_map_pb2.SortedMap:
        state = SortedMapAlphaServicer._state.get()
        if state is None:
            raise RuntimeError(
                "`state` property is only relevant within a `Servicer` method"
            )
        workflow = SortedMapAlphaServicer._workflow.get()
        if workflow:
            raise RuntimeError(
    ...
    async def _Insert(
        ...
        # `_state is None`.
        assert SortedMapAlphaServicer._state.get() is None
        SortedMapAlphaServicer._state.set(state)
        try:
            return await self._instance(
                context.state_id
            ).Insert(context, request)
        finally:
            SortedMapAlphaServicer._state.set(None)
...
class SortedMapAlpha:

    Servicer: IMPORT_typing.TypeAlias = SortedMapAlphaServicer


class SortedMap:

    alpha: IMPORT_typing.TypeAlias = SortedMapAlpha
