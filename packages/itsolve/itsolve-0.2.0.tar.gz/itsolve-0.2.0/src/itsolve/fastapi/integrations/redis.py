...
class RedisClient:
    def __init__(self, settings: RedisSettings | dict[str, Any]) -> None:
        if isinstance(settings, dict):
            settings = RedisSettings.model_validate(settings)
    ...
    async def hset(
        ...
        """
        response = (
            pipe.hset(*args, **kwargs)
            if pipe
            else self.redis.hset(*args, **kwargs)
        )
