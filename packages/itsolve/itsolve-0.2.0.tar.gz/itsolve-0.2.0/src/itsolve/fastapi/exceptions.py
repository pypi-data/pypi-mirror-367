...
class GlobalAppException(AppException):
    STATUS_CODE: int = status.HTTP_400_BAD_REQUEST
    ERROR: str
    DESCRIPTION: str | None = None
    ...
    def __init__(
        self, message: str | None = None, ctx: dict | None = None
    ) -> None:
        super().__init__(
...
class DeclareModuleAppException(GlobalAppException):
    MODULE: str
...
class NotFoundInstanceException(GlobalAppException):
    STATUS_CODE = status.HTTP_404_NOT_FOUND
    ERROR = "not_found"
    DESCRIPTION = "Server can not find such instance"


class AlreadyExistsInstanceException(GlobalAppException):
    STATUS_CODE = status.HTTP_409_CONFLICT
    ERROR = "already_exists"
    DESCRIPTION = "Such instance exists"


class ValidationDataInstanceException(GlobalAppException):
    STATUS_CODE = status.HTTP_422_UNPROCESSABLE_ENTITY
    ERROR = "validation_data"
    DESCRIPTION = "Wrong data sent"


class PermissionsDeniedException(GlobalAppException):
...
class UnknownException(GlobalAppException):
    STATUS_CODE = status.HTTP_400_BAD_REQUEST
    ERROR = "unknown"
    DESCRIPTION = "Something went wrong"

    def __init__(
        self, module: str | None = None, ctx: dict | None = None
    ) -> None:
        self.ERROR = f"{module}.{self.ERROR}" if module else self.ERROR
        super().__init__(ctx=ctx)


class ResourceException(GlobalAppException):
    STATUS_CODE: int = status.HTTP_400_BAD_REQUEST
...
class ResourceCreateException(ResourceException):
    ERROR: str = "create_error"
    DESCRIPTION = "Creation failed"


class ResourceRetrieveException(ResourceException):
    ERROR: str = "retrieve_error"
    DESCRIPTION = "Error occurred while retrieving"


class ResourceUpdateException(ResourceException):
    ERROR: str = "update_error"
    DESCRIPTION = "Error occurred while updating"


class ResourceDeleteException(ResourceException):
...
def uncaught_error(
    exc: type[GlobalAppException] | GlobalAppException, msg: str | None = None
) -> Callable:
    def decorator(func: Callable) -> Callable:
        async def wrapper(obj: Any, *args: Any, **kwargs: Any) -> Any:
            try:
                ...
                logger.warning(
                    f"{error_msg} ({e}) | ctx: {
                        json.dumps(ctx_info, default=str)
                    }"
                )
