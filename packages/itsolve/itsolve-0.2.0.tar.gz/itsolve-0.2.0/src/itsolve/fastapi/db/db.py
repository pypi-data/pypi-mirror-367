...
class Database:
    def __init__(self, settings: DatabaseSettings | dict[str, Any]) -> None:
        if isinstance(settings, dict):
            settings = DatabaseSettings.model_validate(settings)
...
class TestDatabase(Database):
    ...
    async def async_session_factory(self) -> AsyncIterator[AsyncSession]:
        async with self._session_factory() as session:
            ...
            if mode == "test":
                pytest_xdist_worker = os.environ.get(
    ...
    @contextlib.asynccontextmanager
    async def lifespan_database(self) -> AsyncIterator[None]:
        async with self._engine.begin() as conn:
