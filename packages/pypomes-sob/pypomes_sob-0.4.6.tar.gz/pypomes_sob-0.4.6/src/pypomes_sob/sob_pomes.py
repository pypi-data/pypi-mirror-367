...
from .sob_config import (
    ...
    sob_attrs_map, sob_attrs_unique, sob_cls_references
)
...
class PySob:
    ...
    def load_references(self,
        ...
        cls_name: str = f"{self.__class__.__module__}.{self.__class__.__qualname__}"
        tbl_name: str = sob_db_specs[cls_name][0]
        for name in (sob_cls_references.get(cls_name) or []):
    ...
    def initialize(db_specs: tuple[Type[StrEnum], int | str] |
        ...
        if cls:
            ...
            # register the fully-qualified names of subclasses referred to by the current class
            if sob_references:
                ref_names: list[str] = [f"{c.__module__}.{c.__qualname__}" for c in sob_references]
                sob_cls_references.update({cls_name: ref_names})
    ...
    def retrieve(errors: list[str] = None,
        ...
        if not errors:
            ...
            if not errors:
                ...
                for rec in recs:
                    data: dict[str, Any] = {}
                    for inx, attr in sob_col_names.get(cls_name):
                        data[attr] = rec[inx]
                    sob: Type[Sob] = cls()
                    sob.set(data=data)
                    sob._is_new = False
                    if load_references:
                        sob.load_references(errors=errors,
                                            db_engine=db_engine,
                                            db_conn=db_conn)
                    if errors:
                        break
