...
async def get_estimates(
    ...
    Use this to check prices before booking immediate rides or scheduling
    future rides. The response indicates which vehicle types support scheduling.
    
    IMPORTANT: "Courier" is for packages/items only - NOT for passengers.
    Only suggest Courier for deliveries, not for human transportation.
    ...
    for estimate in result:
        ...
        duration_seconds = estimate.get("duration_estimate_seconds")
        distance = estimate.get("distance_estimate")
        ...
        if duration_seconds:
            ...
            duration_text = f" • {duration_mins}min trip"
        
        # Add distance if available
        distance_text = ""
        if distance:
            distance_text = f" • {distance:.1f}mi"
        
        # Add capacity warning for Courier
        capacity_text = ""
        if capacity == 0:
            capacity_text = " (📦 packages only)"
        
        surge_text = f" (🔥 {surge}x surge)" if surge > 1.0 else ""
        estimates_text.append(f"{available} {display_name}: {price} • ETA: {eta}min{duration_text}{distance_text}{surge_text}{capacity_text}")
