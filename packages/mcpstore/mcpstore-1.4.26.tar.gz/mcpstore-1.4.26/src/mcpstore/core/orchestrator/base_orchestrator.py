...
class MCPOrchestrator(
    ...
    async def setup(self, auto_register_on_startup: bool = True):
        """初始化编排器资源

        Args:
            auto_register_on_startup: 是否在启动时自动注册mcp.json中的服务，默认为True
        """
        # 检查是否已经初始化
        ...
        try:
            ...
            await self._setup_sync_manager(auto_register_on_startup=auto_register_on_startup)
    ...
    async def _setup_sync_manager(self, auto_register_on_startup: bool = True):
        """设置统一同步管理器

        Args:
            auto_register_on_startup: 是否在启动时自动注册mcp.json中的服务
        """
        try:
            ...
            if not self.standalone_config_manager:
                ...
                await self.sync_manager.start(auto_register_on_startup=auto_register_on_startup)
