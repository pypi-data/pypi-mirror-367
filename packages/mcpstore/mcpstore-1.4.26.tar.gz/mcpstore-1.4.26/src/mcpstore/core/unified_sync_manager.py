...
class UnifiedMCPSyncManager:
    ...
    async def start(self, auto_register_on_startup: bool = True):
        """启动同步管理器
        ...
        try:
            ...
            # 🔧 根据auto_register_on_startup参数决定是否执行启动时同步
            if auto_register_on_startup:
                logger.info("Auto-register on startup enabled: executing initial sync from mcp.json")
                await self.sync_global_agent_store_from_mcp_json()
            else:
                logger.info("Auto-register on startup disabled: skipping initial sync from mcp.json")
