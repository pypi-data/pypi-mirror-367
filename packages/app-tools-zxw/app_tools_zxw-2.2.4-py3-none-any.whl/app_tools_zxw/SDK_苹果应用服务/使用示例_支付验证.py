...
from pems.config_苹果支付 import (
    私钥文件路径, 密钥ID, 发行者ID, 应用包ID, 是否沙盒环境, 苹果ID,
    共享密钥, receipt, transaction_id, originalTransactionIdentifierIOS,
    应用包ID2, transactionReceipt2, transactionIdentifier2, receipt3
)

receipt = transactionReceipt2
...
async def 示例_基本功能():
    ...
    async with 获取支付服务() as 支付服务:
        # 1. 验证收据
        print("1. 验证收据...")
        验证结果 = await 支付服务.验证收据_从应用收据(receipt3)
        print(f"验证结果: {验证结果.model_dump()}")

        # 2. 获取最新交易
        print("\n2. 获取最新交易...")
        最新交易 = await 支付服务.获取最新交易("2000000978241468")
        print(f"最新交易: {最新交易.model_dump()}")

        # 3. 验证特定交易
        print("\n3. 验证特定交易...")
        交易验证结果 = await 支付服务.验证特定交易("2000000978241468")
...
测试数据：
- 收据数据长度: {len(receipt)} 字符
- 交易ID: {transaction_id}
- 原始交易ID: {originalTransactionIdentifierIOS}
