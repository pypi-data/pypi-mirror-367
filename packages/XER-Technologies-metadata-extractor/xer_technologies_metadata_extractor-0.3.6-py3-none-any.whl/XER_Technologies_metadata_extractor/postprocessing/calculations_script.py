#!/usr/bin/env python3
"""
XER Data Analysis Calculations Script

This script implements the complete calculation pipeline for power system analysis,
including PMU power, engine power, and system efficiency calculations.

Usage:
    python calculations_script.py input.csv [--reference-data-dir public/reference_data] [--output output.csv]
"""
...
class XERDataCalculator:
    """Main calculator class for XER data analysis."""
    ...
    def calculate_system_efficiency(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Apply 10-point rolling average with center alignment
        print("Applying rolling average to System efficiency...")
        smoothed_efficiency = self._apply_rolling_average(efficiency, window_size=10)
        df['System_efficiency'] = smoothed_efficiency
        
        valid_efficiencies = [e for e in smoothed_efficiency if e > 0]
    ...
    def process_csv(self, input_file: str, output_file: str = None) -> pd.DataFrame:
        """
        Process a CSV file with all calculations.
...
def main():
    ...
    parser = argparse.ArgumentParser(description='XER Data Analysis Calculations')
    parser.add_argument('input_csv', help='Input CSV file path')
    parser.add_argument('--reference-data-dir', default='reference_data',
                       help='Directory containing reference RPM/power CSV files')
    parser.add_argument('--output', help='Output CSV file path (optional)')
    parser.add_argument('--filter-power', type=float, default=100,
                       help='Minimum power threshold for filtering (default: 100W)')
    ...
    # Create calculator
    calculator = XERDataCalculator(args.reference_data_dir)
    
    # Process the file
    result_df = calculator.process_csv(args.input_csv, args.output)
    
    if result_df is not None:
        # Apply optional filtering
        if args.filter_power > 0:
            result_df = calculator.filter_data_by_power(result_df, args.filter_power)
        
        if args.remove_outliers:
            result_df = calculator.remove_outliers(result_df, args.outlier_threshold)
        
        # Save final results if filtering was applied and output specified
        if args.output and (args.filter_power > 0 or args.remove_outliers):
            try:
                result_df.to_csv(args.output, index=False)
                print(f"Final filtered results saved to: {args.output}")
            except Exception as e:
                print(f"Error saving final results: {e}")
        
        # Print summary statistics
        print("\n=== CALCULATION SUMMARY ===")
        if 'PMU_power' in result_df.columns:
            pmu_stats = result_df['PMU_power'].describe()
            print(f"PMU Power (W): Mean={pmu_stats['mean']:.1f}, Std={pmu_stats['std']:.1f}, Range=[{pmu_stats['min']:.1f}, {pmu_stats['max']:.1f}]")
        
        if 'Engine_power' in result_df.columns:
            engine_stats = result_df['Engine_power'].describe()
            print(f"Engine Power (W): Mean={engine_stats['mean']:.1f}, Std={engine_stats['std']:.1f}, Range=[{engine_stats['min']:.1f}, {engine_stats['max']:.1f}]")
        
        if 'System_efficiency' in result_df.columns:
            eff_valid = result_df[result_df['System_efficiency'] > 0]['System_efficiency']
            if len(eff_valid) > 0:
                eff_stats = eff_valid.describe()
                print(f"System Efficiency (%): Mean={eff_stats['mean']:.1f}, Std={eff_stats['std']:.1f}, Range=[{eff_stats['min']:.1f}, {eff_stats['max']:.1f}]")
                print(f"Valid efficiency points: {len(eff_valid)}/{len(result_df)} ({100*len(eff_valid)/len(result_df):.1f}%)")
            else:
                print("System Efficiency: No valid efficiency values calculated")
        
        print(f"Final dataset: {len(result_df)} rows")
    else:
