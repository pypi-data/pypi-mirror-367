...
def example_fdn(args):
    ...
    new_vals = torch.abs(torch.randn(10, 3, 1)).repeat(1, 1, N)
    new_vals[:, -1, :] = torch.sigmoid(new_vals[:, -1, :])
    new_vals[:, -1, 1] = new_vals[:, -1, 1] * torch.tensor(2)
    new_vals[:, -1, 2] = new_vals[:, -1, 2] * torch.tensor(3) # make the last band a shelving filter
    new_vals[:, 1, :] = new_vals[:, 1, :] + torch.sqrt(torch.tensor(2)) / 2
  # make the last band a shelving filter
    attenuation.assign_value(new_vals)
