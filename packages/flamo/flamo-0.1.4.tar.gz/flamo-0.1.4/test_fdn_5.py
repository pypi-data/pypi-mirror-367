...
from flamo.optimize.loss import sparsity_loss
from losses import edr_loss, edc_loss, mss_loss
    ...
    with torch.no_grad():
        ...
        for key, value in param.items():
            try:
                ...
                if perturb:
                    tensor_value = torch.randn_like(tensor_value) * tensor_value.std() + tensor_value.mean()
            except:
                continue
            if key == "A":
...
def example_fdn(args):
    ...
    boradband_attenuation.assign_value(torch.ones(N,))
    ...
    for index in range(len(data)):
        ...
        if target_rir.shape[1] < args.nfft:
            target_rir = torch.nn.functional.pad(
                target_rir, (0, 0, (args.nfft - target_rir.shape[1])//2, (args.nfft - target_rir.shape[1])//2)
            )
        ...
        trainer.register_criterion(mse_loss(args.nfft, delays = delay_lengths), 1, requires_model=True)
        ...
        core = model.get_core()
        Hatt = core.feedback_loop.feedback.attenuation(torch.fft.rfft(input_imp, n=args.nfft, dim=1).repeat(1, 1, N))
        # Hatt *= core.feedback_loop.feedback.broadband_attenuation(torch.fft.rfft(input_imp, n=args.nfft, dim=1).repeat(1, 1, N))
        ...
        plt.plot([20.0000, 43.0887, 92.8318, 200.0000, 430.8870, 928.3179, 2000.0002,  4308.8696,  9283.1777, 20000.00000], RT, "x", label="target")
