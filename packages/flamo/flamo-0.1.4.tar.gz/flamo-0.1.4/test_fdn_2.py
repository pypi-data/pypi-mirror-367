...
def example_fdn(args):
    ...
    ## create target 
    param = scipy.io.loadmat("data/params/parameters_init.mat")
    perturb = True
    ...
    gain = -60 / 48000 / rt 
    gain_m = torch.einsum("i,j->ij", gain, delay_lengths)
    # get the minimum value of the gain along the first axis
    min_gain = torch.min(gain_m, dim=0)[0] + 1 # shift by one to avoid staying on the 0db axis  
    gain_m = gain_m - min_gain
    freqs = -1000*torch.ones(10, 6) # this is just the bias 
    R = torch.ones(10, 6) #  / torch.sqrt( 2* torch.ones(10, 6))
    new_param = torch.cat((freqs.unsqueeze(1), R.unsqueeze(1), gain_m.unsqueeze(1)), dim=1)
    ## assign target PEQ
    core = model.get_core()
    core.feedback_loop.feedback.peq.assign_value(new_param)
    core.feedback_loop.feedback.broadband_attenuation.assign_value(10**(min_gain/20))
    model.set_core(core)
    ...
    core.feedback_loop.feedback.broadband_attenuation.assign_value(10**((min_gain * torch.rand_like(min_gain))/20))
    model.set_core(core)
