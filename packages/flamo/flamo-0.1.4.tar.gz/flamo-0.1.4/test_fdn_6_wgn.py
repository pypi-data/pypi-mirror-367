...
def example_fdn(args):
    ...
    #     save_audio(
    #         os.path.join(args.train_dir,  f"ir_init.wav"),
    #         ir_init / torch.max(torch.abs(ir_init)),
    #         fs=args.samplerate,
    #     )

    # load WGN target RIR 
    target_rir, fs = sf.read("./rirs/lrf/target_ir.wav")
    target_rir = torch.tensor(target_rir, dtype=torch.float32).unsqueeze(0).unsqueeze(-1)
    # save_audio(
    #     os.path.join(args.train_dir, f"target.wav"),
    #     target_rir.squeeze() / torch.max(torch.abs(target_rir)),
    #     fs=args.samplerate,
    # )
    ...
    # normalize the energy so that it mataches the one of the FDN
    target_rir = target_rir * torch.sqrt( torch.norm(ir_init, p=2) / torch.norm(target_rir, p=2) )
    ...
    trainer.register_criterion(sparsity_loss(), 2, requires_model=True)
    ...
    #     save_audio(
    #         os.path.join(args.train_dir, f"ir_optim.wav"),
    ...
    # plt.xlabel("Frequency (Hz)")
    # plt.ylabel("RT60 (s)")
    # plt.savefig(os.path.join(args.train_dir, f"RT60.png"))
