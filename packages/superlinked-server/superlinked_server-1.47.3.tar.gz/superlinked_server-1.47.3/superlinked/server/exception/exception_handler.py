...
async def handle_generic_exception(_: Request, exception: Exception) -> JSONResponse:
    ...
    _record_exception(exception, external=False)
    return _build_json_response(exception, status.HTTP_500_INTERNAL_SERVER_ERROR)


def _build_json_response(exception: Exception, status_code: int) -> JSONResponse:
    return JSONResponse(
        status_code=status_code,
        content={"exception": str(exception.__class__.__name__), "detail": str(exception)},
    )


def _record_exception(exception: Exception, external: bool) -> None:
    if external:
        logger.info("user error", reason=str(exception))
    else:
        logger.warning("internal error", reason=str(exception), exception_type=type(exception).__name__)
        capture_exception(exception)

    metric_suffix = "external" if external else "internal"
    labels = {"exception_type": type(exception).__name__, "exception_message": str(exception)}
