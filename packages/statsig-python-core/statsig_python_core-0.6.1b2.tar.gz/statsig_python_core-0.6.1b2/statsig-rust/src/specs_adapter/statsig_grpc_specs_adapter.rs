...
pub struct StatsigGrpcSpecsAdapter {
    ...
    cancel_poll_tx: Arc<broadcast::Sender<u64>>,
}
...
impl StatsigGrpcSpecsAdapter {
    pub fn new(
        ...
        let (cancel_poll, _) = broadcast::channel(1);
        Self {
            ...
            cancel_poll_tx: Arc::new(cancel_poll),
        }
    }

    fn spawn_poll_from_statsig_thread(
        http_spec_adapter: Arc<StatsigHttpSpecsAdapter>,
        cancel_notify_tx: Arc<broadcast::Sender<u64>>,
        shutdown_notify: Arc<Notify>,
        ops_stats: Arc<OpsStatsForInstance>,
    ) {
        ops_stats.log(ObservabilityEvent::new_event(
            MetricType::Increment,
            "grpc_start_fallback_polling".to_string(),
            1.0,
            None,
        ));
        let mut rx = cancel_notify_tx.subscribe();
        ...
        tokio::task::spawn(async move {
            loop {
                tokio::select! {
                    ...
                    res = rx.recv() => {
                        if res.unwrap_or_default() > (Utc::now().timestamp_millis() as u64 - 10_000) {
                            ops_stats.log(ObservabilityEvent::new_event(
                                MetricType::Increment,
                                "grpc_cancel_fallback_polling_background".to_string(),
                                1.0,
                                None,
                            ));
    ...
    async fn spawn_grpc_streaming_thread(
        ...
        println!("calling spawn_grpc_streaming_thread");
                ...
                result = self.handle_grpc_request_stream() => {
                    if let Err(err) = result {
                        ...
                        if attempt == FALL_BACK_TO_POLLING_THREASHOLD {
                            ...
                            Self::spawn_poll_from_statsig_thread(self.http_specs_adapter.clone(), self.cancel_poll_tx.clone(), self.shutdown_notify.clone(), self.ops_stats.clone());
                        }
        ...
        loop {
            match stream.message().await {
                Ok(Some(config_spec)) => {
                    ...
                    _ =  self
                        .cancel_poll_tx
                        .send(Utc::now().timestamp_millis() as u64);
                    let _ = self
                        .initialization_tx
