...
def analyze_results(all_results):
    ...
    if successful_models:
        # Best for cost
        cheapest = min(successful_models.items(), 
        ...
        # Best for speed
        fastest = min(successful_models.items(),
                     key=lambda x: x[1]['total_latency'] / x[1]['successes'])
        print(f"⚡ Fastest: {fastest[0]} ({fastest[1]['total_latency'] / fastest[1]['successes']:.2f}s avg)")
        
        # Best cache performance
        best_cache = max(successful_models.items(),
...
def main():
    ...
    for scenario in test_scenarios:
        results = run_comparison_test(models_to_test, scenario['prompt'], scenario['name'])
