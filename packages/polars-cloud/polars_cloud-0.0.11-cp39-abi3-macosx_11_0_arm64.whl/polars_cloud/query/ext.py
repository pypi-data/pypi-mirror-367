...
class LazyFrameRemote:
    ...
    def partition_by(
        ...
        exec._check_partition_by_broadcast_over()

        return exec

    def broadcast_over(self, over: Broadcast | list[list[list[Path]]]) -> ExecuteRemote:
        """Run this query in parallel over the given source paths.
        ...
        if isinstance(over, Broadcast):
            over = over.finish()  # type: ignore[assignment]
        ...
        exec = ExecuteRemote(
            ...
            labels=self._labels,
            engine=self._engine,
            broadcast_over=over,  # type: ignore[arg-type]
        )
        exec._check_partition_by_broadcast_over()
    ...
    def execute(self) -> InteractiveQuery | BatchQuery:
        ...
        This is useful for interactive workloads to cache the results of a query.
...
class ExecuteRemote:
    ...
    def __init__(
        ...
        self._broadcast_over: None | list[list[list[Path]]] = broadcast_over
        self._engine: Engine = engine
    ...
    def _check_partition_by_broadcast_over(self) -> None:
