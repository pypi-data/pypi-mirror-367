...
class EthernityCloudRunner:
    def __init__(self, network_name="BLOXBERG", network_type="TESTNET") -> None:  # type: ignore
        ...
        self.block_time = self.network_config.BLOCK_TIME
    ...
    def check_order_for_task(self) -> bool:
        ...
        initial_delay = self.block_time  # Start with the original block_time
        max_delay = 600  # Example: max delay of 10 minutes (adjust as needed)
        backoff_factor = 2  # Multiply delay by this factor on each retry
        ...
        while True:
            try:
                if self.find_order(self.do_request):
                    ...
                    return True  # Explicit return True on success
                else:
                # Reset backoff after a successful (no exception) poll, even if order not found
                    retry_count = 0
                    current_delay = initial_delay
                    time.sleep(self.block_time)
            except Web3RPCError as e:
                # Log detailed RPC info if available
                error_code = e.error.get('code', 'Unknown')
                error_msg = e.error.get('message', 'No detailed message')
                self.log_append(
                    f"Web3RPCError: Code {error_code} - {error_msg}. Retrying after {current_delay} seconds...", ECLog.ERROR
                )
                ...
                # Fallback for other exceptions, log type and full message
                self.log_append(
                    f"Unexpected error: {type(e).__name__}: {str(e)}. Retrying after {current_delay} seconds...", ECLog.ERROR
                )
                time.sleep(current_delay)
    ...
    def close(self) -> None:
        try:
            # Stop the task thread if running
            if self.task_thread and self.task_thread.is_alive():
                self.log_append("Stopping task thread...", ECLog.INFO)
                self.running = False  # Signal thread to stop
                self.task_thread.join(timeout=5.0)  # Wait up to 5 seconds for thread to finish
                if self.task_thread.is_alive():
                    self.log_append("Task thread did not terminate gracefully", ECLog.WARNING)
            ...
            while not self.event_queue.empty():
                try:
                    ...
                    break
            self.event_queue = Queue()  # Reset queue

            # Close Web3 provider connections
            if hasattr(self, 'contract') and self.contract:
                try:
                    provider = self.contract.get_provider()
                    if provider and hasattr(provider, 'http') and provider.http:
                        provider.http.close()  # Close HTTP session
                        self.log_append("Closed Web3 HTTP provider", ECLog.DEBUG)
                    if provider and hasattr(provider, 'ws') and provider.ws:
                        provider.ws.close()  # Close WebSocket if used
                        self.log_append("Closed Web3 WebSocket provider", ECLog.DEBUG)
                except Exception as e:
                    self.log_append(f"Error closing Web3 provider: {e}", ECLog.WARNING)

            # Close image registry contract provider
            if hasattr(self, 'image_registry_contract') and self.image_registry_contract:
                try:
                    provider = self.image_registry_contract.get_provider()
                    if provider and hasattr(provider, 'http') and provider.http:
                        provider.http.close()
                        self.log_append("Closed image registry Web3 HTTP provider", ECLog.DEBUG)
                    if provider and hasattr(provider, 'ws') and provider.ws:
                        provider.ws.close()
                        self.log_append("Closed image registry Web3 WebSocket provider", ECLog.DEBUG)
                except Exception as e:
                    self.log_append(f"Error closing image registry provider: {e}", ECLog.WARNING)

            # Close IPFS client
            global ipfsClient
            if 'ipfsClient' in globals():
                try:
                    ipfsClient.close()  # Assuming IPFSClient has a close method
                    self.log_append("Closed IPFS client", ECLog.DEBUG)
                except AttributeError:
                    self.log_append("IPFS client does not support close method", ECLog.WARNING)
                except Exception as e:
                    self.log_append(f"Error closing IPFS client: {e}", ECLog.WARNING)
                ipfsClient = None  # Clear global reference
            ...
            self.log_append("Runner resources closed successfully", ECLog.INFO)

        except Exception as e:
            self.log_append(f"Error during close: {e}", ECLog.ERROR)
            self.status = ECStatus.ERROR
            self.last_error = str(e)