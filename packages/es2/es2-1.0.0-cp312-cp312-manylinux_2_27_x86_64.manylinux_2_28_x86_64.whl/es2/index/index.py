...
from es2.utils.utils import Position
...
class Index:
    ...
    >>> index.insert(data=data, metadata=metadata, auto_id=True)
    ...
    def insert(
        ...
        elif isinstance(data[0], list) and isinstance(data[0][0], float) or isinstance(data[0], np.ndarray):
        ...
        for i in tqdm(
            ...
            desc="Insert data (batch)",
            total=(len(data) + batch_size - 1) // batch_size,
        ):
            if self.index_config.index_encryption and not isinstance(data[0], CipherBlock):
                input_data = []
                for j in range(i, min(i + batch_size, len(data))):
                    input_data.append(self.cipher.encrypt(data[j], encode_type="item").data)
            else:
                if self.index_config.index_encryption:
                    input_data = [d.data for d in data[i : i + batch_size]]
                else:
            ...
            self.indexer.insert_data(self.index_config.index_name, input_data, batch_metadata)
            self.num_entities += len(input_data)

        logger.debug("Data insertion completed successfully.")
        return self

    def search(self, query: Union[List[float], np.ndarray], top_k: int = 3, output_fields: List[str] = None):
        """
        Searches the index.
        ...
        >>> print(results)
        """
        result_ctxt = self.scoring(query)
        result = self.cipher.decrypt_score(result_ctxt)
        output_result = self.get_topk_metadata_results(result, top_k, output_fields)
        return output_result

    def scoring(self, query: Union[List[float], np.ndarray]):
        """
        ...
        if isinstance(query[0], list):
            raise ValueError("Only Single Query is allowed.")
        query = np.array(query)
        if query.shape[0] != self.index_config.dim:
            raise ValueError(
                f"Query dimension {query.shape[0]} does not match index dimension {self.index_config.dim}."
            )
        query = [query]
        if self.index_config.query_encryption:
            encrypted_query = self.cipher.encrypt(query, encode_type="query")
        ...
        return CipherBlock(result_ctxt)
    ...
    def get_topk_metadata_results(self, result, top_k: int, output_fields: List[str] = None):
        ...
        """
        data_list = []
        for vec_idx, vec in enumerate(result):
            for inner_idx, value in enumerate(vec):
                data_list.append(((vec_idx, inner_idx), value))

        topk_result = sorted(data_list, key=lambda x: x[1], reverse=True)[:top_k]
