...
class HoloOps:
    ...
    def getDir(self, name: str) -> Optional[Path]:
        # Check dynamically assigned attributes first (from mappingConfig)
        d = getattr(self, name, None)
        if isinstance(d, Path):
            return d
        # Fallback: check baseDirs
        if self.baseDirs and name in self.baseDirs:
            return self.baseDirs[name]
        return None

    def dirNames(self) -> list[str]:
        mapped = []
        if self.mappingConfig:
            for dmap in self.mappingConfig.values():
                mapped.extend(list(dmap.keys()))
        bases = list(self.baseDirs.keys()) if self.baseDirs else []
        # Deduplicate while preserving order
        seen = set()
        unique = [n for n in mapped + bases if not (n in seen or seen.add(n))]
        return unique
