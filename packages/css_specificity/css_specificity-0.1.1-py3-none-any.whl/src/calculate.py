...
class SpecificityCalculator:
    ...
    def calculate(cls, selector: str) -> Specificity:
        ...
        """
        try:
            work_selector = parse_one_declaration(selector, skip_comments=True).serialize()
...
if __name__ == "__main__":
    print(
        SpecificityCalculator.calculate(
            "body:not([arial]) .foo #er3 div:hover::after:has(em, strong#bar) * .foo:where(em, #foo#bar#baz) /* baz */"
        )
    )
