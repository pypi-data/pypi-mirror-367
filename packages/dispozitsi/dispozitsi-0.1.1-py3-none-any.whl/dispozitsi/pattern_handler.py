...
from dispozitsi.pattern_file_handler import PatternFileHandler, loaded_patterns
...
class GrokPatternHandler:
    ...
    def dictify_pattern_matches(self, sample_text: str) -> list[dict] | None:
        """Makes matches into a dictionary - will be used for display purposes."""
        results: list[dict] = []
        try:
            for line in sample_text:
                one = re.search(self.pattern_replacement, line.__str__())
                if one is not None:
                    dictit = one.groupdict()
                    results.append(dictit)
    ...
    def __check_pattern_and_semantic_validity(self, m: re.Match[str], semantics_in_use) -> str | None:
        try:
            pattern = loaded_patterns[m.group(1)]
            semantic = str(m.group(2))

            if not semantic.isidentifier():
                self.invalid_semantic_identifier_detected = True
                raise SemanticNotValidIdentifierError
            ...
            raise UnknownPatternError(
                known_patterns=list(loaded_patterns.keys())
            ) from KeyError
        except re.error:
            pass
        else:
    ...
    def __replace_pattern_reference_with_regex(self, this_pattern) -> str:
        ...
        while True:
            new_pattern = re.sub(
                PATTERN_REFERENCE,
                lambda m: "(" + self.pattern_file_handler.patterns[m.group(1)] + ")",
                this_pattern,
            )
