...
class ProjectProgress:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.failed_functions, list):
            raise ValidationError(f"Failed functions must be a list, got {type(self.failed_functions)}")
            
        for i, func_name in enumerate(self.failed_functions):
            if not isinstance(func_name, str):
                raise ValidationError(f"Failed function at index {i} must be a string, got {type(func_name)}: {func_name}")
            if not func_name.strip():
                raise ValidationError(f"Failed function name at index {i} cannot be empty")
