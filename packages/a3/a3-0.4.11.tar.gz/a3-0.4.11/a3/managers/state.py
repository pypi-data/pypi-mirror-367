...
class StateManager(BaseStateManager):
    ...
    def save_progress(self, phase: ProjectPhase, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Update with additional data with type checking
            if 'total_functions' in data:
                total_funcs = data['total_functions']
                if isinstance(total_funcs, int):
                    current_progress.total_functions = total_funcs
                else:
                    current_progress.total_functions = int(total_funcs) if total_funcs is not None else 0
                    
            if 'implemented_functions' in data:
                impl_funcs = data['implemented_functions']
                if isinstance(impl_funcs, int):
                    current_progress.implemented_functions = impl_funcs
                elif isinstance(impl_funcs, list):
                    current_progress.implemented_functions = len(impl_funcs)
                else:
                    current_progress.implemented_functions = int(impl_funcs) if impl_funcs is not None else 0
                    
            if 'failed_functions' in data:
                failed_funcs = data['failed_functions']
                if isinstance(failed_funcs, list):
                    current_progress.failed_functions = failed_funcs
                elif isinstance(failed_funcs, int):
                    # If it's an int, it might be a count - convert to empty list for now
                    current_progress.failed_functions = []
                else:
                    current_progress.failed_functions = []
            
            # Validate before saving with better error reporting
            try:
                current_progress.validate()
            except Exception as validation_error:
                # Log detailed information about the validation failure
                print(f"Validation error details:")
                print(f"  current_phase: {current_progress.current_phase} (type: {type(current_progress.current_phase)})")
                print(f"  total_functions: {current_progress.total_functions} (type: {type(current_progress.total_functions)})")
                print(f"  implemented_functions: {current_progress.implemented_functions} (type: {type(current_progress.implemented_functions)})")
                print(f"  failed_functions: {current_progress.failed_functions} (type: {type(current_progress.failed_functions)})")
                print(f"  completed_phases: {current_progress.completed_phases} (type: {type(current_progress.completed_phases)})")
