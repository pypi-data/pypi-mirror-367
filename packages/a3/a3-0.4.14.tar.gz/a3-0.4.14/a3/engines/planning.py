...
from ..core.models import (
    ...
    EnhancedFunctionSpec, ValidationLevel, ValidationErrorCategory
)
...
"""
    ...
    def _create_dependency_graph(self, modules: List[Module]) -> DependencyGraph:
        ...
        try:
            ...
            # Perform planning-specific dependency analysis (structural validation only)
            analysis_result = self.dependency_analyzer.analyze_planning_dependencies(modules)
            
            if not analysis_result.is_valid:
                # Create detailed error message with phase context
                error_details = []
                error_details.append(f"Planning phase dependency validation failed ({analysis_result.get_phase_summary()}):")
                
                # Group errors by category for better organization
                structural_errors = analysis_result.get_errors_by_category(ValidationErrorCategory.STRUCTURAL)
                if structural_errors:
                    error_details.append("\nStructural Issues:")
                    for error in structural_errors:
                        error_details.append(f"  • {error.message}")
                        if error.suggestions:
                            error_details.append(f"    Suggestions: {'; '.join(error.suggestions)}")
                
                config_errors = analysis_result.get_errors_by_category(ValidationErrorCategory.CONFIGURATION)
                if config_errors:
            ...
            if analysis_result.warnings:
                self.logger.info(f"Planning phase validation completed with warnings:")
                for warning in analysis_result.warnings:
                    self.logger.info(f"  • {warning}")
            else:
                self.logger.info("Planning phase dependency validation passed successfully")
            ...
            if isinstance(e, PlanGenerationError):
                ...
                raise PlanGenerationError(f"Failed to create dependency graph during planning phase: {str(e)}")
    ...
    def _create_enhanced_dependency_graph(self, modules: List[Module]) -> EnhancedDependencyGraph:
        ...
        try:
            ...
            self.logger.info(f"Successfully created enhanced dependency graph with {len(enhanced_graph.function_nodes)} functions")
            
            return enhanced_graph
            
        except Exception as e:
            # Log the specific error for debugging
            self.logger.warning(f"Failed to create enhanced dependency graph: {str(e)}")
            if isinstance(e, PlanGenerationError):
                raise
            else:
                # Return a basic enhanced graph instead of failing completely
                basic_graph = EnhancedDependencyGraph()
                for module in modules:
                    for function in module.functions:
                        basic_graph.add_function(function.name, module.name)
                self.logger.info(f"Created basic enhanced dependency graph with {len(basic_graph.function_nodes)} functions")
                return basic_graph
