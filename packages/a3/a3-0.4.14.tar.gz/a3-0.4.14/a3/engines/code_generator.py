...
class CodeGenerator(BaseCodeGenerator):
    ...
    def _create_code_generation_prompt(self, spec: FunctionSpec, retry_count: int = 0) -> str:
        ...
        # Create context about the module and project structure
        module_context = f"This function belongs to the '{spec.module}' module."
        
        # Add information about available project modules to prevent internal imports
        project_modules_context = ""
        if hasattr(self, 'state_manager') and self.state_manager:
            try:
                # Get project plan to understand module structure
                project_plan = self.state_manager.load_project_plan()
                if project_plan and hasattr(project_plan, 'modules'):
                    available_modules = [m.name for m in project_plan.modules if m.name != spec.module]
                    if available_modules:
                        project_modules_context = f"""
PROJECT STRUCTURE CONTEXT:
- Current module: {spec.module}
- Other available modules: {', '.join(available_modules)}
- DO NOT import from other project modules inside the function - these imports will be handled automatically at the module level
- Only use standard library and third-party imports if absolutely necessary inside the function"""
            except Exception:
                pass
...
10. DO NOT include any import statements inside the function - all imports will be handled at the module level
11. DO NOT import from other project modules (like from ...utils.validators import *) - these dependencies are handled automatically

CRITICAL IMPORT RULES:
- NO import statements inside the function body
- NO imports from other project modules (those starting with . or .. or project names)
- Only use functions and classes that are already available in the current scope
- Standard library imports (if absolutely necessary) will be detected and moved to module level automatically
- Assume all project dependencies are already imported at module level
...
def function_name(arg1: type1, arg2: type2 = default) -> return_type:
    ...
    # Implementation here (no import statements)
    return result
"""
    ...
    def _extract_required_imports(self, code: str, func_spec: FunctionSpec, 
        ...
        # Common patterns to detect third-party library imports needed
        import_patterns = [
            ...
            (r'\bre\.', 'import re'),
            (r'BeautifulSoup', 'from bs4 import BeautifulSoup'),
            (r'\bfeedparser\.', 'import feedparser'),
            (r'feedparser\.parse', 'import feedparser')
        ]
        
        # Check for third-party library patterns
        ...
        # Extract project-internal imports that might be needed based on function dependencies
        if specs and hasattr(specs, 'modules'):
            current_module_name = func_spec.module
            
            # Look for functions or classes that might be from other modules
            for module in specs.modules:
                if module.name != current_module_name:
                    # Check if any functions from this module are referenced in the code
                    if hasattr(module, 'functions'):
                        for func in module.functions:
                            # Look for function calls that might be from other modules
                            if re.search(rf'\b{func.name}\s*\(', code):
                                # Generate relative import based on module structure
                                relative_import = self._generate_relative_import(current_module_name, module.name)
                                if relative_import and relative_import not in imports:
                                    imports.append(relative_import)
        
        return imports
    
    def _generate_relative_import(self, from_module: str, to_module: str) -> Optional[str]:
        """
        Generate a relative import statement from one module to another.
        
        Args:
            from_module: Source module name (e.g., 'parser.article_parser')
            to_module: Target module name (e.g., 'utils.validators')
            
        Returns:
            Relative import statement or None if not applicable
        """
        try:
            ...
            if len(from_parts) == len(to_parts):
                # Same level: from ..utils.validators import *
                return f"from ..{to_parts[-1]} import *"
            elif len(from_parts) > len(to_parts):
                # Going up: from ...utils.validators import *
                levels = len(from_parts) - len(to_parts) + 1
                dots = '.' * levels
                return f"from {dots}{to_parts[-1]} import *"
            else:
                # Going down: from .submodule import *
                return f"from .{to_parts[-1]} import *"
                
        except Exception:
            return None
    
    def _clean_imports_from_code(self, code: str) -> Tuple[str, List[str]]:
        """
        Remove import statements from generated function code and extract them.
        This includes imports at the top level and inside function bodies.
        
        Args:
            code: Generated function code that may contain imports
            
        Returns:
            Tuple of (cleaned_code, extracted_imports)
        """
        lines = code.split('\n')
        cleaned_lines = []
        extracted_imports = []
        ...
        for i, line in enumerate(lines):
            ...
            if inside_function and line.strip() and current_indent <= function_indent:
                inside_function = False
            ...
            if stripped.startswith('import ') or stripped.startswith('from '):
                ...
                # If we're inside a function, we need to maintain indentation structure
                if inside_function:
                    # Add an empty line to maintain structure if needed
                    if i > 0 and lines[i-1].strip() and not lines[i-1].strip().startswith(('import ', 'from ')):
                        # Only add empty line if previous line wasn't also an import
                        pass
                continue
            else:
        ...
        # Remove empty lines at the beginning of the function
        while final_lines and not final_lines[0].strip():
            final_lines.pop(0)
            
        return '\n'.join(final_lines), extracted_imports
    ...
    def _write_function_to_file(self, func_spec: FunctionSpec, code: str, specs: SpecificationSet) -> None:
        ...
        # Clean imports from generated code and extract them
        cleaned_code, extracted_imports = self._clean_imports_from_code(code)
        
        # Analyze cleaned code to extract required imports
        required_imports = self._extract_required_imports(cleaned_code, func_spec, module_info, specs)
        
        # Add extracted imports from the generated code
        required_imports.extend(extracted_imports)
        ...
        # Update or add function using cleaned code
        updated_functions = self._update_function_in_content(functions_section, func_spec.name, cleaned_code)
    ...
    def _merge_imports(self, existing_imports: str, new_imports: List[str]) -> str:
        ...
        for new_import in new_imports:
            ...
            if new_import and new_import not in existing_set:
                existing_lines.append(new_import)
                existing_set.add(new_import)
