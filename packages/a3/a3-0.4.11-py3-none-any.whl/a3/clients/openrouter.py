...
class OpenRouterClient(BaseAIClient):
    ...
    def __init__(self, api_key: Optional[str] = None, default_model: Optional[str] = None, config: Optional[Any] = None):
        """
        ...
        Args:
            ...
            config: A3Config instance for configuration settings
        """
        ...
        self.fallback_models = []  # No default fallback models - user must configure explicitly
        self.config = config  # Store config for use_fallback_models setting
    ...
    def generate_with_retry(self, prompt: str, max_retries: int = 3, model: Optional[str] = None, use_fallbacks: Optional[bool] = None) -> str:
        """
        ...
        Args:
            ...
            model: Model to use for completion (defaults to default_model if None)
            use_fallbacks: Whether to use fallback models if primary model fails (defaults to config setting)
        ...
        if use_fallbacks is None:
            # Use config setting if available, otherwise default to False
            use_fallbacks = self.config.use_fallback_models if self.config else False
        ...
        # Only add fallback models if explicitly requested or configured
        if use_fallbacks:
            ...
            # No automatic default fallbacks - user must configure explicitly
