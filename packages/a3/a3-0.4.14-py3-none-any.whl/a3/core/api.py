...
from ..core.code_syntax_fixer import CodeSyntaxFixer
...
from .models import (
    Module, ProjectPlan, ProjectStatus, SpecificationSet, ImplementationResult, 
...
class A3:
    ...
    def set_api_key(self, api_key: str) -> None:
        ...
        try:
            ...
            try:
                ...
                client = OpenRouterClient(self._api_key, config=config)
    ...
    def integrate(self, project_path: str = ".", generate_tests: bool = False) -> IntegrationResult:
        ...
        try:
            ...
            self._handle_integration_result(result, integration_engine, plan.modules, generate_tests)
            ...
            if generate_tests and result.test_result:
                if result.test_result.success:
                    ...
                    if result.test_result.test_files_created:
                        self._logger.info(f"Created test files: {', '.join(result.test_result.test_files_created)}")
                else:
                    self._logger.warning(f"Test generation encountered {len(result.test_result.errors)} errors:")
                    for error in result.test_result.errors:
            ...
            if result.success:
                ...
                if result.import_errors:
                    self._logger.info("Note: Integration succeeded despite some non-critical import warnings.")
            else:
                self._logger.warning("Integration completed with errors. Some functionality may be limited.")
            ...
            # Add integration-specific progress data
            progress_data['integration'] = {
                'integrated_modules': result.integrated_modules,
                'import_errors_count': len(result.import_errors),
            ...
            # Allow completion even with some import errors if modules were integrated
            should_complete = result.success or (len(result.integrated_modules) > 0 and len(result.import_errors) < len(result.integrated_modules))
            
            if should_complete:
                self._state_manager.save_progress(ProjectPhase.COMPLETED, progress_data)
                if not result.success:
                    self._logger.warning("Integration marked as completed despite errors due to partial success.")
            else:
                # Log detailed failure information with enhanced categorization
                error_details = []
                if result.import_errors:
                    error_details.append(f"{len(result.import_errors)} import errors")
                if generate_tests and result.test_result and not result.test_result.success:
                    error_details.append(f"{len(result.test_result.errors)} test generation errors")
                if len(result.integrated_modules) == 0:
                    error_details.append("no modules successfully integrated")
                
                error_msg = f"Integration failed with {', '.join(error_details)}"
                
                # Provide specific guidance based on error types
                guidance_parts = ["Check the error details above"]
                if any('path' in error.lower() or 'not found' in error.lower() for error in result.import_errors):
                    guidance_parts.append("verify all module paths are correct")
                if any('syntax' in error.lower() for error in result.import_errors):
                    guidance_parts.append("check for syntax errors in generated code")
                if any('dependency' in error.lower() or 'circular' in error.lower() for error in result.import_errors):
                    guidance_parts.append("review module dependencies for circular references")
                
                guidance_parts.append("ensure all dependencies are properly installed and accessible")
                
                raise OperationError(error_msg, ". ".join(guidance_parts) + ".")
    ...
    def _handle_integration_result(self, result: IntegrationResult, integration_engine, modules: List[Module], generate_tests: bool) -> None:
        """
        Handle integration result with enhanced error processing and syntax fixing.
        
        Args:
            result: Integration result from the engine
            integration_engine: The integration engine instance
            modules: List of modules being integrated
            generate_tests: Whether test generation was requested
        """
        # Categorize and report import errors with improved messaging
        if result.import_errors:
            self._categorize_and_report_errors(result.import_errors)
            
            # Attempt syntax fixes for import-related errors
            syntax_errors = [error for error in result.import_errors 
                           if any(keyword in error.lower() for keyword in ['syntax', 'import', 'indentation', 'unexpected'])]
            
            self._logger.debug(f"Checking for syntax errors: {len(syntax_errors)} found from {len(result.import_errors)} total errors")
            
            if syntax_errors:
                self._logger.info(f"Detected {len(syntax_errors)} potential syntax errors. Attempting automatic fixes...")
                
                syntax_fixer = CodeSyntaxFixer()
                fix_results = syntax_fixer.fix_directory(str(self.project_path))
                
                self._logger.debug(f"Syntax fixer results: fixed_files={syntax_fixer.fixed_files}, errors={syntax_fixer.errors}")
                
                if syntax_fixer.fixed_files:
                    self._logger.info(f"Successfully fixed syntax errors in {len(syntax_fixer.fixed_files)} files:")
                    for fixed_file in syntax_fixer.fixed_files:
                        self._logger.info(f"  - {fixed_file}")
                    
                    # Re-run integration after fixing syntax errors
                    self._logger.info("Re-running integration after syntax fixes...")
                    try:
                        new_result = integration_engine.integrate_modules(modules, generate_tests=generate_tests)
                        self._logger.debug(f"Re-integration result: success={new_result.success}, modules={len(new_result.integrated_modules)}, errors={len(new_result.import_errors)}")
                        
                        # Update the original result with improvements
                        if len(new_result.import_errors) < len(result.import_errors):
                            fixed_errors = len(result.import_errors) - len(new_result.import_errors)
                            self._logger.info(f"Syntax fixes resolved {fixed_errors} import errors")
                            
                            # Update result with improved data
                            result.import_errors = new_result.import_errors
                            result.success = new_result.success
                            result.integrated_modules = new_result.integrated_modules
                            if new_result.test_result:
                                result.test_result = new_result.test_result
                            self._logger.debug(f"Updated original result: success={result.success}, modules={len(result.integrated_modules)}, errors={len(result.import_errors)}")
                        else:
                            self._logger.warning("Syntax fixes did not resolve import errors")
                            
                    except Exception as e:
                        self._logger.error(f"Failed to re-run integration after syntax fixes: {e}")
                        result.import_errors.append(f"Re-integration after syntax fixes failed: {e}")
                else:
                    self._logger.warning("No files were fixed by syntax fixer")
                
                # Report any syntax fixer errors
                if syntax_fixer.errors:
                    self._logger.warning(f"Syntax fixer encountered {len(syntax_fixer.errors)} errors:")
                    for error in syntax_fixer.errors:
                        self._logger.warning(f"  - {error}")
                        
            else:
                self._logger.info("No syntax-related errors detected. Skipping automatic syntax fixes.")
        
        # Handle warnings from integration result
        if hasattr(result, 'warnings') and result.warnings:
            self._logger.info(f"Integration completed with {len(result.warnings)} warnings:")
            for warning in result.warnings:
                self._logger.warning(f"  - {warning}")
    
    def _categorize_and_report_errors(self, import_errors: List[str]) -> None:
        """
        Categorize and report import errors with enhanced messaging.
        ...
        for error in import_errors:
            ...
            if any(keyword in error_lower for keyword in ['syntax', 'unexpected token', 'indentation']):
                syntax_errors.append(error)
            elif any(keyword in error_lower for keyword in ['path', 'not found', 'cannot find', 'no module']):
                path_errors.append(error)
            elif any(keyword in error_lower for keyword in ['circular', 'dependency', 'cycle']):
        ...
        self._logger.warning(f"Integration completed with {total_errors} import errors:")
        
        if syntax_errors:
            self._logger.warning(f"  Syntax Errors ({len(syntax_errors)}):")
            for error in syntax_errors[:3]:  # Show first 3 to avoid log spam
                self._logger.warning(f"    - {error}")
            if len(syntax_errors) > 3:
                self._logger.warning(f"    ... and {len(syntax_errors) - 3} more syntax errors")
        
        if path_errors:
            self._logger.warning(f"  Path/Module Errors ({len(path_errors)}):")
            for error in path_errors[:3]:
                self._logger.warning(f"    - {error}")
            if len(path_errors) > 3:
                self._logger.warning(f"    ... and {len(path_errors) - 3} more path errors")
        
        if dependency_errors:
            self._logger.warning(f"  Dependency Errors ({len(dependency_errors)}):")
            for error in dependency_errors:
                self._logger.warning(f"    - {error}")
        
        if validation_errors:
            self._logger.warning(f"  Validation Errors ({len(validation_errors)}):")
            for error in validation_errors:
                self._logger.warning(f"    - {error}")
        
        if other_errors:
            self._logger.warning(f"  Other Errors ({len(other_errors)}):")
            for error in other_errors[:2]:
                self._logger.warning(f"    - {error}")
            if len(other_errors) > 2:
                self._logger.warning(f"    ... and {len(other_errors) - 2} more errors")
