...
class SensorControler(Sensor):
    ...
    def fail_scan(error_message: str, scontroler=None) -> NoReturn:
        ...
        # upload fail dump
        if scontroler and scontroler.sensor_type in FILES_BY_SENSOR_TYPE:
            upload_log_dump(FILES_BY_SENSOR_TYPE[scontroler.sensor_type], scontroler.scan_id)
    ...
    def finish_scan(scontroler) -> NoReturn:
        ...
        # upload log dump
        if scontroler.sensor_type in FILES_BY_SENSOR_TYPE:
            upload_log_dump(FILES_BY_SENSOR_TYPE[scontroler.sensor_type], scontroler.scan_id)
