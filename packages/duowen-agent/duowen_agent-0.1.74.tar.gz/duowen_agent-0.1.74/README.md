...
vdb = KDTreeVector(llm_embeddings_instance=emb, lex_synth=LexSynth())
...
vdb = KDTreeVector(llm_embeddings_instance=self.emb, lex_synth=self.lex_synth)

query = "苹果公司最新发布的iPhone 15 Pro有哪些新功能？"

print(f"question: {query}")

print("语义相似")
for i in vdb.semantic_search(query=query, top_k=5):
    print(i.vector_similarity_score, i.result.page_content)

print("全文相似")
for i in vdb.full_text_search(query=query, top_k=5):
    print(i.token_similarity_score, i.result.page_content)
```