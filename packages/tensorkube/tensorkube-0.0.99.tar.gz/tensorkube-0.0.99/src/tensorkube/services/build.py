...
def get_buildkit_command(sanitised_project_name: str, image_tag: str, image_url: Optional[str] =None, upload_to_nfs: bool= False, convert_to_nydus_image: bool = False, secrets: List[str] = []):
...
def apply_k8s_buildkit_config(sanitised_project_name: str, image_tag: str,
                              env_name: Optional[str] = None, context_name: Optional[str] = None, image_url: Optional[str] = None, upload_to_nfs: bool = False,convert_to_nydus_image: bool = False, secrets: List[str] = []):
    if not context_name:
    ...
    buildkit_config['spec']['template']['spec']['containers'][0]['command'] = get_buildkit_command(sanitised_project_name, image_tag, image_url, upload_to_nfs, convert_to_nydus_image, secrets)
