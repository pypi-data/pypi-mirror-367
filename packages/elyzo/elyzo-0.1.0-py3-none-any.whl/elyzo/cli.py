...
def main():
    ...
    test_parser.add_argument(
        "--input",
        action="append", # Allows the argument to be repeated, e.g., --input key1=val1 --input key2=val2
        dest="inputs",
        metavar="KEY=VALUE",
    ...
    if args.command == "deploy":
        ...
        api_key = get_api_key(args.api_key)
        # Use an empty list if no inputs are provided
        input_args = args.inputs or []
        test_agent_run(api_key, input_args, args.output_path)
    elif args.command == "run":
        api_key = get_api_key(args.api_key)
        input_args = args.inputs or []
        execute_agent_run(args.agent_id, api_key, input_args, args.output_path)
    else:
        parser.print_help()
