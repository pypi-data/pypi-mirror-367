...
DimensionError.__module__ = "tlib"
    ...
    if len(exprs) == 1:
        ...
        return [
            tlib.expr.Equation(
                expr,
                tensor_shape,
                desc1=f'{tlib.expr.idx_to_ordinal(i)} input expression ("{expr}")',
                desc2=f"{tlib.expr.idx_to_ordinal(i)} input tensor{shape_to_str(tensor_shape)}",
            )
...
def output_equations(exprs):
    if len(exprs) == 1:
        ...
        return [
            tlib.expr.Equation(
                expr,
                desc1=f'{tlib.expr.idx_to_ordinal(i)} output expression ("{expr}")',
            )
