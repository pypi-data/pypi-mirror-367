    ...
    *,
    ...
    if nboot > 0:
        ...
        if outcome_type == "survival":
            ...
            trt_names = sorted(k for k in boot_list[0].keys() if k != "matching")
            boot_mat = np.array([[d[k] for k in trt_names] for d in boot_list])
        ...
        if outcome_type == "survival":
            ...
            est_vec = np.array([theta_hat[k] for k in trt_names])
            lo_vec, hi_vec = np.percentile(boot_mat, [pct_lo, pct_hi], axis=0)

            df_means = pd.DataFrame(
                {"treatment": trt_names, "mean": est_vec, "lo": lo_vec, "hi": hi_vec}
            )
    ...
    if outcome_type == "survival":
        ...
        return {"pairwise": df_pairs, "matching": cluster_mat}
    ...
    if not do_stacking:
        ...
        treatment_names = results[0][3]
        retdict = {treatment_names[i]: avg_arr[i] for i in range(len(treatment_names))}
        return {**retdict, "matching": cluster_mat}
