...
class Button_perm_list(ModalScreen):
    ...
    def __init__(self, role, perms, file_type, all_perms_data):
        super().__init__()
        self.role = role
        self.perms = perms
        self.file_type = file_type
        self.all_perms_data = all_perms_data
    ...
    def check_and_warn(self):
        is_dangerous, warning_msg = Validator_dangerous.is_safe(self.file_type, self.role, self.perms)
        warning_label = self.query_one("#warning_label", Label)
        save_button = self.query_one("#save_button", Button)

        if is_dangerous:
            warning_label.update(warning_msg)
            warning_label.styles.color="red"
    ...
    def save(self, event: Button.Pressed):
        if event.button.id == "save_button" and not event.button.disabled:
    ...
    async def perm_button(self, event: Button.Pressed):
        ...
        if role == 'save' or role == 'cancel':
            ...
            if perms:
                ...
                self.app.push_screen(Button_perm_list(role, perms, self.data['file_type'], self.data['perm']), self.save_data)
...
class Validator_dangerous(Validator):
    @staticmethod
    def is_safe(file_type: str, role: str, new_perms: list) -> tuple[bool, str]:
        
        if role != "owner":
            return False, ""
            
        if file_type == 'directory':
            if 'execute' not in new_perms:
                return True, "Remove execute permission from directories will lock you out!!"
            
        elif file_type == "file":
            if not new_perms:
                return True, "Removing  all permission will make this file inaccessible!!"
class Main_app(App):
    CSS_PATH = "main.tcss"
