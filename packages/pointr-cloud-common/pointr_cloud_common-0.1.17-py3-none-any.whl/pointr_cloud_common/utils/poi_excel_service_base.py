...
class PoiExcelServiceBase:
    ...
    def _process_poi_to_row(self, feature):
        ...
        # Basic fields
        row = [feature.get("type", ""), p.get("typeCode", ""), p.get("sid", ""), p.get("bid", ""), 
               str(p.get("lvl", "")), p.get("fid", ""), p.get("name", ""), p.get("description", ""), 
               p.get("logo", ""), str(p.get("images", [])) if p.get("images") else "",
               str(p.get("tags", [])) if p.get("tags") else "", p.get("eid", ""), 
               str(p.get("price", "")), str(p.get("priceMax", "")), p.get("priceSign", ""), 
               str(p.get("rating", "")), str(p.get("ratingMax", "")), str(p.get("numberOfRatings", "")),
        ...
        row.extend([contact["email"], contact["phone"], contact["website"], contact["order_url"], 
                   contact["order_override"], contact["book_url"], contact["book_override"], 
                   contact["menu_url"], contact["menu_override"]])
        
        # Extra and style
        row.extend([str(p.get("extra", {})) if p.get("extra") else "", 
        ...
        for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
            ...
            if day_hours and len(day_hours) > 0 and len(day_hours[0]) >= 2:
                ...
                row.extend([open_time, close_time, str(is_closed).upper(), str(is_always_open).upper()])
            else:
