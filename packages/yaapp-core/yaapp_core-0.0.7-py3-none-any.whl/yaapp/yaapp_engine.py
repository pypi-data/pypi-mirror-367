...
class Command:
    ...
    async def execute(self, **kwargs) -> Result[Any]:
        ...
        """
        # If it's a group (class or object instance), we need to execute its methods
        if self.metadata.get("type") == "group":
            # The 'run' method of ClassExposer/ObjectExposer returns an instance
            instance_result = self.exposer.run(self.metadata.get("obj"), **kwargs)
            ...
            # If a method name is provided via dot notation, execute that method on the instance
            if method_name:
                return await self.engine.object_exposer.run(
                    instance, method_name, yaapp_engine=self.engine, **kwargs
                )
            else:
                # If no method_name, and it's a group, return the instance itself
                return Ok(instance)
...
class YaappEngine:
    ...
    def __init__(self):
        ...
        self._config = None  # Config storage
    ...
    async def _load_config_async(self) -> Result[bool]:
        ...
        Future: This will support network-based config loading.
        """
        try:
            # For now, delegate to sync config manager
            # TODO: Replace with async config loading
            from .config_manager import ConfigManager

            config_manager = ConfigManager()
            config_manager.load(yaapp_engine=self)
    ...
    async def _activate_plugins_async(self) -> Result[bool]:
        ...
        try:
            ...
            # TODO: Add async plugin activation here
            # For now, plugins are activated during config loading
    ...
    async def execute(self, ____name: str, **kwargs) -> Result[Any]:
        ...
        Args:
            ____name: Command name to execute. Supports dot notation for methods (e.g., "class.method")
                     Uses ugly ____name to avoid conflicts with user kwargs.
            **kwargs: Arguments to pass to the command
        """
        # Handle dot notation for method calls (class.method)
        method_name = None
        if "." in ____name:
            class_name, method_name = ____name.split(".", 1)
            ____name = class_name
        ...
        if metadata.get("type") == "group":
            ...
            instance_result = exposer.run(metadata.get("obj"), **kwargs)
            if not inspect.isawaitable(instance_result):
                if instance_result.is_err():
                    return instance_result
                instance = instance_result.unwrap()
            ...
            # If a method name is provided via dot notation, execute that method on the instance
            if method_name:
                # For ObjectExposer, we already have the instance, so call the method directly
                if (
                    hasattr(exposer, "__class__")
                    and exposer.__class__.__name__ == "ObjectExposer"
                ):
                    return await exposer.run(
                        metadata.get("obj"), method_name, yaapp_engine=self, **kwargs
                    )
                else:
                    return await self.object_exposer.run(
                        instance, method_name, yaapp_engine=self, **kwargs
                    )
            else:
                # If no method_name, and it's a group, return the instance itself
                return Ok(instance)
    ...
    def activate_plugin(self, plugin_name: str, plugin_config: Dict[str, Any]) -> None:
        ...
        try:
            from .discovery import discover_plugins_metadata, load_plugin_class
    ...
    def get_plugins(self) -> Dict[str, Any]:
        ...
        from .discovery import discover_plugins_metadata
    ...
    def get_method_help(self, command_name: str, method_name: str) -> str:
        ...
        if isinstance(method_info, dict) and 'help' in method_info:
            help_text = method_info['help']
