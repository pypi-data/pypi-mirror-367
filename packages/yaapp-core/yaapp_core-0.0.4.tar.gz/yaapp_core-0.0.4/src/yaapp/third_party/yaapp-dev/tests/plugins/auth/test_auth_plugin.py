...
class TestAuthPlugin:
    ...
    def setup_method(self):
        ...
        # Create yaapp instance with auth plugin
        self.app = Yaapp(auto_discover=False)
        
        # Configure storage plugin (required by auth)
        storage_config = {"backend": "memory"}
        self.app.expose(self._create_storage_plugin(storage_config), name="storage")
        ...
        self.app.expose(auth_plugin, name="auth")
        
        # Set yaapp reference for auth plugin
        auth_plugin.yaapp = self.app
