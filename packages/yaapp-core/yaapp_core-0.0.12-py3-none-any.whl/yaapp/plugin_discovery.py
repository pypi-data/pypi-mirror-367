...
def load_plugin_class(plugin_metadata: Dict[str, Any]) -> Optional[Any]:
    ...
    try:
        ...
        # Always use external plugin approach - no hardcoded "builtin" detection
        # Just add the plugin directory to path and import directly
        plugins_dir = Path(search_path)
        if str(plugins_dir) not in sys.path:
            sys.path.insert(0, str(plugins_dir))
        
        # Use simple module path - no yaapp.plugins prefix
