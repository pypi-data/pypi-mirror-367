...
def _add_rest_endpoints(app, yaapp_engine, command_tree: dict, path_prefix: str = ""):
    ...
    # Add endpoints for each plugin
    for plugin_name, plugin_info in command_tree.items():
        plugin_path = f"{path_prefix}/{plugin_name}"
        ...
        def make_plugin_describe(plugin_name, plugin_info):
            def plugin_describe():
                return {
                    ...
                    "methods": list(plugin_info.get('commands', {}).keys()) if 'commands' in plugin_info else []
                }
            return plugin_describe
        
        app.get(f"{plugin_path}/_describe")(make_plugin_describe(plugin_name, plugin_info))
        
        # Add method endpoints
        methods = plugin_info.get('commands', {}) if plugin_info.get('type') == 'group' else {}
        for method_name in methods:
            ...
            endpoint_func = make_method_endpoint(plugin_name, method_name)
            app.post(
                method_path,
                summary=f"Execute {plugin_name}.{method_name}",
                description=f"Execute {method_name} method on {plugin_name} plugin"
            )(endpoint_func)
