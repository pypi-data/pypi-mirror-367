...
class PluginDiscovery:
    ...
    def _find_plugin_class_in_module(self, module, section_name: str) -> Optional[Type]:
        """Find plugin class in a module using common naming patterns."""
        ...
        class_name_patterns = [
            ...
            f"{section_name.title()}",        # storage -> Storage
            "Plugin",                         # Generic Plugin class
            f"{section_name.replace('_', '').title()}Plugin",  # session_handler -> SessionHandlerPlugin
        ]
        
        for class_name in class_name_patterns:
            if hasattr(module, class_name):
                cls = getattr(module, class_name)
                # Return the class (works for both real classes and mocks in tests)
                return cls
