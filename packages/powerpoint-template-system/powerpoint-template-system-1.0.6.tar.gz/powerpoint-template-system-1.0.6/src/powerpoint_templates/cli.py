...
def merge_and_generate(style_path: str, content_path: str, output_dir: str = ".", merged_path: str = None) -> None:
    """Merge style and content templates, then generate presentation"""
    try:
        ...
        if merged_path:
            ...
            print(f"✅ Merged configuration saved to '{merged_path}'")
        
        # Generate presentation from merged config
        print("🎯 Generating presentation from merged configuration...")
        generate_from_json_config(merged_config, output_dir)
        
        print("🎉 Successfully merged templates and generated presentation!")
        
    except ImportError:
        print("❌ Error: Could not import merge functionality. Make sure you're running from the project root.")
    except Exception as e:
        print(f"❌ Error in merge and generate: {e}")


def generate_from_json_config(config: Dict[str, Any], output_dir: str = ".", presentation_name: Optional[str] = None) -> None:
    """Generate presentations from JSON configuration object (internal use)"""
    generator = EnhancedVisualGenerator()
...
Documentation:
  ...
  Separated Templates: Use merge-generate to combine style and content templates
        """
    )
    ...
    # Merge and generate command
    merge_parser = subparsers.add_parser('merge-generate', help='Merge style and content templates, then generate presentation')
    merge_parser.add_argument('style', help='Style template JSON file path')
    merge_parser.add_argument('content', help='Content template JSON file path')
    merge_parser.add_argument('--output', '-o', default='.', help='Output directory (default: current)')
    merge_parser.add_argument('--merged', '-m', help='Save merged configuration to file (optional)')
    ...
    elif args.command == 'merge-generate':
        merge_and_generate(args.style, args.content, args.output, args.merged)
