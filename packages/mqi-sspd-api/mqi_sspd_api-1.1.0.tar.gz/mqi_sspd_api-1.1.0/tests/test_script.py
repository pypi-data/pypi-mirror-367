...
def test_mqi_api():
    ...
    status,biasboxes_count = m.get_number_of_biasboxes()
    ...
    status = m.get_status(biasbox_ID)
    ...
    status_set_current_response, body  = m.set_current(biasbox_ID, [1, 2], 0.0000005)
    print(f"Set Current Response: {status_set_current_response}\n")

    # Test: Set voltage for specific channels
    print("Testing set_voltage() for Biasbox ID 1, channels [1, 2], value 3...")
    status_set_voltage_response, body  = m.set_voltage(biasbox_ID, [1, 2,3], 0.1)
    ...
    status_set_voltage_response, num_channels = m.get_number_of_channels(biasbox_ID)
    ...
    set_channels_active_response, body  = m.set_channels_active(biasbox_ID, [1, 2])
    ...
    status, active_channels = m.get_channels_active(biasbox_ID)
    ...
    status, adc_voltage = m.get_voltage(biasbox_ID, [1, 2])
    ...
    status, target_voltage = m.get_target_voltage(biasbox_ID, [1, 2])
    ...
    status, max_volt_current = m.get_max_volt_current(biasbox_ID,[])
    ...
    status, id = m.get_id(biasbox_ID)
    print(f"Biasbox ID: {id}\n")

    # Test: Get the configuration of the system
    print("Testing get_config()...")
    status, config = m.get_config()
    print(f"System Configuration: {config}\n")

    # Test: Set the configuration of the system
    print("Testing set_config() with sample config...")
    sample_config = {"setting1": "value1", "setting2": "value2"}
    set_config_response = m.set_config(config)
