...
def send_slack_notification(report, test_run, test_env, test_run_id) -> None:
    ...
    failed_tests_count_high = len(
        [test for test in report['tests'] if test['outcome'] == 'failed' or test['outcome'] == 'error' and 'high' in test.get('keywords', [])])
    failed_tests_count_med = len(
        [test for test in report['tests'] if test['outcome'] == 'failed' or test['outcome'] == 'error' and 'medium' in test.get('keywords', [])])
    failed_tests_count_low = len(
        [test for test in report['tests'] if test['outcome'] == 'failed' or test['outcome'] == 'error' and 'low' in test.get('keywords', [])])
