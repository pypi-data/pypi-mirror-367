...
def hierarchical_clustering(df: pd.DataFrame, n_clusters: int = 2,title1: str = '层次聚类谱系图',title2: str = None,title3: str = '层次聚类二维散点图(PCA降维)',max_clusters: int = 10):
    ...
    输出: 谱系图、肘部图、PCA图、聚类中心表格和用户命名聚类结果表格
    '''
    # -------- [1] 拆分样本名与指标 --------
    df = df.copy()
    ...
    for k in possible_clusters:
        ...
        for i in range(1, k + 1):
            cluster_points = df_scaled[clusters_k == i]
            cluster_centers[i - 1] = cluster_points.mean(axis=0)
        sse_val = sum(np.sum((df_scaled[clusters_k == i + 1].values - cluster_centers[i]) ** 2)
                      for i in range(k))
        sse.append(sse_val)
    plt.figure(figsize=(10, 6))
