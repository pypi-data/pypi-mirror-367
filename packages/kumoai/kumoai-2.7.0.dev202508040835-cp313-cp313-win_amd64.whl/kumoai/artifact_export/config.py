...
class TrainingTableExportConfig(OutputConfig):
    ...
    def validate(cls, values: Dict) -> Dict:
        output_types = values.get('output_types')
        if output_types != {'training_table'}:
            raise ValueError("output_type must be set(['training_table'])"
                             f" (got {output_types})")
        if values.get('output_connector') is None:
            raise ValueError("output_connector is required")
        if values.get('output_table_name') is None:
            raise ValueError("output_table_name is required")
        if values.get('output_metadata_fields') is not None:
            raise ValueError(
