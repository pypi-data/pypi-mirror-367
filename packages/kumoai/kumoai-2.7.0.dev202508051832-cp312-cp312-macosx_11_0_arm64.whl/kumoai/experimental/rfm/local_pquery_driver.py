...
class LocalPQueryDriver:
    ...
    def collect_test(
        ...
        if not _coverage_warned and not reached_end and len(node) < size // 2:
            ...
            warnings.warn(f"Failed to collect {size:,} many test examples in "
                          f"{max_iterations} iterations. It is advised to "
                          f"increase the number of PQ iterations via the "
                          f"'max_pq_iterations' option to improve the "
                          f"coverage. This warning will be suppressed for the "
                          f"remainder of the script.")
    ...
    def collect_train(
        ...
        if not _coverage_warned and not reached_end and len(node) < size // 2:
            _coverage_warned = True
            warnings.warn(f"Failed to collect {size:,} many context examples "
                          f"in {max_iterations} iterations. It is advised to "
                          f"increase the number of PQ iterations via the "
                          f"'max_pq_iterations' option to improve the "
                          f"coverage. This warning will be suppressed for the "
                          f"remainder of the script.")
