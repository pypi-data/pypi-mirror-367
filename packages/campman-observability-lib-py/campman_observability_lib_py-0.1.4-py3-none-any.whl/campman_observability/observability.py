...
def setup_tracing(
    ...
    try:
        ...
        print(f"Tracing setup completed for service: {service_name}")
        return trace.get_tracer(__name__)
        
    except Exception as e:
        print(f"Failed to setup tracing: {e}. Application will continue without tracing.")
    ...
    try:
        ...
        if trace_ctx_str:
            ...
            if parsed_trace:
                ...
                print(f"Using existing trace_id: {parsed_trace['trace_id']} from {source}")
                return
        ...
        if current_span.is_recording():
            ...
            _set_span_attribute(current_span, TRACE_HEADER_KEY, trace_ctx_str)
            print("No existing trace found. Using current context.")
            
    except Exception as e:
        print(f"Error setting up trace context: {e}")
        ...
        if envelope and isinstance(envelope, dict):
            ...
            if isinstance(pubsub_message, dict):
                ...
                if isinstance(attributes, dict):
                    ...
                    if trace_ctx_str:
                        print(f"Found trace context in Pub/Sub message: {trace_ctx_str}")
                        return trace_ctx_str, "Pub/Sub message attributes"
    except Exception as e:
        print(f"Failed to get trace context from Pub/Sub: {e}")
    
    # Try HTTP headers
    try:
        trace_ctx_str = request.headers.get(HTTP_TRACE_HEADER)
        if trace_ctx_str:
            print(f"Found trace context in HTTP headers: {trace_ctx_str}")
            return trace_ctx_str, "HTTP headers"
    except Exception as e:
        print(f"Failed to get trace context from HTTP headers: {e}")

    # Try environment variable as fallback
    try:
        trace_ctx_str = os.environ.get('TRACE_CONTEXT')
        if trace_ctx_str:
            print(f"Found trace context in environment variable: {trace_ctx_str}")
            return trace_ctx_str, "environment variable"
    except Exception as e:
        print(f"Failed to get trace context from environment variable: {e}")
    
    print("No trace context found in any source.")    
    return None, None
...
def _update_log_fields_with_trace(
    ...
    try:
        ...
        print(f"Missing required trace field: {e}")
        raise
    except Exception as e:
        print(f"Failed to update log fields with trace: {e}")
        raise


def _set_span_attribute(current_span: trace.Span, key: str, value: str) -> None:
    """
    ...
    try:
        ...
        print(f"Failed to set span attribute: {e}")
...
def _parse_trace_context(trace_ctx_str: str) -> Optional[Dict[str, Union[str, bool]]]:
    ...
    try:
        ...
        if len(trace_span_components) != 2:
            print(f"Invalid trace context format: {trace_ctx_str}")
            return None
        ...
        if not trace_id or not span_id:
            print(f"Empty trace_id or span_id in: {trace_ctx_str}")
            return None
        ...
        if len(trace_id) != 32 or not all(c in '0123456789abcdefABCDEF' for c in trace_id):
            print(f"Invalid trace_id format: {trace_id}")
            return None
        ...
        try:
            ...
            print(f"Invalid span_id format: {span_id}")
            return None
        ...
        if len(parts) > 1:
            ...
            if options_part.startswith('o='):
                if options_part in ('o=0', 'o=1'):
                    ...
                    print(f"Invalid sampled flag: {options_part}")
                    return None
            else:
                print(f"Invalid options format: {options_part}")
                return None
        ...
        print(f"Failed to parse trace context '{trace_ctx_str}': {e}")
        return None
...
def _get_current_trace_context() -> str:
    ...
    try:
        ...
        print(f"Failed to get current trace context: {e}")
...
def get_trace_id() -> Optional[str]:
    ...
    try:
        ...
        print(f"Failed to get trace ID: {e}")
    return None
...
def add_span_attributes(**attributes: Any) -> None:
    ...
    try:
        ...
        print(f"Failed to add span attributes: {e}")


def create_child_span(name: str, **attributes: Any) -> Optional[trace.Span]:
    """
    ...
    try:
        ...
        print(f"Failed to create child span '{name}': {e}")
        return None