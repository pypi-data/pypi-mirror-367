...
class RuleControl(EntityXControl):
    ...
    # {'bigOAINet.base.member.UserControl.logout:ip:127.0.0.1': datetime.datetime(2025, 7, 17, 10, 0, 58, 162890)}
    # {'bigOAINet.base.member.UserControl.logout:user:1': datetime.datetime(2025, 7, 17, 10, 0, 58, 162890)}
    _cache_last_access_time = {}
    # 只存储最终计算后的规则
    # 匿名用户 key 统一为 anonymous，用户用 userId 作为 key
    # 'bigOAINet.base.member.UserControl.logout:user:1' = {'allow': False, 'min_frequency': 0, 'min_max_return': 0, 'count_param_name': None, 'deny_reason': '无权限'}
    # 'bigOAINet.base.member.UserControl.logout' = {'allow': False, 'min_frequency': 0, 'min_max_return': 0, 'count_param_name': None, 'deny_reason': '无权限'}
    _cache_rule = {}
    ...
    def check_access(self, functionName: str, params: object = None, 
        ...
        Args:
            functionName: 函数名称，有可能为缩写如：vote.VoteControl.add，要通过缩写变成全拼
            params: 参数，返回次数校验要用到
            user_id: 用户ID(-1表示未登录)
            user_roles: 用户角色列表
            user_rights: 用户权限列表
        ...
        if compiled_rule.min_frequency > 0:
            ...
            if elapsed_ms < compiled_rule.min_frequency:
                return im.set_error(f"访问频率过高。请等待 int({compiled_rule.min_frequency - elapsed_ms}) 毫秒后再试。", 429)
