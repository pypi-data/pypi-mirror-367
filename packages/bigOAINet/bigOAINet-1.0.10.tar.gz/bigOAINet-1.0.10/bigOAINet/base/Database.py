...
# 在这里执行，是因为 bigo 被引用的时候，所有实体都会被初始化，
# 所以在这里初始化数据库，确保数据库连接在所有实体初始化之前完成。
print('connect_database')
db, dh = mu.DatabaseHelper.init('database')
...
def init_inner_data():
    ...
    api_configs = [
        ...
        {
            'name': 'bigo login,register',
            'space': 'bigOAINet.base.member.UserControl',
            'codes': 'login,register',
            'sort': 1,
            'desc': 'bigo 会员登录、注册',
            'rules': [
                {
        ...
        {
            'name': 'bigo upload',
            'space': 'mxupy.disk.file',
            'codes': 'upload_user_file',
            'sort': 1,
            'desc': 'bigo 上传文件',
            'rules': [
                {
    ...
    # 用户
    if (im := bigo.UserControl.inst().register('bigo', 'bigo', 'bigO12#$')).error:
