...
class AcelerAIHttpClient():
    ...
    async def _write_to_file(self, ikey:str, query, data:list[dict], retries:int, retry_type:TYPE_OF_RETRY) -> None:
    ...
    async def fetch_bigpage(self, ikey: str, query: dict, retries: int = 0, retry_type: TYPE_OF_RETRY = TYPE_OF_RETRY.RETRY_FROM_BEGINNING) -> None:
        async with SEM:
            headers = {
                "Authorization": f"A2G {self.token}",
                "ikey": ikey,
            ...
            my_body = { "query": json.dumps(query, cls=CustomJSONEncoder), "retries": retries, "retry_type": retry_type.value }
                                ...
                                while len(buffer) >= 4:  # Asegurarse de que al menos 4 bytes están disponibles
                                    ...
                                    if not isinstance(data, list):
                                        ...
                                        if "aceler_retry" in data:
                                            logger.info("Reintentando descarga de datos...")
                                        else:
                                    ...
                                    if "aceler_retry" in data:           
                                        ...
                                        await self._write_to_file(ikey, query, data, retries=retries, retry_type=retry_type)
                    logger.info(f"Downloaded data on retry {retries}")
                    break  # Si todo va bien, salimos del bucle
                except (httpx.StreamError, httpx.ConnectError, httpx.ReadTimeout) as e:
                    # Manejar errores específicos de conexión y reinicio
