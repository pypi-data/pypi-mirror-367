...
Allocate a DoorDash session for a client with automatic credential management, cart restoration, and address isolation.

**🚀 Automatic Features:**
- **Credential Pool Management**: Automatically assigns available DoorDash credentials
- **Cart Restoration**: Restores previous cart contents if session existed before
- **Address Isolation**: Complete address privacy with automatic restoration from snapshots
- **Session Persistence**: Cart state and addresses saved automatically when session ends
- **Multi-Tenant Support**: Isolated sessions for different users with zero cross-contamination

**🏠 Address Requirements:**
- **New Users**: Must provide a valid address during first session acquisition
- **Existing Users**: Addresses automatically restored from encrypted snapshots
- **Privacy Guaranteed**: Users never see other users' addresses
- **Clean Validation**: Address validation uses read-only operations only

**📋 Required First Step:**
- **Always call this first** before using any other DoorDash tools
- Returns client_id needed for all subsequent operations
- Session expires after TTL (default: 60 minutes)
- Use release_session() when done to free up credentials and save snapshots

**🔄 Session Lifecycle:**
1. initialize_doordash() → get client_id (address required for new users)
2. Use client_id for search, cart, order operations  
3. release_session() → save cart state, address snapshots, and free credentials
    ...
    Returns:
        Dict containing session info with client_id for subsequent operations
    """
    try:
        if not org_id:
...
Release a session, automatically save cart state and address snapshots, and free up credentials for reuse.

**🔄 Automatic Snapshot Management:**
- **Cart State**: All cart contents saved to encrypted snapshots
- **Address Isolation**: User addresses captured and saved securely
- **Tenant Cleanup**: All user addresses removed from tenant account
- **Privacy Protection**: Only tenant default address remains (hidden from users)

**💾 What Gets Saved:**
- Complete cart contents with item details and options
- All user addresses with selection state
