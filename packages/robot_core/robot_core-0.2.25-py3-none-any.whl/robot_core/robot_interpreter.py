...
def main(raw_input):
	...
	try:
		...
		logger.info('流程结束运行', result=json.dumps(result, default=custom_default))
	except Exception as e:
		logger.exception(e)
		logger.info('流程运行失败')
		raise e
...
def init_log(robot_inputs):
	...
	logger.remove()
	# 添加 JSON 格式的处理器（控制台）
	logger.add(sys.stdout, format='{time:YYYY-MM-DD HH:mm:ss.SSS} | {level} | {message}', serialize=True)
