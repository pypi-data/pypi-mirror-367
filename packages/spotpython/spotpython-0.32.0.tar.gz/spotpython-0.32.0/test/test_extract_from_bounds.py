def test_extract_from_bounds():
    ...
    # Kriging:
    S = Kriging(name="kriging", seed=124, noise=False, cod_type="norm")
    S.fit(X, y)
    print(f"S.k: {S.k}, S.n: {S.n}, S.nat_X.shape: {S.nat_X.shape}, S.nat_y.shape: {S.nat_y.shape}")
    print(f"S.n_theta: {S.n_theta}")
