...
def test_de_bounds():
    ...
    n = 2
    p = 4
    S = Kriging(name="kriging", seed=124, n_p=p, optim_p=True)
    S._set_de_bounds()
    assert len(S.de_bounds) == n + p
    S = Kriging(name="kriging", seed=124, n_p=p, optim_p=False)
    S._set_de_bounds()
    assert len(S.de_bounds) == n
...
def test_extract_from_bounds():
    n = 2
    p = 2
    S = Kriging(name="kriging", seed=124, n_p=p, optim_p=True, noise=False)
...
def test_optimize_model():
    ...
    n = 2
    p = 2
    S = Kriging(name="kriging", seed=124, n_p=p, optim_p=True, noise=True)
...
def test_likelihood():
    ...
    n = 2
    p = 1
    S = Kriging(name="kriging", seed=124, n_p=p, optim_p=True, noise=False, theta_init_zero=True)
    S._initialize_variables(nat_X, nat_y)
    S._set_variable_types()
    S._set_theta_values()
    S._initialize_matrices()
