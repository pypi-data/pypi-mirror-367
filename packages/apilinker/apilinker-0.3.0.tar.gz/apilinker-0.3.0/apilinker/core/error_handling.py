...
class DeadLetterQueue:
    ...
    def get_items(self, 
        ...
        for file_path in dlq_files[:limit]:
            try:
                ...
                # Apply filters
                if error_category and item['error']['error_category'] != error_category.value:
                    continue
    ...
    def retry_item(self, item_id: str, operation: Callable[[Any], Any]) -> Tuple[bool, Optional[Any], Optional[ApiLinkerError]]:
        """
...
class CircuitBreaker(Generic[T]):
    ...
    def __init__(
        ...
        logger.info(f"Initialized circuit breaker '{name}' with failure threshold {failure_threshold}")
    ...
    def state(self) -> CircuitBreakerState:
        ...
        if (self._state == CircuitBreakerState.OPEN and
            time.time() - self._last_failure_time >= self.reset_timeout_seconds):
    ...
    def execute(self, operation: Callable[[], T]) -> Tuple[Optional[T], Optional[ApiLinkerError]]:
        """
        ...
        if current_state == CircuitBreakerState.HALF_OPEN and self._half_open_calls >= self.half_open_max_calls:
            logger.warning(f"Circuit '{self.name}' is HALF_OPEN and call limit reached - failing fast")
            return None, ApiLinkerError(
        ...
        try:
            ...
            # Handle failure
            error = ApiLinkerError.from_exception(e, additional_context={'circuit_breaker': self.name})
            self._failure()
            return None, error
    ...
    def _failure(self) -> None:
        ...
        if self._state == CircuitBreakerState.CLOSED and self._failure_count >= self.failure_threshold:
            logger.warning(f"Circuit '{self.name}' opening after {self._failure_count} consecutive failures")
            self._state = CircuitBreakerState.OPEN
            
        elif self._state == CircuitBreakerState.HALF_OPEN:
            logger.warning(f"Circuit '{self.name}' reopening after failure in HALF_OPEN state")
            self._state = CircuitBreakerState.OPEN
            
        logger.debug(f"Circuit '{self.name}' recorded failure ({self._failure_count}/{self.failure_threshold})")
...
class ErrorRecoveryManager:
    ...
    def __init__(self, dlq: Optional[DeadLetterQueue] = None):
        ...
        self.default_strategies: Dict[ErrorCategory, List[RecoveryStrategy]] = {
            ...
            ErrorCategory.SERVER: [RecoveryStrategy.CIRCUIT_BREAKER, RecoveryStrategy.EXPONENTIAL_BACKOFF],
        ...
        self.custom_strategies: Dict[str, Dict[ErrorCategory, List[RecoveryStrategy]]] = {}
    ...
    def set_strategy(
        ...
        logger.debug(f"Set {len(strategies)} recovery strategies for {error_category.value}" + 
        ...
        for strategy in strategies:
            ...
            # Exponential backoff retry strategy
                ...
                for attempt in range(max_retries):
                    try:
                        if attempt > 0:
                            logger.info(f"Waiting {current_delay:.2f}s before retry {attempt+1}/{max_retries}")
...
class ErrorAnalytics:
    ...
    def __init__(self, max_errors: int = 1000):
        ...
        self.error_rates: Dict[str, List[Tuple[float, int]]] = {}  # Category -> [(timestamp, count)]
    ...
    def record_error(self, error: ApiLinkerError) -> None:
        ...
        for cat in self.error_rates:
            self.error_rates[cat] = [
                (ts, count) for ts, count in self.error_rates[cat]
                if ts > one_hour_ago
            ]
    
    def get_error_rate(self, category: Optional[ErrorCategory] = None, minutes: int = 5) -> float:
        """
        ...
        for cat in categories:
            if cat in self.error_rates:
                error_count += sum(
                    count for ts, count in self.error_rates[cat]
                    if ts > since
                )
...
def create_error_handler(dlq_dir: Optional[str] = None) -> Tuple[DeadLetterQueue, ErrorRecoveryManager, ErrorAnalytics]:
    """
