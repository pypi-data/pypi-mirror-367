...
from earnix_elevate.clients.data.models.generate_llm_recipe_request import (
    GenerateLlmRecipeRequest,
)
from earnix_elevate.clients.data.models.lean_transformation_graph_response import (
    LeanTransformationGraphResponse,
)
from earnix_elevate.clients.data.models.llm_recipe_response import LlmRecipeResponse
...
class DataSetServiceApi:
    ...
    def generate_llm_recipe(
        ...
        generate_llm_recipe_request: Annotated[
            GenerateLlmRecipeRequest,
            Field(..., description="Generate LLM Recipe Request"),
        ],
        **kwargs,
    ) -> LlmRecipeResponse:
        """Generates a recipe using LLM for the specified data table and data version in the data set.
        ...
        :param generate_llm_recipe_request: Generate LLM Recipe Request (required)
        :type generate_llm_recipe_request: GenerateLlmRecipeRequest
        :return: Returns the result object.
        :rtype: LlmRecipeResponse
        """
        kwargs["_return_http_data_only"] = True
        if "_preload_content" in kwargs:
            message = "Error! Please call the generate_llm_recipe_with_http_info method with `_preload_content` instead and obtain raw data from ApiResponse.raw_data"
            raise ValueError(message)
        return self.generate_llm_recipe_with_http_info(
            data_set_id,
            data_set_node_id,
            data_table_id,
            data_version_number,
            generate_llm_recipe_request,
            **kwargs,
        )

    @validate_arguments
    def generate_llm_recipe_with_http_info(
        self,
        data_set_id: Annotated[
            StrictInt,
            Field(..., description="Unique identifier of the DataSet to operate on"),
        ...
        **kwargs,
        ...
        :type _content_type: string, optional: force content-type for the request
        :return: Returns the result object.
        :rtype: tuple(LlmRecipeResponse, status_code(int), headers(HTTPHeaderDict))
        """
        ...
        for _key, _val in _params["kwargs"].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method generate_llm_recipe" % _key
                )
            _params[_key] = _val
        del _params["kwargs"]
        ...
        # process the body parameter
        _body_params = None
        if _params["generate_llm_recipe_request"] is not None:
            _body_params = _params["generate_llm_recipe_request"]
        ...
        _response_types_map = {
            "200": "LlmRecipeResponse",
        }

        return self.api_client.call_api(
            "/v1/data-sets/{data-set-id}/nodes/{data-set-node-id}/data-tables/{data-table-id}/data-versions/{data-version-number}/generate-llm-recipe",
            "POST",
            _path_params,
            _query_params,
            _header_params,
