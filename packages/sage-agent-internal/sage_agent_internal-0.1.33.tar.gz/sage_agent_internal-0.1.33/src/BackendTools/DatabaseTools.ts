...
print("[DatabaseTools] DB_URL environment variable set in temporary kernel")
      ...
      return {
        ...
        cleanup: () => {
          console.log(`[DatabaseTools] Cleaning up temporary session: ${session.id}`);
          sessionManager.shutdown(session.id).catch((error) => {
            console.warn(`[DatabaseTools] Error shutting down session ${session.id}:`, error);
          });
        }
      };
    } catch (error) {
...
try:
    ...
    print(json.dumps({"error": f"Required modules not installed: {str(e)}. Please install sqlalchemy and psycopg2-binary"}))
    exit()
...
`;
  ...
  async executeQuery(query: string, dbUrl?: string): Promise<string> {
    try {
      console.log('[DatabaseTools] Executing SQL query via temporary kernel...');
