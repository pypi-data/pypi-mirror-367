...
//! ### Python quick-start
//! ```python
//! import rust_pyo3_timeseries as rts
...
//! ## Rust quick-start
//! ```rust
//! use rust_pyo3_timeseries::utils::extract_f64_array;
...
#[pymodule]
fn rust_pyo3_timeseries<'py>(_py: Python<'py>, m: &Bound<'py, PyModule>) -> PyResult<()> {
...
fn statistical_tests<'py>(
    _py: Python,
    rust_pyo3_timeseries: &Bound<'py, PyModule>,
) -> PyResult<()> {
    let statistical_tests = PyModule::new(rust_pyo3_timeseries.py(), "statistical_tests")?;
    statistical_tests.add_class::<EscancianoLobato>()?;
    rust_pyo3_timeseries.add_submodule(&statistical_tests)?;
    Ok(())
}
