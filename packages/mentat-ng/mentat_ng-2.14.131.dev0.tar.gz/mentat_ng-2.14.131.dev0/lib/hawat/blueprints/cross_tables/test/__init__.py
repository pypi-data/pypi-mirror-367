...
"""
Unit tests for :py:mod:`hawat.blueprints.cross_tables`.
"""
...
class SearchTestCase(TestRunnerMixin, HawatTestCase):
    """
    Class for testing ``cross_tables.search`` endpoint.
    """
    ...
    def _attempt_succeed(self):
        self.assertGetURL(
            "/cross_tables/search",
            200,
            [b"Calculate event cross tables"],
        )
...
class APISearchTestCase(TestRunnerMixin, HawatTestCase):
    """
    Class for testing ``cross_tables.apisearch`` endpoint.
    """

    def _attempt_fail_unauthorized(self):
        self.assertGetURL(
            "/api/cross_tables/search",
            401,
            [b"Unauthorized"],
        )
