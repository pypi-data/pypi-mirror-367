...
def enter_synthesis_mode(db: Database, config: Config) -> None:
    ...
    try:
        ...
        from litai.synthesis_tools import SynthesisOrchestrator, SynthesisConversation
        ...
        while True:
            try:
                ...
                if user_input.startswith("/"):
                    if user_input.startswith("/list"):
                    ...
                    elif user_input.startswith("/papers"):
                        logger.info(f"Slash command: /papers, papers in session: {len(conversation.session.selected_papers)}")
                        # Show current papers in session
                        if conversation.session.selected_papers:
                            console.print("\n[bold]Current Papers:[/bold]")
                        ...
                        console.print("[yellow]Unknown command. Use /list, /papers, /context, /clear, or /exit[/yellow]")
                        continue
...
class SynthesisCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        if text.startswith("/"):
            ...
            commands = {
                "/list": "Show all papers in your collection",
                "/papers": "Show current papers in session",
...
def show_help() -> None:
    ...
    console.print(
        "[cyan]/synthesize[/cyan] — Enter interactive synthesis mode for collaborative paper exploration"
    )
    console.print(
