...
class SimplePlotManager:
    ...
    def create_progress_plot(self, fig, canvas, x_range=None, y_range=None, 
        ...
        try:
            ...
            if "hypervolume" not in history_df.columns:
                self._plot_message(fig, "Hypervolume data not available")
                canvas.draw()
                return
            ...
            # Handle both old format (float) and new format (dict) for hypervolume
            hypervolumes = []
            for _, row in history_df.iterrows():
                hv_value = row["hypervolume"]
                if isinstance(hv_value, dict):
                    # Use raw hypervolume from new format
                    hypervolumes.append(hv_value.get("raw_hypervolume", 0.0))
                else:
                    # Use legacy format (simple float)
                    hypervolumes.append(
                        float(hv_value) if hv_value is not None else 0.0
                    )
            ...
            if has_normalized and show_normalized_hv:
                ...
                for _, row in history_df.iterrows():
                    hv_value = row["hypervolume"]
                    if isinstance(hv_value, dict):
                        normalized_hvs.append(
                            hv_value.get("normalized_hypervolume", 0.0)
                        )
                    else:
                        normalized_hvs.append(
                            0.0
                        )  # No normalized data for legacy format
            ...
            latest_iteration = history_df.iloc[-1] if not history_df.empty else None
            if (
                latest_iteration is not None
                and "convergence_analysis" in latest_iteration
            ):
            ...
            # Add convergence status annotation if available
            if convergence_info:
                status_text = (
                    f"Status: {convergence_info.get('recommendation', 'unknown')}"
                )
                if convergence_info.get("converged", False):
                    status_color = "lightgreen"
                    status_text += " (Converged)"
                elif "improvement" in convergence_info:
                    improvement = convergence_info["relative_improvement"]
            ...
            self._plot_message(fig, f"Progress plot error: {str(e)}")
    ...
    def _add_model_diagnostics_text(self, ax, model, n_predictions: int):
        ...
        try:
            ...
            props = dict(boxstyle="round", facecolor="wheat", alpha=0.8, fontsize=8)
            ax.text(
                0.02,
                0.98,
                textstr,
            ...
            logger.debug(f"Could not add model diagnostics: {e}")
    ...
    def create_3d_surface_plot(
        ...
        try:
            ...
            if response_name not in models:
                self._plot_message(
                    fig,
                    f"Model for {response_name} not available.\\nNeed more data for GP modeling.",
                )
                if canvas:
                    canvas.draw()
                return
            ...
            # Generate prediction points efficiently
            ...
            if Z_mean is None:
                self._plot_message(fig, "Failed to get GP surface predictions")
                if canvas:
                    canvas.draw()
                return

            # Create enhanced 3D visualization
            ax = self._create_enhanced_3d_plot(
                fig,
                X1,
                X2,
                Z_mean,
