...
async def async_pg_query(*args, **kwargs):
    """Async wrapper for pg_query using asyncio.to_thread"""
    return await asyncio.to_thread(pg_query, *args, **kwargs)
...
class CubeQueryService:
    ...
    @staticmethod
    async def execute_sql_query(payload: CubeSqlRequestPayload) -> list[tuple]:
        """Execute SQL query using psycopg2 with connection pooling only for sight client"""

        logger.info(f"Executing SQL query on {payload.host}:{payload.port}/{payload.user} for client_code='{payload.client_code}'")

        # Use the async version of pg_query with conditional connection pooling
