...
class CubeSqlMessageHandler(BaseMessageHandler):
    ...
    async def handle(self, message: WebSocketMessage, ws_connector: WebSocketConnector):
        ...
        result_data = None
        try:
            # Execute SQL query using CubeQueryService
            result_data = await cube_query_service.execute_sql_query(payload)
        except Exception as e:
            logger.error(f"SQL query execution failed: {str(e)}")
            result_data = str(e)
        finally:
            execution_time = (time.time() - start_time) * 1000  # Convert to milliseconds
            returned_msg = f"{len(result_data)} rows" if isinstance(result_data, list) else result_data
            logger.info(f"SQL query executed in {execution_time:.2f}ms, returned {returned_msg}")

        response_payload = CubeSqlResponsePayload(success=True, data=result_data, execution_time_ms=execution_time)
