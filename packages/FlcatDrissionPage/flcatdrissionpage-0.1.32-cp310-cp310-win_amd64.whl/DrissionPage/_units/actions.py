...
from DrissionPage.mouse_trajectory import request_trajectory
...
class Actions:
    ...
    def move_to(self, ele_or_loc, offset_x=None, offset_y=None, duration=.5):
        ...
        if isinstance(ele_or_loc, (tuple, list)):
            ...
            if not ele_or_loc.states.is_whole_in_viewport or ele_or_loc.states.is_covered:
    ...
    def move(self, offset_x=0, offset_y=0, duration=.5):
        ...
        relative_trajectory = request_trajectory([offset_x, offset_y])['trajectory']

        points = []
        start_x = self.curr_x
        start_y = self.curr_y
        for tp in relative_trajectory:
            points.append((start_x + int(tp[0]), start_y + int(tp[1])))
