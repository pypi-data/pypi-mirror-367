...
def positional_gmm():
    ...
    acim = acim[['LID','read_index', 'position', 'contig', 'event_level_mean', 'event_length']]
    acim = acim.groupby(by=['read_index', 'position', 'contig']).mean().reset_index()
    acim.fillna(0, inplace=True)

    dfn = pd.DataFrame(columns=['LID', 'position', 'contig', 'read_depth', 'percent_modified', 'Predict'])

    for lid in acim['LID'].unique():
        a = acim[acim['LID']==lid]
        a = a.loc[a['event_level_mean']!=0]

        #iterate over positions
        for pos in a['position'].unique():
            at = a.loc[a['position']==pos]

            ### acim data for all reads at position i ###
            b = pd.concat([at['event_level_mean'],at['event_length']], axis=1)
            b = b.to_numpy()

            ### dmso data for all reads at position i training ###
            c = pd.concat([d.loc[d['position']==pos, 'event_level_mean'],d.loc[d['position']==pos,'event_length']], axis=1)
            if len(c) < 2: #gmm requires at least 2
                dfn.loc[len(dfn.index)] = [lid, pos, seq, len(c), 0, 0]
            else:
                c = c.to_numpy()
                ...
                dfn.loc[len(dfn.index)] = [lid, pos, seq, len(labels), percent_modified, predict]
