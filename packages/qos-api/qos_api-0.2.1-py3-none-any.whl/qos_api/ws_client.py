...
class QOSWebSocketClient:
    def __init__(self, api_key: str):
        ...
        self._running = False
        self._subscriptions = {
            WSType.SNAPSHOT.value: [],
            WSType.TRADE.value: [],
            WSType.DEPTH.value: [],
            WSType.KLINE.value: []
        }

    def _record_subscription(self, type_: str, codes: List[str], extra: Optional[Dict] = None):
        """记录订阅请求参数"""
        sub = {"codes": codes}
        if extra:
            sub.update(extra)
        self._subscriptions[type_].append(sub)

    async def connect(self):
        """持续尝试建立WebSocket连接，直到成功"""
    ...
    async def _send_heartbeat_loop(self):
        ...
        try:
            ...
            logging.error(f"quit Heartbeat loop error: {e}")
            # 不进行 reconnect，由 _listen_messages 控制
        logging.warning("_send_heartbeat_loop end")
    ...
    async def _listen_messages(self):
        """持续监听消息"""
        logging.warning("WebSocket _listen_messages begin")
        while self._running and self.websocket:
            try:
                ...
                break
        logging.warning("WebSocket _listen_messages end")
        await self._reconnect()

    async def _reconnect(self):
        """触发重连"""
        logging.warning("Reconnecting WebSocket...")
    ...
    async def _resubscribe_all(self):
        """重连后恢复订阅，每秒一个"""
        for tp, items in self._subscriptions.items():
            for sub in items:
                request = {"type": tp}
                request.update(sub)
                try:
                    await self._send_request(request)
                    logging.warning(f"Resubscribed: {tp} - {sub}")
                except Exception as e:
                    logging.warning(f"Failed to resubscribe {tp}: {e}")
                await asyncio.sleep(1)  # 每隔1秒发一次请求
    ...
    async def heartbeat(self):
        logging.warning("send heartbeat")
        """5.1 发送心跳"""
    ...
    async def subscribe_snapshot(self, codes: List[str]):
        ...
        self._record_subscription(WSType.SNAPSHOT.value, codes)
    ...
    async def subscribe_trades(self, codes: List[str]):
        ...
        self._record_subscription(WSType.TRADE.value, codes)
        await self._send_request({
    ...
    async def subscribe_depth(self, codes: List[str]):
        ...
        if len(codes) > MAX_SUB_CODES:
            raise QOSAPIError(f"Max subscription count is {MAX_SUB_CODES}")
        self._record_subscription(WSType.DEPTH.value, codes)
        await self._send_request({
    ...
    async def subscribe_kline(self, codes: List[str], ktype: int):
        ...
        if len(codes) > MAX_SUB_CODES:
            raise QOSAPIError(f"Max subscription count is {MAX_SUB_CODES}")
        self._record_subscription(WSType.KLINE.value, codes)
        await self._send_request({
