...
class QOSWebSocketClient:
    ...
    async def connect(self):
        ...
        while self.websocket is None:
            try:
                ...
                self._running = True
                logging.info("WebSocket connected success")
                asyncio.create_task(self._listen_messages())
                asyncio.create_task(self._send_heartbeat_loop())
            except Exception as e:
                logging.warning(f"WebSocket connect failed: {e}")
    ...
    async def _send_heartbeat_loop(self):
        ...
        try:
            ...
            logging.warning(f"quit Heartbeat loop error: {e}")
    ...
    async def _listen_messages(self):
        """持续监听消息"""
        logging.info("WebSocket _listen_messages begin")
        while self._running and self.websocket:
            try:
                ...
                await self._reconnect()
                break
        logging.info("WebSocket _listen_messages end")
