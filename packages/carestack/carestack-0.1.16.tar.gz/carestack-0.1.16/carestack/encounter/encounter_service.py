...
class Encounter(BaseService):
    ...
    async def generate_fhir_from_files(
        ...
        """
        try:
            # Step 1: if enable_extraction is true Encrypt case sheets and call the _generate_discharge_summary method to extract data
            fhir_payload: Any = {"caseType": case_type}

            if document_references:
                encrypted_document_references_resp = await self.utilities.encryption(
                    payload={"files": document_references}
                )
                fhir_payload["encryptedDocRefs"] = encrypted_document_references_resp
            ...
            if enable_extraction:
                ...
                encrypted_extracted_data = await self.utilities.encryption(
                    payload={"data": casesheet_info.extracted_data}
                )
                fhir_payload["encryptedData"] = encrypted_extracted_data
                ...
                if patient_details and doctor_details:
                    fhir_payload["patientDetails"] = patient_details.model_dump(
                        by_alias=True
                    )
                    fhir_payload["practitionerDetails"] = doctor_details[0].model_dump(
                        by_alias=True
                    )
                    fhir_payload["enableExtraction"] = enable_extraction
                else:
    ...
    async def generate_fhir_from_sections(
        ...
        try:
            ...
            if patient_details and doctor_details:
                ...
                fhir_payload["practitionerDetails"] = [
                    doc.model_dump(by_alias=True) for doc in doctor_details
                ]
            else:
                raise ValueError(
                    "When extraction is disabled, patient and practitioner details are required."
                )

            if enabled_extraction:
                encrypted_sections_resp = await self.utilities.encryption(
                    payload={"data": sections.copy()}
                )
                fhir_payload["encryptedSections"] = encrypted_sections_resp
