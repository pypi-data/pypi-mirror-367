...
class AiService(BaseService):
    ...
    async def generate_fhir_bundle(
        ...
        try:
            ...
            if not fhir_bundle_dto.extracted_data:
                raise ValueError("No extracted data is provided.")
            else:
                encrypted_extracted_data = await self.utilities.encryption(
                    payload={"data": fhir_bundle_dto.extracted_data}
                )
            encrypted_document_references = await self.utilities.encryption(
                payload={"files": fhir_bundle_dto.document_references}
            )

            payload = {
                "caseType": fhir_bundle_dto.case_type,
                "enableExtraction": fhir_bundle_dto.enable_extraction,
                "encryptedData": encrypted_extracted_data,
                "encryptedDocRefs": encrypted_document_references,
            }
