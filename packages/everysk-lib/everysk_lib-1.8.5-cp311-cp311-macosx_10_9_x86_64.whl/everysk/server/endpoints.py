...
class BaseEndpoint:
    ...
    async def _log_error(self, error: Exception) -> None:
        ...
        if getattr(error, 'status_code', 500) in HTTP_STATUS_CODES_LOG:
            ...
            msg = str(error)
            # Print only the first 200 characters of the error message the full message is in the extra context
            log.error(msg[:200], extra={'http_payload': payload, 'labels': {'error': msg}})
...
class JSONEndpoint(BaseEndpoint):
    ...
    def _make_response(self, content: Any = None, status_code: int = 200) -> JSONResponse:
        ...
        """
        response = super()._make_response(content=content, status_code=status_code)
        response.serialize_dumps_params = self._serialize_dumps_params
        return response
