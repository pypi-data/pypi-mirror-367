...
class AzureBearerTokenAuthProvider(AuthProvider):
    """Azure Bearer token authentication - forwards user's token to MCP server."""

    def __init__(self, required_scopes: Optional[list] = None):
        self.required_scopes = required_scopes or []
        logger.info(f"🔑 AzureBearerTokenAuthProvider initialized with required_scopes: {required_scopes}")
    ...
    async def authenticate(self, request_headers: Dict[str, str]) -> AuthContext:
        ...
        try:
            # Simple JWT parsing without verification for basic claims extraction
            import base64
            ...
            token_scopes = claims.get("scp", "").split() if claims.get("scp") else claims.get("scope", "").split()

            # Check required scopes if specified
            if self.required_scopes:
                if not all(scope in token_scopes for scope in self.required_scopes):
                    logger.warning(f"Token missing required scopes. Has: {token_scopes}, Required: {self.required_scopes}")
                    raise AuthenticationError("Insufficient scopes")

            logger.info(f"✅ Bearer token authenticated for user: {claims.get('upn', claims.get('sub', 'unknown'))}")

            return AuthContext(
                method=AuthMethod.AZURE_BEARER,
                access_token=token,
                user_id=claims.get("sub"),
                tenant_id=claims.get("tid"),
                scopes=token_scopes,
            ...
            raise AuthenticationError(f"Invalid JWT payload: {e}")
        except Exception as e:
            raise AuthenticationError(f"Invalid token: {e}")

    def get_environment_vars(self, auth_context: AuthContext) -> Dict[str, str]:
        """Pass user's bearer token to MCP server for direct Azure API access."""
        if not auth_context.access_token:
            raise AuthenticationError("No access token available")

        env_vars = {
            # Pass the user's token directly
            "AZURE_ACCESS_TOKEN": auth_context.access_token,
            "AZURE_TOKEN_TYPE": "bearer",
            
            # Add user context
            "AZURE_USER_ID": auth_context.user_id or "",
            "AZURE_TENANT_ID": auth_context.tenant_id or "",
            "AZURE_TOKEN_SCOPES": " ".join(auth_context.scopes or []),
            
            # Disable other credential types to force token usage
            "AZURE_EXCLUDE_CLI_CREDENTIAL": "true",
            "AZURE_EXCLUDE_BROWSER_CREDENTIAL": "true", 
            "AZURE_EXCLUDE_SHARED_TOKEN_CACHE_CREDENTIAL": "true",
            "AZURE_EXCLUDE_VISUAL_STUDIO_CREDENTIAL": "true",
            "AZURE_EXCLUDE_VISUAL_STUDIO_CODE_CREDENTIAL": "true",
            "AZURE_EXCLUDE_AZURE_POWERSHELL_CREDENTIAL": "true",
            "AZURE_EXCLUDE_AZURE_DEVELOPER_CLI_CREDENTIAL": "true",
            "AZURE_EXCLUDE_MANAGED_IDENTITY_CREDENTIAL": "true",
            "AZURE_EXCLUDE_ENVIRONMENT_CREDENTIAL": "true",
        }

        logger.info("📋 Passing user bearer token to MCP server for direct Azure API access")
        logger.info(f"  Token scopes: {auth_context.scopes}")
        logger.info(f"  User ID: {auth_context.user_id}")
        logger.info(f"  Tenant ID: {auth_context.tenant_id}")
