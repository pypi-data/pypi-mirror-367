...
class AzureDefaultAuthProvider(AuthProvider):
    ...
    def get_environment_vars(self, auth_context: AuthContext) -> Dict[str, str]:
        ...
        # If we have managed identity variables but no AZURE_CLIENT_ID, 
        # try to get it from function app configuration
        if ("MSI_ENDPOINT" in env_vars or "IDENTITY_ENDPOINT" in env_vars) and "AZURE_CLIENT_ID" not in env_vars:
            # Check for user-assigned managed identity client ID in various places
        ...
        logger.info(f"=== PASSING {passed_vars + 1} ENVIRONMENT VARIABLES TO MCP SERVER ===")
