"""CSO Module management commands."""
...
class ModuleSpec:
    ...
    def __init__(self, path: Path):
        ...
        self.path = path
        self.cso_dir = path / ".cso"
        self.spec_file = self.cso_dir / "module.yaml"
        self.spec = None
    ...
    def is_valid(self) -> bool:
        """Check if module has valid .cso directory."""
        return self.cso_dir.exists() and self.spec_file.exists()
    ...
    def deployment_type(self) -> str:
        """Get deployment type."""
        if self.spec:
            return self.spec.get("spec", {}).get("deployment", {}).get("type", "unknown")
        return "unknown"
...
def validate_module(
    ...
    """Validate a module against CSO specification."""
    
    console.print(f"[bold cyan]Validating module at:[/bold cyan] {path}")
    
    # Check for .cso directory
    module = ModuleSpec(path)
    
    if not module.is_valid:
        console.print("[red]✗ No .cso/module.yaml found![/red]")
        console.print("\n[yellow]Tip:[/yellow] Initialize module with: [cyan]cso module init[/cyan]")
        raise typer.Exit(1)
    ...
    with Progress(
        ...
        if module_info.get("tier") not in ["core", "standard", "custom", "experimental"]:
        ...
        if monitoring.get("metrics", {}).get("enabled", True):
            metrics_path = monitoring["metrics"].get("path", "/metrics")
        ...
        # 9. Check for additional files in .cso directory
        progress.update(task, description="Checking additional configurations...")
        
        expected_files = ["module.yaml", "compliance.yaml", "dependencies.yaml", "security/network-policy.yaml"]
        for file in expected_files:
            file_path = module.cso_dir / file
            if file != "module.yaml" and not file_path.exists():
                if verbose:
                    warnings.append(f"Optional file not found: .cso/{file}")
...
def init_module(
    ...
    deployment: str = typer.Option("kustomize", "--deployment", "-d", help="Deployment type"),
    force: bool = typer.Option(False, "--force", "-f", help="Overwrite existing files")
):
    """Initialize a new CSO module structure."""
    
    module_path = path / name if path.name != name else path
    cso_dir = module_path / ".cso"
    
    # Check if already exists
    if cso_dir.exists() and not force:
    ...
    module_spec = {
        ...
        "spec": {
            "module": {
                ...
                "version": "0.1.0",
                "tier": tier,
                "description": f"{name} module for CloudStack Orchestrator",
                "maintainer": {
    ...
    spec_file = cso_dir / "module.yaml"
    with open(spec_file, "w") as f:
        yaml.dump(module_spec, f, default_flow_style=False, sort_keys=False)
    
    console.print(f"[green]✓[/green] Created .cso/module.yaml")
    ...
    if deployment == "kustomize":
        ...
        console.print(f"[green]✓[/green] Created k8s/base/kustomization.yaml")
        ...
        console.print(f"[green]✓[/green] Created k8s/base/namespace.yaml")
...
"""
    ...
    console.print(f"\nNext steps:")
    console.print(f"  1. Update .cso/module.yaml with your module details")
    console.print(f"  2. Implement your module in {module_path}")
    console.print(f"  3. Validate with: [cyan]cso module validate {module_path}[/cyan]")
    console.print(f"  4. Deploy with: [cyan]cso module deploy {module_path}[/cyan]")


@app.command("list")
def list_modules(
    path: Path = typer.Option(None, "--path", "-p", help="Search path (defaults to parent projects directory)"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON")
):
    """List all CSO modules in a directory."""
    
    # Default to parent projects directory
    if path is None:
        # Go up from cso-cli to cloudstack-orchestrator to projects
        search_path = Path.cwd().parent.parent if "cso-cli" in str(Path.cwd()) else Path.cwd().parent
    else:
        search_path = path
    
    modules = []
    
    # Search recursively for modules with .cso directories
    for cso_dir in search_path.rglob(".cso"):
        if cso_dir.is_dir():
            module_dir = cso_dir.parent
    ...
    if json_output:
        ...
        if not modules:
            console.print("[yellow]No CSO modules found[/yellow]")
            return
        
        table = Table(title="CSO Modules")
...
def deploy_module(
    ...
    if not module.is_valid:
        console.print("[red]Invalid module: missing .cso/module.yaml[/red]")
        raise typer.Exit(1)
    ...
    if not dry_run:
        ...
        console.print(f"  cso module status {path}")
...
def test_module(
    path: Path = typer.Argument(Path.cwd(), help="Module path"),
    test_type: str = typer.Option("all", "--type", "-t", help="Test type: unit, e2e, integration, all"),
