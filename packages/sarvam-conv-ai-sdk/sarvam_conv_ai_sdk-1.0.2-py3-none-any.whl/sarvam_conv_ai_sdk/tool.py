...
class InteractionTurnRole(StrEnum):
    USER = "user"
    AGENT = "agent"


class InteractionTurn(BaseModel):
    role: InteractionTurnRole = Field(description="Role of the speaker")
...
class InteractionTranscript(BaseModel):
    app_id: str
    app_version: int
    interaction_transcript: list[InteractionTurn] = Field(
...
class SarvamOnEndToolContext(SarvamToolBaseContext):
    user_identifier: str
    interaction_transcript: Optional[InteractionTranscript] = Field(
    ...
    def get_interaction_transcript(self) -> Optional[InteractionTranscript]:
