...
def _write_wrapper_script(path: Path):
...
def mommify_patch_venv(venv_dir: Optional[Path] = None):
    ...
    for path in list(bin_path.iterdir()):
        ...
        if path.is_symlink():
            ...
            expected_returncode = 161
            proc = subprocess.run(
                [str(path), '-c', f'exit({expected_returncode})'],
                timeout=0.1,
            ...
            if proc.returncode != expected_returncode:
                serious_logger.debug("%s doesn't link to a python interpreter")
                continue

            if name.startswith("inner_"):
                wrapper_script = path.parent / path.name.replace("inner_", "", 1)
                serious_logger.info("updating %s", wrapper_script)
                mommy_logger.info("%s only updates the wrapper %s", MOMMY.ROLE, wrapper_script)
                _write_wrapper_script(wrapper_script) 
            else:
                serious_logger.info("newly wrapping %s", path)
                mommy_logger.info("%s wrapt the interpreter %s", MOMMY.ROLE, path)
