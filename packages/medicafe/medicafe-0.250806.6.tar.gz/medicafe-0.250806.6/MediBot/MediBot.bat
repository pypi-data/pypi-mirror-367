...
echo.
echo DEBUG: Package version is: %package_version%
echo DEBUG: Update script path is: %upgrade_medicafe%
echo.

:: Check if update_medicafe.py exists in the expected location, with fallback
if exist "%upgrade_medicafe%" (
    echo Found update script at: %upgrade_medicafe%
    ...
    echo Update script not found at expected location: %upgrade_medicafe%
    echo Using relative path: MediBot\update_medicafe.py
    echo Starting update process...
    start "Medicafe Update" cmd /c py "MediBot\update_medicafe.py" %package_version% > upgrade_log.txt 2>&1 && (
        echo %DATE% %TIME% Upgrade initiated. >> "%temp_file%"
        echo Exiting batch to complete the upgrade.
    ) || (
        echo %DATE% %TIME% Update failed. Check logs. >> upgrade_log.txt
    ...
    echo.
    echo Please ensure the update script is available and try again.
