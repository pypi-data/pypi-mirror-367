...
from .providers import (
    ColorfulTableProvider, ProxyModelProvider, TableProvider,
    TemplateProvider, SingleMergedTableProvider, ProxyPDFModelProvider, AddTableProvider
)
...
class CheckBoxReportView(SqlReportView):
    ...
    def post(self, request):
        fields = tuple(field for field in request.POST.keys() if
                       field != 'csrfmiddlewaretoken' and field != 'date_min'
                       and field != 'date_max' and field != 'report_format')
        form = self.form_class(request.POST, form_fields=self.form_fields)
...
class ReplacerView(TemplateReportView):
    ...
    def add_data(self, pk: int, func_args: list) -> Iterable:
        obj = self.model.proxy_objects.get_all_data().get(pk=pk)
        data = {'replace_data': obj.get_report_data(),
                'table_data': run_sql_functions(self.sql_function, func_args)}
        return self.update_data(data)
...
class SqlReportViewOptional(FileNameMixin, BaseReportView):
    ...
    Атрибуты:
        sql_function (Union[str, dict]): Название SQL-функции или словарь {ключ: функция} для множественных запросов.
        fields (Union[tuple, dict, str]): Список или отображение полей формы. По умолчанию '__all__'.
        template_name (str): Шаблон формы, отображаемый в модальном окне.
        task_name (callable): Функция для запуска асинхронной задачи генерации отчёта.
        provider (BaseProvider): Класс для вставки таблиц в отчёт (например, Word/Excel).
