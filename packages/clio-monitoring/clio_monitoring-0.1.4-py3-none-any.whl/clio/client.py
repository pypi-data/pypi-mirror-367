...
class ClioMonitor:
        ...
        try:
            ...
            async def on_page_created(page: Page) -> None:
                ...
                async def on_page_close(closing_page: Page) -> None:
                    """Handle page close event - just log, don't upload video yet"""
                    logger.info(f"🔒 Page closed: {closing_page.url}")
                    
                    # Just update the video path, but don't upload yet
                    # Videos are only flushed when context closes
                    with self._lock:
                        run_info = self._active_runs.get(context)
                        if run_info and closing_page in run_info["pages"]:
                            ...
                            if not page_info["video_path"] and closing_page.video:
                                try:
                                    ...
                                    logger.info(f"🎥 Stored video path: {mask_sensitive_data(page_info['video_path'])}")
                                except Exception as e:
                                    logger.warning(f"Could not get video path: {e}")
                            
                            logger.info(f"📝 Page video will be uploaded when context closes")
            ...
            async def patched_close(*, reason: Optional[str] = None) -> None:
                """Patched close method that saves trace before closing context"""
                logger.info(f"🔒 Context close initiated, saving trace first...")
                
                # Stop and save trace before closing
                await stop_and_save_trace()
                
                # Upload trace and any remaining videos
                logger.info(f"📤 Uploading trace and remaining videos...")
                try:
                ...
                # Now call the original close method
    ...
    async def _handle_context_upload(self, context: BrowserContext) -> None:
        ...
        try:
            ...
            # Upload any remaining videos that weren't uploaded on page close
            for _page, page_info in pages_info.items():
                video_path_str = page_info.get("video_path")
                if video_path_str and not page_info.get("uploaded"):
                    logger.info(f"📤 Uploading remaining video: {mask_sensitive_data(video_path_str)}")
                    try:
                        ...
                        if video_path.exists():
                            ...
                            if video_success:
                                ...
                                any_video_uploaded = True
                                logger.info(f"✅ Uploaded remaining video")
                    except Exception as e:
                        logger.error(f"Failed to upload remaining video: {e}")
