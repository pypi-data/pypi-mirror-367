...
# tool_dict is now loaded dynamically in _filter_tools_by_config
...
class CodeInstance:
    ...
    def __init__(self, type: str, config: Dict, stat: ToolCallStats = None, auto_start: bool = False):
        ...
        self.jupyter_enabled = config.get('jupyter_enabled', False)
        ...
        # Filter tool_dict based on Jupyter configuration
        self.tool_dict = self._filter_tools_by_config()
        
        # Auto-start if requested
        if auto_start:
            self.start_sync()
    
    def _filter_tools_by_config(self) -> Dict:
        """Filter tool_dict based on configuration settings."""
        from .tools.tool_dict import tool_dict
        
        # Start with all tools
        filtered_tools = dict(tool_dict)
    ...
    def get_method_tools(self):
        new_method_list = []
        for key in self.tool_dict:
            tool = {
                "method_name": key,
                "method_type": "inbuilt",
                "description": self.tool_dict[key]
            }
