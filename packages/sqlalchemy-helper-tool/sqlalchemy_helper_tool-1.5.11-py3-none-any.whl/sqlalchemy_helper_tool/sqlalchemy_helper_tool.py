...
class DbApi:
    ...
    def write_sql_key2(self, df, table_name):
        ...
        # Asegurar que los nombres de columnas sean strings y estén limpios
        df.columns = df.columns.astype(str)
        
        # Crear un alias sin espacios para los placeholders
        alias_map = {col: col.strip().replace('`', '').replace(' ', '_') for col in df.columns}

        sql_columns = ', '.join(f"`{col}`" for col in alias_map.keys())
        placeholders = ', '.join(f":{alias}" for alias in alias_map.values())

        query = text(
            f"INSERT IGNORE INTO `{self.database}`.`{table_name}` ({sql_columns}) VALUES ({placeholders})"
        )

        # Reemplazar NaN con None y renombrar las claves de los dicts
        df_clean = df.where(pd.notnull(df), None)
        tuples = [
            {alias_map[k]: v for k, v in row.items()}
            for row in df_clean.to_dict(orient='records')
        ]

        # Ejecutar dentro de una transacción
        with self.con.begin() as conn:
