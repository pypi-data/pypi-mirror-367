...
class DbApi:
    ...
    def write_sql_key2(self, df, table_name):
        ...
        clean_cols = [col.strip().replace('`', '') for col in df.columns]

        # Columnas escapadas para el SQL
        sql_columns = ', '.join(f"`{col}`" for col in clean_cols)
        ...
        # Limpiar valores NaN y convertir en lista de dicts
        df_clean = df.where(pd.notnull(df), None)
        tuples = df_clean.to_dict(orient='records')
