...
class DbApi:
    ...
    def table_in_db(self, table_name):
        tables_list = self.engine.table_names()
        ...
        if dict_params:  # Only execute if dict_params is not None or empty
            with self.engine.connect() as connection:
                for k, v in dict_params.items():
                    connection.execute(text(f"SET @{k} := :val"), {'val': v})

        # Execute SQL
        return pd.read_sql_query(sql=my_query, con=self.engine)
    ...
    def write_sql_key(self, df, table_name):
        ...
        with self.engine.connect() as connection:
            connection.execute(text(query), tuples)
    ...
    def replace_sql_values(self, df, table_name, column_replace, columns_key):
        ...
        query = f"""
            INSERT INTO `{table_name}` ({values_columns}) 
            VALUES {tuples} 
            ON DUPLICATE KEY UPDATE `{column_replace}`=VALUES(`{column_replace}`);
        """
        self.execute_query(query, commit=True)
