...
class CustomLitellmModel(LitellmModel):
    """LitellmModel that passes credentials as parameters"""
    def __init__(self, model_name: str, **kwargs):
        model_cfg = get_model_config().get_model(model_name)
        if not model_cfg:
            raise ValueError(f"Model '{model_name}' not found in models.json")
        
        # Pass credentials through model_kwargs
        if 'model_kwargs' not in kwargs:
            kwargs['model_kwargs'] = {}
        kwargs['model_kwargs'].update({
            'api_key': model_cfg['api_key'],
            'api_base': model_cfg['endpoint'],
            'custom_llm_provider': 'openai'
        })
        
        super().__init__(model_name=model_cfg['model'], **kwargs)
    ...
    def run(self, prompt: str, model="gpt-4o", system_prompt=None, cli="mini-swe"):
        if cli == "mini-swe":
            temp_agent = DefaultAgent(
                CustomLitellmModel(model_name=model),
