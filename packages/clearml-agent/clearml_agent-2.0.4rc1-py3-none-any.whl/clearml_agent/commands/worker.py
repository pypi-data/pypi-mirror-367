...
class Worker(ServiceCommandSection):
                ...
                for queue in queues:
                    ...
                    try:
                        ...
                        if gpu_queues and gpu_queues.get(queue):
                            self.log.debug(f"Allocating dynamic GPUs for queue {queue}")
                            gpus = []
                            fractions = []
                            ...
                            self.log.debug(f" - gpus: {gpus}")
                            self.log.debug(f" - available_gpus: {available_gpus}")
                            ...
                            self.log.debug(f" - worker_id: {self.worker_id}")
    ...
    def _dynamic_gpu_get_available(self, gpu_indexes):
        ...
        self.log.debug(f"Getting available dynamic GPUs")
        ...
        self.log.debug(f" - dyn_gpu workers: {', '.join(our_workers)}")
        ...
        self.log.debug(f" - gpus: {gpus}")
        self.log.debug(f" - allocated_gpus: {allocated_gpus}")
        self.log.debug(f" - available_gpus: {available_gpus}")
    ...
    def _setup_dynamic_gpus(self, gpu_queues, gpu_indexes):
        ...
        self.log.debug(f"Setting up dynamic GPUs:")
        self.log.debug(f" - available_gpus raw: {available_gpus}")
        ...
        self.log.debug(
            f" - setting runtime: {','.join('{}_{}'.format(g, str(v)[2:]) for g, v in available_gpus.items())}"
        )
    ...
    def _parse_dynamic_gpus(self, kwargs, queues):
        ...
        self.log.debug(
            f"Dynamic GPUs mode:\n - Queue Names: {', '.join(queue_names)}\n - GPU Idxs: {', '.join(gpu_indexes)}"
        )
        ...
        self.log.debug(
            f"Dynamic GPUs parsed:\n - dynamic_gpus: {dict(dynamic_gpus)}\n - gpu_indexes: {', '.join(gpu_indexes)}\n - queues: {', '.join(queues)}"
        )
