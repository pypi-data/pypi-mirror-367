        ...
        try:
            ...
            while True:
                ...
                if current_time - start_time > self.timeout:
                    ...
                    process.terminate()
                    process.wait()
                    self.error_signal.emit(f"Process timed out after {self.timeout} seconds - requires manual download")
                    return
...
class MarketSwimmerGUI(QMainWindow):
    ...
    def download_data(self):
        ...
        self.console_output.append("⚠️ NOTE: This will open a browser window where you need to manually download the XLSX file")
        self.console_output.append("After downloading, click 'Calculate Owner Earnings' to process the file")
        self.console_output.append("The process will timeout after 10 seconds")
        self.disable_buttons()
        self.show_progress()
        
        # Use the modern CLI for data analysis with timeout
        command = f'python -m marketswimmer analyze {self.current_ticker} --force'
        self.worker_thread = WorkerThread(command, timeout=10)
    ...
    def run_full_analysis(self):
        ...
        self.console_output.append("⚠️ NOTE: This will open a browser for manual download (10s timeout)")
        self.console_output.append("Please wait...\n")
        
        self.disable_buttons()
        self.show_progress()
        
        # Use the new MarketSwimmer CLI automation with timeout
