...
class AnalysisWorkflow:
    ...
    def _save_fair_value_analysis(self, calculator, output_file):
        ...
        try:
            ...
            console.print("[dim]DEBUG: Starting fair value analysis...[/dim]")
            ...
            console.print(f"[dim]DEBUG: Found {len(owner_earnings)} years of owner earnings data[/dim]")
            
            # Extract shares outstanding data for each period
            shares_data = []
            for year in sorted(owner_earnings.keys(), reverse=True):
                try:
                    console.print(f"[dim]DEBUG: Extracting shares for year {year}...[/dim]")
                    shares_outstanding = calculator.extract_shares_outstanding(year)
                    console.print(f"[dim]DEBUG: Year {year} shares: {shares_outstanding}[/dim]")
                    ...
                    if shares_outstanding and shares_outstanding > 0:
                        shares_data.append({
                            ...
                            'Owner_Earnings_Per_Share': owner_earnings[year]['owner_earnings'] / shares_outstanding
                        })
                        console.print(f"[dim]DEBUG: Added data for year {year}[/dim]")
                    else:
                        console.print(f"[dim]DEBUG: No valid shares data for year {year}[/dim]")
                except Exception as e:
                    console.print(f"[yellow]WARNING: Could not get shares for {year}: {e}[/yellow]")
                    continue
            
            if not shares_data:
                console.print("[yellow]WARNING: No shares outstanding data found for fair value analysis[/yellow]")
                console.print("[dim]DEBUG: Attempting to create basic fair value estimates without shares data...[/dim]")
                
                # Create a simplified version without shares data
                basic_fair_values = []
                avg_owner_earnings = sum(data['owner_earnings'] for data in owner_earnings.values()) / len(owner_earnings)
                
                # Use a default shares outstanding estimate (this should be improved)
                default_shares = 1000000000  # 1 billion shares as fallback
                avg_oe_per_share = avg_owner_earnings / default_shares
                
                for multiple in [10, 15, 20, 25]:
                    fair_price = avg_oe_per_share * multiple
                    basic_fair_values.append({
                        'Valuation_Multiple': f"{multiple}x",
                        'Fair_Price_Estimate': fair_price,
                        'Avg_Owner_Earnings_Per_Share': avg_oe_per_share,
                        'Note': 'Estimated with default shares count'
                    })
                
                # Save basic fair value estimates
                fair_value_file = output_file.parent / f"fair_value_estimates_{output_file.stem.split('_')[-1]}.csv"
                basic_df = pd.DataFrame(basic_fair_values)
                basic_df.to_csv(fair_value_file, index=False)
                
                console.print(f"[yellow]>> Basic fair value estimates saved (without shares data):[/yellow]")
            ...
            console.print(f"[dim]DEBUG: Created DataFrame with {len(shares_df)} rows[/dim]")
            
            # Calculate average owner earnings per share
            avg_oe_per_share = shares_df['Owner_Earnings_Per_Share'].mean()
            console.print(f"[dim]DEBUG: Average OE per share: {avg_oe_per_share}[/dim]")
            ...
            console.print(f"[dim]DEBUG: Saved shares data to {output_file}[/dim]")
            
            # Save fair value estimates
            fair_value_file = output_file.parent / f"fair_value_estimates_{output_file.stem.split('_')[-1]}.csv"
            fair_values_df = pd.DataFrame(fair_values)
            fair_values_df.to_csv(fair_value_file, index=False)
            console.print(f"[dim]DEBUG: Saved fair value estimates to {fair_value_file}[/dim]")
            ...
            import traceback
            console.print(f"[red]DEBUG: {traceback.format_exc()}[/red]")
    ...
    def _generate_visualizations(self, ticker: str) -> bool:
        ...
        try:
            with Progress(
                ...
                console.print("[dim]DEBUG: Starting visualization generation...[/dim]")
                ...
                try:
                    ...
                    console.print(f"[dim]DEBUG: Calling visualization_main with ticker: {ticker}[/dim]")
                    ...
                    console.print(f"[yellow]WARNING: Visualization dependencies not available: {e}[/yellow]")
