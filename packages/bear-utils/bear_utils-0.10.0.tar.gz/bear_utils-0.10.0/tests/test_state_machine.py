import pytest
from bear_utils.extras.state_tracking import StateMachine, State, StateTransitionError, auto_value_decorator, state
...
class TestState:
    ...
    def test_state_equality_with_state(self):
        ...
        assert 0 == state1  # Any enum with same value will be equal
    ...
    def test_state_multiple_states(self):
        ...
        assert int(state.value) == 0
        assert int(state2.value) == 1
...
class TestAutoValueSystem:
    def test_string_conversion(self):
        ...
        assert sm.INFO.value == 0
        assert sm.WARNING.value == 1
        assert sm.ERROR.value == 2
        ...
        assert sm2.START.value == 0
        assert sm2.MIDDLE.value == 1
        assert sm2.END.value == 2
