...
from bear_utils.extras.state_tracking._common import StateTransitionError, get_original, is_auto
...
class StateMachine:
    ...
    def _process_state_map(dict_items: dict) -> dict[str, State]:
        ...
        for state_name, state in raw_states.items():
            if _counter == 0:
                state.initial = True
            if is_auto(state.value):
                local_value: int = get_original(state.value)
                if local_value > _counter:
                    state.value = local_value
                    _counter.set(local_value).increment()
                elif local_value == 0:
                    state.value = _counter.get(increment_after=True)
            elif not is_auto(state.value) and isinstance(state.value, int):
                if state.value > _counter:
                    _counter.set(state.value).increment()
                elif state.value == 0:
                    state.value = _counter.get(increment_after=True)
            processed_map[state_name] = state
