...
class OrderBuilder:
    ...
    def limit(self, price: Union[Decimal, int, float]) -> OrderBuilder:
        ...
        self._limit_price = price_decimal
        return self
    ...
    def stop(self, price: Union[Decimal, int, float]) -> OrderBuilder:
        ...
        self._stop_price = price_decimal
        self._limit_price = None
        return self
    ...
    def stop_limit(
        ...
        Raises:
            ValueError: If prices are not positive
        """
        stop_decimal = Decimal(str(stop_price))
        limit_decimal = Decimal(str(limit_price))
        if stop_decimal <= 0:
            raise ValueError("Stop price must be positive")
        if limit_decimal <= 0:
            raise ValueError("Limit price must be positive")
        self._order_type = OrderType.STOP_LIMIT
        self._stop_price = stop_decimal
        self._limit_price = limit_decimal
        return self
