...
class TestCLI:
    ...
    def test_cli_new_command(self, capsys, tmp_path):
        """Test CLI new command creates strategy file."""
        # Change to tmp directory to avoid creating files in test directory
        import os
        ...
        try:
            with patch.object(sys, "argv", ["tektii", "new", "my_strategy"]):
                ...
                assert exc_info.value.code == 0
                captured = capsys.readouterr()
                assert "✓ Created my_strategy.py" in captured.out
                assert "Next steps:" in captured.out
                # Check file was created
                assert os.path.exists("my_strategy.py")
        finally:
            os.chdir(original_cwd)

    def test_cli_new_command_aliases(self, capsys, tmp_path):
        """Test CLI new command aliases."""
        import os
        ...
        try:
            ...
            # Remove file before next test
            if os.path.exists("my_strategy.py"):
                os.remove("my_strategy.py")
