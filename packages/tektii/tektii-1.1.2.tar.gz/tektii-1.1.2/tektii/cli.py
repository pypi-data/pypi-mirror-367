...
from tektii.commands.test import cmd_test
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  tektii new my_strategy --advanced --with-tests
  tektii validate my_strategy.py
  tektii validate my_strategy.py --fast
  tektii test test_my_strategy.py
  tektii serve my_strategy.py MyStrategy --port 50051
  tektii push my_strategy.py MyStrategy --save-config
  tektii backtest my_strategy.py MyStrategy --start 2024-01-01 --end 2024-12-31
  tektii analyze backtest_results.json
  tektii analyze --backtest-id abc123 --save-local
...
For help on specific commands: tektii <command> --help
    ...
    new_parser.add_argument("--advanced", action="store_true", help="Use advanced template with indicators and risk management")
    new_parser.add_argument("--with-tests", action="store_true", help="Create test file alongside strategy")
    ...
    test_parser = subparsers.add_parser("test", aliases=["t"], help="Run strategy tests")
    test_parser.add_argument("file", help="Test file to run")
    ...
    serve_parser.add_argument("--broker", "-b", help="Broker service address (e.g., localhost:50052)")
    ...
    # Status command - quick health check and overview
    status_parser = subparsers.add_parser("status", help="Show current environment status and health checks")
    ...
    if command == "new":
        ...
        from tektii.commands.status import cmd_status
