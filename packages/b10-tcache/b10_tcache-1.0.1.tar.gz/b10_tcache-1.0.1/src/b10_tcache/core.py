...
def load_compile_cache() -> bool:
    ...
    logger.info("Starting load_compile_cache")
    ...
    try:
        ...
        try:
            logger.info(f"Copying cache file from b10fs to local temp: {cache_file} -> {temp_path}")
        ...
        logger.info(f"load_compile_cache finished in {elapsed:.2f}s")
...
def save_compile_cache() -> bool:
    ...
    try:
        ...
        temp_file = b10fs_dir / f"{cache_filename}_{hostname}.incomplete.{uuid.uuid4()}.tar.gz"
        ...
        try:
            ...
            logger.info(f"Renaming temp file to final cache file: {temp_file} -> {final_file}")
