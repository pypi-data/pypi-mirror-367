...
:::"""
        ...
        # Format response
        if raw_directive:
            # Return in artifact format for direct rendering
            # First element is what AI sees, second is the artifact content
            directive = f":::lightdash-dashboard{{url=\"{embed_url}\" title=\"{title}\" height=\"{embed_height}\"}}\n:::"
            
            # Return as a tuple/list that mimics artifact format
            # The directive will be the "artifact" content that renders directly
            return [
                TextContent(type="text", text=f"Dashboard '{title}' has been embedded."),
                {"content": directive}
            ]
