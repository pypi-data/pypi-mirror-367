        ...
        if include_embed_url and "uuid" in created_chart:
            try:
                ...
                embed_request = {
                    "content": {
                        "type": "chart",
                        "uuid": created_chart['uuid']
                    },
                    "expiresIn": "8h"
                }
                
                embed_endpoint = f"/projects/{config.lightdash_config.project_id}/embed/get-embed-url"
                embed_response = await client._make_request(
                    method="POST",
                    endpoint=embed_endpoint,
                    data=embed_request
                )
                
                if embed_response["status"] == "ok":
                    embed_url = embed_response["results"]["url"]
                    # Add markdown directive to result
                    result += f"\n\n:::lightdash-chart{{url=\"{embed_url}\" title=\"{name}\" height=\"400\"}}\n:::"
            except Exception as e:
                logger.warning(f"Failed to generate embed URL: {str(e)}")
                # Continue without embed URL
