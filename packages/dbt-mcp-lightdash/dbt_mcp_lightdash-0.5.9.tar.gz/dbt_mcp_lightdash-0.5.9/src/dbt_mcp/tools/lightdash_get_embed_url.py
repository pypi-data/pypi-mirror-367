...
:::"""
        ...
        if raw_directive:
            # Return just the directive for LibreChat to extract
            directive = f":::lightdash-dashboard{{url=\"{embed_url}\" title=\"{title}\" height=\"{embed_height}\"}}\n:::"
            logger.info(f"[DEBUG] Returning raw directive: {directive[:100]}...")
            return [TextContent(type="text", text=directive)]
