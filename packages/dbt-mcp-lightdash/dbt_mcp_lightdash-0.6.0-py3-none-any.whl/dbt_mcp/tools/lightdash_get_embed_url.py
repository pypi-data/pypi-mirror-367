...
:::"""
        ...
        if raw_directive:
            # Return instruction for AI to output the directive verbatim
            directive = f":::lightdash-dashboard{{url=\"{embed_url}\" title=\"{title}\" height=\"{embed_height}\"}}\n:::"
            instruction = f"Please output the following directive exactly as shown, without any additional text or explanation:\n\n{directive}"
            logger.info(f"[DEBUG] Returning instruction to output directive: {directive[:100]}...")
