...
class ToolsManager:
    ...
    async def get(self, user_id: str, tools: Optional[List[str]] = None) -> List[ChatCompletionToolParam]:
        ...
        for tool in validated_tools:
            if tool in APP_TO_ACTIONS:
                await self._process_app_constant(tool, user_id, tool_schemas, processed_providers)
            else:
    ...
    async def _process_app_constant(
        ...
        try:
            ...
            if provider_service_key not in processed_providers:
                ...
                actions_response = await self._get_available_actions(provider, service, user_id)
    ...
    async def _get_available_actions(self, provider: str, service: str, user_id: str) -> Dict[str, Dict[str, Any]]:
        """
