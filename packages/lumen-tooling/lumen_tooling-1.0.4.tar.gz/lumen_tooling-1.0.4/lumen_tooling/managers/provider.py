...
from lumen_tooling.exceptions.exceptions import LumenError
...
class ProviderManager:
    ...
    def _extract_tool_calls(self, response: Any) -> List[Any]:
        ...
        """
        try:
            # Handle direct dict format: {'action': 'send_email', 'params': {...}, 'account_identifier': '...'}
            if isinstance(response, dict) and 'action' in response and 'params' in response:
                # Convert dict format to tool call-like object
                class DictToolCall:
    ...
    async def _execute_single_tool_call(
        ...
        # Handle dict format with account_identifier
        if hasattr(arguments, '__dict__') and hasattr(arguments, 'get'):
            # This is likely from the dict format case
            payload = {
