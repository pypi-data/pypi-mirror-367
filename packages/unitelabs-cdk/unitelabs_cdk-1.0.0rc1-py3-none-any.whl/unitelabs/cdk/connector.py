...
class Connector:
    ...
    def __init__(self, config: typing.Optional[ConnectorBaseConfig] = None):
        ...
        sila_server = self.config.sila_server.to_dict()
        cloud_server_endpoint = self.config.cloud_server_endpoint.to_dict()

        sila_server["version"] = parse_version(sila_server.get("version", "0.0.0"))
        sila_server["tls"] = sila_server.get("tls", False)
        cloud_server_endpoint["tls"] = cloud_server_endpoint.get("tls", False)
        ...
        } | cloud_server_endpoint.get("options", {})
        cloud_server_endpoint["options"] = framework.ChannelOptions(**options)

        self._sila_server = server.Server(sila_server)
        self._discovery = server.Discovery(self._sila_server)
        # HERE WE ASSUME THE CLOUD SERVER IS ALWAYS ENABLED; allow broadcast to be optional based on configuration
