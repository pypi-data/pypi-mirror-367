...
class TestConnector:
    ...
    async def test_create_new_connector_with_default_values(self, no_broadcast):
        ...
        assert connector.config.sila_server.version == "0.0.0"
        ...
        assert not connector.config.cloud_server_endpoint.tls
        ...
        assert connector.config.cloud_server_endpoint.reconnect_delay == 5
        assert not connector.config.cloud_server_endpoint.options
