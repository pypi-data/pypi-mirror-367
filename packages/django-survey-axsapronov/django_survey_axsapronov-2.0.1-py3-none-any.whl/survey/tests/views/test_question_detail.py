...
class TestQuestionDetailViewCorrectAnswerDisplay:
    ...
    def test_form_shows_correct_answer_after_submission(self, client):
        ...
        # Проверяем, что правильный ответ отображается
        assert "Правильный ответ:" in content
        assert "Вариант 2" in content
        assert "Ваш ответ:" in content
        assert "Вариант 1" in content
        assert "Корректный ответ и давай дальше" in content
    ...
    def test_form_does_not_show_correct_answer_before_submission(self, client):
        ...
        # Проверяем, что правильный ответ не отображается
        assert "Правильный ответ:" not in response.content.decode()
        assert "Завершить опрос" in response.content.decode()
    ...
    def test_correct_answer_display_for_different_question_types(self, client):
        ...
        assert "Правильный ответ:" in response.content.decode()
        assert "Ваш ответ:" in response.content.decode()
