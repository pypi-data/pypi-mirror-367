...
class OutputCollector:
    ...
    def collect_outputs(self, output_dir: str, outputs_config: Dict) -> List[Optional[str]]:
        ...
        print(f"收集完成，共 {len([f for f in output_files if f])} 个有效输出文件")
        return output_files
    ...
    def _handle_missing_output(self, field_name: str, field_config: Dict, field_type: str):
        ...
        if field_config.get("required", True):
            print(f"警告: 未找到必填输出{'文件夹' if field_type == 'folder' else '文件'}: {field_name}")
        else:
            print(f"可选输出{'文件夹' if field_type == 'folder' else '文件'}未找到: {field_name} (根据输入参数，此项可能不会生成)")