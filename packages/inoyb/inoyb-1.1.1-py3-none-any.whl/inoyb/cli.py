...
def cmd_images(args):
    ...
    try:
        ...
        if args.action == "list":
            ...
            if not args.name:
                print("❌ 请指定要导出的镜像名称")
                print("💡 用法: inoyb images export <镜像名称> [-o 输出文件] [--path 导出目录]")
                print("💡 示例: inoyb images export inoyb/my-model:abc123")
                print("💡 示例: inoyb images export inoyb/my-model:abc123 --path ./exports")
                sys.exit(1)
...
网络问题解决方案:
  ...
  注意: inoyb 会自动重试网络相关错误 (最多3次)

常见问题解决:
  # Docker 日志驱动错误: "configured logging driver does not support reading"
  
  这是无害的警告，不影响构建。如果想彻底解决:
  1. 清理 Docker 缓存: inoyb images prune
  2. 重启 Docker 服务: 
     - macOS: 重启 Docker Desktop
     - Linux: sudo systemctl restart docker
  3. 检查 Docker 配置: docker info | grep "Logging Driver"
  4. 或者忽略此警告，inoyb 会自动过滤这些信息
        """,
    )
