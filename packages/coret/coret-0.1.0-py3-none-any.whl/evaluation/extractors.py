...
class DefaultExtractor(BaseExtractor):
    def extract(self, cur_emb, embeddings, params):
        """
        Extract concepts for a given embedding using the Core instance.

        Args:
            cur_emb (np.ndarray): Single embedding vector (1D or 2D).
            embeddings (np.ndarray): Full dataset embeddings.
            params (dict):
                - main: dict of core extraction parameters
        ...
        # The Core instance is required as part of params
        core_instance = params.get("core_instance")
        if core_instance is None:
            raise ValueError("core_instance must be passed in params['core_instance']")
        ...
        # Perform main Core concept search
        concepts_dict = core_instance(
            query=cur_emb.reshape(1, -1),
            **params["main"]
        )
...
class RetrievalExtractor(BaseExtractor):
    ...
    def extract(self, cur_emb: np.ndarray, embeddings: np.ndarray, params: dict):
        ...
        concepts_retrieved_emb = np.stack(concepts_retrieved_list, axis=0)  # (num_rounds, num_imgs_per_concept, emb_dim)
...
class KMeansExtractor(BaseExtractor):
    ...
    def extract(
        ...
        top_k               = min(main.get("top_k", num_clusters * num_imgs_per_concept),
                                  embeddings.shape[0])
