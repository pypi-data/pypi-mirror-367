...
def load_pre_calculated_embeddings_from_paths(paths):
     ...
     for path in tqdm(paths, desc="Loading embeddings", leave=False):
         try:
             ...
             if 'urls' in data or 'successful_urls' in data or 'image_urls' in data:  # both keys are used in different datasets
                 urls = data.get('urls', data.get('successful_urls', data.get('image_urls', [None for i in range(len(labels))])))
             else:
     ...
     logger.info(f"Loaded {embeddings_s.shape[0]} embeddings in {time() - start_time:.2f} seconds. \n  embedding memory usage: {embeddings_s.nbytes / (1024 ** 2):.2f} MB")
...
def truncated_pca_transform_cpu(embedding, pca_model, n_components_to_use):
    ...
    reconstructed = np.dot(truncated_coeffs, pca_model.components_[:n_components_to_use]) + pca_model.mean_
...
def calculate_projection(base_emb, emb_to_subtract):
    projection_coefficients = np.einsum(
        "ij,ij->i", base_emb, emb_to_subtract
    ) / np.einsum("ij,ij->i", emb_to_subtract, emb_to_subtract)
...
def cal_leftover_embeddings_pca(embedding, pca_to_subtract, number_of_pca_components_to_use=3, pca_explained_variance_to_use=None, change_dataset_percentage=0):
    if type(pca_explained_variance_to_use) is float:
        logging.info(f'pca_explained_variance_to_use: {pca_explained_variance_to_use}, OVERWRITING number_of_pca_components_to_use')
        ...
        number_of_pca_components_to_use = np.argmax(cum_sum_explained_variance > pca_explained_variance_to_use)
        
    pca_explained_variance = pca_to_subtract.explained_variance_ratio_[:number_of_pca_components_to_use].sum()
    logging.info(f'number_of_pca_components_to_use: {number_of_pca_components_to_use}, which explains {pca_explained_variance*100:.2f}% of the variance')
    ...
    if change_dataset_percentage > 0:
        ...
        logging.info(f'num_of_embedding_to_change: {num_of_embedding_to_change} out of {embedding.shape[0]}')
    ...
    emb_to_mod_pca_inv = truncated_pca_transform_auto(embedding_to_change, pca_to_subtract, number_of_pca_components_to_use)
    emb_to_mod_pca_inv = emb_to_mod_pca_inv / np.linalg.norm(emb_to_mod_pca_inv, axis=1).reshape(-1, 1)
...
class ConceptRetrieval:
    ...
    def fit(self, embeddings: np.ndarray = None, paths: List[str] = None):
        ...
        if embeddings is not None:
            ...
            self.embeddings, self.labels, self.urls = load_pre_calculated_embeddings_from_paths(paths)
        else:
        ...
        self.index = EmbeddingIndex(self.embeddings, gpu=self.configs.get('gpu', 0) if self.configs else 0)

        self.fitted = True
        self.logger.info(f"ConceptRetrieval fitted with {self.embeddings.shape[0]} embeddings in {time() - start_time:.2f} seconds.")
    ...
    def retrieve(self, 
        ...
        if query.ndim != 2 and query.ndim != 1:
            raise ValueError("Query must be a 2D inner_product with shape (1, d), or a 1D inner_product with shape (d,).")
        # check if the query is a 1D inner_product and reshape it to 2D
        if query.ndim == 1:
            query = query.reshape(1, -1)
        if query.shape[1] != self.embeddings.shape[1]:
            raise ValueError(f"Query dimension {query.shape[1]} does not match embeddings dimension {self.embeddings.shape[1]}.")

        threshold_min_number_of_samples = self.configs.get('threshold_min_number_of_samples', 50) if self.configs else 50

        if num_of_std_to_add_for_first_threshold is None:
            num_of_std_to_add_for_first_threshold = self.configs.get('num_of_std_to_add_for_first_threshold', None) if self.configs else None
        if num_of_std_to_add_for_first_threshold is not None : # Slow option better to leave it as None and set a constant for large_neighborhood_number_of_samples
            ...
            logging.info(f"Percentage of first stage filtered samples (mean+num_of_std_to_add_for_first_threshold*std): {(indexes_above_threshold.shape[0] / similarity.shape[0]) * 100 :.2f}%, Number of samples above threshold: {indexes_above_threshold.shape[0]}, Out of {similarity.shape[0]}, n_std: {num_of_std_to_add_for_first_threshold}, Threshold: {threshold:.3f}")
            if len(indexes_above_threshold) < threshold_min_number_of_samples*2:
                logging.info(f"Too few samples above the threshold, returning the {threshold_min_number_of_samples*2} most similar samples.")
            large_neighborhood_number_of_samples = max(len(indexes_above_threshold), threshold_min_number_of_samples*2)
        else:
            large_neighborhood_number_of_samples = self.configs.get('large_neighborhood_number_of_samples', 10_000) if self.configs else 10_00

        if small_neighborhood_number_of_samples is None:
            small_neighborhood_number_of_samples = self.configs.get('small_neighborhood_number_of_samples', 750) if self.configs else 750
        start_time = time()
        ...
        index = EmbeddingIndex(curr_embedding_s, gpu=self.configs.get('gpu', 0) if self.configs else 0)
        ...
        for concept_index in range(number_of_concepts): 
        # get the large neighborhood of the query
            neighbors_distances, neighbors_indices = index.search(query, large_neighborhood_number_of_samples)
            self.logger.info(f"Retrieved {len(neighbors_indices)} neighbors in {time() - start_time:.2f} seconds.")

            # get the embeddings of the large neighborhood
            large_neighborhood_embeddings = curr_embedding_s[neighbors_indices]
            small_neighborhood_embeddings = large_neighborhood_embeddings[:small_neighborhood_number_of_samples]

            # the query embedding as the last element in the large neighborhood
            large_neighborhood_embeddings = np.concatenate((large_neighborhood_embeddings, anchor), axis=0)
            small_neighborhood_embeddings = np.concatenate((small_neighborhood_embeddings, anchor), axis=0)
            ...
            inner_products = cosine_similarity(large_neighborhood_embeddings, small_neighborhood_embeddings)
            ...
            ind_to_bin_ind_s = np.zeros((inner_products.shape), dtype=int) # for each index in the row, the bin index it belongs to
            logging.info(f"Calculating histograms for {inner_products.shape[0]} rows.")
            start_time_histogram = time()

            hist_s, ind_to_bin_ind_s = vectorized_histogram_uniform(inner_products, value_range, bins)
            ...
            samples_inds_largest_s = {i : {} for i in range(inner_products.shape[0])}  # dict of lists to hold indices of samples in the largest gaussian
            ...
            for i, (hist, peaks) in enumerate(zip(hist_s, peaks_s)):
                ...
                mean_anchor_dist_of_elem_in_largest_s[i] = np.mean(inner_products[samples_inds_largest_s[i], -1]) # -1 should be the distance to the anchor (query) embedding

            # Check Conditions
            logging.info(f" failed_to_find_peaks_s: {failed_to_find_peaks_s.mean()*100:.2f}%, (number of peaks: {(~failed_to_find_peaks_s).sum()})")
            anchor_failed_test = np.zeros(large_neighborhood_embeddings.shape[0], dtype=bool)
            
            min_number_of_sample_in_largest_failed_test = samples_in_largest_s < threshold_min_number_of_samples
            ratio_of_failed_min_samples_in_largest = (
                (min_number_of_sample_in_largest_failed_test).sum()
                / inner_products.shape[0]
            )
            logging.info(f"ratio_of_failed_min_samples_in_largest: {ratio_of_failed_min_samples_in_largest * 100:.2f}%, (number of rows with less than {threshold_min_number_of_samples} samples in the largest gaussian: {(min_number_of_sample_in_largest_failed_test).sum()} out of {inner_products.shape[0]})")
            do_anchor_similarity = self.configs.get('do_anchor_similarity', None) if self.configs else None
            if do_anchor_similarity is not None:
                # Uncomment below if doing anchor-in-largest test
                # if do_anchor_in_largest_test:
                anchor_failed_test = (
                    (inner_products[:,-1] < bin_centers[separation_idx_s]) |
                    (separation_idx_s < 0)
                )
                ...
                if ratio_of_failed_anchor == 1.0:
                    logging.warning("All surviving rows failed anchor test => skipping anchor check")
                    anchor_failed_test[:] = False

            passed_rows = ~failed_to_find_peaks_s & ~anchor_failed_test & ~min_number_of_sample_in_largest_failed_test
            if passed_rows.sum() == 0:
                logging.warning("No rows passed the tests, treating the smaller neighborhood as a single concept.")
                ...
                best_row_embeddings_in_largest = small_neighborhood_embeddings[best_row_samples_inds_in_largest]

            ## tmp save hist_s as an image and save TODO Debug
            do_plot_histograms = False
            if do_plot_histograms:
                from other.plot_histogram import plot_histograms_matrix, plot_row_inds_distribution, plot_histogram
                plot_histograms_matrix(hist_s[passed_rows], input_bin=ind_to_bin_ind_s[:,-1][passed_rows], path="tmp/neighborhood_histograms")
                plot_row_inds_distribution(np.where(passed_rows)[0], path="tmp/row_inds_distribution.png")
                
                logging.info(f"Best row index: {best_row_idx_in_original}, with mean anchor distance: {mean_anchor_dist_of_elem_in_largest_s[best_row_idx_in_original]:.2f}")
                # plot the best row histogram
                plot_histogram(hist_s[best_row_idx_in_original], input_bin=ind_to_bin_ind_s[:,-1][best_row_idx_in_original], path="tmp/neighborhood_histograms", title=f"best_row_histogram_row_index_{best_row_idx_in_original}")
            ...
            pca_explained_variance_to_use = self.configs.get('pca_explained_variance_to_use', 0.5) if self.configs else 0.5
            ...
            if type(pca_explained_variance_to_use) is float:
                ...
                logging.info(
                    f"number_of_pca_components_to_use: {number_of_pca_components_to_use}"
                )
            ...
            concept_score = cosine_similarity(
                query.reshape(1, -1),
                concept_embed.reshape(1, -1)
            ).item()
            logging.info(f"Element embedding score: {concept_score:.4f}")

            # retrieve the top k concepts based on the element embedding with the original index
            top_k_concepts_distances, top_k_concepts_indices = self.index.search(concept_embed.reshape(1, -1), number_of_samples_per_concept)

            # save the results

            concept_embed_s.append(concept_embed)
            concept_score_s.append(concept_score)
            ...
            pca_explained_variance_to_use = self.configs.get('pca_explained_variance_to_use', 0.5) if self.configs else 0.5
            change_dataset_percentage = self.configs.get('change_dataset_percentage', 0.1) if self.configs else 0.1
            curr_embedding_s = cal_leftover_embeddings_pca(curr_embedding_s, pca, number_of_pca_components_to_use=number_of_pca_components_to_use, change_dataset_percentage=change_dataset_percentage, pca_explained_variance_to_use=pca_explained_variance_to_use)
            anchor = cal_leftover_embeddings_pca(anchor, pca, number_of_pca_components_to_use=number_of_pca_components_to_use, change_dataset_percentage=0, pca_explained_variance_to_use=pca_explained_variance_to_use)
            index = EmbeddingIndex(curr_embedding_s, gpu=self.configs.get('gpu', 0) if self.configs else 0)

            
        concept_embed_s = np.stack(concept_embed_s)
        concept_score_s = np.stack(concept_score_s)
        ...
        return_dict = {
            ...
            "concept_score_s": concept_score_s,
...
def get_concept_neighbors(concepts_dict,
                          core_instance,
    ...
    Args:
        ...
        core_instance:   object with .index.search(queries, k) method
    ...
    for concept_ind in range(concepts_dict['concept_embed_s'].shape[0]):
        ...
        if noise_std>0:
            ...
            noisy_proj = proj_rep + np.random.normal(
                loc=0.0,
                scale=noise_std,
                size=proj_rep.shape
            )
            ...
            dists, idxs = core_instance.index.search(queries, 1)
            ...
            faiss.normalize_L2(queries)
            dists, idxs = core_instance.index.search(queries,
                                                              number_of_samples_per_concept)
