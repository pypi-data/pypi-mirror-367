...
class AsyncBaseModbusServer(ABC):
    ...
    def __init__(self, 
        ...
        self._logger.info(f"Modbus服务器初始化完成 | Modbus server initialized: 从站地址 | Slave ID {slave_id}")
    ...
    def process_request(self, slave_id: int, pdu: bytes) -> bytes:
        ...
        try:
            ...
            if slave_id != self.slave_id and slave_id != 0:  # 0是广播地址 | 0 is broadcast address
                self._logger.debug(f"忽略非本站请求 | Ignoring request for different slave: {slave_id}")
                return b''  # 不响应非本站请求 | Don't respond to requests for other slaves
            
            function_code = pdu[0]
            data = pdu[1:]
            
            self._logger.debug(f"处理请求 | Processing request: 功能码 | Function Code 0x{function_code:02X}, 数据长度 | Data Length {len(data)}")
            ...
            if function_code in self._function_handlers:
                ...
                self._logger.debug(f"请求处理完成 | Request processed: 响应长度 | Response Length {len(response_pdu)}")
                return response_pdu
            else:
                # 不支持的功能码 | Unsupported function code
                self._logger.warning(f"不支持的功能码 | Unsupported function code: 0x{function_code:02X}")
                return self._create_exception_response(function_code, 0x01)  # 非法功能码 | Illegal function code
                
        except ModbusException as e:
            self._logger.warning(f"Modbus异常 | Modbus exception: {e}")
            return self._create_exception_response(pdu[0], e.exception_code)
        except Exception as e:
            self._logger.error(f"处理请求时发生错误 | Error processing request: {e}")
            return self._create_exception_response(pdu[0] if len(pdu) > 0 else 0x00, 0x04)  # 从站设备故障 | Slave device failure

    def _create_exception_response(self, function_code: int, exception_code: int) -> bytes:
        """
        创建异常响应PDU
        
        Create Exception Response PDU
