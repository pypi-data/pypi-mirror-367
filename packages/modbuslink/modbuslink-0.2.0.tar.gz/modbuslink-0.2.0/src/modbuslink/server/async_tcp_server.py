...
class AsyncTcpModbusServer(AsyncBaseModbusServer):
    ...
    def __init__(self, 
        ...
        self._logger.info(f"TCP服务器初始化 | TCP server initialized: {host}:{port}, 最大连接数 | Max connections: {max_connections}")
            ...
            while self._running:
                try:
                    ...
                    if len(pdu_data) != pdu_length:
                        self._logger.warning(f"PDU数据长度不匹配 | PDU data length mismatch: expected {pdu_length}, got {len(pdu_data)}")
                        break
                    
                    self._logger.debug(f"接收到请求 | Received request: 事务ID | Transaction ID {transaction_id}, 单元ID | Unit ID {unit_id}, PDU长度 | PDU Length {len(pdu_data)}")
                    ...
                    if response_pdu:  # 只有非广播请求才响应 | Only respond to non-broadcast requests
                        ...
                        self._logger.debug(f"发送响应 | Sent response: 事务ID | Transaction ID {transaction_id}, 响应长度 | Response Length {len(response_pdu)}")
