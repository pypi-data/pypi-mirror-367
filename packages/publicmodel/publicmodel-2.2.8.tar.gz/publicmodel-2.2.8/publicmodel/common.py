...
class DownloadFile:
    def __init__(self, url, filename, path=None, block=1024, progress=False, multithreading=False,
                 automatic_repair=False, info=True, auto_resume=False, max_retries=10, retry_delay=10):
        """
        初始化下载文件类
    ...
    def _check_file_completeness(self):
            ...
            try:
                ...
                if response.status_code == 416:  # Range Not Satisfiable
                    ...
                    if 'content-range' in response.headers:
                        ...
                        if self.info:
                            print(
                                f"[INFO] Server returned 416, assuming file is complete")
        ...
        if server_size > 0 and local_size == server_size:
            ...
            if self.info:
                print(
                    f"[WARNING] Local file is larger than server file, removing corrupted file")
    ...
    def _download_file(self):
        ...
        # 首先检查文件是否已经完整
        is_complete, server_size = self._check_file_completeness()
        if is_complete:
            if self.info:
                print(
                    f"[INFO] File already complete: {self.filename} ({server_size} bytes)")

            # 如果启用了进度条，返回100%完成的状态
            if self.progress:
                self.downloaded_size = server_size
                # 计算块数
                chunk_size = max(1024, server_size //
                                 self.block) if server_size > 0 else 1024
...
if __name__ == "__main__":
    downloader = DownloadFile(
        ...
        block=8192,
        progress=True,
        multithreading=False,  # 多线程模式下无法使用进度条
    ...
    try:
        # 使用进度报告
        for total_blocks, current_block, elapsed_time, remaining_time in downloader.download():
            # 计算百分比
            ...
            bar_length = 40
            filled_length = int(
                bar_length * current_block // total_blocks) if total_blocks > 0 else 0
            ...
            remaining_str = f"{int(remaining_time // 60):02d}:{int(remaining_time % 60):02d}" if remaining_time > 0 else "--:--"

            # 显示进度
            sys.stdout.write(
                f'\r[{bar}] {percent:.1f}% ({current_block}/{total_blocks}) 已用:{elapsed_str} 剩余:{remaining_str}')
            sys.stdout.flush()
