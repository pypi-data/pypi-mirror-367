...
class FinancialStatementAnalyzer(BaseResource):
    ...
    async def analyze_income_statement(
        ...
        try:
            ...
            results = self._calculator.analyze_income_statement(
                revenue_data, expense_data, period
            )
            ...
            # Format output
            formatted_output = self._formatter.format_income_statement_analysis(
                results, period
            )
    ...
    async def analyze_cash_flow(
        ...
        try:
            ...
            # Perform calculations
            results = self._calculator.analyze_cash_flow(
                operating_data, investing_data, financing_data, begin_cash, period
            )
    ...
    async def analyze_balance_sheet(
        ...
        try:
            ...
            # Perform calculations
            results = self._calculator.analyze_balance_sheet(
                asset_data, liability_data, equity_data, period_end
            )
...
class FinancialCalculations:
    ...
    def analyze_income_statement(
        ...
        if len(revenue_data) > 1 and all('Q' in k or 'M' in k for k in revenue_data.keys()):
            ...
            results['period_growth_rates'] = {
                f"{periods[i]}→{periods[i+1]}": rate 
        ...
        operating_expenses = sum(
            v for k, v in expense_data.items() 
            if k.lower() not in ['cogs', 'interest', 'tax', 'taxes']
        )
    ...
    def analyze_balance_sheet(
        ...
        current_assets = sum(
            v for k, v in asset_data.items() 
            if any(ca in k.lower() for ca in current_asset_keys)
        )
        
        current_liability_keys = ['payables', 'short_term', 'current_liabilities']
        current_liabilities = sum(
            v for k, v in liability_data.items()
            if any(cl in k.lower() for cl in current_liability_keys)
        )
        ...
        long_term_debt = sum(
            v for k, v in liability_data.items()
            if 'long_term' in k.lower() or 'debt' in k.lower()
        )
    ...
    def analyze_growth_trends(
        ...
        for i in range(1, len(values)):
            if values[i-1] != 0:
                ...
                growth_rates.append({
                    'period': f"{periods[i-1]}→{periods[i]}",
                    'growth_rate': growth
                })
    ...
    def compare_financials(
        ...
        if comparison_type in ['common_size', 'all']:
            ...
            results['common_size_a'] = {
                k: (v / base_a * 100) if base_a != 0 else 0 
                for k, v in data_a.items()
            }
            results['common_size_b'] = {
                k: (v / base_b * 100) if base_b != 0 else 0 
