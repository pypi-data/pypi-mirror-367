...
class TaskSpecificFresherAgent:
    ...
    def __init__(self, domain: str, role: str, tasks: List[str], domain_cls: DefaultDomain, 
                 confidence_threshold: float = 85.0, max_iterations: int = 10):
        """
...
{{
    ...
    def improve_questions_for_gaps(self, questions: Dict[str, List[str]], gaps: List[Dict]) -> Dict[str, List[str]]:
        ...
        current_questions_for_gaps = {
            cat: questions.get(cat, []) for cat in gap_categories
        }
...
{{
    ...
    def generate_comprehensive_questions(self) -> Dict:
        ...
        logger.warning("Using legacy category-based question generation. Consider using generate_comprehensive_questions_from_tree() for tree-based generation.")
