...
from business.pdf.PDFExtractor import ConfigAI, PDFExtractor
from storage.StorageService import OCIStorageManager
import tempfile
...
class ExtractorFile:
    ...
    def run(self, task_id, use_ai: bool = False, ai_provider: str = 'google', extract_insurance_data: bool = True, file_path: str = None, config: ConfigAI = ConfigAI()) -> Dict[str, Any]:
        ...
        try:
            if task_id and task_id > 0:
                ...
                return self._process_batch_from_oci(task_id=task_id, use_ai=use_ai, ai_provider=ai_provider, extract_insurance_data=extract_insurance_data, config=config)
            elif file_path:
                logger.info(f"Processando arquivo único: {file_path}")
                return self._process_single_file(use_ai=use_ai, ai_provider=ai_provider, extract_insurance_data=extract_insurance_data, file_path=file_path, config=config)
            else:
                return {
                    "status": "Erro",
    ...
    def _process_batch_from_oci(self, task_id, use_ai: bool = False, ai_provider: str = "google", extract_insurance_data: bool = True, config: ConfigAI = ConfigAI()) -> Dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for oci_path in file_names:
                ...
                try:
                    ...
                    result = self._process_single_file(use_ai=use_ai, ai_provider=ai_provider, extract_insurance_data=extract_insurance_data, file_path=local_file_path, config=config)
    ...
    def _process_single_file(self, use_ai: bool = False, ai_provider: str = "google", extract_insurance_data: bool = True, file_path: str = None, config: ConfigAI = ConfigAI()) -> Dict[str, Any]:
        ...
        # Lógica de extração...
        if use_ai:
            extractor = PDFExtractor(ai_provider=ai_provider, config=config)
        else:
            extractor = PDFExtractor(ai_provider="none", config=config)
...
def main():
    ...
    try:
        ...
        if os.path.exists(test_file):
            ...
            result1 = extractor.run(task_id=task_id, ai_provider='google', use_ai=True, file_path=test_file, config=config)
            ...
            print("-" * 40)
            extractor = ExtractorFile()
            result2 = extractor.run(task_id=task_id, ai_provider='azure', use_ai=True, file_path=test_file, config=config)
            ...
            print("-" * 40)
            extractor = ExtractorFile()
            result3 = extractor.run(task_id=task_id, use_ai=False, file_path=test_file, config=config)
