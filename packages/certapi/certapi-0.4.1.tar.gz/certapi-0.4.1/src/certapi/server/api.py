...
def create_api_resources(api_ns, cert_manager:AcmeCertManager):
    ...
    issued_cert_model = api_ns.model('IssuedCertData', {
    ...
    certificate_response_model = api_ns.model('CertificateResponseData', {
    ...
    obtain_parser.add_argument('hostname', type=str, action='append', required=True, help='List of hostnames for the certificate')
    ...
    class SignCsr(Resource):
        @api_ns.doc(description='Signs a Certificate Signing Request (CSR)', 
                     responses={200: 'CSR signed successfully', 400: 'Invalid CSR', 500: 'Failed to sign CSR'})
        @api_ns.expect(api_ns.model('CSR', {'csr_pem': fields.String(required=True, description='PEM encoded CSR')}), 
                       validate=True)
        def post(self):
            try:
