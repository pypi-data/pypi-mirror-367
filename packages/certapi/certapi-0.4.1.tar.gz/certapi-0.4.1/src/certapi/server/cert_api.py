...
def create_cert_resources(cert_ns, key_store):
    ...
    cert_model = cert_ns.model('Certificate', {
    ...
    cert_input_model = cert_ns.model('CertificateInput', {
        'private_key_id': fields.String(required=True, description='ID of the private key associated with the certificate'),
        'cert': fields.String(required=True, description='PEM encoded certificate string'),
        'domains': fields.List(fields.String, required=True, description='List of domains covered by the certificate'),
            ...
            if domain:
                ...
                if cert_info:
                    ...
                    if isinstance(cert_list, list):
                        ...
                        pem_content = cert_to_pem(cert_list).decode("utf-8") if hasattr(cert_list, 'public_bytes') else cert_list

                    return [{
                        "id": domain, # Using domain as ID for this endpoint
                        "key_id": key_id,
                        "pem": pem_content
                    }]
                else:
        ...
        def get(self, cert_id):
            ...
            if cert_info:
                ...
                if isinstance(cert_list, list):
                    ...
                    pem_content = cert_to_pem(cert_list).decode("utf-8") if hasattr(key_obj, 'public_bytes') else cert_list

                return {
                    "id": cert_id,
                    "key_id": key_obj.id if hasattr(key_obj, 'id') else None, # Assuming Key object has an 'id' attribute
                    "pem": pem_content
                }
            else:
