...
def display_mol_info(moleculein, flagsum=1):
    ...
    for ii, imol in enumerate(molzz):
        ...
        if flagsum == 1:
            ...
            print("#%s %-12s with %.6f eV (%.6f eV) (f=%.2f)" %(jj, imol.info['i'], imol.info['e'], deltae, fitness[ii]))
        else:
            print("#%s %-12s with %.6f eV (%.6f eV)" %(jj, imol.info['i'], imol.info['e'], deltae))
#-------------------------------------------------------------------------------
def conformational(inputfile='INPUT.txt'):
    ...
    xopt=calculator_anix_all(xrand, opt=calculator, preclist=[1E-03])
    print('')
    ...
    for igen in range(nof_generations):
        ...
        print('Optimization at %s:' %(calculator))
        generation_opt=calculator_anix_all(atoms_list_out+atoms_list_mut, opt=calculator, preclist=[1E-03])
