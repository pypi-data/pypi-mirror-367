...
class LegalOrganization(XMLBaseModel):
    ...
    Attributes:
        id: Legal entity identifier (LEI) using ISO 17442 standard.
            Format: 20 characters, alphanumeric string
    ...
    id: Optional[str] = Field(
        default=None,
        description="Legal Entity Identifier (LEI) following ISO 17442",
        pattern=r"^[0-9A-Z]{20}$",
        min_length=20,
        max_length=20
    )
    ...
    @field_validator('id')
    def validate_lei(cls, value: Optional[str]) -> Optional[str]:
        """Validates the Legal Entity Identifier (LEI) format.

        Args:
            value: LEI to validate

        Returns:
            The validated LEI

        Raises:
            ValueError: If the LEI format is invalid
        """
        if value is not None:
            if not value.strip():
                raise ValueError("LEI cannot be empty when provided")
            if len(value) != 20:
                raise ValueError("LEI must be exactly 20 characters long")
            if not value.isalnum():
                raise ValueError("LEI must contain only letters and numbers")
        return value
    ...
    def to_xml(self, element_name: str, profile: InvoiceProfile) -> ET.Element:
        ...
        Raises:
            ValueError: If trading_business_name is missing for EN16931 or higher profiles
        """
        if profile >= InvoiceProfile.EN16931 and not self.trading_business_name:
            raise ValueError(
                f"Trading business name is required for profile {profile.name}"
            )
