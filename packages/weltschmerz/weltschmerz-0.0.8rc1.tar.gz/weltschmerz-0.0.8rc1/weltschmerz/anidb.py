...
def lookup_files(
    ...
    for folder in folders:
        unknown_files_folder_query = adbc.dbs.session.query(anime.LocalFile)\
            .filter(anime.LocalFile.directory.like(f'{folder.rstrip("/")}%'))\
            .filter((anime.LocalFile.fid == None) | (anime.LocalFile.aid == None))
        if online:
            unknown_files_folder_query = unknown_files_folder_query.join(anime.AnidbFileResponse, anime.LocalFile.hash_ed2k == anime.AnidbFileResponse.hash_ed2k, isouter=True)\
                .filter(anime.AnidbFileResponse.hash_ed2k == None)
        else:
            unknown_files_folder_query = unknown_files_folder_query.join(anime.AnidbFileResponse, anime.LocalFile.hash_ed2k == anime.AnidbFileResponse.hash_ed2k, isouter=False)\
                .filter(anime.AnidbFileResponse.data == None)
        if debug:
            print(unknown_files_folder_query.statement.compile())
        unknown_files_folder = unknown_files_folder_query.all()
        unknown_files += unknown_files_folder
        print(f"DEBUG: unknown files to look up ({folder}): {len(unknown_files_folder)}")
