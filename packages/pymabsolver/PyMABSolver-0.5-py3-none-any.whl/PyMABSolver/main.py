...
class MABSolver:
    def __init__(self, n, t, tf = None, eps = None, init_t = None, c = None):
        self.n = n
        self.t = t
        self.tf = tf
        self.epsilon = eps
    ...
    def ucb(self):
        ...
        for _ in range(self.init_t):
            for i in range(len(self.selection_prob_list)):
                init_selected_arms.append(i)

        for _ in range(len(init_selected_arms)):
            rand_probs2.append(round(random.uniform(0 ,1), 2))

        for i in range(len(rand_probs2)):
            if rand_probs2[i] <= self.win_prob_list[init_selected_arms[i]]:
                rewards.append(1)
                win_dict[init_selected_arms[i]] += 1
            
            else:
                rewards.append(0)

        for i in range(self.n):
            count_selection[i] = init_selected_arms.count(i)
        ...
        for _ in range(self.t):
            ucb_val = []
            for i in range(self.n):
                ucb_val.append(actual_win_prob[i] + self.c * (math.log(self.t)/count_selection[i]))
            
            selected_arms.append(ucb_val.index(max(ucb_val)))
            rand_prob = round(random.uniform(0 ,1), 2)

            if rand_prob <= self.win_prob_list[selected_arms[-1]]:
                rewards.append(1)
                win_dict[selected_arms[-1]] += 1
