...
from snowflake.snowflake_data_validation.utils.model.column_metadata import ColumnMetadata


EXPECTED_ROW_CONCATENATED_QUERY: Literal = (
"""CREATE TEMPORARY TABLE IF NOT EXISTS ROW_CONCATENATED_datatypes_test_123 (
...
EXPECTED_ROW_CONCATENATED_INSERTION_QUERY: Literal = (
"""INSERT INTO ROW_CONCATENATED_datatypes_test_123
SELECT
...
EXPECTED_ROW_MD5_QUERY: Literal = (
"""CREATE TEMPORARY TABLE IF NOT EXISTS ROW_MD5_datatypes_test_123(
...
EXPECTED_ROW_MD5_INSERTION_QUERY: Literal = (
"""INSERT INTO ROW_MD5_datatypes_test_123
SELECT
...
class TestQueryGeneratorRedshift:
    ...
    def test_generate_compute_md5_chunk_query(self):
        mock_table_context = Mock()
        mock_table_context.index_column_collection = [
            ColumnMetadata(name='pk', data_type='BIGINT', is_primary_key=True, nullable=False, calculated_column_size_in_bytes=256, properties={})
        ]
        mock_table_context.columns_to_validate = [
            ColumnMetadata(name='text_col', data_type='TEXT', is_primary_key=False, nullable=False, calculated_column_size_in_bytes=256, properties={}),
        ]
        ...
        templates_dir_path = str(Path(__file__).parent.parent.parent.parent.parent / 
                                "src/snowflake/snowflake_data_validation/redshift/extractor/templates")
        mock_table_context.sql_generator = SQLQueriesTemplateGenerator(templates_dir_path)
        ...
        assert EXPECTED_ROW_CONCATENATED_QUERY in results
        assert EXPECTED_ROW_CONCATENATED_INSERTION_QUERY in results
        assert EXPECTED_ROW_MD5_QUERY in results
        assert EXPECTED_ROW_MD5_INSERTION_QUERY in results
        ...
        assert any(expected_final_insert in stmt for stmt in results), f"Expected final insert statement not found in: {results}"
