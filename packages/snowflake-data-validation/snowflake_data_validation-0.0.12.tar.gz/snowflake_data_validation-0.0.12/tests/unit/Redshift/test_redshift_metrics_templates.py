...
def load_redshift_and_snowflake_templates():
    ...
    base_path = (
        Path(__file__).parent.parent.parent.parent / "src/snowflake/snowflake_data_validation"
    )
...
def test_redshift_type_mappings():
    ...
    for redshift_type, snowflake_type in type_mappings.items():
        redshift_metrics = set(redshift_df[redshift_df["type"] == redshift_type]["metric"])

        snowflake_metrics = set(snowflake_df[snowflake_df["type"] == snowflake_type]["metric"])

        if len(redshift_metrics) > 0 and len(snowflake_metrics) > 0:
            compatible_metrics = redshift_metrics.intersection(snowflake_metrics)
            assert len(compatible_metrics) > 0, \
...
def test_redshift_incompatible_metric_templates():
    ...
    assert (
        "REDSHIFT_SPECIFIC" in diff["values_changed"]["root['metric_query']"]["old_value"]
    )
    assert (
        "SNOWFLAKE_SPECIFIC" in diff["values_changed"]["root['metric_query']"]["new_value"]
    )
...
def test_redshift_query_generation_with_missing_metric(mock_redshift_context):
    ...
    assert 'SOME_AGG_FUNCTION(CAST("test_column" AS DOUBLE PRECISION))' in redshift_query
    assert 'SOME_AGG_FUNCTION(CAST("test_column" AS DOUBLE PRECISION))' not in snowflake_query
...
def test_redshift_query_generation_with_where_clause(mock_redshift_context):
    ...
    assert "WHERE status = 'completed'" in query, "Query should contain the WHERE clause"
...
def test_redshift_template_file_existence():
    ...
    base_path = (
        Path(__file__).parent.parent.parent.parent / "src/snowflake/snowflake_data_validation/redshift/extractor/templates"
    )
    ...
    for file_name in required_files:
        ...
        assert file_path.exists(), f"Required Redshift template file {file_name} should exist"
        assert file_path.stat().st_size > 0, f"Template file {file_name} should not be empty" 