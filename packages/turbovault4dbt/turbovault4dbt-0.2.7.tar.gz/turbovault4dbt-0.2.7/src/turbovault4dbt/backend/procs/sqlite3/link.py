...
def generate_link(data_structure):
  ...
  for link in link_list:
    ...
    # Query for output_dir for this link
    if has_column(cursor, "standard_link", "output_dir"):
        cursor.execute("SELECT output_dir FROM standard_link WHERE Link_Identifier = ? LIMIT 1", (link_id,))
