...
class Ad(Model, TsTrait):
    ...
    price: float = Dcm11Type
    amount: float = FloatField(null=True)
    quantity: float = Dcm11Type
...
class Rival(Model, TsTrait):
    ...
    ad_id: int
    snap_price: float = Dcm11Type

    class Meta:
        unique_together = (("ad_id", "snap_price"),)
...
class RaceStat(Model):
    race: ForeignKeyRelation[Race] = ForeignKeyField("models.Race", "race")
    race_id: int
