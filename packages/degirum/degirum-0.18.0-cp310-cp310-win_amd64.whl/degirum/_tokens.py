...
import time, threading, os, json, copy, yaml, datetime, secrets, webbrowser, socket, urllib
...
class TokenManager:
    ...
    def token_create(self):
        ...
        message = f"New token is created by PySDK CLI request from host {hostname} ({ip_address})"
...
def _token_args(parser):
    ...
    parser.add_argument(
        ...
        default="status",
        help="token command: show token status, install a token, create new token, renew expired token (default is 'status')",
    )
