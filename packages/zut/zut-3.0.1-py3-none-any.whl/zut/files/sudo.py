...
class SudoAdapter(BaseAdapter[str]):
    ...
    def exists(cls, path: str|os.PathLike):
        ...
        cp = run_process(['sh', '-c', f'[ -e "{escaped_path}" ] && echo 1 || echo 0'], check=True, capture_output='rstrip-newline', sudo=True)
        if cp.stdout == '1':
            return True
        elif cp.stdout == '0':
            return False
    ...
    def _makedirs(cls, path: str|os.PathLike):
        ...
        run_process(['mkdir', '-p', path], check=True, capture_output='rstrip-newline', sudo=True)
