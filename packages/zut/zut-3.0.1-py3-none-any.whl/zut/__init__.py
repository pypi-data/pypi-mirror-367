"""
Top-level API: most commonly reused utilities.
"""
...
class NotImplementedBy(NotImplementedError):
    def __init__(self, by: str, feature: str|None = None):
        self.by = by
        self.feature = feature or f"{inspect.stack()[1].function}()"
        super().__init__(f"Not implemented by {self.by}: {self.feature}")
...
class NotSupportedBy(Exception):
    def __init__(self, by: str, feature: str|None = None):
        self.by = by
        self.feature = feature or f"{inspect.stack()[1].function}()"
        super().__init__(f"Not supported by {self.by}: {self.feature}")
