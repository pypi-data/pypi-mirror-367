...
from zut import Color, get_logger, log_warnings, skip_utf8_bom

__shortcuts__ = (get_logger, log_warnings)


#region Logging

def configure_logging(level: str|int|None = None, *, names = False, file_level: str|int|None = None, verbose_level: str|int|None = None, verbose_loggers: Sequence[str]|None = None, count = True, exit_handler = True):
    config = get_logging_config(level=level, names=names, file_level=file_level, verbose_level=verbose_level, verbose_loggers=verbose_loggers, count=count, exit_handler=exit_handler)
    logging.config.dictConfig(config)


def get_logging_config(level: str|int|None = None, *, names = False, file_level: str|int|None = None, verbose_level: str|int|None = None, verbose_loggers: Sequence[str]|None = None, count = True, exit_handler = True):
...
def get_locale_date_format() -> str|None:
    """
    Return current locale date format, if known (e.g. "%d/%m/%Y").
    """
    global _locale_date_format
    if _locale_date_format is None:
        with use_locale():
            try:
                _locale_date_format = locale.nl_langinfo(locale.D_FMT) # type: ignore
            except AttributeError:
                name = get_locale_name()
                if name.startswith(('fr_FR.', 'French_France.')):
                    _locale_date_format = '%d/%m/%Y'
                else:
