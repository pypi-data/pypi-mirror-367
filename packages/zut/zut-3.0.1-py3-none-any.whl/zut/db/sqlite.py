...
class SqliteDb(Db[Connection]):
    ...
    def _get_table_unique_keys(self, table: DbObj) -> list[tuple[str,...]]:
        ...
        return [tuple(columns) for columns in sorted(unique_keys, key=lambda u: tuple(positions[c] for c in u))]
    ...
    def table_exists(self, table: str|tuple|type|DbObj) -> bool:
        ...
        if table.schema == 'temp':
            ...
            raise NotSupportedBy(self.scheme, f'schema "{table.schema}"')
    ...
    def use_database(self, name: str) -> None:
        raise NotSupportedBy(self.scheme)
