...
class Db(Generic[T_Connection]):
    ...
    def create_connection(self, autocommit: bool|None = None) -> T_Connection:
        raise NotImplementedBy(self.scheme)
    ...
    def _get_aware_now_sql(self) -> str:
        if not self._accept_aware_datetimes:
            raise NotSupportedBy(self.scheme, 'now sql without specifying if timezone is local or utc')
    ...
    inserted_at_default_column: str = 'inserted_at'
    updated_at_default_column: str = 'updated_at'
    ...
    def get_unique_keys(self, table: str|tuple|type|DbObj) -> list[tuple[str,...]]:
        table = self.parse_obj(table)
    ...
    def _get_django_unique_keys(self, model: type[Model]) -> list[tuple[str,...]]:
            from django.db import models
    ...
    def create_table(self, table: str|tuple|type|DbObj, columns: Sequence[str|Column]|Mapping[str,str|type|Column], *, if_not_exists = False, primary_key: str|bool = False, unique_keys: Iterable[Sequence[str]]|None = None, unique_nulls = False, foreign_keys: Sequence[ForeignKey]|dict[str, str]|None = None, inserted_at_column: str|bool = False, updated_at_column: str|bool = False, ignore_decimal = False, ignore_not_null = False) -> None:
        # Analyze arguments
        table = self.parse_obj(table)
        ...
        if primary_key:
            if not primary_key_columns:
                ...
                if not primary_key in columns:
                    columns_sql += ("\n    ," if columns_sql else "") + f"{self.escape_identifier(primary_key)} {self.get_sql_type(int)} NOT NULL PRIMARY KEY {self._identity_sql}"
        ...
        if len(primary_key_columns) == 1:
            ...
            if column.identity:
                columns_sql += f" {self._identity_sql}"
            columns_mapping.pop(column.name)
        ...
        if inserted_at_column:
            if isinstance(inserted_at_column, bool):
                inserted_at_column = self.inserted_at_default_column
            columns_sql += ("\n    ," if columns_sql else "") + self._get_column_sql(Column(inserted_at_column, type=datetime, not_null=True, default='now'), foreign_key=single_foreign_key_by_column.get(inserted_at_column))
            
        if updated_at_column:
            if isinstance(updated_at_column, bool):
                updated_at_column = self.updated_at_default_column
            columns_sql += ("\n    ," if columns_sql else "") + self._get_column_sql(Column(updated_at_column, type=datetime, not_null=True, default='now'), foreign_key=single_foreign_key_by_column.get(updated_at_column))
        ...
        if unique_keys:
            for unique_key in unique_keys:                    
                ...
                if unique_nulls is False:
                    constraints_sql += " NULLS NOT DISTINCT"
                constraints_sql += " (" + ', '.join(self.escape_identifier(column) for column in unique_key) + ")"
    ...
    def clear_table(self, table: str|tuple|type|DbObj, *, truncate: bool|Literal['cascade'] = False, if_exists = False, loglevel = logging.DEBUG):
        ...
        if truncate == 'cascade':
            if not self._can_cascade_truncate:
                raise NotSupportedBy(self.scheme, "cascade truncate")
            sql += " CASCADE"
    ...
    def schema_exists(self, name: str) -> bool:        
        if self._default_schema is None:
            raise NotSupportedBy(self.scheme, 'schemas')
        raise NotImplementedBy(self.scheme)
    ...
    def _merge_tables(self, src_table: str|tuple|type|DbObj, dst_table: str|tuple|type|DbObj, columns: Sequence[str]|None = None, *, key: str|Sequence[str]|None = None) -> Upserted:
        raise NotImplementedError() #FIXME: ROADMAP, rename to public method name
        from zut.db.operations.merge import MergeOperation
        op = MergeOperation(self, src_table, dst_table, columns, key=key)
        return op.run()
    
    def _actual_merge_tables(self, src_table: DbObj, dst_table: DbObj, columns: Sequence[str], *, key: Sequence[str], updated_at_column: str|None, src_pk: Column, dst_pk: Column) -> Upserted:
        raise NotImplementedBy(self.scheme)

    def _load_csv(self, file: str|os.PathLike|TextIO, table: str|tuple|type|DbObj|None = None, columns: Sequence[str|Column|Literal['*']]|Mapping[str,str|type|Column|Literal['*']]|None = None, *,
                 key: str|Sequence[str]|None = None, encoding='utf-8', create_table: bool|None = None, slugify_columns: Callable[[str],str]|bool = False) -> Upserted:

        raise NotImplementedError() #FIXME: ROADMAP, rename to public method name
        from zut.db.operations.load import LoadOperation
        op = LoadOperation(self, file, table, columns, key=key, encoding=encoding, create_table=create_table, slugify_columns=slugify_columns)
        return op.run()
    
    def _actual_load_csv(self, file: str|os.PathLike, table: DbObj, headers: Sequence[str], delimiter: str, newline: str) -> int:
        """
        Load CSV data in the given table.
        
        """
        raise NotImplementedError() #FIXME: ROADMAP
        #ROADMAP: Interprete empty strings as nulls by default. Make this configurable?
