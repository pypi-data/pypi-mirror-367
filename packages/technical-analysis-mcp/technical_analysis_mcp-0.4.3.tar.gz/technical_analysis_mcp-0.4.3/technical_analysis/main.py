...
@mcp.tool(name="etf_technical_analyzer", description="分析ETF技术指标，包括RSI、布林带、移动平均线等")
...
@mcp.tool(name="stock_technical_analyzer", description="分析股票历史数据技术指标，包括RSI、布林带、移动平均线等")
...
@mcp.tool(name="etf_anomaly_screener", description="筛选ETF异动行情")
def screen_etf_anomaly_in_tech(etf_codes=("513050"), base_date: str = None,
                                    lookback_days: int = 60, top_k=10):
    return screen_etf_anomaly_cached(etf_codes, lookback_days=lookback_days).take(top_k).to_markdown()
