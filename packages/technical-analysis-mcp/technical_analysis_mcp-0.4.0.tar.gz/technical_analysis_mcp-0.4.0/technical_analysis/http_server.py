...
@app.get("/api/etf/anomaly", summary="ETF行情异动检测")
async def detect_etf_anomaly(etf_code: str = '510300', threshold: float = 2.0, base_date: str = None, lookback_days: int = 20) -> dict:
    """
    ETF行情异动检测
    - **etf_code**: ETF代码 (例如'510300')
    - **threshold**: 异动检测阈值(标准差倍数，默认为2.0)
    - **base_date**: 基准日期(格式YYYYMMDD)，默认为当前日期
    - **lookback_days**: 回溯天数(默认为20天)
    ...
    字段说明:
    - is_price_anomaly: 价格是否异常(基于布林带)
    - is_volume_anomaly: 成交量是否异常(基于历史平均)
    - price_deviation: 价格偏离标准差倍数
    - volume_deviation: 成交量偏离标准差倍数
    """
    df = get_etf_data(etf_code=etf_code, end_date=base_date, duration=lookback_days+10)
    if df is None:
        return {"error": "无法获取数据"}
    ...
    # 计算价格异常
    latest = df.iloc[-1]
    price_deviation = (latest['close'] - latest['boll_middle']) / ((latest['boll_upper'] - latest['boll_lower'])/4)
    is_price_anomaly = abs(price_deviation) > threshold
    
    # 计算成交量异常
    volume_mean = df['volume'].mean()
    volume_std = df['volume'].std()
    volume_deviation = (latest['volume'] - volume_mean) / volume_std if volume_std > 0 else 0
    is_volume_anomaly = volume_deviation > threshold
    ...
    return {
        ...
        "volume_deviation": round(volume_deviation, 2),
        "v": app.version
    }

@app.get("/api/etf/anomaly/advanced", summary="ETF高级异动检测")
async def detect_etf_anomaly_advanced(etf_codes: str = None, 
    ...
    """
    from .etf_screener import screen_etf_anomaly
    ...
    # 处理ETF代码参数
    etf_list = etf_codes.split(',') if etf_codes else None
    
    # 调用etf_screener模块
    results = screen_etf_anomaly(
        etf_codes=etf_list,
        end_date=end_date,
    ...
    return {"results": results, "v": app.version}
