...
class LackeyStorage:
    ...
    def __init__(self, workspace_path: str = "."):
        ...
        self.lackey_dir = self.workspace_path / ".lackey"
    ...
    def get_task(self, project_id: str, task_id: str) -> Task:
        """Get task by ID from specified project."""
        project_dir = self.projects_dir / project_id
        ...
        try:
            ...
            raise StorageError(f"Failed to read task: {e}")
    ...
    def _parse_notes(self, notes_content: str) -> List[str]:
        """Parse notes from markdown content."""
        if not notes_content.strip():
            return []

        # Split by lines and filter out empty lines
        notes = [line.strip() for line in notes_content.split("\n") if line.strip()]
        return notes
