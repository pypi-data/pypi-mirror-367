...
from lackey.rate_limiting import get_rate_limit_manager, rate_limit
from lackey.security import security_required
...
def _configure_rate_limiting() -> None:
    """Configure rate limiting rules for different MCP endpoints."""
    ...
    for endpoint in read_endpoints:
        rate_manager.assign_rule_to_endpoint(endpoint, "lenient")
    ...
    for endpoint in write_endpoints:
        rate_manager.assign_rule_to_endpoint(endpoint, "default")
    ...
    for endpoint in sensitive_endpoints:
        rate_manager.assign_rule_to_endpoint(endpoint, "strict")

    # Adaptive endpoints (complex operations)
    adaptive_endpoints = ["validate_task_dependencies_integrity", "search_task_notes"]

    for endpoint in adaptive_endpoints:
        rate_manager.assign_rule_to_endpoint(endpoint, "adaptive")
...
@mcp_server.tool()
@rate_limit(rule_name="default")
@security_required(
    rate_limit_key="mcp_client",
    validate_input=True,
    validation_rules={
...
@mcp_server.tool()
@rate_limit(rule_name="lenient")  # More lenient for read operations
