...
def pseudo_beta_fn(aatype, all_atom_positions, all_atom_mask):
    ...
    if aatype.shape[0] > 0:
        # Handle one-hot encoded aatype
        if len(aatype.shape) == 3:
            # aatype is one-hot encoded (num_templates, num_residues, num_aa_types)
            # Extract glycine probability (index 7)
            is_gly = aatype[..., 7] > 0.5  # (num_templates, num_residues)
        else:
            # aatype is integer encoded
            is_gly = torch.eq(aatype, rc.restype_order["G"])
        ca_idx = rc.atom_order["CA"]
        cb_idx = rc.atom_order["CB"]
        
        # Handle template dimension correctly
        if len(is_gly.shape) == 2 and len(all_atom_positions.shape) == 4:
            # Template case: is_gly is (num_templates, num_residues), all_atom_positions is (num_templates, num_residues, num_atoms, 3)
            # We need to reshape is_gly to match the atom positions
            is_gly_expanded = is_gly.unsqueeze(-1).expand(-1, -1, 3)  # (num_templates, num_residues, 3)
            ca_positions = all_atom_positions[..., ca_idx, :]  # (num_templates, num_residues, 3)
            cb_positions = all_atom_positions[..., cb_idx, :]  # (num_templates, num_residues, 3)
            pseudo_beta = torch.where(is_gly_expanded, ca_positions, cb_positions)
        else:
            # Non-template case: use original logic
...
def atom37_to_torsion_angles(
    ...
    if len(aatype.shape) == 3:
        ...
        aatype_int = torch.argmax(aatype, dim=-1)
    else:
        aatype_int = aatype

    aatype_int = torch.clamp(aatype_int, max=20)
    ...
    atom_indices = chi_atom_indices[..., aatype_int, :, :]
    ...
    chis_mask = chi_angles_mask[aatype_int, :]
    ...
    mirror_torsion_angles = torch.cat(
        [
            all_atom_mask.new_ones(*aatype_int.shape, 3),
