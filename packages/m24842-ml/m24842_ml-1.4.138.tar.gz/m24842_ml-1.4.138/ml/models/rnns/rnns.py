...
class SeqLinear(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        if src_len % self.chunk_size != 0:
            ...
            x_f = F.pad(x_f, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
            x_b = F.pad(x_b, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
            O_f = F.pad(O_f, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
            O_b = F.pad(O_b, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
            I_f = F.pad(I_f, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
            I_b = F.pad(I_b, (0, 0, 0, 0, 0, pad_src_len - src_len), value=0.0)
...
class DecayBlock(nn.Module):
    ...
    def bidirectional(self, x):
        ...
        if cuda_causal_conv1d is not None:
            ...
            x_f = self.conv(x.transpose(1, 2).contiguous()).transpose(1, 2)[:, :pad_src_len]
            x_b = self.conv(x.flip(1).transpose(1, 2).contiguous()).transpose(1, 2)[:, :pad_src_len]
        
        x_f = rearrange(x_f, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        x_b = rearrange(x_b, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        
        w_f = F.silu(w * self.w_base).transpose(1, 2)
        w_b = w_f.flip(1)
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            ...
            w = F.softmax(-F.silu(w * self.w_base), dim=1)
            out = torch.einsum('bthd, bsh -> bthd', x, w)
