...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_model, n_heads,
        ...
        self.alpha = (6 / (self.d_state + self.d_inner)) ** 0.5
        d_in_proj = 2 * self.d_state + self.d_inner + 2 * self.n_heads
        ...
        self.out_proj = nn.Linear(d_inner, d_model, bias=False, device=device)
    ...
    def _reset_parameters(self):
        ...
        nn.init.uniform_(self.w_base, -0.1, 0.1)
        nn.init.xavier_uniform_(self.out_proj.weight)
    ...
    def bidirectional(self, x):
        ...
        xIOw_f = self.conv(self.in_proj(x).transpose(1, 2).contiguous()).transpose(1, 2)[:, :src_len]
        xIOw_b = self.conv(self.in_proj(x).flip(1).transpose(1, 2).contiguous()).transpose(1, 2)[:, :src_len]
        
        x_f, I_f, O_f, w_f, g_f = torch.split(
            xIOw_f,
            [
                self.d_state,
                self.d_state,
        ...
        x_b, I_b, O_b, w_b, g_b = torch.split(
            xIOw_b,
            [
                self.d_state,
                self.d_state,
        ...
        x_f = rearrange(self.alpha * x_f, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        I_f = rearrange(I_f, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        O_f = rearrange(O_f, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        
        x_b = rearrange(self.alpha * x_b, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        I_b = rearrange(I_b, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        O_b = rearrange(O_b, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()

        decay_f = F.silu(w_f * self.w_base).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
        decay_b = F.silu(w_b * self.w_base).transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
        ...
        out = out_f * F.sigmoid(g_f).unsqueeze(-1) + out_b * F.sigmoid(g_b).unsqueeze(-1)
    ...
    def forward(self, x, mode="causal"):
        ...
        xIOw = self.conv(self.in_proj(x).transpose(1, 2).contiguous()).transpose(1, 2)[:, :src_len]
        
        x, I, O, w, g = torch.split(
            xIOw,
            [
                self.d_state,
                self.d_state,
        ...
        x = rearrange(self.alpha * x, 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        ...
        out = out * F.sigmoid(g).unsqueeze(-1)
        out = rearrange(out, 'b s h d -> b s (h d)')
        out = self.out_proj(out)
        return out
