...
class ThePile(Dataset):
    def __init__(self, split, tokenizer, min_len=1, max_len=1000, warmup_epochs=0, num_proc=4, root=None, shard_size=10_000_000):
        ...
        cache_dir = os.path.expanduser(os.path.join(pile_dir, f"{split}_cache"))
        ...
        if not shard_dirs:
            ...
            if split == "train":
                ...
                download_path = hf_hub_download("monology/pile-uncopyrighted", "val.jsonl.zst", cache_dir=pile_dir)
                ...
                download_path = hf_hub_download("monology/pile-uncopyrighted", "test.jsonl.zst", cache_dir=pile_dir)
                with open(download_path, "rb") as compressed, open(path, "wb") as out_f:
                    dctx = zstd.ZstdDecompressor()
                    reader = dctx.stream_reader(compressed)
                    shutil.copyfileobj(reader, out_f)
