...
class SeqLinear(nn.Module):
    ...
    def forward(self, x, mode="causal"):
        ...
        if mode == "causal":
            decay = w.transpose(1, 2).unsqueeze(-2).repeat(1, 1, src_len, 1)
            mask = torch.tril(torch.ones(src_len, src_len, dtype=torch.bool, device=self.device), diagonal=-1)
            ...
            out = torch.einsum('bthd, bhts, bshd, bshe -> bthe', x, decay, I, O * torch.exp(-w).unsqueeze(-1))
        elif mode == "noncausal":
            decay = F.softmax(-w, dim=1)
            out = torch.einsum('bthd, bsh, bshd, bshe -> bthe', x, decay, I, O * torch.exp(-w).unsqueeze(-1))
