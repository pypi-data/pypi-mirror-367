...
class SeqLinear(nn.Module):
    ...
    def __init__(self, d_in, d_out, n_heads,
        ...
        self.alpha = (6 / (d_in + d_out)) ** 0.5
        d_in_proj = d_in + d_out + 2 * self.n_heads
        self.in_proj = nn.Linear(d_in, d_in_proj, bias=False, device=device)
        self.conv = nn.Conv1d(
        ...
        self.residual = nn.Linear(d_in, d_out, bias=False, device=device)
    ...
    def bidirectional(self, x):
        ...
        IOw = self.conv(self.in_proj(x).transpose(1, 2).contiguous()).transpose(1, 2)[:, :src_len]
        residual = rearrange(self.residual(x), 'b s (h d) -> b s h d', h=self.n_heads).contiguous()
        ...
        out_f = torch.einsum('bthd, bhts, bshd, bshe -> bthe', x, decay_f, I, O)
        out_b = torch.einsum('bthd, bhts, bshd, bshe -> bthe', x, decay_b, I, O)
        
        out = (out_f + out_b) * g.unsqueeze(-1) + residual * (1 - g.unsqueeze(-1))
    ...
    def forward(self, x, mode="causal"):
        ...
        out = out * g.unsqueeze(-1) + residual * (1 - g.unsqueeze(-1))
        out = rearrange(out, 'b s h d -> b s (h d)')
        return out
