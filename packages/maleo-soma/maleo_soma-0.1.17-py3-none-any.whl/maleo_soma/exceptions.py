...
def from_http(
    ...
    error_mapping: Dict[int, Tuple[Type[Error], str]] = {
        400: (
            BadRequest,
            f"Failed requesting {resource_name} due to Bad Request response",
        ),
        401: (
            Unauthorized,
            f"Failed requesting {resource_name} due to Unauthorized response",
        ),
        403: (
            Forbidden,
            f"Failed requesting {resource_name} due to Forbidden response",
        ),
        404: (NotFound, f"Failed requesting {resource_name} due to Not Found response"),
        405: (
            MethodNotAllowed,
            f"Failed requesting {resource_name} due to Method Not Allowed response",
        ),
        422: (
            UnprocessableEntity,
            f"Failed requesting {resource_name} due to Unprocessable Entity response",
        ),
        429: (
            TooManyRequests,
            f"Failed requesting {resource_name} due to Too Many Requests response",
        ),
        500: (
            InternalServerError,
            f"Failed requesting {resource_name} due to Internal Server Error response",
        ),
        501: (
            NotImplemented,
            f"Failed requesting {resource_name} due to Not Implemented response",
        ),
        502: (
            BadGateway,
            f"Failed requesting {resource_name} due to Bad Gateway response",
        ),
        503: (
            ServiceUnavailable,
            f"Failed requesting {resource_name} due to Service Unavailable response",
        ),
    }
    ...
    error_class, summary = error_mapping.get(
        ...
        (
            InternalServerError,
            f"Failed requesting {resource_name} due to unexpected error",
        ),
    )
