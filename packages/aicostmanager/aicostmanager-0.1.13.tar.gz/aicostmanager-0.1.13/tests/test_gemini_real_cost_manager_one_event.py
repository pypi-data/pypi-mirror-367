...
def verify_event_delivered(
    ...
    for attempt in range(max_attempts):
        try:
            ...
            print(f"  Attempt {attempt + 1}: GET /api/v1/usage/events/ -> {events_response.status_code}")
            ...
            print(f"  Event not found yet, waiting 3s...")
...
def test_gemini_single_event_delivery():
    ...
    for i, config in enumerate(tracked_client.configs):
        print(f"   Config {i+1}: api_id={config.api_id}, config_id={config.config_id}")
    ...
    assert len(final_payloads) == 1, f"Expected exactly 1 payload, got {len(final_payloads)}"
    ...
    if response_id:
        ...
        if event:
            print(f"✅ Event successfully delivered and verified!")
            ...
            print(f"❌ Event verification failed - event not found on server")
            pytest.fail(f"Usage event for response_id {response_id} was not delivered to server")
    else:
        print(f"⚠️  No response_id available - cannot verify server delivery")
        print(f"   This might indicate a payload generation issue")
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, "/Users/keytonweissinger/home/aicostmanager/aicostmanager-python")
    
    # Set default environment if not already set
    if not os.environ.get("AICM_API_KEY"):
        os.environ["AICM_API_KEY"] = "sk-api01-361e196e-de00-4be4-94cc-88dc1a4bffc4.7dbb634f-00f3-4506-9979-559dd22807d0"
