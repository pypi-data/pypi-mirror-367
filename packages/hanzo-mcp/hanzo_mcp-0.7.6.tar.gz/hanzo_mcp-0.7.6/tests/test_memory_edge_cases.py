...
class TestMemoryEdgeCases:
    ...
    def test_concurrent_operations(self, tool_helper,mock_get_service, mock_ctx):
        ...
        mock_service.create_memory = Mock(side_effect=lambda *args, **kwargs: 
            asyncio.create_task(mock_create_async(*args, **kwargs)))
        mock_get_service.return_value = mock_service
        ...
        result = asyncio.run(tool.call(
            mock_ctx,
            operations={
                "create": ["Memory A", "Memory B", "Memory C"]
            }
        ))
    ...
    async def test_service_timeout_handling(self, tool_helper,mock_get_service, mock_ctx):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                tool.call(mock_ctx, queries=["test"]),
                timeout=0.1
            )
...
class TestMemoryToolsSecurity:
    ...
    def test_user_isolation(self, tool_helper,mock_get_service, mock_ctx):
        ...
        mock_service.delete_memory = Mock(side_effect=lambda uid, mid: accessed_users.append(uid))
        mock_get_service.return_value = mock_service
