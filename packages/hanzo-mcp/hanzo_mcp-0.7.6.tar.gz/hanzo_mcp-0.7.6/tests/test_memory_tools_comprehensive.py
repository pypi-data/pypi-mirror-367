...
class MockMemoryService:
    ...
    def create_memory(self, user_id: str, project_id: str, content: str, 
                     metadata: Dict = None, importance: float = 1.0, **kwargs):
        """Mock create memory."""
    ...
    def search_memories(self, user_id: str, query: str, project_id: str = None,
                       limit: int = 10, **kwargs):
        ...
        for memory in self.memories.values():
            if memory.user_id == user_id:
                ...
                results.append(MockMemoryWithScore(
                    **memory.__dict__,
...
class TestMemoryTools:
    ...
    def test_update_memories(self, tool_helper,mock_get_service, mock_memory_service, mock_ctx):
        ...
        result = asyncio.run(tool.call(
            mock_ctx,
            updates=[
                {"id": memory.memory_id, "statement": "Updated content"}
            ]
        ))
    ...
    def test_delete_memories(self, tool_helper,mock_get_service, mock_memory_service, mock_ctx):
        ...
        memory1 = mock_memory_service.create_memory(
            user_id="test_user",
            project_id="test_project",
            content="Memory to delete 1"
        )
        memory2 = mock_memory_service.create_memory(
            user_id="test_user",
            project_id="test_project", 
            content="Memory to delete 2"
        )
    ...
    def test_manage_memories_batch(self, tool_helper,mock_get_service, mock_memory_service, mock_ctx):
        ...
        result = asyncio.run(tool.call(
            ...
            operations={
                ...
                "update": [{"id": memory.memory_id, "statement": "Updated existing"}],
                "delete": ["invalid_id"]
            }
        ))
...
class TestKnowledgeTools:
    ...
    def test_recall_facts(self, tool_helper,mock_get_service, mock_memory_service, mock_ctx):
        ...
        asyncio.run(store_tool.call(
            mock_ctx,
            facts=["Python fact 1", "Python fact 2"],
            kb_name="python_kb"
        ))
        ...
        result = asyncio.run(recall_tool.call(
            mock_ctx,
            queries=["Python"],
            kb_name="python_kb"
        ))
...
class TestMemoryToolsIntegration:
    ...
    def test_full_memory_workflow(self, tool_helper,mock_get_service, mock_memory_service, mock_ctx):
        ...
        result = asyncio.run(create_tool.call(
            mock_ctx,
            statements=[
        ...
        result = asyncio.run(kb_tool.call(
            mock_ctx,
            action="create",
            kb_name="user_prefs",
        ...
        result = asyncio.run(facts_tool.call(
            mock_ctx,
            facts=[
