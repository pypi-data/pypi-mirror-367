...
try:
    from hanzo_network import (
        ...
        Network, Agent, Tool, State,
        create_agent_network, NetworkConfig
    )
...
class TestHanzoNetworkIntegration:
    ...
    async def test_agent_communication(self, tool_helper,basic_network):
        ...
        async def handle_message(message: Dict[str, Any]):
            ...
            return {"status": "received", "content": f"Acknowledged: {message['content']}"}
    ...
    async def test_local_compute_orchestration(self, tool_helper,basic_network):
        ...
        simple_result = await basic_network.delegate_task(
            task="Format this JSON: {\"name\":\"test\"}",
            complexity="simple"
        )
    ...
    async def test_agent_network_persistence(self, tool_helper,basic_network, temp_dir):
        ...
        new_network = await create_agent_network(
            NetworkConfig(name="restored-network")
        )
