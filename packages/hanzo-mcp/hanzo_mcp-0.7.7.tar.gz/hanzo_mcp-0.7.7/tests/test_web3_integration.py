...
try:
    from hanzo_agents import (
        Agent, State, Network, Tool,
        WalletConfig, AgentWallet, generate_shared_mnemonic,
        TEEConfig, TEEProvider, ConfidentialAgent,
        WEB3_AVAILABLE
    )
...
try:
    from hanzo_network import (
        ...
        InferenceRequest, ModelConfig, ModelProvider,
...
class TestWeb3Integration:
    ...
    async def test_agent_payment(self):
        ...
        payment_request = await agent2.request_payment(
            from_address=agent1.address,
            amount_eth=1.0,
            task_description="Test service"
        )
...
class TestLocalComputeIntegration:
    ...
    async def test_compute_marketplace_integration(self):
        ...
        user_agent = Web3Agent(
            name="user",
            description="Needs AI inference"
        )
