...
class ProjectAuthorization:
    ...
    @abstractmethod
    def verify_anonymous(self, jwt_token, typically):
        """
        hàm kiểm tra token là loại anonymous hay không, check signature và exp có thỏa mãn không
        :param jwt_token:
        :param typically:
        :param method:
        :return: True or False
    ...
    @abstractmethod
    def verify_anonymous_signature(self, jwt_token, typically):
        """
        hàm kiểm tra token là loại anonymous hay không, chỉ check signature có thỏa mãn không
        :param jwt_token:
        :param typically:
        :param method:
        :return: True or False
...
class HttpJwtAuth(object):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if request.method != "OPTIONS":  # pragma: no cover
                if auth:
                    ...
                    if not self.project_auth.verify_anonymous(token, auth_type):
                        print(
                            "authenticate for token %s api_name  %s error %s"
                            % (token, method, str(request.data)[0])
                        )
    ...
    def verify_anonymous_signature(self, f):
        method = f.__name__
        ...
        def decorated_function(*args, **kwargs):
            ...
            if request.method != "OPTIONS":  # pragma: no cover
                if auth:
                    ...
                    if not self.project_auth.verify_anonymous_signature(token, auth_type):
                        print(
                            "authenticate for token %s api_name  %s error %s"
                            % (token, method, str(request.data)[0])
                        )
