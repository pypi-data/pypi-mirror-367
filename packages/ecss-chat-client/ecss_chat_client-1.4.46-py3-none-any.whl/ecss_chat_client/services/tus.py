...
class TusService(Base):
    ...
    def get_file(self, room_id: str, file_id: str, token: str) -> Response:
        """Получить файл по tus.

        :param room_id: uid комнаты
        :param file_id: uid файла
        :param token: токен для доступа к файлу

        :return: Response
        """
        return self._make_request(
            endpoint=f'elph_chat/file-upload/files/rooms/{room_id}/uploads'
                     f'/{file_id}',
            params={'token': token},
            method='GET',
            tus_path=True,
