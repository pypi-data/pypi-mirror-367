...
from . import Stage, WorkMode, ChooseMode, UnitType, DisplayAttitude, QueryOp, add_to_layout, img, \
    current_theme, alignment_flag
...
class YFieldWidget(QTextEdit):
    ...
    def setup_ui(self):
        ...
        if self.col_spec.unit_type() != UnitType.RICH_TEXT:
    ...
    def field_alignment(self):
        ...
        elif spec.unit_type() in (UnitType.CURRENCY, UnitType.INT, UnitType.NUMBER):
            return Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
        elif spec.unit_type() in (UnitType.CHOOSE, UnitType.LOOKUP, UnitType.DATE):
    ...
    def get_field_label(self):
        ...
        label: str = get_attr(self, 'label', None)
        if label is None:
...
class YDisplayDelegate(QStyledItemDelegate):
    ...
    def __init__(self, view: QTableView | QTreeView, col_spec: ColumnSpec, **kwargs):
        ...
        self.utype = self.factory.deduce_unit_type(col_spec=self.col_spec)  # UnitType(col_spec.unit_type())
...
class YFdChoose(YFieldEditor):
    def __init__(self, parent=None, **kwargs):
        ...
        if self.code_dict is None:
            self.code_dict = {}
        self.query_op = get_attr(self, 'op', QueryOp.IN)
        self.mode = get_attr(self, 'mode', ChooseMode.LIST.value)
        self.format = get_attr(self, 'format', '[{code}] - {name}')
        # 如果是查询场景，mode自动设置为checkbox,布局根据可选项数量确定为'V'或'G2'
        if self.stage == Stage.IN_QUERY and self.query_op != QueryOp.EQU:
...
class YFpChoose(YFieldPainter):
    ...
    def get_color(self, value, text):
        ...
        for att, v in self.attitude.items():
            if value in v:
                return self.get_attitude_color(DisplayAttitude(att))
        return None
...
class YFdCriteria(QWidget):
    ...
    def __init__(self,
        ...
        self.label = self.column_spec.title()
    ...
    def get_default_op(self):
        return {
            YFdText: QueryOp.LIKE_IN,
            YFdNumber: QueryOp.BETWEEN,
            YFdInt: QueryOp.BETWEEN,
            YFdYear: QueryOp.BETWEEN,
            YFdCurrency: QueryOp.BETWEEN,
            YFdDate: QueryOp.BETWEEN,
            YFdDateTime: QueryOp.BETWEEN,
            YFdYearMonth: QueryOp.BETWEEN,
            YFdChoose: QueryOp.IN,
            YFdLookup: QueryOp.IN,
        }[self.editor_cls]
    ...
    def is_editor_pair(self):
        return self.op == QueryOp.BETWEEN
