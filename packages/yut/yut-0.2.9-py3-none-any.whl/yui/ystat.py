...
import yui
from yui import UnitType, img, current_theme, create_layout, create_button, load_icon
...
class StatsCalculator:
    ...
    @staticmethod
    def group_stats(df, group_field, stats_type, value_fields, value_columns=None, units=1):
        """
        执行分组统计
        参数:
            df: 原始DataFrame
    ...
    @staticmethod
    def cross_stats(df, row_field, col_field, value_field, stats_type, units=1):
        """
        执行交叉统计
        参数:
            df: 原始DataFrame
...
class GroupStatsWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.stats_combo.setCurrentText(StatsType.SUM.value)
    ...
    def set_data(self, df, cn_mapping, value_columns=None):
        ...
        for i, col in enumerate(self.value_columns):
            ...
            item.setCheckState(Qt.CheckState.Unchecked if i > 0 else Qt.CheckState.Checked)  # 默认选中第一个
...
class YStatWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.model = None
        self.df = None
        self.group_tab = GroupStatsWidget()
        self.cross_tab = CrossStatsWidget()
    ...
    def prepare_dataframe(self):
        ...
        df = self.model.data_frame().copy()
        column_specs = self.model.column_specs()
        choose_columns = [spec.name for spec in column_specs if
                          spec.unit_type() in (UnitType.CHOOSE, UnitType.LOOKUP)]
        for col in choose_columns:
            try:
                col_idx = self.model._display_columns.index(col)
                new_dat = [self.model.data(self.model.index(i, col_idx),
                                           Qt.ItemDataRole.DisplayRole) for i in range(self.model.rowCount())]
                df[col] = new_dat
            except ValueError:
                pass
        return df
    ...
    def set_model(self, model: PandasModel, value_columns=None):
        self.model = model
        df = self.prepare_dataframe()  # 预处理之后的df
        column_specs = self.model.column_specs()
        col_map = {sp.name: sp.comment for sp in column_specs}
        if not value_columns:
            value_columns = [sp.name for sp in column_specs if sp.unit_type() in (UnitType.CURRENCY, UnitType.INT)]
        self.set_data_frame(df, column_names=col_map, value_columns=value_columns)
