...
class EnapSearchElastic(Page):
	...
	def normalize_servico(self, item):
		return {
			...
			"descricao": item.get("enap_designsystem_enapcomponentes__descricao_filter", ""),
			"url": item.get("enap_designsystem_enapcomponentes__url_filter", ""),
			"data_atualizacao": item.get("enap_designsystem_enapcomponentes__data_atualizacao_filter", ""),
			"data_publicacao": item.get("first_published_at_filter", ""),
		}
...
def custom_serve(self, request, *args, **kwargs):
    ...
    try:
        ...
        template_name = 'enap_designsystem/templates/404.html'  
