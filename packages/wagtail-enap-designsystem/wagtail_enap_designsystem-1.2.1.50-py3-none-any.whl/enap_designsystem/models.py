...
class ENAPComponentes(Page):
	...
	@property
	def categoria_filter(self):
		"""Categoria do serviço"""
		return "Serviços"

	@property
	def categoria(self):  # ← ADICIONE ESTA LINHA
		"""Categoria do serviço"""
		return "Serviços"
...
class EnapSearchElastic(Page):
	...
	def search_wagtail_services(self, query, page, rows_per_page, ordenacao):
		"""Busca nos modelos de serviço do Wagtail - MÉTODO AJUSTADO"""
		from django.core.paginator import Paginator
		from django.contrib.contenttypes.models import ContentType
		
		try:
			# Busca todos os content types e filtra pelo verbose_name
			...
			for ct in content_types:
				...
				if model_class and hasattr(model_class, '_meta'):
					if getattr(model_class._meta, 'verbose_name', '') == 'serviços Enap':
						content_type = ct
						break
			...
			queryset = Page.objects.live().public().filter(content_type=content_type)
			...
			elif ordenacao == "relevantes":
				# Para relevantes, se houver query use score, senão recentes
				if query:
					queryset = queryset.order_by('-_score')
				else:
					queryset = queryset.order_by('-last_published_at')
			elif ordenacao == "vistos":
			...
			for item in page_obj:
				# ✅ AJUSTE: Estrutura seguindo padrão normalize_servico
				result = {
					"titulo": getattr(item, 'titulo_filter', item.title),
					"descricao": getattr(item, 'descricao_filter', ''),
					"url": getattr(item, 'url_filter', item.url),
					"data_publicacao": item.first_published_at,
					"data_atualizacao": item.last_published_at,
					"imagem": getattr(item, 'imagem_filter', None),
				}
			...
			print(f"Erro na busca Wagtail: {e}")
			return {"results": [], "total": 0}

	def count_wagtail_services(self, query):
		"""Conta total de serviços no Wagtail"""
		try:
			from django.contrib.contenttypes.models import ContentType
	...
	def parse_images(self, item, tipo):
		try:
			...
			# Caso 3: nenhum dos dois — define None ou imagem default
				...
				elif tipo == "servicos":  # ✅ AJUSTE: Adicionado caso para serviços
					item["imagem"] = "/static/enap_designsystem/icons/thumb-servicos.png"
				else:
					item["imagem"] = None
	...
	def normalize_servico(self, item):
		# ✅ AJUSTE: Mudança no prefixo para ENAPService
		return {
			"titulo": item.get("enap_designsystem_enapservice__titulo_filter", ""),
			"descricao": item.get("enap_designsystem_enapservice__descricao_filter", ""),
			"url": item.get("enap_designsystem_enapservice__url_filter", ""),
			"data_atualizacao": item.get("enap_designsystem_enapservice__data_atualizacao_filter", ""),
			"data_publicacao": item.get("first_published_at_filter", ""),
			"imagem": item.get("enap_designsystem_enapservice__imagem_filter", ""),
		}
	...
	def get_context(self, request, *args, **kwargs):
		...
		for chave, endpoint in endpoints.items():
			if chave == "servicos":
				tabs_totais[chave] = self.count_wagtail_services(query)
			else:
				# Usa a nova função para outras abas
		...
		# ✅ BUSCA MODIFICADA - Serviços usa Wagtail local
		if tipo == "servicos":
			search_result = self.search_wagtail_services(query, page, rows_per_page, ordenacao)
			context["results"] = search_result["results"]
			context["results_count"] = search_result["total"]
			...
			if total_results == 0 or len(search_result["results"]) == 0:
				start_display = 0
				end_display = 0
			else:
				start_display = ((page - 1) * rows_per_page) + 1
				end_display = start_display + len(search_result["results"]) - 1
		...
		elif tipo in endpoints and endpoints[tipo] != "wagtail_local":
			try:
...
class ENAPService(Page):
	...
	search_fields = Page.search_fields + [
		...
		index.SearchField("titulo_filter", name="titulo"),
		index.SearchField("descricao_filter", name="descricao"),
		index.FilterField("categoria_filter", name="categoria"),
		index.SearchField("url_filter", name="url"),
