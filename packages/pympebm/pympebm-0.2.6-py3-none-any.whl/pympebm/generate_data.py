        ...
        for healthy_ratio in rs:
            for variant in range(num_of_datasets_per_combination):
                ...
                if mixed_pathology:
                    ...
                    combined_ordering, ordering_array = mp_utils.get_combined_order(params, low_num, high_num, low_length, high_length, rng=rng, method=mp_method, mcmc_iterations=mcmc_iterations)
                    ...
                    if mp_method == 'PL':
                        PL = mp_utils.PlackettLuce(ordering_array=ordering_array, rng=rng, sample_count=sample_count)
                        if sample_count > 1:
                            certainty = PL.compute_certainty()
                    else:
                        mcmc_sampler = mp_utils.MCMC(ordering_array=ordering_array, method=mp_method, sample_count=sample_count, rng=rng, iterations=mcmc_iterations)
                        if sample_count > 1:
