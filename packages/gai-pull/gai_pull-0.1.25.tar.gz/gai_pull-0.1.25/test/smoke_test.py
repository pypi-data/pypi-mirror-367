...
def smoke_test(use_editable: bool = False):
    ...
    print(f"[yellow]🔍 Testing gai-pull version: {version}[/yellow]")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # simpler: grep gai-pull version from pip list
        output = subprocess.check_output(
            f"{py} -m pip list --format=freeze | grep gai-pull",
            shell=True,
            env=env,
            text=True,
        )
        # output is like "gai-pull==1.2.3\n"
        installed_version = output.strip().split("==", 1)[1]
        print(f"[green]✅ Installed gai-pull version: {installed_version}[/green]")
