...
from variable_dict import s2s_variables, model_origin, origin_latency_hours, fc_weekday_initials
...
def check_model_name(model):
    ...
    all_models = []
    for model, origin in s2s_variables.items():
...
def check_leadtime_hours(leadtime_hour,variable,origin_id):
    ...
    time_resolution = argument_output.get_timeresolution(origin_id)
