...
def scan_households(source1: str | bytes, source2: str | bytes):
    lf = pl.concat(
        (
            pl.scan_csv(source1, separator=";", schema_overrides=SCHEMA, null_values=["a"]),
            pl.scan_csv(source2, separator=";", schema_overrides=SCHEMA, null_values=["a"]),
        ),
        how="vertical",
    )
    return lf
...
def standardize_households(
    ...
    lf = scan_households(source1, source2)
    lf = lf.rename(
        {
...
def standardize_cars(source1: str | bytes, source2: str | bytes, households: pl.LazyFrame):
    lf = scan_households(source1, source2)
    # Add household_id.
    ...
    lf = pl.concat(
        (
            lf.select(
                ...
                ownership=(pl.col(f"M11{a}") + pl.col(f"MA{a}")).replace_strict(CAR_OWNERSHIP_MAP),
...
def standardize_motorcycles(source1: str | bytes, source2: str | bytes, households: pl.LazyFrame):
    lf = scan_households(source1, source2)
    # Add household_id.
    lf = lf.with_columns(original_household_id=pl.struct("ECH", "STM")).join(
        households.select("original_household_id", "household_id"),
