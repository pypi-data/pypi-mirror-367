...
def scan_legs(source1: str | bytes, source2: str | bytes):
    lf = pl.concat(
        (
            pl.scan_csv(source1, separator=";", schema_overrides=SCHEMA, null_values=["aa"]),
            pl.scan_csv(source2, separator=";", schema_overrides=SCHEMA, null_values=["aa"]),
        ),
        how="vertical",
    )
    return lf
...
def standardize_legs(
    ...
    # For EDGT, the walking legs are not recorded explicitly, instead, the walking time before and
