...
def standardize_persons(
    ...
    lf = lf.with_columns(
        ...
        # corresponds to the month (January or February). The exact date is unknown. We set the date
        # to either 1st January or 2nd February (given that having a date that is a bit off is
        # probably better than having a NULL date).
        # The `min_horizontal`s are then required in both then `then` and the `otherwise` because
        # polars evaluate both cases each time.
        trip_date=pl.when(pl.col("MOIS").eq(14))
        .then(pl.date(year="ANNEE", month=pl.min_horizontal("DATE", 12), day="DATE"))
        .otherwise(pl.date(year="ANNEE", month=pl.min_horizontal("MOIS", 12), day="DATE")),
