...
SCHEMA = {
    ...
    "PMET": pl.UInt8,  # Méthode d'enquête du ménage
    ...
    "AN": pl.UInt16,  # Année du jour d’enquête
    ...
    "P13B": pl.UInt8,  # Pratique du télétravail
    "P18": pl.UInt8,  # Facilités de stationnement sur le lieu de travail ou d'études
...
def standardize_persons(
    ...
    # Add household_id.
    lf = lf.with_columns(original_household_id=pl.struct(METH="PMET", ECH="ECH", STM="STP")).join(
    ...
    lf = lf.with_columns(
        original_person_id=pl.struct("PMET", "ECH", "STP", "PER"),
        trip_date=pl.date(year="AN", month="MOIS", day="DATE"),
    ...
    lf = lf.with_columns(
        ...
        work_car_parking=pl.when(pl.col("is_student").not_()).then(
            pl.col("P18").replace_strict(WORK_STUDY_CAR_PARKING_MAP)
        ),
