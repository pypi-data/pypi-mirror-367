...
SCHEMA = {
    ...
    "TMET": pl.UInt8,  # Méthode d'enquête du ménage
    ...
    "T3A": pl.UInt8,  # Voiture sans permis
...
def scan_legs(source: str | bytes):
    lf = pl.scan_csv(source, separator=";", schema_overrides=SCHEMA)
    return lf
...
def standardize_legs(
    ...
    lf = lf.with_columns(
        original_trip_id=pl.struct(DMET="TMET", ECH="ECH", STD="STT", PER="PER", NDEP="NDEP")
    ).join(
        trips.select(
            "original_trip_id",
            "trip_id",
