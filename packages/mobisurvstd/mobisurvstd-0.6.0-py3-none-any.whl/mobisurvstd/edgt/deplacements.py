...
def scan_trips(source1: str | bytes, source2: str | bytes):
    lf = pl.concat(
        (
            pl.scan_csv(source1, separator=";", schema_overrides=SCHEMA),
            pl.scan_csv(source2, separator=";", schema_overrides=SCHEMA),
        ),
        how="vertical",
    )
    return lf
...
def standardize_trips(
    ...
    lf = scan_trips(source1, source2)
    # Add household_id, person_id, and trip date.
    lf = lf.with_columns(original_person_id=pl.struct(ECH="ECH", STP="STD", PER="PER")).join(
    ...
    lf = lf.with_columns(
        original_trip_id=pl.struct("ECH", "STD", "PER", "NDEP"),
