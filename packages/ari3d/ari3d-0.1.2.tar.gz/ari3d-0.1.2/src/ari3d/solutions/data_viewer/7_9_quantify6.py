...
def quantify_liberated_regions(
    ...
    for i, (index, row) in enumerate(histograms_subdata.iterrows()):
        ...
        if np.count_nonzero(peaks > background_peak) == 1:
            ...
            if enable_PVE:
                ...
                volumeAnalysed += weighted_q_phase_1 + q_phase_1
            else:
                ...
                volumeAnalysed += q_phase_1

            # surface quantification
            s_q_liberated = surface_mesh_subdata.iloc[
    ...
    # datasets to return
    q_1_phases = pd.DataFrame(
        q_1_phases_append, columns=["Quantification_phase_1"]
    )
    s_q_1_phases = pd.DataFrame(
        surface_q_append, columns=["Surface_quantification"]
    )
    if enable_PVE:
    ...
    for i in range(1, 6):
        ...
        q_1_phase_sorted[f"Peak_{i}"] = np.where(
...
def quantify3_phases_particle(
    ...
    for i in range(1, 6):
        ...
        q_3_phase_sorted[f"Peak_{i}"] = np.where(
            mask, peaks_1_phase["Peak_1"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_3_phase_sorted_1[f"Peak_{i}"] = np.where(
    ...
    for i in range(1, 6):
        ...
        q_3_phase_sorted_2[f"Peak_{i}"] = np.where(
    ...
    q_3_phase_sorted = q_3_phase_sorted.mask(
        q_3_phase_sorted == 0, q_3_phase_sorted_1
    )
    q_3_phase_sorted = q_3_phase_sorted.mask(
        q_3_phase_sorted == 0, q_3_phase_sorted_2
    )
    ...
    if enable_PVE:
        ...
        for i, (index, row) in enumerate(
            ...
            if (np.count_nonzero(peaks > background_peak) == 2) and i > -1:
                ...
                sum_phase_1 = inner_hist_subdata.iloc[i, background_peak:particle_peak_1].sum()
                ...
                voxels_p1_to_p2 = inner_hist_subdata.iloc[i, particle_peak_1:particle_peak_2]
                voxels_p1_to_p2 = np.array(voxels_p1_to_p2)

                # weighting for phase 1 to phase 2
                weights_p1_to_p2 = np.arange(0, 1, 1 / ((particle_peak_2) - particle_peak_1))
                ...
                # Voxels pure phase 2
                outer_volume_pure_phase_2 = outer_hist_subdata.iloc[i, particle_peak_2:65535].sum()

                # weighting BG to phase 1
                weights_p1_to_bg = np.arange(
                    0, 1, 1 / ((particle_peak_1 - 1) - background_peak)
                )
                weights_p1_to_bg = np.array(
                    weights_p1_to_bg
                )

                # voxels to background to phase 1
                voxels_bg_to_p1 = outer_hist_subdata.iloc[i, background_peak:particle_peak_1 - 1]
                ...
                weights_bg_to_p2 = np.array(
                    weights_bg_to_p2
                )

                # voxels to background to phase 2
                voxel_bg_to_p2 = outer_hist_subdata.iloc[i, background_peak: particle_peak_2 - 1]
                ...
                vol_phase_1_within_phase_2 = q_outer_phase_2_array[background_peak:particle_peak_1]
                vol_phase_1_within_phase_2 = vol_phase_1_within_phase_2.sum()

                # Sum of outer phase 1 and outer phase 2
                q_outer_phase_2_array = q_outer_phase_2_array.sum() - vol_phase_1_within_phase_2
                q_outer_phase_1_array = q_outer_phase_1_array.sum()

                # assemble the PVE adjusted outer volume
                pve_adjusted_volume = outer_volume_pure_phase_2 + q_outer_phase_1_array + q_outer_phase_2_array
                ...
                # calculate surface volume
                surface_ratio = (
                                    surface_mesh_subdata.iloc[i,
                                    background_peak: int(gradient_ratio * phase_limit)].sum()
                ...
                q_outer_phase_2_volume = (
                        pve_adjusted_volume - q_outer_phase_1_volume
                )
                ...
                regions_2_phases += 1
                regionsAnalysed += 1
                volumeAnalysed += sum_phase_1 + sum_phase_2 + out_of_peak_volume_1 + out_of_peak_volume_2 + q_outer_phase_1_volume + q_outer_phase_2_volume
        ...
        for i in range(1, 6):
            ...
            q_2_phase_sorted[f"Peak_{i}"] = np.where(
                mask, peaks_1_phase["Peak_1"], 0
            )
        ...
        for i in range(1, 6):
            ...
            q_2_phase_sorted_1[f"Peak_{i}"] = np.where(
                mask, peaks_2_phase["Peak_2"], 0
            )
        ...
        for i, (index, row) in enumerate(surface_mesh_subdata.iterrows()):
            ...
            if (np.count_nonzero(peaks > background_peak) == 2) and i > -1:
                ...
                # Taking the sum of ith row from phase 1 minimum threshold greyscale value till Phase1_max_limit
                sum_phase_2 = histograms_subdata.iloc[i, int((particle_peak_1 + particle_peak_2) / 2):
                              ].sum()
                ...
                phase_2_s_vol = surface_mesh_subdata.iloc[i, int(phase_limit_1 * gradient_ratio):].sum()
                ...
                regions_2_phases += 1
                regionsAnalysed += 1
                volumeAnalysed += sum_phase_1 + sum_phase_2
...
def quaternary_regions(
    ...
    for i in range(1, 6):
        ...
        q_4_phase_sorted[f"Peak_{i}"] = np.where(
            mask, peaks_1_phase["Peak_1"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_4_phase_sorted_1[f"Peak_{i}"] = np.where(
            mask, peaks_2_phase["Peak_2"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_4_phase_sorted_2[f"Peak_{i}"] = np.where(
            mask, peaks_3_phase["Peak_3"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_4_phase_sorted_3[f"Peak_{i}"] = np.where(
    ...
    q_4_phase_sorted = q_4_phase_sorted.mask(
        q_4_phase_sorted == 0, q_4_phase_sorted_1
    )
    q_4_phase_sorted = q_4_phase_sorted.mask(
        q_4_phase_sorted == 0, q_4_phase_sorted_2
    )
    q_4_phase_sorted = q_4_phase_sorted.mask(
        q_4_phase_sorted == 0, q_4_phase_sorted_3
    )
...
def quinary_regions(
    ...
    for i, (index, row) in enumerate(surface_mesh_subdata.iterrows()):
        ...
        if (np.count_nonzero(peaks > background_peak) == 5) and i > -1:
            ...
            regions3Phases += 1
            regionsAnalysed += 1
            volumeAnalysed += sum_phase_1 + sum_phase_2 + sum_phase_3 + sum_phase_4 + sum_phase_5
    ...
    for i in range(1, 6):
        ...
        q_5_phase_sorted[f"Peak_{i}"] = np.where(
            mask, peaks_1_phase["Peak_1"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_5_phase_sorted_1[f"Peak_{i}"] = np.where(
            mask, peaks_2_phase["Peak_2"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_5_phase_sorted_2[f"Peak_{i}"] = np.where(
            mask, peaks_3_phase["Peak_3"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_5_phase_sorted_3[f"Peak_{i}"] = np.where(
            mask, peaks_4_phase["Peak_4"], 0
        )
    ...
    for i in range(1, 6):
        ...
        q_5_phase_sorted_4[f"Peak_{i}"] = np.where(
    ...
    q_5_phase_sorted = q_5_phase_sorted.mask(
        q_5_phase_sorted == 0, q_5_phase_sorted_1
    )
    q_5_phase_sorted = q_5_phase_sorted.mask(
        q_5_phase_sorted == 0, q_5_phase_sorted_2
    )
    q_5_phase_sorted = q_5_phase_sorted.mask(
        q_5_phase_sorted == 0, q_5_phase_sorted_3
    )
    q_5_phase_sorted = q_5_phase_sorted.mask(
        q_5_phase_sorted == 0, q_5_phase_sorted_4
    )
...
enable_PVE = parameters_dict["enablePVE"]
...
################## online Arguments ##################
    ...
    with tabFindPeaks:  # table with threshold inputs
        ...
        with col1:
            # histogram smoothing checkbox
            enable_PVE = st.checkbox(
                "Activate Partial Volume correction",
                value=parameters_dict["enablePVE"],
