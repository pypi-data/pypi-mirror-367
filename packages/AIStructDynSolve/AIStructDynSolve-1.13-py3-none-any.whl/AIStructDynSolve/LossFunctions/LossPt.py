...
class LossPt:
    def __init__(
        ...
        self.dimensionsless_mass = torch.max(torch.diag(mass))/torch.min(torch.diag(mass)).to(DEFAULT_DTYPE).to(DEVICE)
        self.dimensionsless_stiffness = self.dimensionsless_mass * torch.max(torch.diag(stiffness/mass)).to(DEFAULT_DTYPE).to(DEVICE)
        self.dimensionsless_damping = self.dimensionsless_mass * torch.max(torch.diag(damping/mass)).to(DEFAULT_DTYPE).to(DEVICE)

        if train_params is not None and "mass" in {key.lower() for key in train_params}:
            self.mass_dimensionsless = torch.nn.Parameter(mass/self.dimensionsless_mass , requires_grad=True)
        else:
            self.mass = mass

        if train_params is not None and "stiffness" in {key.lower() for key in train_params}:
            self.stiffness_dimensionsless = torch.nn.Parameter(stiffness/self.dimensionsless_stiffness, requires_grad=True)
        else:
            self.stiffness = stiffness

        if train_params is not None and "damping" in {key.lower() for key in train_params}:
            self.damping_dimensionsless = torch.nn.Parameter(damping/self.dimensionsless_damping , requires_grad=True)
        else:
    ...
    def lossODE(self):
        ...
        if self.train_params is not None and "mass" in {key.lower() for key in self.train_params}:
            self.mass = self.mass_dimensionsless * self.dimensionsless_mass
        if self.train_params is not None and "stiffness" in {key.lower() for key in self.train_params}:
            self.stiffness = self.stiffness_dimensionsless * self.dimensionsless_stiffness
        if self.train_params is not None and "damping" in {key.lower() for key in self.train_params}:
            self.damping = self.damping_dimensionsless * self.dimensionsless_damping
        # 计算整体的残差
