...
async def db_lifespan(server: FastMCP) -> AsyncIterator[DbContext]:
    ...
    try:
        # Test connection on startup and fetch schema for all databases
        for db_config in db_context.db_configs.values():
            try:
                await db_config.query_runner.test_connection()
                print(f"Successfully connected to {db_config.description}")
                
                # Fetch schema after the connection is confirmed to be ready
                db_config.schema = await db_config.query_runner.get_schema()
                print(f"Successfully fetched schema for {db_config.description}")
            except Exception as e:
                print(f"Warning: Connection test or schema fetch failed for {db_config.description}: {str(e)}")
    ...
    try:
        ...
        if database_id is not None and database_id != "all":
            ...
            if not db_config.schema:
                # If schema was not fetched on startup, try to fetch it now
                try:
                    db_config.schema = asyncio.run(db_config.query_runner.get_schema())
                except Exception as e:
                    return f"Error: Could not fetch schema for database {database_id}: {str(e)}"
...
async def _execute_and_get_results_async(query: str, ctx: Context, db_id: str) -> Dict[str, Any]:
    ...
    result = await query_runner.run_query(query)
...
# Define tools
@mcp.tool()
async def execute_query(query: str, ctx: Context, db_id: str) -> str:
    """Execute a SQL query and return results as a markdown table"""
    try:
        result = await _execute_and_get_results_async(query, ctx, db_id)
...
@mcp.tool()
async def execute_query_json(query: str, ctx: Context, db_id: str) -> str:
...
@mcp.tool()
async def get_table_columns(table_name: str, ctx: Context, db_id: str) -> str:
    ...
    try:
        ...
        db_config = db_context.db_configs[db_id]
        columns = await db_config.query_runner.get_table_columns(table_name)
...
@mcp.tool()
async def get_table_types(table_name: str, ctx: Context, db_id: str) -> str:
    ...
    try:
        ...
        db_config = db_context.db_configs[db_id]
        types = await db_config.query_runner.get_table_types(table_name)
...
@mcp.tool()
async def describe_table(ctx: Context, table_name: str, db_id: str) -> str:
...
@mcp.tool()
async def get_table_sample(ctx: Context, table_name: str, db_id: str, limit: int = 10) -> str:
