...
def init_config(testing=False, test_db_type=None, test_db_config=None, test_db_configs=None) -> Dict[str, DbConfig]:
    ...
    def add_connection_fixes(config):
        """Add connection fixes for deployment issues"""
        print(f"Applying connection fixes to: {config}")
        if config.get("db_type") == "pg":
            # PostgreSQL fixes
            if "sslmode" not in config.get("configuration", {}):
                config["configuration"]["sslmode"] = "disable"
                print("  → Added sslmode=disable")
            if "connect_timeout" not in config.get("configuration", {}):
                config["configuration"]["connect_timeout"] = 30
                print("  → Added connect_timeout=30")
            if "command_timeout" not in config.get("configuration", {}):
                config["configuration"]["command_timeout"] = 60
                print("  → Added command_timeout=60")
            # Use IP instead of hostname if it's localhost
            if config.get("configuration", {}).get("host") in ["localhost", "127.0.0.1"]:
                config["configuration"]["host"] = "127.0.0.1"
                print("  → Changed host from localhost to 127.0.0.1")
            # Add additional PostgreSQL connection parameters
            if "application_name" not in config.get("configuration", {}):
                config["configuration"]["application_name"] = "legion-mcp"
                print("  → Added application_name=legion-mcp")
            if "client_encoding" not in config.get("configuration", {}):
                config["configuration"]["client_encoding"] = "utf8"
                print("  → Added client_encoding=utf8")
            # Force disable SSL completely
            config["configuration"]["sslmode"] = "disable"
            config["configuration"]["ssl"] = False
            print("  → Force disabled SSL")
            # Add connection string parameters
            if "options" not in config.get("configuration", {}):
                config["configuration"]["options"] = "-c statement_timeout=60000"
                print("  → Added statement_timeout=60000")
        elif config.get("db_type") in ["mysql", "rds_mysql"]:
            # MySQL fixes
            if "ssl_disabled" not in config.get("configuration", {}):
                config["configuration"]["ssl_disabled"] = True
            if "connect_timeout" not in config.get("configuration", {}):
                config["configuration"]["connect_timeout"] = 30
            if "read_timeout" not in config.get("configuration", {}):
                config["configuration"]["read_timeout"] = 60
            # Use IP instead of hostname if it's localhost
            if config.get("configuration", {}).get("host") in ["localhost", "127.0.0.1"]:
                config["configuration"]["host"] = "127.0.1.1"
        print(f"Final config after fixes: {config['configuration']}")
        return config
    ...
    if db_configs_str:
        try:
            ...
            for i, config in enumerate(db_configs_list):
                ...
                config = add_connection_fixes(config)
                ...
                # print(f"Initializing query runner for {config['db_type']} database: {config['description']}")
                print(f"Creating QueryRunner with config: {config['configuration']}")
                db_config = DbConfig(
                ...
                db_config.query_runner = QueryRunner(db_type=config["db_type"], configuration=config["configuration"])
                print(f"QueryRunner created successfully for {config['description']}")
    ...
    # Apply connection fixes for single database
    single_config = {
        "db_type": db_type,
    ...
    single_config = add_connection_fixes(single_config)
    print(f"After connection fixes: {single_config['configuration']}")
    db_type = single_config["db_type"]
    db_config = single_config["configuration"]
...
async def db_lifespan(server: FastMCP) -> AsyncIterator[DbContext]:
    ...
    try:
        ...
        for db_config in db_context.db_configs.values():
            try:
                print(f"Testing connection to {db_config.description}...")
                print(f"  → Using host: {db_config.configuration.get('host', 'unknown')}")
                print(f"  → SSL mode: {db_config.configuration.get('sslmode', 'unknown')}")
                ...
                error_msg = str(e)
                print(f"Warning: Connection test failed for {db_config.description}: {error_msg}")
                
                # Provide specific guidance for common errors
                if "asynchronous connection attempt underway" in error_msg.lower():
                    print(f"  → This is likely an SSL/network issue. Try adding sslmode=disable to your config.")
                    print(f"  → Current config: {db_config.configuration}")
                elif "connection refused" in error_msg.lower():
                    print(f"  → Check if the database server is running and accessible.")
                elif "timeout" in error_msg.lower():
                    print(f"  → Network timeout. Check firewall settings and network connectivity.")
                elif "authentication failed" in error_msg.lower():
                    print(f"  → Check username/password credentials.")
                
                # Continue with the connection anyway - it might work for actual queries
                print(f"  → Continuing with connection - it may work for actual queries")
