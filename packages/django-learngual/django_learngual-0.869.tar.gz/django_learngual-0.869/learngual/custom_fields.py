...
class TransformDataMixin:
    def transform_data(self, value: str) -> str:
        """
...
class PydanticModelField(TransformDataMixin, models.JSONField):
    retry_count = 2
    """
    ...
    def __to_python(
        ...
        if isinstance(value, str):
            try:
                value = json.loads(self.transform_data(value))
                if isinstance(value, str):
                    value = json.loads(self.transform_data(value))
            ...
            try:
                if isinstance(self.pydantic_model, (list, tuple)):
                    ...
                    try:
                        ...
                        # User-friendly error message for API response
                        raise custom_exceptions.ServiceUnavailable(
                            "Invalid data: the provided data does not match the expected format. "
                            "Please check the data structure and required fields."
                        ) from e
                else:
                    # Detailed internal logging for debugging
                    logger.exception(
    ...
    def get_prep_value(
        ...
        if isinstance(self.pydantic_model, (list, tuple)):
            ...
            if not _type:
                # Detailed internal logging for debugging
                logger.exception(
                    f"Missing type attribute for polymorphic field '{getattr(self, 'name', 'unknown')}': "
...
class PydanticModelSerializerField(TransformDataMixin, serializers.JSONField):
    """
    ...
    def to_internal_value(self, data: dict):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Invalid data: the provided data does not match the required format. "
                "Please check the required fields and data types."
            )
    ...
    def to_representation(self, value) -> str | dict | list | None:
        ...
        try:
            ...
            if isinstance(data, str):
                data = json.loads(self.transform_data(data))
