...
* hostname: The hostname of the machine the oops was created on. (default)
...
* reporter: Describes the program creating the report. For instance you might
  ...
  Amazon EC2 availability zone might be given the same reporter.
  Differentiated backend services for the same front end site would usually
  get different reporters as well. (e.g. auth, cache, render, ...)
* topic: The subject or context for the report. With a command line tool you
  might put the subcommand here, with a web site you might put the template
  (as opposed to the url). This is used as a weak correlation hint: reports
  from the same topic are more likely to have the same cause than reports from
  different topics.
* timeline: A sequence of (start, stop, category, detail) tuples describing
  the events leading up to the OOPS. One way to populate this is the
  oops-timeline package. Consumers should not assume the length of the tuple
  to be fixed - additional fields may be added in future to the right hand
  side (e.g. backtraces).
"""
...
from oops.publishers import convert_result_to_list, publish_to_many


class Config:
    """The configuration for the OOPS system.

    :ivar on_create: A list of callables to call when making a new report.
        Each will be called in series with the new report and a creation
        context dict. The return value of the callbacks is ignored.
    ...
    def publish(self, report):
        ...
        If any publisher raises an exception, that will propagate to the
        caller.
