...
def _simple_copy(key):
    ...
    copy_key.__doc__ = (
        f"Copy the {key} field from context to report, if present."
    )
    return copy_key
...
def safe_unicode(obj):
    ...
    try:
        ...
        value = f"<unprintable {(str(type(obj).__name__))} object>"
...
def attach_exc_info(report, context):
    ...
    * tb_text
    ...
    if isinstance(info[2], str):
        ...
        tb_text = "".join(map(safe_unicode, traceback.format_tb(info[2])))
