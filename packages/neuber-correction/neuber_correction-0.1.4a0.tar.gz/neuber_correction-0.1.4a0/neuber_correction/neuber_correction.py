...
class NeuberSolverSettings:
    ...
    def __init__(
        ...
        self.memoization_precision = memoization_precision
        self.hash = hash(
            (self.tolerance, self.max_iterations, self.memoization_precision)
        )
...
class NeuberCorrection:
    ...
    def __new__(
        ...
        instance_hash = (material.hash, settings.hash)
    ...
    def __init__(
        ...
        if material.sigma_u <= 0:
            raise ValueError("sigma_u (tensile strength) must be positive")
        if material.elastic_mod <= 0:
            raise ValueError("elastic_mod (Young's modulus) must be positive")
        if material.eps_u <= 0:
            raise ValueError("facture_strain (strain at UTS) must be positive")
        if settings.tolerance <= 0:
            raise ValueError("tolerance must be positive")
        if settings.max_iterations <= 0:
            raise ValueError("max_iterations must be positive")

        # Validate that tensile strength is greater than yield strength
        if material.sigma_u <= material.yield_strength:
            raise ValueError(
    ...
    def _calculate_neuber_correction(self, stress: float) -> float:
        ...
        n = (math.log(plastic_ultimate) - math.log(0.002)) / math.log(
            self.material.sigma_u / self.material.yield_strength
        )

        stress_corr = stress
        stress_corr_min_diff = stress_corr
        ...
        for _ in range(self.settings.max_iterations):
            ...
            plastic_strain = (
                ...
                * (stress_corr / self.material.yield_strength) ** n
                if stress_corr > self.material.yield_strength
                else 0.0
            )
            total_strain = elastic_strain + plastic_strain

            # Calculate Neuber strain (with safeguard)
            if stress_corr > 0:
                neuber_strain = (stress**2) / (self.material.elastic_mod * stress_corr)
            else:
                # This should not happen due to the safeguard above, but just in case
                neuber_strain = 0.0
            ...
            # Calculate d_neuber with safeguard
            if stress_corr > 0:
        ...
        # Always store result in memoization table, whether converged or not
        if final_difference < self.settings.tolerance:
            # Converged successfully
            self._insert_sorted(stress, stress_corr)
            return stress_corr
        else:
            # Failed to converge, return best solution found
            print(f"Neuber correction failed for stress {stress}, returning minimum difference: {min_difference}")
            self._insert_sorted(stress, stress_corr_min_diff)
            return stress_corr_min_diff
    ...
    def plot_neuber_diagram(
        ...
        plot_pretty_name = "" if not plot_pretty_name else f": {plot_pretty_name}"
        ...
        # Create stress range for plotting (avoid zero for log scale)
        ...
        ax.plot(
            ...
            "mo",
            markersize=10,
            label=f"Corrected Point (σ={corrected_stress:.0f} MPa)",
        )
