...
class TestNeuberCorrection:
    ...
    def test_neuber_correction_elastic_range(self):
        ...
        # Correction should be minimal for elastic stresses
        assert (
            abs(corrected_stress - stress_elastic) < 1.0
        )  # Less than 1 MPa difference
    ...
    def test_multiple_stress_values_comprehensive(self):
        ...
        for original, corrected in zip(stress_values, corrected_values):
            ...
            # Elastic range (below yield): minimal correction
            if original <= 315:
                assert abs(corrected - original) < 2.0
