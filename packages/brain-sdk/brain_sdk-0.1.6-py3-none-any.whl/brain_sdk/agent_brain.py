...
class AgentBrain:
    ...
    async def register_with_brain_server(self, port: int):
        ...
        log_info(f"Resolved callback URL: {self.agent.base_url}")
        
        if self.agent.dev_mode:
            log_debug(f"Resolved callback URL: {self.agent.base_url}")
    ...
    async def register_with_fast_lifecycle(self, port: int) -> bool:
        ...
        # Always log the resolved callback URL for debugging
        log_debug(f"Fast lifecycle - Resolved callback URL: {self.agent.base_url}")
