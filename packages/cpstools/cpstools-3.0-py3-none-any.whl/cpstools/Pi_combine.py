...
def IGS_extract(input_file, fasta_dir, info_dir):
    for rec in SeqIO.parse(input_file, format='genbank'):
        ...
        # MODIFIED: Correctly handle filenames with multiple dots.
        base_name = os.path.splitext(os.path.basename(input_file))[0]
        save_file = os.path.join(info_dir, base_name + '_intergenic_location.txt')
        ...
        if int(end_gene_info[-2]) < int(start_gene_info[1]):
            # NOTE: There is a small syntax error in your original code here: `start_gene_info}[0]`
            # I am correcting it to `start_gene_info[0]` to prevent a crash.
        ...
        # MODIFIED: Correctly handle filenames with multiple dots.
        base_name = os.path.splitext(os.path.basename(input_file))[0]
        all_fasta_file = os.path.join(fasta_dir, base_name + '_IGS.fasta')
        ...
        with open(file_path, 'w') as fasta_file:
            for gb_file in os.listdir(work_dir):
                if gb_file.endswith('gb') or gb_file.endswith('gbk'):
                    ...
                    # MODIFIED: Correctly handle filenames with multiple dots.
                    base_name = os.path.splitext(gb_file)[0]
...
def calculate_Pi_values(work_dir):
    ...
    for align_fasta in os.listdir(work_dir):
        if align_fasta.endswith('.fasta'):
            ...
            # MODIFIED: Correctly get the base name for the results file.
            base_name = os.path.splitext(align_fasta)[0]
            all_pi_results.append(f"{base_name}\t{final_pi}")
    
    # NOTE: The original code saved Pi results inside the alignment directory.
    # I am keeping this logic. If you want it outside, change the first argument of os.path.join.
    ...
    if not args.work_dir:
        ...
        for i in os.listdir(args.work_dir):
            if i.endswith('gb') or i.endswith('gbk'):
                ...
                file_name = os.path.join(fasta_dir, base_name + '_IGS.fasta')
                # print(file_path)
...
if __name__ == "__main__":
    ...
    if not args.work_dir:
        ...
        # NOTE: Your main workflow is duplicated. I am applying the fix to the second, active block.
        save_results_dir = common_gene_extract(os.path.abspath(args.work_dir))
        # print(save_results_dir)
        # extract fasta
        fasta_dir = os.path.abspath(os.path.join(os.path.dirname(args.work_dir), 'IGS'))
