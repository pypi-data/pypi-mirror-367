...
def compare_genbank_files(ref_file, test_file):
    ref_dict, test_dict = defaultdict(lambda: {"count": 0, "length": []}), defaultdict(lambda: {"count": 0, "length": []})
        ...
        for feature in rec.features:
            if feature.type in ['CDS', 'rRNA', 'tRNA']:
                if feature.qualifiers.get('gene'):
                    ...
                    ref_dict[gene_name]["count"] += 1
                    ref_dict[gene_name]["length"].append(len(feature.extract(rec.seq)))
                else:
    ...
    for rec in test_records:
        for feature in rec.features:
            if feature.type in ['CDS', 'rRNA', 'tRNA']:
                if feature.qualifiers.get('gene'):
                    gene_name = feature.qualifiers.get('gene')[0]
                    test_dict[gene_name]["count"] += 1
                    test_dict[gene_name]["length"].append(len(feature.extract(rec.seq)))
                else:
                    gene_loc = feature.location
                    gene_start = int(gene_loc.start) + 1
                    gene_end = int(gene_loc.end)
    ...
    for gene in ref_dict.keys():
        if ref_dict[gene]["count"] != test_dict[gene]["count"]:
            print(f'{gene} in {ref_file} number is {ref_dict[gene]["count"]} \
                but in {test_file} is {test_dict[gene]["count"]}')
        if ref_dict[gene]["length"] != test_dict[gene]["length"]:
            print(f'{gene} in {ref_file} length is {ref_dict[gene]["length"]} \
                but in {test_file} is {test_dict[gene]["length"]}')
