...
from oops_datedir_repo import serializer, serializer_bson
...
class DateDirRepo:
    ...
    * The behaviour of this class is to assign OOPS file names by hashing the
    ...
    def __init__(
        self, error_dir, serializer=None, inherit_id=False, stash_path=False
    ):
    ...
    def republish(self, publisher):
        ...
        for dirname, (y, m, d) in self._datedirs():
            ...
            for candidate in map(partial(os.path.join, dirpath), files):
                ...
                if (report is None and prune) or (
                    report is not None and oopsid
                ):
                    os.unlink(candidate)
        ...
        for dirname, (y, m, d) in self._datedirs():
            ...
            for candidate in map(partial(os.path.join, dirpath), files):
                ...
                with open(candidate, "rb") as report_file:
                    ...
                    if (
                        report_time is None
                        or getattr(report_time, "date", None) is None
                        or report_time.date() < dirdate
                        or report_time.date() > dirdate
                    ):
                    ...
                    if (
                        ...
                        and report_time <= stop_time
                        and report["id"] not in references
                    ):
