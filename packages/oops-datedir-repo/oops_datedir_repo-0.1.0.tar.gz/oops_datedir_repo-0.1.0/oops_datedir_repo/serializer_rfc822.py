...
def _safestr(obj):
    if isinstance(obj, str):
        return obj.replace("\\", "\\\\").encode("ASCII", "backslashreplace")
    ...
    if isinstance(obj, bytes):
        ...
        try:
            ...
            logging.getLogger("oops_datedir_repo.serializer_rfc822").exception(
                "Error while getting a str " "representation of an object"
            )
            value = "<unprintable %s object>" % (str(type(obj).__name__))
    ...
    value = re.sub(
        rb"[\x80-\xff]",
        lambda match: ("\\x%02x" % ord(match.group(0))).encode("UTF-8"),
        value,
    )
    return value
    ...
    if "timeline" in report:
        for row in report["timeline"]:
            ...
            chunks.append(
                ...
                + _safestr(category)
                + b" "
                + _normalise_whitespace(_safestr(statement))
                + b"\n"
            )
        chunks.append(b"\n")
