...
class LottieAnimation(QQuickPaintedItem):
    ...
    def _render_current_frame(self) -> Optional[Any]:
        ...
        if image:
            ...
            if len(self._base_image_cache) > 30:
                keys_to_remove = list(self._base_image_cache.keys())[:-20]
    ...
    def _on_size_changed(self) -> None:
        ...
        if (self._last_item_size.width() > 0 and self._last_item_size.height() > 0):
            width_change = abs(current_size.width() - self._last_item_size.width()) / self._last_item_size.width()
            height_change = abs(current_size.height() - self._last_item_size.height()) / self._last_item_size.height()
