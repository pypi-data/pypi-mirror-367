...
class FieldPlotTab(QWidget):
    ...
    def reload_quiver_data(self) -> None:
        ...
        sampling = self.quiver_settings.sampling_rate
        ...
        coord1: NDArray = getattr(self.top.selected_monitor, self.plot_type[0].lower())
        coord2: NDArray = getattr(self.top.selected_monitor, self.plot_type[1].lower())

        # Create a mesh grid of the given coordinates.
        X, Y = np.meshgrid(coord1, coord2, indexing="ij")
        X_sampled = X[::sampling, ::sampling]
        Y_sampled = Y[::sampling, ::sampling]
        if self.quiver:
            self.quiver.remove()

        self.quiver = self.ax.quiver(X_sampled, Y_sampled, np.zeros_like(X_sampled), np.zeros_like(X_sampled))
    ...
    def reinit_axes(self) -> None:
        ...
        if "Plane" in plot_type:
            ...
            if plot_vectors:
                ...
                X_sampled = X[::sampling, ::sampling]
                Y_sampled = Y[::sampling, ::sampling]
                self.quiver = self.ax.quiver(X_sampled, Y_sampled, U, V)
                self.quiver_settings.reapply_quiver_config()
