...
class Source(SimulationObject, ABC):
    ...
    def polarization(self, angle: float) -> None:
        """
        Sets the polarization of the source relative to it's direction. Angle theta = angle phi = 0 and polarization 0
        yields x-polarized light. Will only work if polarization definition is angle
        """
        definition = self._get("polarization definition", str)
        if definition != "angle":
            raise ValueError(f"Polarization angle for a source can only be set if the 'polarization definition' is "
                             f"set to 'angle'.")
        angle = angle % 360
