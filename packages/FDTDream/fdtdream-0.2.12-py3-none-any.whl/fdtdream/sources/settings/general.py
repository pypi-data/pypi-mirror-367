...
class General(Module):
    ...
    def set_angle_theta(self, theta: float) -> None:
        ...
        if self._get("direction definition", str) == "axis":
            self._set("angle theta", theta)
        else:
            raise ValueError(f"You cannot set the pangle theta when "
    ...
    def set_angle_phi(self, phi: float) -> None:
        ...
        if self._get("direction definition", str) == "axis":
            self._set("angle phi", phi)
        else:
            raise ValueError(f"You cannot set the angle phi when "
                             f"'direction definition' is not 'axis'.")
