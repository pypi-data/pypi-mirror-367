...
class Simulation(SimulationInterface):
    ...
    def _get_simulation_objects_in_scope(self, groupscope: str, autoset_new_unique_names: bool,
        ...
        for i in range(num_objects):
            ...
            used_names = [sim_object["name"] for sim_object in iterated]
    ...
    def _load_objects_from_file(self) -> None:
        ...
        for sim_object in simulation_objects:
            ...
            if sim_object["name"] == "FDTD":
                ...
                setattr(self, sim_object["name"].replace(" ", "_"), instantiated_object)
    ...
    def run(self,
            ...
            simulation_category: str = None,
            parameters: Dict[str, Union[str, float, int, bool]] = None,
            info_text: str = None,
            save_path: str = None) -> SimulationResults:
        """
        Runs the simulation and stores the results either in a database or as a file.

        Parameters:
            simulation_name (str, optional): The name of the simulation. Required if saving to a database.
            database_path (str, optional): Path to the database file. If provided, simulation results are saved here.
                                           If the database file does not exist, it will be created. The directory must exist.
                                           If only a filename is provided, it will be placed in the current working directory.
            simulation_category (str, optional): Category (folder/group) in the database. Required if saving to a database.
            parameters (dict, optional): Dictionary of parameters to associate with the simulation.
            info_text (str, optional): Additional text information to attach to the simulation.
            save_path (str, optional): File path to save the simulation output if not using a database.
                                       If only a filename is provided, it will be placed in the current working directory.

        Raises:
            ValueError: If neither a database nor a save path is provided.
            ValueError: If database path is provided but simulation name or category is missing.
            FileNotFoundError: If the directory for the save path or database path does not exist.

        Returns:
            SimulationResults: The results of the simulation.
        """

        if not any([database_path, save_path]):
            raise ValueError("Neither a database nor a save path is selected. The simulation results will be saved "
                             "nowhere, defeating the purpose of the simulation.")

        if database_path:
            if not all([simulation_name, simulation_category]):
                raise ValueError(
                    "When saving a simulation to a database, both simulation name and category are required.")
            db_dir = os.path.dirname(database_path) or os.getcwd()
            if not os.path.exists(db_dir):
                raise FileNotFoundError(f"The directory '{db_dir}' for the database path does not exist.")

        if save_path:
            save_dir = os.path.dirname(save_path) or os.getcwd()
            if not os.path.exists(save_dir):
                raise FileNotFoundError(f"The directory '{save_dir}' for the save path does not exist.")
        ...
        self.save(temp_fsp_path, print_confirmation=False)  # Save simulation to temp file

        # Run the simulation
        self._lumapi().switchtolayout()
        self._lumapi().run()

        # Save the .fsp file with results to the save path provided, if any
        if save_path:
            self.save(save_path)
        ...
        self._lumapi().switchtolayout()
        self.save(temp_fsp_path, print_confirmation=False)
