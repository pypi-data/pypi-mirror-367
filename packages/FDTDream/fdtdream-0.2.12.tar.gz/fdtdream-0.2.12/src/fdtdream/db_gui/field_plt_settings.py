...
from PyQt6.QtWidgets import QDoubleSpinBox, QWidget, QVBoxLayout, QHBoxLayout, QSpinBox
...
class FieldSettings(QWidget):
    ...
    def _create_combos(self) -> None:
        ...
        # Color Scale font size
        self.cscale_fontsize = QSpinBox()
        self.cscale_fontsize.setRange(1, 100)
        self.cscale_fontsize.setValue(12)
        self.cscale_fontsize.valueChanged.connect(self.on_cscale_fontsize_change)  # type: ignore
        self._layout.addWidget(self.cscale_fontsize)
    ...
    def on_cscale_fontsize_change(self, size: int):
        try:
            self._parent.colorbar.ax.tick_params(labelsize=size)
            for tl in self._parent.colorbar.ax.get_yticklabels():
                tl.set_fontfamily('times new roman')
            self._update_data()
        except:
            pass
