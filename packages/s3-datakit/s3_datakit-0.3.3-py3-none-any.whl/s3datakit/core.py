...
def _read_file_to_df(file_path: str, low_memory: bool = True) -> pd.DataFrame:
...
def download_s3_file(
    ...
    if not replace and path_obj.exists():
        ...
        if to_df:
            return _read_file_to_df(str(path_obj), low_memory=low_memory)
        return str(path_obj)
    ...
    if to_df:
        return _read_file_to_df(str(path_obj), low_memory=low_memory, replace=replace))
    return str(path_obj)