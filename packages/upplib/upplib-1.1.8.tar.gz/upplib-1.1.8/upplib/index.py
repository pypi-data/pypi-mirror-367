...
def to_datetime(
    ...
    default_tz: Union[str, timezone] = 'UTC+8'  # 默认时区（可配置）
) -> Union[datetime, str]:
    """
    将字符串 s 转化为 datetime 对象，支持带时区的 ISO 8601 格式和自定义模式。
    如果没有时区信息，使用 default_tz 作为默认时区。

    Args:
        s: 输入字符串或数字（时间戳）。如果为 None 或空，返回当前时间。
        pattern: 自定义解析格式（如 "%Y-%m-%d %H:%M:%S"）。
        r_str: 是否返回字符串格式。
        tz: 目标时区（如 "UTC+7" 或 datetime.timezone 对象），最终结果会转换到此。
        default_tz: 当输入无时区时使用的默认时区（默认 UTC）。

    Returns:
        datetime 对象或字符串（如果 r_str=True）。

    Examples:
        >>> to_datetime("2025-08-06 16:30:01")  # 无时区输入
        datetime(2025, 8, 6, 16, 30, 1, tzinfo=timezone.utc)  # 默认 UTC

        >>> to_datetime("2025-08-06 16:30:01", default_tz="UTC+8")
        datetime(2025, 8, 6, 16, 30, 1, tzinfo=timezone(timedelta(hours=8)))
    """
    if s is None or s == '':
