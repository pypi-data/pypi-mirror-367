...
from typing_extensions import Any, Generic, Literal, cast, overload
...
from agentlin.route.mcp_proxy_client_session import ClientSession
from agentlin.route.mcp_proxy_transports import (
    ClientTransportT,
    FastMCP1Server,
    FastMCPTransport,
    MCPConfigTransport,
...
class Client(Generic[ClientTransportT]):
    ...
    def __init__(
        self,
        transport: ClientTransportT | FastMCP | AnyUrl | Path | MCPConfig | dict[str, Any] | str,
        # Common args
        ...
        if sampling_handler is not None:
            self._session_kwargs["sampling_callback"] = create_sampling_callback(sampling_handler)

        if elicitation_handler is not None:
            self._session_kwargs["elicitation_callback"] = create_elicitation_callback(elicitation_handler)
    ...
    def set_sampling_callback(self, sampling_callback: SamplingHandler) -> None:
        ...
        self._session_kwargs["sampling_callback"] = create_sampling_callback(sampling_callback)

    def set_elicitation_callback(self, elicitation_callback: ElicitationHandler) -> None:
        """Set the elicitation callback for the client."""
        self._session_kwargs["elicitation_callback"] = create_elicitation_callback(elicitation_callback)
    ...
    async def read_resource(self, uri: AnyUrl | str) -> list[mcp.types.TextResourceContents | mcp.types.BlobResourceContents]:
        ...
        if isinstance(uri, str):
            try:
                ...
                raise ValueError(f"Provided resource URI is invalid: {str(uri)!r}") from e
    ...
    # --- Prompt ---
    async def get_prompt_mcp(self, name: str, arguments: dict[str, Any] | None = None) -> mcp.types.GetPromptResult:
        ...
        result = await self.session.get_prompt(name=name, arguments=serialized_arguments)
        return result

    async def get_prompt(self, name: str, arguments: dict[str, Any] | None = None) -> mcp.types.GetPromptResult:
