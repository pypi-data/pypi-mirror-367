...
class FileFlowHandleV2:
    ...
    def entry_point(
        ...
        return self._file_flow_entry_point_internal(
            records, start_stage, target_stage, tags, priority, source_type,
            urgent=False
        )
...
if __name__ == '__main__':
    file_flow = FileFlowHandleV2()

    # 插入用例
    records = [{"id": "test_id1", "s3_path_info": [{"store_path": "s3://orbit-common-resources/fyx_test/FC060083175_20150115.pdf", "file_name": "test_file_name"}]}]
    start_stage = "convert"
    target_stage = "embedding"
    tags = ["test_data"]
    priority = "1"
    data_source = "test_data"
    status, ids, message = file_flow.entry_point(records, start_stage, target_stage, tags, priority=priority, source_type=data_source)
    print(f"Status: {status}, IDs: {ids}, Message: {message}")

    # 读取用例
    result = file_flow.exit_point(tags=['test_data'], limit_size=1)
    for data in result:
