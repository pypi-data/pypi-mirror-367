...
class CognitoStack(IStack):
    ...
    def _ssm_export(self, user_pool: cognito.UserPool):
        ...
        if self.cognito_config.ssm_export_user_pool_id:
            parameter_name = self._build_parameter_name("user-pool-id")
            ssm.StringParameter(
                scope=self,
                id="UserPoolId",
        ...
        if self.cognito_config.ssm_export_user_pool_name:
            parameter_name = self._build_parameter_name("user-pool-name")
            ssm.StringParameter(
                scope=self,
                id="UserPoolName",
                string_value=self.cognito_config.user_pool_name,
                parameter_name=f"/{parameter_name}",
                description="User Pool Name for Cognito User Pool",
            )

        if self.cognito_config.ssm_export_user_pool_arn:
            parameter_name = self._build_parameter_name("user-pool-arn")
            ssm.StringParameter(
                scope=self,
                id="UserPoolArn",
    ...
    def _build_parameter_name(self, name: str) -> str:
        return f"{self.deployment.environment}/{self.deployment.workload_name}/{name}"
