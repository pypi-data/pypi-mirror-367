...
async def _check_gcs_bucket_accessibility(storage: Storage, bucket_name: str):
    ...
    try:
        ...
        # Just check if we can access the bucket by trying to list blobs
        # We'll break after the first item to avoid listing all blobs in large buckets
        async for _ in bucket.list_blobs():
            break
        return True
    except Exception as e:
