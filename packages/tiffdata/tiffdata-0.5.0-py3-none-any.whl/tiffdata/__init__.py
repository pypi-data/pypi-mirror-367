...
class TIFF(object):
    ...
    def save(
        ...
        logger.debug(
            "%s.save(filepath: %s, overwrite: %s, order: %s, format: %s)",
            self.__class__.__name__,
            filepath,
            overwrite,
            order,
        ...
        with tempfile.NamedTemporaryFile(
            ...
            for index, element in enumerate(self.elements, start=1):
                logger.debug(" > writing element %d/%d", index, count)
                ...
                if isinstance(element, Data) and not (
                    ...
                    datafirst = data = element.encode(order=order, format=format)
                    datalength += len(data)
                    handle.write(data)
