...
def main():
    ...
    def add_booking(purpose: str,room_name: str, reserved_by: str, start_time: str, end_time: str) -> str:
        ...
        try:
            ...
            session.commit()
            return f"預約成功!!，ID：{new_booking.id}，傳入的資料：{purpose}，{room_name}，{reserved_by}，{start_time}，{end_time}"
        except Exception as e:
            return f"新增失敗：{str(e)}"
    ...
    def get_upcoming_bookings() -> list:
        ...
        bookings = (
            ...
            #.filter(MeetingBooking.start_time >= now)
            .order_by(MeetingBooking.start_time.asc())
            .all()
        )

        if not bookings:
            return [{"message": "目前沒有任何即將到來的會議預約!now:"+str(now)}]
