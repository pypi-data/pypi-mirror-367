...
def recover_mnemonic(shares: List[Share]) -> Mnemonic:
    ...
    mnemonic = Mnemonic()
    mnemonic.seed = original_key.to_bytes(32, "big")
...
def recover_from_phrase(phrase: List[str], language: Language) -> Mnemonic:
    Mnemonic.validate_phrase(phrase, language)

    mnemonic = Mnemonic()
    for i, word in enumerate(phrase):
        mnemonic.set_word(i, word, language)
