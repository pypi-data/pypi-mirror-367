...
class GetSolutionUsingGit:
    ...
    @classmethod
    async def download(cls, selected):
        env = os.environ.copy()
        env["GIT_TERMINAL_PROMPT"] = (
            "0"  # prevents hanging on username input if url invalid
        )

        clone_dir = Path.cwd() / Path(selected["name"].replace(" ", ""))
        ...
        Audit.submit("solution_download", Audit.Type.Log, command=command)

        process = subprocess.Popen(
            command, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, env=env
        )  # git outputs updates over stderr

        await git_clone_progress(process, selected["name"])
