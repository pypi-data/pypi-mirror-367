...
from shoestring_assembler.display import Display, OptionPrompt, TailoredConfirm
...
class PlainCLI:
    ...
    def create_progress_section(self,msg:SectionEvent):
        if msg.entered:
            self.progress_manager = RichProgress()  #TODO presentation - can use fields to include a label rather than a 0/3
    ...
    def diplay_progress_update(self, progress_event:ProgressEvent):
        if self.progress_manager is not None:
            if self.progress_task_map.get(progress_event.key) not in self.progress_manager.task_ids:
                task_id = self.progress_manager.add_task(progress_event.label,total=progress_event.total,completed=progress_event.value)
    ...
    def handle(self, step):
        match (step):
            ...
            case steps.PromptToAssemble():
                if self.do_prompt_for_next_phase:
                    answer = TailoredConfirm.ask(
                        "? Do you want to assemble the solution now?",
                        default=True
                    )
                else:
                    answer = True
            ...
            case steps.PromptToBuild():
                if self.do_prompt_for_next_phase:
                    answer = TailoredConfirm.ask(
                        "? Do you want to build the solution now?",
                        default=True
                    )
                else:
                    answer = True
            ...
            case steps.PromptToStart():
                if self.do_prompt_for_next_phase:
                    answer = TailoredConfirm.ask(
                        "? Do you want to start the solution now?",
                        default=True
                    )
                else:
                    answer = True
