from shoestring_assembler.model.recipe import Recipe, SolutionSpec
...
class SolutionModel:
    ...
    async def saturate(self):
        if not self.saturated:
            ...
            sources_definition = {
                name:{"spec":spec,"fs":self.fs.get_source_fs(name),"root":self.fs.root} for name,spec in recipe.sources.items()
            }
            self.__sources = ModelMap.generate(SourceModel, sources_definition)
            sm_definitions = {
                name:{"spec":spec,"fs":self.fs.get_module_fs(name),"source":self.__sources[spec["source"]]} for name,spec in recipe.service_modules.items()
            }
            self.__service_modules = ModelMap.generate(
                ServiceModuleModel, sm_definitions
            )
            inf_definitions = {
                name:{"spec":spec,"fs":self.fs.get_module_fs(name),"source":self.__sources[spec["source"]]} for name,spec in recipe.infrastructure.items()
            }
...
class VersionControl:
    ...
    def available_updates(self):
        if self.__available_updates is None:
