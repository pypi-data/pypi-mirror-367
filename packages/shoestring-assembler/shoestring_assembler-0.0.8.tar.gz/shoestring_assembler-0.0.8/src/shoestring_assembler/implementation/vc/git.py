...
class SolutionGitVC:
    ...
    async def _fetch_updates(cls):
        ...
        await Audit.submit("solution::get_versions_git",Audit.Type.from_boolean(result.returncode == 0),return_code = result.returncode, stdout = result.stdout.decode(), stderr = result.stderr.decode())
    ...
    @classmethod
    async def fetch_version_details(cls, strict_tags_only=True, full_list=True):
        await cls._fetch_updates()
    ...
    @classmethod
    async def do_update(cls, tag_or_branch):
