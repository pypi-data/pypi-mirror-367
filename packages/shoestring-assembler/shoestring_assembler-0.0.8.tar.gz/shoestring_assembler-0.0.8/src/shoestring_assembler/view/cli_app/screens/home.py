...
class Home(Screen):
    ...
    def compose(self):
        ...
        with HorizontalGroup(id="download_bar"):
            ...
            yield Button("Find", id="filesystem")
        with VerticalScroll(can_focus=False):
            for solution in self.solution_list:
                with Collapsible(title=solution.solution_details.name):
                    yield SolutionEntry(solution, classes = "solution_action_dropdown")
        yield Footer()
        # Button("Download Solution", id="download"),
        #
        # Button("Assemble Solution"),
        # Button("Reconfigure Solution"),
        # Button("Build Solution"),
        # Button("Setup Solution"),
        # Button("Start Solution"),
        # Button("Stop Solution"),
...
class SolutionEntry(Container):
    ...
    def compose(self):
        yield Label(f"version {self.solution.solution_details.version}")
