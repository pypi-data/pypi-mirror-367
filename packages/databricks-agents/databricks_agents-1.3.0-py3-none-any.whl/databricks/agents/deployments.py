...
def deploy(
    ...
    **kwargs,
    ...
    if serving_endpoint_opt is None:
        ...
        # If the user changed tags, description, or budget policy id, update them
        if tags is not None:
            curr_tags = set((t.key, t.value) for t in serving_endpoint_opt.tags)
            new_tags = set((key, value) for key, value in tags.items())
            if curr_tags != new_tags:
                w.serving_endpoints.patch(
                    name=endpoint_name,
                    add_tags=list(
                        EndpointTag(key, value) for key, value in new_tags - curr_tags
                    )
                    or None,
                    delete_tags=list(key for key, _ in curr_tags - new_tags) or None,
                )
