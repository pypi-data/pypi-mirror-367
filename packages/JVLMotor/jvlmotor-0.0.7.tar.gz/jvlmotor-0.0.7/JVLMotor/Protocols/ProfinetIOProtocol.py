...
class ProfinetIOProtocol(TemplateProtocol):
    def __init__(self,motor,port="XXX",product="",update=False):
        ...
        self.logger = logging.getLogger("ProfinetIOProtocol")
        # Ensure log directory exists and log file is unique
        log_file = os.path.join("protocol_logs", "ProfinetIOProtocol.log")
        ...
        if (err != CIFX_NO_ERROR):
            self.logger.error("Open channel error: %s", self.driver.getErrorDescriptionDriver(err))
        ...
        if (err != CIFX_NO_ERROR):
            self.logger.error("Error I/O header flags: %s", self.driver.getErrorDescriptionDriver(err))

        footer_flags = 0x8080
        footer_flags = footer_flags.to_bytes(2,byteorder='big')
        self.driver.writeIOChannel(self.channel,0,5+32,footer_flags,2)
        if (err != CIFX_NO_ERROR):
            self.logger.error("Error I/O footer flags: %s", self.driver.getErrorDescriptionDriver(err))

        #Iniatilize I/O read
        err = self.read(self.motor.io_read_words[0])
        if (err != CIFX_NO_ERROR):
            self.logger.error("I/O read no set yet %s", self.driver.getErrorDescriptionDriver(err))

        self.logger.info("ProfinetIO initialized")
    ...
    def readIO(self,reg_num,length=4):
        ...
        for i in range(READ_IO_NUM):
            ...
            if (err != CIFX_NO_ERROR):
                self.logger.error("Error I/O reading: %s", self.driver.getErrorDescriptionDriver(err))
                return
    ...
    def writeIO(self,reg_num,data,length=4):
        ...
        if (err != CIFX_NO_ERROR):
            self.logger.error("Error I/O writing: %s", self.driver.getErrorDescriptionDriver(err))
            return
        time.sleep(4*STATION_TIMING)
    ...
    def readAcyclic(self,reg_num,length=4):
        ...
        for i in range(READ_ACYCLIC_NUM):
            ...
            if err != CIFX_NO_ERROR:
                self.logger.error("Read error: %s", self.driver.getErrorDescriptionDriver(err))
                return
            ...
            if (status != CIFX_NO_ERROR):
                self.logger.warning("Read Status: %s", hex(status))
                self.logger.warning("Profinet error code: %s", int.from_bytes(rcv_packet.abData[8:12],byteorder='big'))
                self.logger.warning("Profinet add value 1: %s", int.from_bytes(rcv_packet.abData[12:14],byteorder='big'))
                self.logger.warning("Profinet add value 2: %s", int.from_bytes(rcv_packet.abData[14:16],byteorder='big'))
                return
    ...
    def writeAcyclic(self,reg_num,data,length=4,no_response=False):
        ...
        rcv_packet, err = self.driver.getPacketChannel(self.channel, PACKET_WAIT_TIMEOUT)
        if err != CIFX_NO_ERROR:
            self.logger.warning("Read error: %s", self.driver.getErrorDescriptionDriver(err))
            return

        status = rcv_packet.tHeader.ulState
        if (status != CIFX_NO_ERROR):
            self.logger.warning("Write Status: %s", hex(status))
            self.logger.warning("Profinet error code: %s", int.from_bytes(rcv_packet.abData[4:8],byteorder='big'))
            self.logger.warning("Profinet add value 1: %s", int.from_bytes(rcv_packet.abData[8:10],byteorder='big'))
            self.logger.warning("Profinet add value 2: %s", int.from_bytes(rcv_packet.abData[10:12],byteorder='big'))
            return
        time.sleep(4*STATION_TIMING)
        return CIFX_NO_ERROR
