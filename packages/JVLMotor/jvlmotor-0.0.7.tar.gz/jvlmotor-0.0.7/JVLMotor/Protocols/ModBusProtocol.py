...
class ModBusProtocol(TemplateProtocol):
    def __init__(self,com_type="Serial",port=None, ip = None,
        ...
        self.netX = netX
        self.logger = logging.getLogger("ModBusProtocol")

        # Ensure log directory exists and log file is unique
        log_file = os.path.join("protocol_logs", "ModBusProtocol.log")
        ...
        if com_type == "Serial":
            ...
            self.logger.info("Serial ModBusProtocol initialized")
        ...
        if com_type == "Ethernet":
            ...
            self.logger.info("TCP ModBusProtocol initialized")
    ...
    def read(self,reg_num,length=4):
        try:
            ...
            if result.isError():
                self.logger.error("Read Error: %s", result)
                return 
    ...
    def comm(self,data):
        ...
        if self.com_type == "Serial":
            ...
            if self.communication.socket:  
                ...
                self.logger.error("Socket is not open.")
                return None
