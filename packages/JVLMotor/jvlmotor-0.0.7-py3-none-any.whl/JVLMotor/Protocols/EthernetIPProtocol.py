...
class EthernetIPProtocol(TemplateProtocol):
    def __init__(self,motor,port="XXX",product="", update=False):
        ...
        self.logger = logging.getLogger("EthernetIPProtocol")
        # Ensure log directory exists and log file is unique
        log_file = os.path.join("protocol_logs", "EthernetIPProtocol.log")
        ...
        self.logger.info("EthernetIPProtocol Initialized")
    ...
    def openClass3Connection(self,max_retries=5):
        for attempt in range(1, max_retries+1):
            ...
            if err != CIFX_NO_ERROR:
                self.logger.error("Open class 3 connection error: %s - %s", err, self.driver.getErrorDescriptionDriver(err))
                continue

            status = rcv_packet.tHeader.ulState
            if (status != CIFX_NO_ERROR):
                self.logger.error(f"Failed Attempt {attempt}: Open Status - {hex(status)}")
                time.sleep(1)
    ...
    def readModule(self,reg_num,length=4):
        ...
        for i in range(1):
            ...
            if err != CIFX_NO_ERROR:
                self.logger.error("Read module error: %s", self.driver.getErrorDescriptionDriver(err))
                return
    ...
    def writeModule(self,reg_num,data,length=4,no_response=True):
        ...
        err = self.driver.putPacketChannel(self.channel,packet,PACKET_WAIT_TIMEOUT)
        if err != CIFX_NO_ERROR:
            self.logger.error("Write module error: %s", self.driver.getErrorDescriptionDriver(err))
            return
        ...
        status = rcv_packet.tHeader.ulState
        if (status != CIFX_NO_ERROR):
            self.logger.warning(f"Write Module Status {reg_num} for {data}: {hex(status)}")
            return
        ...
        for i in range(READ_ACYCLIC_NUM):
            ...
            if (status != CIFX_NO_ERROR):
                ...
                if (status == 0xc01e001e):
                    self.logger.warning("Communication issue probably due to timeout - Reopening Class3")
                    self.openClass3Connection()
                    time.sleep(0.1)
                return
