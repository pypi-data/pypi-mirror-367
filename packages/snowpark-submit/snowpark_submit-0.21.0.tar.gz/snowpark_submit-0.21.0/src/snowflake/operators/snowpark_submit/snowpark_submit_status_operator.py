...
class SnowparkSubmitStatusOperator(BaseOperator):
    ...
    :param log_fetch_delay: Initial delay before fetching logs (default: 20). Only applies when display_logs=True. It may take time for the logs to populate.
    ...
    template_fields = ["snowflake_workload_name", "connections_config"]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log_fetch_delay = log_fetch_delay
    ...
    def execute(self, context: Context) -> StatusInfo:
        ...
        if self.display_logs and self.log_fetch_delay > 0:
            self.log.info(
                f"Waiting {self.log_fetch_delay} seconds for logs to be fully available..."
            )
            time.sleep(self.log_fetch_delay)
