...
class TestSnowparkSubmit:
    ...
    def test_snowpark_submit_cluster_mode_python(
        ...
        with sas_version_set_via_env_vars(sas_version):
            ...
            integration_name, network_rule_name = self._setup_pypi_external_access(
                spcs_snowpark_session
            )
            if not integration_name:
                pytest.fail("Could not set up external access integration for PyPI")
            ...
            self._cleanup_pypi_external_access(
                spcs_snowpark_session, integration_name, network_rule_name
            )
...
EXECUTE JOB SERVICE
    ...
    def _setup_pypi_external_access(self, session):
        uid = generate_random_name(prefix="test_pypi_")
        network_rule_name = f"pypi_network_rule_{uid}".upper()
        integration_name = f"pypi_access_integration_{uid}".upper()
        ...
        try:
            ...
            logger.debug(
                f"Created network rule '{network_rule_name}' with role {current_role}"
            )
            ...
            session.sql(
                ...
                CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION {integration_name}
                  ...
                  ENABLED = true
                  COMMENT = 'External access integration for PyPI package installation (test)'
                """
            ).collect()
            logger.debug(
                f"Created external access integration '{integration_name}' with SYSADMIN role"
            )

            session.sql(
                f"GRANT USAGE ON INTEGRATION {integration_name} TO ROLE {current_role}"
            ).collect()
            logger.debug(f"Granted usage on integration to role {current_role}")

            session.sql(f"USE ROLE {current_role}").collect()

            return integration_name, network_rule_name
        except Exception as e:
            logger.error(f"Failed to set up external access integration: {e}")
            raise Exception(f"Failed to set up external access integration: {e}") from e

    def _cleanup_pypi_external_access(
        self, session, integration_name, network_rule_name
    ):
        try:
            if integration_name:
                session.sql(f"DROP INTEGRATION IF EXISTS {integration_name}").collect()
                logger.debug(f"Dropped integration '{integration_name}'")
            if network_rule_name:
                session.sql(
                    f"DROP NETWORK RULE IF EXISTS {network_rule_name}"
                ).collect()
                logger.debug(f"Dropped network rule '{network_rule_name}'")
        except Exception as e:
            logger.warning(f"Failed to clean up external access resources: {e}")
