...
def detect_space_0_and_space_1(hex_string):
    ...
    # On cherche un motif de type 0^n 1^m répété plusieurs fois
    for size in range(4, len(binary_string) // 2):
        unit = binary_string[:size]
        if re.fullmatch(r'0+1+', unit):
            # Vérifier si ce motif est répété
            repeat_count = len(binary_string) // len(unit)
            repeated = (unit * repeat_count)[:len(binary_string)]
            if repeated == binary_string:
                space_0 = unit.find("1")
                space_1 = len(unit) - space_0
                return space_0, space_1

    raise ValueError("Motif de type 0^n 1^m non trouvé dans l'hexadécimal.")
