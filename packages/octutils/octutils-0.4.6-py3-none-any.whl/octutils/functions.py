...
def resolve_shared_expression(name, shared_expressions):
    for expr in shared_expressions:
        if expr.get("name") == name:
            return expr.get("expression", {}).get("expression", "")
    return ""
...
def get_tables(tmsl, WorkspaceID, DatasetID, Alias, ModifiedTime):
    try:
        ...
        shared_expressions = model.get("expressions", [])
        tables_list = []

        for tableproperty in tables:
            tableName = tableproperty.get('name', 'NA')
            partitions = tableproperty.get('partitions', ['NA'])[0]
            ...
            # resolve expressions like: Source = DatabaseQuery
            if re.search(r'Source\s*=\s*(\w+)', expression):
                match = re.search(r'Source\s*=\s*(\w+)', expression)
                ref_name = match.group(1)
                if ref_name not in ["Sql", "Table", "Source"]:
                    resolved_expr = resolve_shared_expression(ref_name, shared_expressions)
                    if resolved_expr:
            ...
            if expression_type == "calculated":
                ...
                match = re.search(r'Schema\s*=\s*"(?P<schema>[^"]+)"\s*,\s*Item\s*=\s*"(?P<item>[^"]+)"', expression)
                if match:
                    schema = match.group('schema')
                    item = match.group('item')
                    source_table_name = f"{schema}.{item}"
                    ...
                    try:
                        ...
                        source_table_name = "Query Parse Failed"
                    ...
                    if 'Item = "' in expression:
                        item_name = expression.split('Item = "')[1].split('"')[0]
                        source_table_name = schema_name + "." + item_name
                    else:
            ...
            elif 'Navigation = Source{[Name = "' in expression or 'Navigation = Source{[Name="' in expression:
                try:
                    source_table_name = re.search(r'Navigation = Source{\[Name ?= ?\\"([^\"]+)', expression).group(1)
                except:
                    source_table_name = "Navigation Name not found"
                source_type = "Azure" if 'AzureStorage.DataLake' in expression else "Navigation type Not Defined"

            elif 'Json.Document(Binary.Decompress(Binary.FromText("' in expression:
                source_table_name = "Compressed Json Table"
                source_type = "Binary Json Document"

            elif 'Excel.Workbook' in expression:
                match = re.search(r'Excel\\.Workbook\\((?:File|Web)?\\.Contents\\(\"(.*?)\"', expression)
                if match:
            ...
            elif 'AzureStorage.DataLake' in expression:
                source_table_name = expression.split('AzureStorage.DataLake("')[1].split('"')[0]
                source_type = "AzureStorage DataLake"

            elif 'SharePoint.Tables("' in expression or 'SharePoint.Files("' in expression:
                try:
                    source_table_name = re.search(r'SharePoint\\.\w+\\("([^\"]+)', expression).group(1)
                except:
                    source_table_name = "SharePoint Path Not Found"
                source_type = "SharePoint"

            elif 'Databricks.Catalogs(' in expression:
                try:
                    match = re.findall(r'Name="(.*?)",Kind="Table"', expression)
                    source_table_name = match[0] if match else "Databricks Table Not Found"
            ...
            if isinstance(source_table_name, list):
                for stname in source_table_name:
                    tables_list.append((WorkspaceID, DatasetID, mode, source_type, expression, tableName, stname, Alias, ModifiedTime))
            else:
                tables_list.append((WorkspaceID, DatasetID, mode, source_type, expression, tableName, source_table_name, Alias, ModifiedTime))
...
WITH actual_target_check AS (
    SELECT 
        ...
        MAX(CASE 
            WHEN (LOWER(Column_Name) LIKE '%revenue%' OR LOWER(Column_Name) LIKE '%actual%' OR LOWER(Column_Name) LIKE '%acr%')
              AND LOWER(Column_Name) NOT LIKE '%revenuetarget%' 
              AND LOWER(Column_Name) NOT LIKE '%budget%' 
              AND LOWER(Column_Name) NOT LIKE '# The command is not a standard IPython magic command. It is designed for use within Fabric notebooks only.
# %pipeline%' 
              AND LOWER(Column_Name) NOT LIKE '%forecast%' 
              AND LOWER(Column_Name) NOT LIKE '%key' 
            THEN 1 ELSE 0 END) AS has_actuals,
        MAX(CASE 
            WHEN LOWER(Column_Name) LIKE '%quota%' OR LOWER(Column_Name) LIKE '%target%' OR LOWER(Column_Name) LIKE '%revenuetarget%' 
            THEN 1 ELSE 0 END) AS has_targets,
        MAX(CASE 
            WHEN LOWER(Column_Name) LIKE '%tpid%' OR LOWER(Column_Name) LIKE '%accountid%' OR LOWER(Column_Name) LIKE '%tpaccountid%' 
            THEN 1 ELSE 0 END) AS has_accounts
    FROM oct_column
    WHERE LOWER(Data_Type) IN ('int64', 'double', 'decimal')
      AND NOT (LOWER(Column_Name) LIKE 'is%' OR LOWER(Column_Name) LIKE 'has%' OR LOWER(Column_Name) LIKE '%order'
               OR LOWER(Column_Name) LIKE '%type')
...
AppliedRLS AS (
    SELECT DISTINCT a.DatasetID 
    FROM oct_graph_relationships a
    JOIN oct_roles b ON a.DatasetID = b.DatasetID AND a.SourceTable = b.TableName
),
...
security_flags AS (
    SELECT 
        ...
        Workspace_ID,
        MAX(CASE 
            WHEN lower(Final_Shortcut_Name) LIKE '%dimaccountsummarygrouping' THEN 1
            WHEN lower(Final_Shortcut_Name) LIKE '%ext_dimaccountrls_actualssecurity' THEN 1
            WHEN lower(Final_Shortcut_Name) LIKE '%ext_dimaccountrls_targetsecurity' THEN 1
            WHEN lower(Final_Shortcut_Name) LIKE '%dimcyallaccounts' THEN 1
            WHEN lower(Final_Shortcut_Name) LIKE '%proxy%' THEN 1
            WHEN SPLIT(Initial_Shortcut_Name, '\\.')[1] LIKE '%RLS_TargetSecurity' THEN 1
            WHEN SPLIT(Initial_Shortcut_Name, '\\.')[1] LIKE '%RLS_ActualSecurity' THEN 1
            WHEN SPLIT(Initial_Shortcut_Name, '\\.')[1] LIKE '%dimaccountsummarygrouping_CZDL' THEN 1
            WHEN SPLIT(Initial_Shortcut_Name, '\\.')[1] LIKE '%dimcyallaccounts_CZDL' THEN 1
            WHEN SPLIT(Initial_Shortcut_Name, '\\.')[1] LIKE '%proxy%' THEN 1
            ELSE 0 
        END) AS SecurityFlag,
        MAX(CASE 
            WHEN OSOTLakehouseFlag = 'Yes' THEN 1
            ELSE 0
        END) AS OSOTLakehouseFlag
    FROM factoct
    GROUP BY Dataset_ID, Workspace_ID
)
...
SELECT 
    ...
    CASE WHEN (oc.has_actuals = 1 OR oc.has_targets = 1) AND oc.has_accounts = 1 THEN 'Yes' 
        ELSE 'No' 
    END AS Has_ALDA_Required,
    CASE WHEN r.DatasetID IS NOT NULL THEN 'Yes' ELSE 'No' END AS RLSApplied,
    CASE WHEN f.SecurityFlag = 1 THEN 'Yes' ELSE 'No' END AS SecurityFlag,
    CASE WHEN f.OSOTLakehouseFlag = 1 THEN 'Yes' ELSE 'No' END AS OSOTLakehouseFlag

FROM security_flags f
LEFT JOIN actual_target_check oc ON f.Dataset_ID = oc.Dataset_ID
LEFT JOIN AppliedRLS r ON f.Dataset_ID = r.DatasetID """)
    df.write.mode("overwrite").saveAsTable("oct_TablesRLS_Summary")
...
def final_optimize_tables():
    table_paths = [
        ...
        "abfss://07df884c-5185-45e4-9c8e-ebee3aca6605@msit-onelake.dfs.fabric.microsoft.com/2a5d1cfa-70ba-438a-a5ec-d5a288b58d17/Tables/oct_graph_relationships_bridge"
        ]
