...
class ChangeLog:
    ...
    def _add_branch_comparison(self, data: dict):
        ...
        if head and len(tail) >= 1:
            ...
            head_major = re.match(rf"{self.version_prefix}?(\d+)\.", head).group(1)
            tail_major = re.match(rf"{self.version_prefix}?(\d+)\.", tail[0]).group(1)
        ...
        if not major_change:
            prev_major = next(
                (
                    r
                    for r in tail[1:]
                    if re.match(rf"{self.version_prefix}?(\d+)\.", r).group(1)
                    != re.match(rf"{self.version_prefix}?(\d+)\.", head).group(1)
                ),
                None,
            )
            if prev_major:
