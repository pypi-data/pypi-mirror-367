...
class Accounts:
    ...
    def _authenticate_with_cache(self, auth: Authentication, cache: Dict):
        if self.root_mode:
            return

        if auth.api_key not in cache:
    ...
    def _check_admin_with_cache(self, auth: Authentication, cache: Dict):
        if self.root_mode:
            return

        self._authenticate_with_cache(auth, cache)

        account: Account = from_dict(Account, cache[auth.api_key])
