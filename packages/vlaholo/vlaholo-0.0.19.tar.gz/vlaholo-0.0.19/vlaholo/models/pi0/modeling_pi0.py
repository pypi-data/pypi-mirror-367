...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.config = config
        logger.info(
            f"input features order: {[k for k in self.config.input_features.keys() if 'observation' in k]}"
        )
        ...
        if os.path.exists("checkpoints/paligemma-3b-pt-224"):
            ...
            logger.info(
                f"checkpoints/paligemma-3b-pt-224 not found! download from google/paligemma-3b-pt-224"
            )
    ...
    def _transform_state_dict_keys(self, state_dict: dict) -> dict:
        ...
        transformed_dict = {}
        logger.info(
            f"transforming state dict for transformers>=4.52.0 in paligemma changes model names..."
        )
    ...
    def _load_as_safetensor(
        ...
        if packaging.version.parse(safetensors.__version__) < packaging.version.parse(
            ...
            # )
            logger.info(
                f"==> {model_file} map_location:{map_location} strict: {strict}"
            )
            ...
            if version.parse(transformers_version) >= version.parse(
                ...
                logger.info(
                    "Transformers version >= 4.52.0 detected - applying state dict key transformation"
                )
            ...
            try:
                ...
                if "Missing key(s)" in str(e):
                    missing_keys = (
                        str(e)
                        .split("Missing key(s)")[1]
                        .split("Unexpected key(s)")[0]
                        .strip()
                    )
                    logger.warning(
                        f"Missing keys: {missing_keys}, please note these keys when finetuning downstream tasks."
                    )
    ...
    def select_action(
        ...
        if len(self._action_queue) == 0:
            ...
            if verbose:
                logger.info(
                    f"actions: {actions.shape}, original_action_dim: {original_action_dim}"
                )
