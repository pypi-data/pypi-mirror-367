...
def get_safe_version(repo_id: str, version: str | packaging.version.Version) -> str:
    ...
    if lower_major:
        raise RuntimeError(f"Backward compatibility error for repo '{repo_id}' with version '{max(lower_major)}'")

    upper_versions = [v for v in hub_versions if v > target_version]
    assert len(upper_versions) > 0
    # Define ForwardCompatibilityError if not already imported
    class ForwardCompatibilityError(Exception):
        def __init__(self, repo_id, version):
            super().__init__(f"Forward compatibility error for repo '{repo_id}' with version '{version}'")
