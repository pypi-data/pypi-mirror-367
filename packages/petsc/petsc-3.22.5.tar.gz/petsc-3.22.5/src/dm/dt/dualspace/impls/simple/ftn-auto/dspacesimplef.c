...
extern "C" {
#endif
PETSC_EXTERN void  petscdualspacesimplesetdimension_(PetscDualSpace sp,PetscInt *dim, int *ierr)
{
CHKFORTRANNULLOBJECT(sp);
*ierr = PetscDualSpaceSimpleSetDimension(
	(PetscDualSpace)PetscToPointer((sp) ),*dim);
}
PETSC_EXTERN void  petscdualspacesimplesetfunctional_(PetscDualSpace sp,PetscInt *func,PetscQuadrature q, int *ierr)
{
CHKFORTRANNULLOBJECT(sp);
CHKFORTRANNULLOBJECT(q);
*ierr = PetscDualSpaceSimpleSetFunctional(
