...
extern "C" {
#endif
PETSC_EXTERN void  ksprichardsonsetscale_(KSP ksp,PetscReal *scale, int *ierr)
{
CHKFORTRANNULLOBJECT(ksp);
*ierr = KSPRichardsonSetScale(
	(KSP)PetscToPointer((ksp) ),*scale);
}
PETSC_EXTERN void  ksprichardsonsetselfscale_(KSP ksp,PetscBool *scale, int *ierr)
{
CHKFORTRANNULLOBJECT(ksp);
*ierr = KSPRichardsonSetSelfScale(
	(KSP)PetscToPointer((ksp) ),*scale);
}
#if defined(__cplusplus)
}
