...
extern "C" {
#endif
PETSC_EXTERN void  pcbjacobisettotalblocks_(PC pc,PetscInt *blocks, PetscInt lens[], int *ierr)
{
CHKFORTRANNULLOBJECT(pc);
CHKFORTRANNULLINTEGER(lens);
*ierr = PCBJacobiSetTotalBlocks(
	(PC)PetscToPointer((pc) ),*blocks,lens);
}
PETSC_EXTERN void  pcbjacobisetlocalblocks_(PC pc,PetscInt *blocks, PetscInt lens[], int *ierr)
{
CHKFORTRANNULLOBJECT(pc);
CHKFORTRANNULLINTEGER(lens);
