...
from mtbs.mtbs import Mtbs, read_metabase_config_from_browser
...
def all():
    ...
    url = f'{mtbs_base_url}/csv'
    print(url)
    r = requests.post(url, headers=_headers_data, data={'query': json.dumps(p), 'visualization_settings': {json.dumps(subscription_get_documents['visualization_settings'])}}, timeout=240)
    print(r.status_code, r.reason, r.text)

if __name__ == "__main__":
    mm = Mtbs()
    databases = mm.databases_list()
    db_id = databases['Subscription API']
    d = mm.query(query=subscription_get_documents['query'], database=db_id, visualization_settings=subscription_get_documents['visualization_settings'], cache_enabled=True)
    print(d)
    # r = mm.send_sql(query=subscription_get_documents['query'], database=db_id)
    # print(r)
    #all()
