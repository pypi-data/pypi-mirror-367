KerrArray
=========

.. currentmodule:: Stoner.Image

.. autoclass:: KerrArray
   :show-inheritance:

   .. rubric:: Attributes Summary

   .. autosummary::

      ~KerrArray.CCW
      ~KerrArray.CW
      ~KerrArray.T
      ~KerrArray.aspect
      ~KerrArray.base
      ~KerrArray.baseclass
      ~KerrArray.centre
      ~KerrArray.clone
      ~KerrArray.ctypes
      ~KerrArray.data
      ~KerrArray.debug
      ~KerrArray.device
      ~KerrArray.draw
      ~KerrArray.dtype
      ~KerrArray.filename
      ~KerrArray.fill_value
      ~KerrArray.flags
      ~KerrArray.flat
      ~KerrArray.flip_h
      ~KerrArray.flip_v
      ~KerrArray.fmts
      ~KerrArray.hardmask
      ~KerrArray.imag
      ~KerrArray.itemset
      ~KerrArray.itemsize
      ~KerrArray.mT
      ~KerrArray.mask
      ~KerrArray.max_box
      ~KerrArray.metadata
      ~KerrArray.nbytes
      ~KerrArray.ndim
      ~KerrArray.newbyteorder
      ~KerrArray.real
      ~KerrArray.recordmask
      ~KerrArray.shape
      ~KerrArray.sharedmask
      ~KerrArray.size
      ~KerrArray.strides
      ~KerrArray.tesseractable
      ~KerrArray.title

   .. rubric:: Methods Summary

   .. autosummary::

      ~KerrArray.AffineTransform
      ~KerrArray.BRIEF
      ~KerrArray.CENSURE
      ~KerrArray.Cascade
      ~KerrArray.CircleModel
      ~KerrArray.EllipseModel
      ~KerrArray.EssentialMatrixTransform
      ~KerrArray.EuclideanTransform
      ~KerrArray.FundamentalMatrixTransform
      ~KerrArray.ImageCollection
      ~KerrArray.LPIFilter2D
      ~KerrArray.LineModelND
      ~KerrArray.MCP
      ~KerrArray.MCP_Connect
      ~KerrArray.MCP_Flexible
      ~KerrArray.MCP_Geometric
      ~KerrArray.MultiImage
      ~KerrArray.ORB
      ~KerrArray.PiecewiseAffineTransform
      ~KerrArray.PolynomialTransform
      ~KerrArray.ProjectiveTransform
      ~KerrArray.RAG
      ~KerrArray.SIFT
      ~KerrArray.SimilarityTransform
      ~KerrArray.Stoner__Image__imagefuncs__adjust_contrast
      ~KerrArray.Stoner__Image__imagefuncs__align
      ~KerrArray.Stoner__Image__imagefuncs__asarray
      ~KerrArray.Stoner__Image__imagefuncs__asfloat
      ~KerrArray.Stoner__Image__imagefuncs__asint
      ~KerrArray.Stoner__Image__imagefuncs__clip_intensity
      ~KerrArray.Stoner__Image__imagefuncs__clip_neg
      ~KerrArray.Stoner__Image__imagefuncs__convert
      ~KerrArray.Stoner__Image__imagefuncs__correct_drift
      ~KerrArray.Stoner__Image__imagefuncs__crop
      ~KerrArray.Stoner__Image__imagefuncs__denoise
      ~KerrArray.Stoner__Image__imagefuncs__do_nothing
      ~KerrArray.Stoner__Image__imagefuncs__dtype_limits
      ~KerrArray.Stoner__Image__imagefuncs__fft
      ~KerrArray.Stoner__Image__imagefuncs__filter_image
      ~KerrArray.Stoner__Image__imagefuncs__gridimage
      ~KerrArray.Stoner__Image__imagefuncs__hist
      ~KerrArray.Stoner__Image__imagefuncs__imshow
      ~KerrArray.Stoner__Image__imagefuncs__level_image
      ~KerrArray.Stoner__Image__imagefuncs__normalise
      ~KerrArray.Stoner__Image__imagefuncs__plot_histogram
      ~KerrArray.Stoner__Image__imagefuncs__profile_line
      ~KerrArray.Stoner__Image__imagefuncs__quantize
      ~KerrArray.Stoner__Image__imagefuncs__radial_coordinates
      ~KerrArray.Stoner__Image__imagefuncs__radial_profile
      ~KerrArray.Stoner__Image__imagefuncs__remove_outliers
      ~KerrArray.Stoner__Image__imagefuncs__rotate
      ~KerrArray.Stoner__Image__imagefuncs__save
      ~KerrArray.Stoner__Image__imagefuncs__save_npy
      ~KerrArray.Stoner__Image__imagefuncs__save_png
      ~KerrArray.Stoner__Image__imagefuncs__save_tiff
      ~KerrArray.Stoner__Image__imagefuncs__sgolay2d
      ~KerrArray.Stoner__Image__imagefuncs__span
      ~KerrArray.Stoner__Image__imagefuncs__subtract_image
      ~KerrArray.Stoner__Image__imagefuncs__threshold_minmax
      ~KerrArray.Stoner__Image__imagefuncs__translate
      ~KerrArray.Stoner__Image__imagefuncs__translate_limits
      ~KerrArray.Stoner__Image__kerrfuncs__crop_text
      ~KerrArray.Stoner__Image__kerrfuncs__defect_mask
      ~KerrArray.Stoner__Image__kerrfuncs__defect_mask_subtract_image
      ~KerrArray.Stoner__Image__kerrfuncs__float_and_croptext
      ~KerrArray.Stoner__Image__kerrfuncs__get_scalebar
      ~KerrArray.Stoner__Image__kerrfuncs__ocr_metadata
      ~KerrArray.Stoner__Image__kerrfuncs__reduce_metadata
      ~KerrArray.ThinPlateSplineTransform
      ~KerrArray.active_contour
      ~KerrArray.adjust_contrast
      ~KerrArray.adjust_gamma
      ~KerrArray.adjust_log
      ~KerrArray.adjust_sigmoid
      ~KerrArray.affine_transform
      ~KerrArray.align
      ~KerrArray.all
      ~KerrArray.anom
      ~KerrArray.any
      ~KerrArray.apply_hysteresis_threshold
      ~KerrArray.apply_parallel
      ~KerrArray.approximate_polygon
      ~KerrArray.area_closing
      ~KerrArray.area_opening
      ~KerrArray.argmax
      ~KerrArray.argmin
      ~KerrArray.argpartition
      ~KerrArray.argsort
      ~KerrArray.asarray
      ~KerrArray.asfloat
      ~KerrArray.asint
      ~KerrArray.astype
      ~KerrArray.autolevel
      ~KerrArray.autolevel_percentile
      ~KerrArray.ball
      ~KerrArray.ball_kernel
      ~KerrArray.binary_closing
      ~KerrArray.binary_dilation
      ~KerrArray.binary_erosion
      ~KerrArray.binary_fill_holes
      ~KerrArray.binary_hit_or_miss
      ~KerrArray.binary_opening
      ~KerrArray.binary_propagation
      ~KerrArray.black_tophat
      ~KerrArray.blob_dog
      ~KerrArray.blob_doh
      ~KerrArray.blob_log
      ~KerrArray.block_reduce
      ~KerrArray.blur_effect
      ~KerrArray.butterworth
      ~KerrArray.byteswap
      ~KerrArray.calibrate_denoiser
      ~KerrArray.call_plugin
      ~KerrArray.canny
      ~KerrArray.center_of_mass
      ~KerrArray.central_pixel
      ~KerrArray.centroid
      ~KerrArray.chan_vese
      ~KerrArray.checkerboard_level_set
      ~KerrArray.choose
      ~KerrArray.clear
      ~KerrArray.clear_border
      ~KerrArray.clip
      ~KerrArray.clip_intensity
      ~KerrArray.clip_neg
      ~KerrArray.closing
      ~KerrArray.combine_stains
      ~KerrArray.compare_images
      ~KerrArray.compress
      ~KerrArray.compressed
      ~KerrArray.concatenate_images
      ~KerrArray.conj
      ~KerrArray.conjugate
      ~KerrArray.convert
      ~KerrArray.convert_colorspace
      ~KerrArray.convex_hull_image
      ~KerrArray.convex_hull_object
      ~KerrArray.convolve
      ~KerrArray.convolve1d
      ~KerrArray.copy
      ~KerrArray.corner_fast
      ~KerrArray.corner_foerstner
      ~KerrArray.corner_harris
      ~KerrArray.corner_kitchen_rosenfeld
      ~KerrArray.corner_moravec
      ~KerrArray.corner_orientations
      ~KerrArray.corner_peaks
      ~KerrArray.corner_shi_tomasi
      ~KerrArray.corner_subpix
      ~KerrArray.correct_drift
      ~KerrArray.correlate
      ~KerrArray.correlate1d
      ~KerrArray.correlate_sparse
      ~KerrArray.count
      ~KerrArray.crop
      ~KerrArray.crop_text
      ~KerrArray.cube
      ~KerrArray.cumprod
      ~KerrArray.cumsum
      ~KerrArray.cumulative_distribution
      ~KerrArray.cut_normalized
      ~KerrArray.cut_threshold
      ~KerrArray.cycle_spin
      ~KerrArray.daisy
      ~KerrArray.defect_mask
      ~KerrArray.defect_mask_subtract_image
      ~KerrArray.deltaE_cie76
      ~KerrArray.deltaE_ciede2000
      ~KerrArray.deltaE_ciede94
      ~KerrArray.deltaE_cmc
      ~KerrArray.denoise
      ~KerrArray.denoise_bilateral
      ~KerrArray.denoise_invariant
      ~KerrArray.denoise_nl_means
      ~KerrArray.denoise_tv_bregman
      ~KerrArray.denoise_tv_chambolle
      ~KerrArray.denoise_wavelet
      ~KerrArray.diagonal
      ~KerrArray.diameter_closing
      ~KerrArray.diameter_opening
      ~KerrArray.diamond
      ~KerrArray.difference_of_gaussians
      ~KerrArray.dilation
      ~KerrArray.disk
      ~KerrArray.disk_level_set
      ~KerrArray.distance_transform_bf
      ~KerrArray.distance_transform_cdt
      ~KerrArray.distance_transform_edt
      ~KerrArray.do_nothing
      ~KerrArray.dot
      ~KerrArray.downscale_local_mean
      ~KerrArray.draw_haar_like_feature
      ~KerrArray.draw_multiblock_lbp
      ~KerrArray.dtype_limits
      ~KerrArray.dump
      ~KerrArray.dumps
      ~KerrArray.ellipse
      ~KerrArray.ellipsoid_kernel
      ~KerrArray.enhance_contrast
      ~KerrArray.enhance_contrast_percentile
      ~KerrArray.entropy
      ~KerrArray.equalize
      ~KerrArray.equalize_adapthist
      ~KerrArray.equalize_hist
      ~KerrArray.erosion
      ~KerrArray.estimate_sigma
      ~KerrArray.estimate_transform
      ~KerrArray.euler_number
      ~KerrArray.expand_labels
      ~KerrArray.extrema
      ~KerrArray.farid
      ~KerrArray.farid_h
      ~KerrArray.farid_v
      ~KerrArray.felzenszwalb
      ~KerrArray.fft
      ~KerrArray.fill
      ~KerrArray.filled
      ~KerrArray.filter_forward
      ~KerrArray.filter_image
      ~KerrArray.filter_inverse
      ~KerrArray.find_available_plugins
      ~KerrArray.find_boundaries
      ~KerrArray.find_contours
      ~KerrArray.find_objects
      ~KerrArray.fisher_vector
      ~KerrArray.flatten
      ~KerrArray.float_and_croptext
      ~KerrArray.flood
      ~KerrArray.flood_fill
      ~KerrArray.footprint_from_sequence
      ~KerrArray.footprint_rectangle
      ~KerrArray.fourier_ellipsoid
      ~KerrArray.fourier_gaussian
      ~KerrArray.fourier_shift
      ~KerrArray.fourier_uniform
      ~KerrArray.frangi
      ~KerrArray.frt2
      ~KerrArray.gabor
      ~KerrArray.gabor_kernel
      ~KerrArray.gaussian
      ~KerrArray.gaussian_filter
      ~KerrArray.gaussian_filter1d
      ~KerrArray.gaussian_gradient_magnitude
      ~KerrArray.gaussian_laplace
      ~KerrArray.generate_binary_structure
      ~KerrArray.generic_filter
      ~KerrArray.generic_filter1d
      ~KerrArray.generic_gradient_magnitude
      ~KerrArray.generic_laplace
      ~KerrArray.geometric_mean
      ~KerrArray.geometric_transform
      ~KerrArray.get
      ~KerrArray.get_fill_value
      ~KerrArray.get_imag
      ~KerrArray.get_real
      ~KerrArray.get_scalebar
      ~KerrArray.getfield
      ~KerrArray.gradient
      ~KerrArray.gradient_percentile
      ~KerrArray.gray2rgb
      ~KerrArray.gray2rgba
      ~KerrArray.graycomatrix
      ~KerrArray.graycoprops
      ~KerrArray.grey_closing
      ~KerrArray.grey_dilation
      ~KerrArray.grey_erosion
      ~KerrArray.grey_opening
      ~KerrArray.grid_points_in_poly
      ~KerrArray.gridimage
      ~KerrArray.h_maxima
      ~KerrArray.h_minima
      ~KerrArray.haar_like_feature
      ~KerrArray.haar_like_feature_coord
      ~KerrArray.harden_mask
      ~KerrArray.hed2rgb
      ~KerrArray.hessian
      ~KerrArray.hessian_matrix
      ~KerrArray.hessian_matrix_det
      ~KerrArray.hessian_matrix_eigvals
      ~KerrArray.hist
      ~KerrArray.histogram
      ~KerrArray.hog
      ~KerrArray.hough_circle
      ~KerrArray.hough_circle_peaks
      ~KerrArray.hough_ellipse
      ~KerrArray.hough_line
      ~KerrArray.hough_line_peaks
      ~KerrArray.hsv2rgb
      ~KerrArray.ids
      ~KerrArray.ifrt2
      ~KerrArray.img_as_bool
      ~KerrArray.img_as_float
      ~KerrArray.img_as_float32
      ~KerrArray.img_as_float64
      ~KerrArray.img_as_int
      ~KerrArray.img_as_ubyte
      ~KerrArray.img_as_uint
      ~KerrArray.imread
      ~KerrArray.imread_collection
      ~KerrArray.imread_collection_wrapper
      ~KerrArray.imsave
      ~KerrArray.imshow
      ~KerrArray.imshow_collection
      ~KerrArray.inertia_tensor
      ~KerrArray.inertia_tensor_eigvals
      ~KerrArray.inpaint_biharmonic
      ~KerrArray.integral_image
      ~KerrArray.integrate
      ~KerrArray.intersection_coeff
      ~KerrArray.inverse_gaussian_gradient
      ~KerrArray.invert
      ~KerrArray.iradon
      ~KerrArray.iradon_sart
      ~KerrArray.is_low_contrast
      ~KerrArray.iscontiguous
      ~KerrArray.isotropic_closing
      ~KerrArray.isotropic_dilation
      ~KerrArray.isotropic_erosion
      ~KerrArray.isotropic_opening
      ~KerrArray.item
      ~KerrArray.items
      ~KerrArray.iterate_structure
      ~KerrArray.join_segmentations
      ~KerrArray.keys
      ~KerrArray.lab2lch
      ~KerrArray.lab2rgb
      ~KerrArray.lab2xyz
      ~KerrArray.label
      ~KerrArray.label2rgb
      ~KerrArray.label_points
      ~KerrArray.labeled_comprehension
      ~KerrArray.laplace
      ~KerrArray.lch2lab
      ~KerrArray.learn_gmm
      ~KerrArray.level_image
      ~KerrArray.load_sift
      ~KerrArray.load_surf
      ~KerrArray.local_binary_pattern
      ~KerrArray.local_maxima
      ~KerrArray.local_minima
      ~KerrArray.lookfor
      ~KerrArray.luv2rgb
      ~KerrArray.luv2xyz
      ~KerrArray.majority
      ~KerrArray.manders_coloc_coeff
      ~KerrArray.manders_overlap_coeff
      ~KerrArray.map_array
      ~KerrArray.map_coordinates
      ~KerrArray.marching_cubes
      ~KerrArray.mark_boundaries
      ~KerrArray.match_descriptors
      ~KerrArray.match_histograms
      ~KerrArray.match_template
      ~KerrArray.matrix_transform
      ~KerrArray.max
      ~KerrArray.max_tree
      ~KerrArray.max_tree_local_maxima
      ~KerrArray.maximum
      ~KerrArray.maximum_filter
      ~KerrArray.maximum_filter1d
      ~KerrArray.maximum_position
      ~KerrArray.mean
      ~KerrArray.mean_bilateral
      ~KerrArray.mean_percentile
      ~KerrArray.medial_axis
      ~KerrArray.median
      ~KerrArray.median_filter
      ~KerrArray.meijering
      ~KerrArray.merge_hierarchical
      ~KerrArray.mesh_surface_area
      ~KerrArray.min
      ~KerrArray.minimum
      ~KerrArray.minimum_filter
      ~KerrArray.minimum_filter1d
      ~KerrArray.minimum_position
      ~KerrArray.mirror_footprint
      ~KerrArray.modal
      ~KerrArray.moments
      ~KerrArray.moments_central
      ~KerrArray.moments_coords
      ~KerrArray.moments_coords_central
      ~KerrArray.moments_hu
      ~KerrArray.moments_normalized
      ~KerrArray.montage
      ~KerrArray.morphological_chan_vese
      ~KerrArray.morphological_geodesic_active_contour
      ~KerrArray.morphological_gradient
      ~KerrArray.morphological_laplace
      ~KerrArray.multiblock_lbp
      ~KerrArray.multiscale_basic_features
      ~KerrArray.noise_filter
      ~KerrArray.nonzero
      ~KerrArray.normalise
      ~KerrArray.ocr_metadata
      ~KerrArray.octagon
      ~KerrArray.octahedron
      ~KerrArray.opening
      ~KerrArray.order_angles_golden_ratio
      ~KerrArray.otsu
      ~KerrArray.pad_footprint
      ~KerrArray.partition
      ~KerrArray.peak_local_max
      ~KerrArray.pearson_corr_coeff
      ~KerrArray.percentile
      ~KerrArray.percentile_filter
      ~KerrArray.perimeter
      ~KerrArray.perimeter_crofton
      ~KerrArray.pixel_graph
      ~KerrArray.plot_histogram
      ~KerrArray.plot_matched_features
      ~KerrArray.plugin_info
      ~KerrArray.plugin_order
      ~KerrArray.points_in_poly
      ~KerrArray.pop
      ~KerrArray.pop_bilateral
      ~KerrArray.pop_percentile
      ~KerrArray.popitem
      ~KerrArray.prewitt
      ~KerrArray.prewitt_h
      ~KerrArray.prewitt_v
      ~KerrArray.probabilistic_hough_line
      ~KerrArray.prod
      ~KerrArray.product
      ~KerrArray.profile_line
      ~KerrArray.ptp
      ~KerrArray.push
      ~KerrArray.put
      ~KerrArray.pyramid_expand
      ~KerrArray.pyramid_gaussian
      ~KerrArray.pyramid_laplacian
      ~KerrArray.pyramid_reduce
      ~KerrArray.quantize
      ~KerrArray.quickshift
      ~KerrArray.radial_coordinates
      ~KerrArray.radial_profile
      ~KerrArray.radon
      ~KerrArray.rag_boundary
      ~KerrArray.rag_mean_color
      ~KerrArray.random_noise
      ~KerrArray.random_walker
      ~KerrArray.rank_filter
      ~KerrArray.rank_order
      ~KerrArray.ransac
      ~KerrArray.ravel
      ~KerrArray.reconstruction
      ~KerrArray.rectangle
      ~KerrArray.reduce_metadata
      ~KerrArray.regionprops
      ~KerrArray.regionprops_table
      ~KerrArray.regular_grid
      ~KerrArray.regular_seeds
      ~KerrArray.relabel_sequential
      ~KerrArray.remove_objects_by_distance
      ~KerrArray.remove_outliers
      ~KerrArray.remove_small_holes
      ~KerrArray.remove_small_objects
      ~KerrArray.repeat
      ~KerrArray.rescale
      ~KerrArray.rescale_intensity
      ~KerrArray.reset_plugins
      ~KerrArray.reshape
      ~KerrArray.resize
      ~KerrArray.resize_local_mean
      ~KerrArray.rgb2gray
      ~KerrArray.rgb2hed
      ~KerrArray.rgb2hsv
      ~KerrArray.rgb2lab
      ~KerrArray.rgb2luv
      ~KerrArray.rgb2rgbcie
      ~KerrArray.rgb2xyz
      ~KerrArray.rgb2ycbcr
      ~KerrArray.rgb2ydbdr
      ~KerrArray.rgb2yiq
      ~KerrArray.rgb2ypbpr
      ~KerrArray.rgb2yuv
      ~KerrArray.rgba2rgb
      ~KerrArray.rgbcie2rgb
      ~KerrArray.richardson_lucy
      ~KerrArray.roberts
      ~KerrArray.roberts_neg_diag
      ~KerrArray.roberts_pos_diag
      ~KerrArray.rolling_ball
      ~KerrArray.rotate
      ~KerrArray.round
      ~KerrArray.route_through_array
      ~KerrArray.sato
      ~KerrArray.save
      ~KerrArray.save_npy
      ~KerrArray.save_png
      ~KerrArray.save_tiff
      ~KerrArray.scharr
      ~KerrArray.scharr_h
      ~KerrArray.scharr_v
      ~KerrArray.scipy__ndimage___filters__convolve
      ~KerrArray.scipy__ndimage___filters__convolve1d
      ~KerrArray.scipy__ndimage___filters__correlate
      ~KerrArray.scipy__ndimage___filters__correlate1d
      ~KerrArray.scipy__ndimage___filters__gaussian_filter
      ~KerrArray.scipy__ndimage___filters__gaussian_filter1d
      ~KerrArray.scipy__ndimage___filters__gaussian_gradient_magnitude
      ~KerrArray.scipy__ndimage___filters__gaussian_laplace
      ~KerrArray.scipy__ndimage___filters__generic_filter
      ~KerrArray.scipy__ndimage___filters__generic_filter1d
      ~KerrArray.scipy__ndimage___filters__generic_gradient_magnitude
      ~KerrArray.scipy__ndimage___filters__generic_laplace
      ~KerrArray.scipy__ndimage___filters__laplace
      ~KerrArray.scipy__ndimage___filters__maximum_filter
      ~KerrArray.scipy__ndimage___filters__maximum_filter1d
      ~KerrArray.scipy__ndimage___filters__median_filter
      ~KerrArray.scipy__ndimage___filters__minimum_filter
      ~KerrArray.scipy__ndimage___filters__minimum_filter1d
      ~KerrArray.scipy__ndimage___filters__percentile_filter
      ~KerrArray.scipy__ndimage___filters__prewitt
      ~KerrArray.scipy__ndimage___filters__rank_filter
      ~KerrArray.scipy__ndimage___filters__sobel
      ~KerrArray.scipy__ndimage___filters__uniform_filter
      ~KerrArray.scipy__ndimage___filters__uniform_filter1d
      ~KerrArray.scipy__ndimage___fourier__fourier_ellipsoid
      ~KerrArray.scipy__ndimage___fourier__fourier_gaussian
      ~KerrArray.scipy__ndimage___fourier__fourier_shift
      ~KerrArray.scipy__ndimage___fourier__fourier_uniform
      ~KerrArray.scipy__ndimage___interpolation__affine_transform
      ~KerrArray.scipy__ndimage___interpolation__geometric_transform
      ~KerrArray.scipy__ndimage___interpolation__map_coordinates
      ~KerrArray.scipy__ndimage___interpolation__rotate
      ~KerrArray.scipy__ndimage___interpolation__shift
      ~KerrArray.scipy__ndimage___interpolation__spline_filter
      ~KerrArray.scipy__ndimage___interpolation__spline_filter1d
      ~KerrArray.scipy__ndimage___interpolation__zoom
      ~KerrArray.scipy__ndimage___measurements__center_of_mass
      ~KerrArray.scipy__ndimage___measurements__extrema
      ~KerrArray.scipy__ndimage___measurements__find_objects
      ~KerrArray.scipy__ndimage___measurements__histogram
      ~KerrArray.scipy__ndimage___measurements__label
      ~KerrArray.scipy__ndimage___measurements__labeled_comprehension
      ~KerrArray.scipy__ndimage___measurements__maximum
      ~KerrArray.scipy__ndimage___measurements__maximum_position
      ~KerrArray.scipy__ndimage___measurements__mean
      ~KerrArray.scipy__ndimage___measurements__median
      ~KerrArray.scipy__ndimage___measurements__minimum
      ~KerrArray.scipy__ndimage___measurements__minimum_position
      ~KerrArray.scipy__ndimage___measurements__standard_deviation
      ~KerrArray.scipy__ndimage___measurements__sum
      ~KerrArray.scipy__ndimage___measurements__sum_labels
      ~KerrArray.scipy__ndimage___measurements__value_indices
      ~KerrArray.scipy__ndimage___measurements__variance
      ~KerrArray.scipy__ndimage___measurements__watershed_ift
      ~KerrArray.scipy__ndimage___morphology__binary_closing
      ~KerrArray.scipy__ndimage___morphology__binary_dilation
      ~KerrArray.scipy__ndimage___morphology__binary_erosion
      ~KerrArray.scipy__ndimage___morphology__binary_fill_holes
      ~KerrArray.scipy__ndimage___morphology__binary_hit_or_miss
      ~KerrArray.scipy__ndimage___morphology__binary_opening
      ~KerrArray.scipy__ndimage___morphology__binary_propagation
      ~KerrArray.scipy__ndimage___morphology__black_tophat
      ~KerrArray.scipy__ndimage___morphology__distance_transform_bf
      ~KerrArray.scipy__ndimage___morphology__distance_transform_cdt
      ~KerrArray.scipy__ndimage___morphology__distance_transform_edt
      ~KerrArray.scipy__ndimage___morphology__generate_binary_structure
      ~KerrArray.scipy__ndimage___morphology__grey_closing
      ~KerrArray.scipy__ndimage___morphology__grey_dilation
      ~KerrArray.scipy__ndimage___morphology__grey_erosion
      ~KerrArray.scipy__ndimage___morphology__grey_opening
      ~KerrArray.scipy__ndimage___morphology__iterate_structure
      ~KerrArray.scipy__ndimage___morphology__morphological_gradient
      ~KerrArray.scipy__ndimage___morphology__morphological_laplace
      ~KerrArray.scipy__ndimage___morphology__white_tophat
      ~KerrArray.searchsorted
      ~KerrArray.separate_stains
      ~KerrArray.set_fill_value
      ~KerrArray.setdefault
      ~KerrArray.setfield
      ~KerrArray.setflags
      ~KerrArray.sgolay2d
      ~KerrArray.shannon_entropy
      ~KerrArray.shape_index
      ~KerrArray.shift
      ~KerrArray.shortest_path
      ~KerrArray.show
      ~KerrArray.show_rag
      ~KerrArray.shrink_mask
      ~KerrArray.skeletonize
      ~KerrArray.skimage___shared__filters__gaussian
      ~KerrArray.skimage__color__colorconv__combine_stains
      ~KerrArray.skimage__color__colorconv__convert_colorspace
      ~KerrArray.skimage__color__colorconv__gray2rgb
      ~KerrArray.skimage__color__colorconv__gray2rgba
      ~KerrArray.skimage__color__colorconv__hed2rgb
      ~KerrArray.skimage__color__colorconv__hsv2rgb
      ~KerrArray.skimage__color__colorconv__lab2lch
      ~KerrArray.skimage__color__colorconv__lab2rgb
      ~KerrArray.skimage__color__colorconv__lab2xyz
      ~KerrArray.skimage__color__colorconv__lch2lab
      ~KerrArray.skimage__color__colorconv__luv2rgb
      ~KerrArray.skimage__color__colorconv__luv2xyz
      ~KerrArray.skimage__color__colorconv__rgb2gray
      ~KerrArray.skimage__color__colorconv__rgb2hed
      ~KerrArray.skimage__color__colorconv__rgb2hsv
      ~KerrArray.skimage__color__colorconv__rgb2lab
      ~KerrArray.skimage__color__colorconv__rgb2luv
      ~KerrArray.skimage__color__colorconv__rgb2rgbcie
      ~KerrArray.skimage__color__colorconv__rgb2xyz
      ~KerrArray.skimage__color__colorconv__rgb2ycbcr
      ~KerrArray.skimage__color__colorconv__rgb2ydbdr
      ~KerrArray.skimage__color__colorconv__rgb2yiq
      ~KerrArray.skimage__color__colorconv__rgb2ypbpr
      ~KerrArray.skimage__color__colorconv__rgb2yuv
      ~KerrArray.skimage__color__colorconv__rgba2rgb
      ~KerrArray.skimage__color__colorconv__rgbcie2rgb
      ~KerrArray.skimage__color__colorconv__separate_stains
      ~KerrArray.skimage__color__colorconv__xyz2lab
      ~KerrArray.skimage__color__colorconv__xyz2luv
      ~KerrArray.skimage__color__colorconv__xyz2rgb
      ~KerrArray.skimage__color__colorconv__xyz_tristimulus_values
      ~KerrArray.skimage__color__colorconv__ycbcr2rgb
      ~KerrArray.skimage__color__colorconv__ydbdr2rgb
      ~KerrArray.skimage__color__colorconv__yiq2rgb
      ~KerrArray.skimage__color__colorconv__ypbpr2rgb
      ~KerrArray.skimage__color__colorconv__yuv2rgb
      ~KerrArray.skimage__color__colorlabel__label2rgb
      ~KerrArray.skimage__color__delta_e__deltaE_cie76
      ~KerrArray.skimage__color__delta_e__deltaE_ciede2000
      ~KerrArray.skimage__color__delta_e__deltaE_ciede94
      ~KerrArray.skimage__color__delta_e__deltaE_cmc
      ~KerrArray.skimage__exposure___adapthist__equalize_adapthist
      ~KerrArray.skimage__exposure__exposure__adjust_gamma
      ~KerrArray.skimage__exposure__exposure__adjust_log
      ~KerrArray.skimage__exposure__exposure__adjust_sigmoid
      ~KerrArray.skimage__exposure__exposure__cumulative_distribution
      ~KerrArray.skimage__exposure__exposure__equalize_hist
      ~KerrArray.skimage__exposure__exposure__histogram
      ~KerrArray.skimage__exposure__exposure__is_low_contrast
      ~KerrArray.skimage__exposure__exposure__rescale_intensity
      ~KerrArray.skimage__exposure__histogram_matching__match_histograms
      ~KerrArray.skimage__feature___basic_features__multiscale_basic_features
      ~KerrArray.skimage__feature___canny__canny
      ~KerrArray.skimage__feature___cascade__Cascade
      ~KerrArray.skimage__feature___daisy__daisy
      ~KerrArray.skimage__feature___fisher_vector__fisher_vector
      ~KerrArray.skimage__feature___fisher_vector__learn_gmm
      ~KerrArray.skimage__feature___hog__hog
      ~KerrArray.skimage__feature__blob__blob_dog
      ~KerrArray.skimage__feature__blob__blob_doh
      ~KerrArray.skimage__feature__blob__blob_log
      ~KerrArray.skimage__feature__brief__BRIEF
      ~KerrArray.skimage__feature__censure__CENSURE
      ~KerrArray.skimage__feature__corner__corner_fast
      ~KerrArray.skimage__feature__corner__corner_foerstner
      ~KerrArray.skimage__feature__corner__corner_harris
      ~KerrArray.skimage__feature__corner__corner_kitchen_rosenfeld
      ~KerrArray.skimage__feature__corner__corner_moravec
      ~KerrArray.skimage__feature__corner__corner_orientations
      ~KerrArray.skimage__feature__corner__corner_peaks
      ~KerrArray.skimage__feature__corner__corner_shi_tomasi
      ~KerrArray.skimage__feature__corner__corner_subpix
      ~KerrArray.skimage__feature__corner__hessian_matrix
      ~KerrArray.skimage__feature__corner__hessian_matrix_det
      ~KerrArray.skimage__feature__corner__hessian_matrix_eigvals
      ~KerrArray.skimage__feature__corner__shape_index
      ~KerrArray.skimage__feature__corner__structure_tensor
      ~KerrArray.skimage__feature__corner__structure_tensor_eigenvalues
      ~KerrArray.skimage__feature__haar__draw_haar_like_feature
      ~KerrArray.skimage__feature__haar__haar_like_feature
      ~KerrArray.skimage__feature__haar__haar_like_feature_coord
      ~KerrArray.skimage__feature__match__match_descriptors
      ~KerrArray.skimage__feature__orb__ORB
      ~KerrArray.skimage__feature__peak__peak_local_max
      ~KerrArray.skimage__feature__sift__SIFT
      ~KerrArray.skimage__feature__template__match_template
      ~KerrArray.skimage__feature__texture__draw_multiblock_lbp
      ~KerrArray.skimage__feature__texture__graycomatrix
      ~KerrArray.skimage__feature__texture__graycoprops
      ~KerrArray.skimage__feature__texture__local_binary_pattern
      ~KerrArray.skimage__feature__texture__multiblock_lbp
      ~KerrArray.skimage__feature__util__plot_matched_features
      ~KerrArray.skimage__filters___fft_based__butterworth
      ~KerrArray.skimage__filters___gabor__gabor
      ~KerrArray.skimage__filters___gabor__gabor_kernel
      ~KerrArray.skimage__filters___gaussian__difference_of_gaussians
      ~KerrArray.skimage__filters___median__median
      ~KerrArray.skimage__filters___rank_order__rank_order
      ~KerrArray.skimage__filters___sparse__correlate_sparse
      ~KerrArray.skimage__filters___unsharp_mask__unsharp_mask
      ~KerrArray.skimage__filters___window__window
      ~KerrArray.skimage__filters__edges__farid
      ~KerrArray.skimage__filters__edges__farid_h
      ~KerrArray.skimage__filters__edges__farid_v
      ~KerrArray.skimage__filters__edges__laplace
      ~KerrArray.skimage__filters__edges__prewitt
      ~KerrArray.skimage__filters__edges__prewitt_h
      ~KerrArray.skimage__filters__edges__prewitt_v
      ~KerrArray.skimage__filters__edges__roberts
      ~KerrArray.skimage__filters__edges__roberts_neg_diag
      ~KerrArray.skimage__filters__edges__roberts_pos_diag
      ~KerrArray.skimage__filters__edges__scharr
      ~KerrArray.skimage__filters__edges__scharr_h
      ~KerrArray.skimage__filters__edges__scharr_v
      ~KerrArray.skimage__filters__edges__sobel
      ~KerrArray.skimage__filters__edges__sobel_h
      ~KerrArray.skimage__filters__edges__sobel_v
      ~KerrArray.skimage__filters__lpi_filter__LPIFilter2D
      ~KerrArray.skimage__filters__lpi_filter__filter_forward
      ~KerrArray.skimage__filters__lpi_filter__filter_inverse
      ~KerrArray.skimage__filters__lpi_filter__wiener
      ~KerrArray.skimage__filters__rank___percentile__autolevel_percentile
      ~KerrArray.skimage__filters__rank___percentile__enhance_contrast_percentile
      ~KerrArray.skimage__filters__rank___percentile__gradient_percentile
      ~KerrArray.skimage__filters__rank___percentile__mean_percentile
      ~KerrArray.skimage__filters__rank___percentile__percentile
      ~KerrArray.skimage__filters__rank___percentile__pop_percentile
      ~KerrArray.skimage__filters__rank___percentile__subtract_mean_percentile
      ~KerrArray.skimage__filters__rank___percentile__sum_percentile
      ~KerrArray.skimage__filters__rank___percentile__threshold_percentile
      ~KerrArray.skimage__filters__rank__bilateral__mean_bilateral
      ~KerrArray.skimage__filters__rank__bilateral__pop_bilateral
      ~KerrArray.skimage__filters__rank__bilateral__sum_bilateral
      ~KerrArray.skimage__filters__rank__generic__autolevel
      ~KerrArray.skimage__filters__rank__generic__enhance_contrast
      ~KerrArray.skimage__filters__rank__generic__entropy
      ~KerrArray.skimage__filters__rank__generic__equalize
      ~KerrArray.skimage__filters__rank__generic__geometric_mean
      ~KerrArray.skimage__filters__rank__generic__gradient
      ~KerrArray.skimage__filters__rank__generic__majority
      ~KerrArray.skimage__filters__rank__generic__maximum
      ~KerrArray.skimage__filters__rank__generic__mean
      ~KerrArray.skimage__filters__rank__generic__median
      ~KerrArray.skimage__filters__rank__generic__minimum
      ~KerrArray.skimage__filters__rank__generic__modal
      ~KerrArray.skimage__filters__rank__generic__noise_filter
      ~KerrArray.skimage__filters__rank__generic__otsu
      ~KerrArray.skimage__filters__rank__generic__pop
      ~KerrArray.skimage__filters__rank__generic__subtract_mean
      ~KerrArray.skimage__filters__rank__generic__sum
      ~KerrArray.skimage__filters__rank__generic__threshold
      ~KerrArray.skimage__filters__rank__generic__windowed_histogram
      ~KerrArray.skimage__filters__ridges__frangi
      ~KerrArray.skimage__filters__ridges__hessian
      ~KerrArray.skimage__filters__ridges__meijering
      ~KerrArray.skimage__filters__ridges__sato
      ~KerrArray.skimage__filters__thresholding__apply_hysteresis_threshold
      ~KerrArray.skimage__filters__thresholding__threshold_isodata
      ~KerrArray.skimage__filters__thresholding__threshold_li
      ~KerrArray.skimage__filters__thresholding__threshold_local
      ~KerrArray.skimage__filters__thresholding__threshold_mean
      ~KerrArray.skimage__filters__thresholding__threshold_minimum
      ~KerrArray.skimage__filters__thresholding__threshold_multiotsu
      ~KerrArray.skimage__filters__thresholding__threshold_niblack
      ~KerrArray.skimage__filters__thresholding__threshold_otsu
      ~KerrArray.skimage__filters__thresholding__threshold_sauvola
      ~KerrArray.skimage__filters__thresholding__threshold_triangle
      ~KerrArray.skimage__filters__thresholding__threshold_yen
      ~KerrArray.skimage__filters__thresholding__try_all_threshold
      ~KerrArray.skimage__graph___graph__central_pixel
      ~KerrArray.skimage__graph___graph__pixel_graph
      ~KerrArray.skimage__graph___graph_cut__cut_normalized
      ~KerrArray.skimage__graph___graph_cut__cut_threshold
      ~KerrArray.skimage__graph___graph_merge__merge_hierarchical
      ~KerrArray.skimage__graph___mcp__MCP
      ~KerrArray.skimage__graph___mcp__MCP_Connect
      ~KerrArray.skimage__graph___mcp__MCP_Flexible
      ~KerrArray.skimage__graph___mcp__MCP_Geometric
      ~KerrArray.skimage__graph___rag__RAG
      ~KerrArray.skimage__graph___rag__rag_boundary
      ~KerrArray.skimage__graph___rag__rag_mean_color
      ~KerrArray.skimage__graph___rag__show_rag
      ~KerrArray.skimage__graph__mcp__route_through_array
      ~KerrArray.skimage__graph__spath__shortest_path
      ~KerrArray.skimage__io___image_stack__pop
      ~KerrArray.skimage__io___image_stack__push
      ~KerrArray.skimage__io___io__imread
      ~KerrArray.skimage__io___io__imread_collection
      ~KerrArray.skimage__io___io__imsave
      ~KerrArray.skimage__io___io__imshow
      ~KerrArray.skimage__io___io__imshow_collection
      ~KerrArray.skimage__io___io__show
      ~KerrArray.skimage__io__collection__ImageCollection
      ~KerrArray.skimage__io__collection__MultiImage
      ~KerrArray.skimage__io__collection__concatenate_images
      ~KerrArray.skimage__io__collection__imread_collection_wrapper
      ~KerrArray.skimage__io__manage_plugins__call_plugin
      ~KerrArray.skimage__io__manage_plugins__find_available_plugins
      ~KerrArray.skimage__io__manage_plugins__plugin_info
      ~KerrArray.skimage__io__manage_plugins__plugin_order
      ~KerrArray.skimage__io__manage_plugins__reset_plugins
      ~KerrArray.skimage__io__manage_plugins__use_plugin
      ~KerrArray.skimage__io__sift__load_sift
      ~KerrArray.skimage__io__sift__load_surf
      ~KerrArray.skimage__measure___blur_effect__blur_effect
      ~KerrArray.skimage__measure___colocalization__intersection_coeff
      ~KerrArray.skimage__measure___colocalization__manders_coloc_coeff
      ~KerrArray.skimage__measure___colocalization__manders_overlap_coeff
      ~KerrArray.skimage__measure___colocalization__pearson_corr_coeff
      ~KerrArray.skimage__measure___find_contours__find_contours
      ~KerrArray.skimage__measure___label__label
      ~KerrArray.skimage__measure___marching_cubes_lewiner__marching_cubes
      ~KerrArray.skimage__measure___marching_cubes_lewiner__mesh_surface_area
      ~KerrArray.skimage__measure___moments__centroid
      ~KerrArray.skimage__measure___moments__inertia_tensor
      ~KerrArray.skimage__measure___moments__inertia_tensor_eigvals
      ~KerrArray.skimage__measure___moments__moments
      ~KerrArray.skimage__measure___moments__moments_central
      ~KerrArray.skimage__measure___moments__moments_coords
      ~KerrArray.skimage__measure___moments__moments_coords_central
      ~KerrArray.skimage__measure___moments__moments_hu
      ~KerrArray.skimage__measure___moments__moments_normalized
      ~KerrArray.skimage__measure___polygon__approximate_polygon
      ~KerrArray.skimage__measure___polygon__subdivide_polygon
      ~KerrArray.skimage__measure___regionprops__regionprops
      ~KerrArray.skimage__measure___regionprops__regionprops_table
      ~KerrArray.skimage__measure___regionprops_utils__euler_number
      ~KerrArray.skimage__measure___regionprops_utils__perimeter
      ~KerrArray.skimage__measure___regionprops_utils__perimeter_crofton
      ~KerrArray.skimage__measure__block__block_reduce
      ~KerrArray.skimage__measure__entropy__shannon_entropy
      ~KerrArray.skimage__measure__fit__CircleModel
      ~KerrArray.skimage__measure__fit__EllipseModel
      ~KerrArray.skimage__measure__fit__LineModelND
      ~KerrArray.skimage__measure__fit__ransac
      ~KerrArray.skimage__measure__pnpoly__grid_points_in_poly
      ~KerrArray.skimage__measure__pnpoly__points_in_poly
      ~KerrArray.skimage__measure__profile__profile_line
      ~KerrArray.skimage__morphology___flood_fill__flood
      ~KerrArray.skimage__morphology___flood_fill__flood_fill
      ~KerrArray.skimage__morphology___skeletonize__medial_axis
      ~KerrArray.skimage__morphology___skeletonize__skeletonize
      ~KerrArray.skimage__morphology___skeletonize__thin
      ~KerrArray.skimage__morphology__binary__binary_closing
      ~KerrArray.skimage__morphology__binary__binary_dilation
      ~KerrArray.skimage__morphology__binary__binary_erosion
      ~KerrArray.skimage__morphology__binary__binary_opening
      ~KerrArray.skimage__morphology__convex_hull__convex_hull_image
      ~KerrArray.skimage__morphology__convex_hull__convex_hull_object
      ~KerrArray.skimage__morphology__extrema__h_maxima
      ~KerrArray.skimage__morphology__extrema__h_minima
      ~KerrArray.skimage__morphology__extrema__local_maxima
      ~KerrArray.skimage__morphology__extrema__local_minima
      ~KerrArray.skimage__morphology__footprints__ball
      ~KerrArray.skimage__morphology__footprints__cube
      ~KerrArray.skimage__morphology__footprints__diamond
      ~KerrArray.skimage__morphology__footprints__disk
      ~KerrArray.skimage__morphology__footprints__ellipse
      ~KerrArray.skimage__morphology__footprints__footprint_from_sequence
      ~KerrArray.skimage__morphology__footprints__footprint_rectangle
      ~KerrArray.skimage__morphology__footprints__mirror_footprint
      ~KerrArray.skimage__morphology__footprints__octagon
      ~KerrArray.skimage__morphology__footprints__octahedron
      ~KerrArray.skimage__morphology__footprints__pad_footprint
      ~KerrArray.skimage__morphology__footprints__rectangle
      ~KerrArray.skimage__morphology__footprints__square
      ~KerrArray.skimage__morphology__footprints__star
      ~KerrArray.skimage__morphology__gray__black_tophat
      ~KerrArray.skimage__morphology__gray__closing
      ~KerrArray.skimage__morphology__gray__dilation
      ~KerrArray.skimage__morphology__gray__erosion
      ~KerrArray.skimage__morphology__gray__opening
      ~KerrArray.skimage__morphology__gray__white_tophat
      ~KerrArray.skimage__morphology__grayreconstruct__reconstruction
      ~KerrArray.skimage__morphology__isotropic__isotropic_closing
      ~KerrArray.skimage__morphology__isotropic__isotropic_dilation
      ~KerrArray.skimage__morphology__isotropic__isotropic_erosion
      ~KerrArray.skimage__morphology__isotropic__isotropic_opening
      ~KerrArray.skimage__morphology__max_tree__area_closing
      ~KerrArray.skimage__morphology__max_tree__area_opening
      ~KerrArray.skimage__morphology__max_tree__diameter_closing
      ~KerrArray.skimage__morphology__max_tree__diameter_opening
      ~KerrArray.skimage__morphology__max_tree__max_tree
      ~KerrArray.skimage__morphology__max_tree__max_tree_local_maxima
      ~KerrArray.skimage__morphology__misc__remove_objects_by_distance
      ~KerrArray.skimage__morphology__misc__remove_small_holes
      ~KerrArray.skimage__morphology__misc__remove_small_objects
      ~KerrArray.skimage__restoration___cycle_spin__cycle_spin
      ~KerrArray.skimage__restoration___denoise__denoise_bilateral
      ~KerrArray.skimage__restoration___denoise__denoise_tv_bregman
      ~KerrArray.skimage__restoration___denoise__denoise_tv_chambolle
      ~KerrArray.skimage__restoration___denoise__denoise_wavelet
      ~KerrArray.skimage__restoration___denoise__estimate_sigma
      ~KerrArray.skimage__restoration___rolling_ball__ball_kernel
      ~KerrArray.skimage__restoration___rolling_ball__ellipsoid_kernel
      ~KerrArray.skimage__restoration___rolling_ball__rolling_ball
      ~KerrArray.skimage__restoration__deconvolution__richardson_lucy
      ~KerrArray.skimage__restoration__deconvolution__unsupervised_wiener
      ~KerrArray.skimage__restoration__deconvolution__wiener
      ~KerrArray.skimage__restoration__inpaint__inpaint_biharmonic
      ~KerrArray.skimage__restoration__j_invariant__calibrate_denoiser
      ~KerrArray.skimage__restoration__j_invariant__denoise_invariant
      ~KerrArray.skimage__restoration__non_local_means__denoise_nl_means
      ~KerrArray.skimage__restoration__unwrap__unwrap_phase
      ~KerrArray.skimage__segmentation___chan_vese__chan_vese
      ~KerrArray.skimage__segmentation___clear_border__clear_border
      ~KerrArray.skimage__segmentation___expand_labels__expand_labels
      ~KerrArray.skimage__segmentation___felzenszwalb__felzenszwalb
      ~KerrArray.skimage__segmentation___join__join_segmentations
      ~KerrArray.skimage__segmentation___join__relabel_sequential
      ~KerrArray.skimage__segmentation___quickshift__quickshift
      ~KerrArray.skimage__segmentation___watershed__watershed
      ~KerrArray.skimage__segmentation__active_contour_model__active_contour
      ~KerrArray.skimage__segmentation__boundaries__find_boundaries
      ~KerrArray.skimage__segmentation__boundaries__mark_boundaries
      ~KerrArray.skimage__segmentation__morphsnakes__checkerboard_level_set
      ~KerrArray.skimage__segmentation__morphsnakes__disk_level_set
      ~KerrArray.skimage__segmentation__morphsnakes__inverse_gaussian_gradient
      ~KerrArray.skimage__segmentation__morphsnakes__morphological_chan_vese
      ~KerrArray.skimage__segmentation__morphsnakes__morphological_geodesic_active_contour
      ~KerrArray.skimage__segmentation__random_walker_segmentation__random_walker
      ~KerrArray.skimage__segmentation__slic_superpixels__slic
      ~KerrArray.skimage__transform___geometric__AffineTransform
      ~KerrArray.skimage__transform___geometric__EssentialMatrixTransform
      ~KerrArray.skimage__transform___geometric__EuclideanTransform
      ~KerrArray.skimage__transform___geometric__FundamentalMatrixTransform
      ~KerrArray.skimage__transform___geometric__PiecewiseAffineTransform
      ~KerrArray.skimage__transform___geometric__PolynomialTransform
      ~KerrArray.skimage__transform___geometric__ProjectiveTransform
      ~KerrArray.skimage__transform___geometric__SimilarityTransform
      ~KerrArray.skimage__transform___geometric__estimate_transform
      ~KerrArray.skimage__transform___geometric__matrix_transform
      ~KerrArray.skimage__transform___thin_plate_splines__ThinPlateSplineTransform
      ~KerrArray.skimage__transform___warps__downscale_local_mean
      ~KerrArray.skimage__transform___warps__rescale
      ~KerrArray.skimage__transform___warps__resize
      ~KerrArray.skimage__transform___warps__resize_local_mean
      ~KerrArray.skimage__transform___warps__rotate
      ~KerrArray.skimage__transform___warps__swirl
      ~KerrArray.skimage__transform___warps__warp
      ~KerrArray.skimage__transform___warps__warp_coords
      ~KerrArray.skimage__transform___warps__warp_polar
      ~KerrArray.skimage__transform__finite_radon_transform__frt2
      ~KerrArray.skimage__transform__finite_radon_transform__ifrt2
      ~KerrArray.skimage__transform__hough_transform__hough_circle
      ~KerrArray.skimage__transform__hough_transform__hough_circle_peaks
      ~KerrArray.skimage__transform__hough_transform__hough_ellipse
      ~KerrArray.skimage__transform__hough_transform__hough_line
      ~KerrArray.skimage__transform__hough_transform__hough_line_peaks
      ~KerrArray.skimage__transform__hough_transform__probabilistic_hough_line
      ~KerrArray.skimage__transform__integral__integral_image
      ~KerrArray.skimage__transform__integral__integrate
      ~KerrArray.skimage__transform__pyramids__pyramid_expand
      ~KerrArray.skimage__transform__pyramids__pyramid_gaussian
      ~KerrArray.skimage__transform__pyramids__pyramid_laplacian
      ~KerrArray.skimage__transform__pyramids__pyramid_reduce
      ~KerrArray.skimage__transform__radon_transform__iradon
      ~KerrArray.skimage__transform__radon_transform__iradon_sart
      ~KerrArray.skimage__transform__radon_transform__order_angles_golden_ratio
      ~KerrArray.skimage__transform__radon_transform__radon
      ~KerrArray.skimage__util___invert__invert
      ~KerrArray.skimage__util___label__label_points
      ~KerrArray.skimage__util___map_array__map_array
      ~KerrArray.skimage__util___montage__montage
      ~KerrArray.skimage__util___regular_grid__regular_grid
      ~KerrArray.skimage__util___regular_grid__regular_seeds
      ~KerrArray.skimage__util___slice_along_axes__slice_along_axes
      ~KerrArray.skimage__util__apply_parallel__apply_parallel
      ~KerrArray.skimage__util__arraycrop__crop
      ~KerrArray.skimage__util__compare__compare_images
      ~KerrArray.skimage__util__dtype__dtype_limits
      ~KerrArray.skimage__util__dtype__img_as_bool
      ~KerrArray.skimage__util__dtype__img_as_float
      ~KerrArray.skimage__util__dtype__img_as_float32
      ~KerrArray.skimage__util__dtype__img_as_float64
      ~KerrArray.skimage__util__dtype__img_as_int
      ~KerrArray.skimage__util__dtype__img_as_ubyte
      ~KerrArray.skimage__util__dtype__img_as_uint
      ~KerrArray.skimage__util__lookfor__lookfor
      ~KerrArray.skimage__util__noise__random_noise
      ~KerrArray.skimage__util__shape__view_as_blocks
      ~KerrArray.skimage__util__shape__view_as_windows
      ~KerrArray.skimage__util__unique__unique_rows
      ~KerrArray.slic
      ~KerrArray.slice_along_axes
      ~KerrArray.sobel
      ~KerrArray.sobel_h
      ~KerrArray.sobel_v
      ~KerrArray.soften_mask
      ~KerrArray.sort
      ~KerrArray.span
      ~KerrArray.spline_filter
      ~KerrArray.spline_filter1d
      ~KerrArray.square
      ~KerrArray.squeeze
      ~KerrArray.standard_deviation
      ~KerrArray.star
      ~KerrArray.std
      ~KerrArray.structure_tensor
      ~KerrArray.structure_tensor_eigenvalues
      ~KerrArray.subdivide_polygon
      ~KerrArray.subtract_image
      ~KerrArray.subtract_mean
      ~KerrArray.subtract_mean_percentile
      ~KerrArray.sum
      ~KerrArray.sum_bilateral
      ~KerrArray.sum_labels
      ~KerrArray.sum_percentile
      ~KerrArray.swapaxes
      ~KerrArray.swirl
      ~KerrArray.take
      ~KerrArray.thin
      ~KerrArray.threshold
      ~KerrArray.threshold_isodata
      ~KerrArray.threshold_li
      ~KerrArray.threshold_local
      ~KerrArray.threshold_mean
      ~KerrArray.threshold_minimum
      ~KerrArray.threshold_minmax
      ~KerrArray.threshold_multiotsu
      ~KerrArray.threshold_niblack
      ~KerrArray.threshold_otsu
      ~KerrArray.threshold_percentile
      ~KerrArray.threshold_sauvola
      ~KerrArray.threshold_triangle
      ~KerrArray.threshold_yen
      ~KerrArray.to_device
      ~KerrArray.tobytes
      ~KerrArray.tofile
      ~KerrArray.toflex
      ~KerrArray.tolist
      ~KerrArray.torecords
      ~KerrArray.tostring
      ~KerrArray.trace
      ~KerrArray.translate
      ~KerrArray.translate_limits
      ~KerrArray.transpose
      ~KerrArray.try_all_threshold
      ~KerrArray.uniform_filter
      ~KerrArray.uniform_filter1d
      ~KerrArray.unique_rows
      ~KerrArray.unshare_mask
      ~KerrArray.unsharp_mask
      ~KerrArray.unsupervised_wiener
      ~KerrArray.unwrap_phase
      ~KerrArray.update
      ~KerrArray.use_plugin
      ~KerrArray.value_indices
      ~KerrArray.values
      ~KerrArray.var
      ~KerrArray.variance
      ~KerrArray.view
      ~KerrArray.view_as_blocks
      ~KerrArray.view_as_windows
      ~KerrArray.warp
      ~KerrArray.warp_coords
      ~KerrArray.warp_polar
      ~KerrArray.watershed
      ~KerrArray.watershed_ift
      ~KerrArray.white_tophat
      ~KerrArray.wiener
      ~KerrArray.window
      ~KerrArray.windowed_histogram
      ~KerrArray.xyz2lab
      ~KerrArray.xyz2luv
      ~KerrArray.xyz2rgb
      ~KerrArray.xyz_tristimulus_values
      ~KerrArray.ycbcr2rgb
      ~KerrArray.ydbdr2rgb
      ~KerrArray.yiq2rgb
      ~KerrArray.ypbpr2rgb
      ~KerrArray.yuv2rgb
      ~KerrArray.zoom

   .. rubric:: Attributes Documentation

   .. autoattribute:: CCW
   .. autoattribute:: CW
   .. autoattribute:: T
   .. autoattribute:: aspect
   .. autoattribute:: base
   .. autoattribute:: baseclass
   .. autoattribute:: centre
   .. autoattribute:: clone
   .. autoattribute:: ctypes
   .. autoattribute:: data
   .. autoattribute:: debug
   .. autoattribute:: device
   .. autoattribute:: draw
   .. autoattribute:: dtype
   .. autoattribute:: filename
   .. autoattribute:: fill_value
   .. autoattribute:: flags
   .. autoattribute:: flat
   .. autoattribute:: flip_h
   .. autoattribute:: flip_v
   .. autoattribute:: fmts
   .. autoattribute:: hardmask
   .. autoattribute:: imag
   .. autoattribute:: itemset
   .. autoattribute:: itemsize
   .. autoattribute:: mT
   .. autoattribute:: mask
   .. autoattribute:: max_box
   .. autoattribute:: metadata
   .. autoattribute:: nbytes
   .. autoattribute:: ndim
   .. autoattribute:: newbyteorder
   .. autoattribute:: real
   .. autoattribute:: recordmask
   .. autoattribute:: shape
   .. autoattribute:: sharedmask
   .. autoattribute:: size
   .. autoattribute:: strides
   .. autoattribute:: tesseractable
   .. autoattribute:: title

   .. rubric:: Methods Documentation

   .. automethod:: AffineTransform
   .. automethod:: BRIEF
   .. automethod:: CENSURE
   .. automethod:: Cascade
   .. automethod:: CircleModel
   .. automethod:: EllipseModel
   .. automethod:: EssentialMatrixTransform
   .. automethod:: EuclideanTransform
   .. automethod:: FundamentalMatrixTransform
   .. automethod:: ImageCollection
   .. automethod:: LPIFilter2D
   .. automethod:: LineModelND
   .. automethod:: MCP
   .. automethod:: MCP_Connect
   .. automethod:: MCP_Flexible
   .. automethod:: MCP_Geometric
   .. automethod:: MultiImage
   .. automethod:: ORB
   .. automethod:: PiecewiseAffineTransform
   .. automethod:: PolynomialTransform
   .. automethod:: ProjectiveTransform
   .. automethod:: RAG
   .. automethod:: SIFT
   .. automethod:: SimilarityTransform
   .. automethod:: Stoner__Image__imagefuncs__adjust_contrast
   .. automethod:: Stoner__Image__imagefuncs__align
   .. automethod:: Stoner__Image__imagefuncs__asarray
   .. automethod:: Stoner__Image__imagefuncs__asfloat
   .. automethod:: Stoner__Image__imagefuncs__asint
   .. automethod:: Stoner__Image__imagefuncs__clip_intensity
   .. automethod:: Stoner__Image__imagefuncs__clip_neg
   .. automethod:: Stoner__Image__imagefuncs__convert
   .. automethod:: Stoner__Image__imagefuncs__correct_drift
   .. automethod:: Stoner__Image__imagefuncs__crop
   .. automethod:: Stoner__Image__imagefuncs__denoise
   .. automethod:: Stoner__Image__imagefuncs__do_nothing
   .. automethod:: Stoner__Image__imagefuncs__dtype_limits
   .. automethod:: Stoner__Image__imagefuncs__fft
   .. automethod:: Stoner__Image__imagefuncs__filter_image
   .. automethod:: Stoner__Image__imagefuncs__gridimage
   .. automethod:: Stoner__Image__imagefuncs__hist
   .. automethod:: Stoner__Image__imagefuncs__imshow
   .. automethod:: Stoner__Image__imagefuncs__level_image
   .. automethod:: Stoner__Image__imagefuncs__normalise
   .. automethod:: Stoner__Image__imagefuncs__plot_histogram
   .. automethod:: Stoner__Image__imagefuncs__profile_line
   .. automethod:: Stoner__Image__imagefuncs__quantize
   .. automethod:: Stoner__Image__imagefuncs__radial_coordinates
   .. automethod:: Stoner__Image__imagefuncs__radial_profile
   .. automethod:: Stoner__Image__imagefuncs__remove_outliers
   .. automethod:: Stoner__Image__imagefuncs__rotate
   .. automethod:: Stoner__Image__imagefuncs__save
   .. automethod:: Stoner__Image__imagefuncs__save_npy
   .. automethod:: Stoner__Image__imagefuncs__save_png
   .. automethod:: Stoner__Image__imagefuncs__save_tiff
   .. automethod:: Stoner__Image__imagefuncs__sgolay2d
   .. automethod:: Stoner__Image__imagefuncs__span
   .. automethod:: Stoner__Image__imagefuncs__subtract_image
   .. automethod:: Stoner__Image__imagefuncs__threshold_minmax
   .. automethod:: Stoner__Image__imagefuncs__translate
   .. automethod:: Stoner__Image__imagefuncs__translate_limits
   .. automethod:: Stoner__Image__kerrfuncs__crop_text
   .. automethod:: Stoner__Image__kerrfuncs__defect_mask
   .. automethod:: Stoner__Image__kerrfuncs__defect_mask_subtract_image
   .. automethod:: Stoner__Image__kerrfuncs__float_and_croptext
   .. automethod:: Stoner__Image__kerrfuncs__get_scalebar
   .. automethod:: Stoner__Image__kerrfuncs__ocr_metadata
   .. automethod:: Stoner__Image__kerrfuncs__reduce_metadata
   .. automethod:: ThinPlateSplineTransform
   .. automethod:: active_contour
   .. automethod:: adjust_contrast
   .. automethod:: adjust_gamma
   .. automethod:: adjust_log
   .. automethod:: adjust_sigmoid
   .. automethod:: affine_transform
   .. automethod:: align
   .. automethod:: all
   .. automethod:: anom
   .. automethod:: any
   .. automethod:: apply_hysteresis_threshold
   .. automethod:: apply_parallel
   .. automethod:: approximate_polygon
   .. automethod:: area_closing
   .. automethod:: area_opening
   .. automethod:: argmax
   .. automethod:: argmin
   .. automethod:: argpartition
   .. automethod:: argsort
   .. automethod:: asarray
   .. automethod:: asfloat
   .. automethod:: asint
   .. automethod:: astype
   .. automethod:: autolevel
   .. automethod:: autolevel_percentile
   .. automethod:: ball
   .. automethod:: ball_kernel
   .. automethod:: binary_closing
   .. automethod:: binary_dilation
   .. automethod:: binary_erosion
   .. automethod:: binary_fill_holes
   .. automethod:: binary_hit_or_miss
   .. automethod:: binary_opening
   .. automethod:: binary_propagation
   .. automethod:: black_tophat
   .. automethod:: blob_dog
   .. automethod:: blob_doh
   .. automethod:: blob_log
   .. automethod:: block_reduce
   .. automethod:: blur_effect
   .. automethod:: butterworth
   .. automethod:: byteswap
   .. automethod:: calibrate_denoiser
   .. automethod:: call_plugin
   .. automethod:: canny
   .. automethod:: center_of_mass
   .. automethod:: central_pixel
   .. automethod:: centroid
   .. automethod:: chan_vese
   .. automethod:: checkerboard_level_set
   .. automethod:: choose
   .. automethod:: clear
   .. automethod:: clear_border
   .. automethod:: clip
   .. automethod:: clip_intensity
   .. automethod:: clip_neg
   .. automethod:: closing
   .. automethod:: combine_stains
   .. automethod:: compare_images
   .. automethod:: compress
   .. automethod:: compressed
   .. automethod:: concatenate_images
   .. automethod:: conj
   .. automethod:: conjugate
   .. automethod:: convert
   .. automethod:: convert_colorspace
   .. automethod:: convex_hull_image
   .. automethod:: convex_hull_object
   .. automethod:: convolve
   .. automethod:: convolve1d
   .. automethod:: copy
   .. automethod:: corner_fast
   .. automethod:: corner_foerstner
   .. automethod:: corner_harris
   .. automethod:: corner_kitchen_rosenfeld
   .. automethod:: corner_moravec
   .. automethod:: corner_orientations
   .. automethod:: corner_peaks
   .. automethod:: corner_shi_tomasi
   .. automethod:: corner_subpix
   .. automethod:: correct_drift
   .. automethod:: correlate
   .. automethod:: correlate1d
   .. automethod:: correlate_sparse
   .. automethod:: count
   .. automethod:: crop
   .. automethod:: crop_text
   .. automethod:: cube
   .. automethod:: cumprod
   .. automethod:: cumsum
   .. automethod:: cumulative_distribution
   .. automethod:: cut_normalized
   .. automethod:: cut_threshold
   .. automethod:: cycle_spin
   .. automethod:: daisy
   .. automethod:: defect_mask
   .. automethod:: defect_mask_subtract_image
   .. automethod:: deltaE_cie76
   .. automethod:: deltaE_ciede2000
   .. automethod:: deltaE_ciede94
   .. automethod:: deltaE_cmc
   .. automethod:: denoise
   .. automethod:: denoise_bilateral
   .. automethod:: denoise_invariant
   .. automethod:: denoise_nl_means
   .. automethod:: denoise_tv_bregman
   .. automethod:: denoise_tv_chambolle
   .. automethod:: denoise_wavelet
   .. automethod:: diagonal
   .. automethod:: diameter_closing
   .. automethod:: diameter_opening
   .. automethod:: diamond
   .. automethod:: difference_of_gaussians
   .. automethod:: dilation
   .. automethod:: disk
   .. automethod:: disk_level_set
   .. automethod:: distance_transform_bf
   .. automethod:: distance_transform_cdt
   .. automethod:: distance_transform_edt
   .. automethod:: do_nothing
   .. automethod:: dot
   .. automethod:: downscale_local_mean
   .. automethod:: draw_haar_like_feature
   .. automethod:: draw_multiblock_lbp
   .. automethod:: dtype_limits
   .. automethod:: dump
   .. automethod:: dumps
   .. automethod:: ellipse
   .. automethod:: ellipsoid_kernel
   .. automethod:: enhance_contrast
   .. automethod:: enhance_contrast_percentile
   .. automethod:: entropy
   .. automethod:: equalize
   .. automethod:: equalize_adapthist
   .. automethod:: equalize_hist
   .. automethod:: erosion
   .. automethod:: estimate_sigma
   .. automethod:: estimate_transform
   .. automethod:: euler_number
   .. automethod:: expand_labels
   .. automethod:: extrema
   .. automethod:: farid
   .. automethod:: farid_h
   .. automethod:: farid_v
   .. automethod:: felzenszwalb
   .. automethod:: fft
   .. automethod:: fill
   .. automethod:: filled
   .. automethod:: filter_forward
   .. automethod:: filter_image
   .. automethod:: filter_inverse
   .. automethod:: find_available_plugins
   .. automethod:: find_boundaries
   .. automethod:: find_contours
   .. automethod:: find_objects
   .. automethod:: fisher_vector
   .. automethod:: flatten
   .. automethod:: float_and_croptext
   .. automethod:: flood
   .. automethod:: flood_fill
   .. automethod:: footprint_from_sequence
   .. automethod:: footprint_rectangle
   .. automethod:: fourier_ellipsoid
   .. automethod:: fourier_gaussian
   .. automethod:: fourier_shift
   .. automethod:: fourier_uniform
   .. automethod:: frangi
   .. automethod:: frt2
   .. automethod:: gabor
   .. automethod:: gabor_kernel
   .. automethod:: gaussian
   .. automethod:: gaussian_filter
   .. automethod:: gaussian_filter1d
   .. automethod:: gaussian_gradient_magnitude
   .. automethod:: gaussian_laplace
   .. automethod:: generate_binary_structure
   .. automethod:: generic_filter
   .. automethod:: generic_filter1d
   .. automethod:: generic_gradient_magnitude
   .. automethod:: generic_laplace
   .. automethod:: geometric_mean
   .. automethod:: geometric_transform
   .. automethod:: get
   .. automethod:: get_fill_value
   .. automethod:: get_imag
   .. automethod:: get_real
   .. automethod:: get_scalebar
   .. automethod:: getfield
   .. automethod:: gradient
   .. automethod:: gradient_percentile
   .. automethod:: gray2rgb
   .. automethod:: gray2rgba
   .. automethod:: graycomatrix
   .. automethod:: graycoprops
   .. automethod:: grey_closing
   .. automethod:: grey_dilation
   .. automethod:: grey_erosion
   .. automethod:: grey_opening
   .. automethod:: grid_points_in_poly
   .. automethod:: gridimage
   .. automethod:: h_maxima
   .. automethod:: h_minima
   .. automethod:: haar_like_feature
   .. automethod:: haar_like_feature_coord
   .. automethod:: harden_mask
   .. automethod:: hed2rgb
   .. automethod:: hessian
   .. automethod:: hessian_matrix
   .. automethod:: hessian_matrix_det
   .. automethod:: hessian_matrix_eigvals
   .. automethod:: hist
   .. automethod:: histogram
   .. automethod:: hog
   .. automethod:: hough_circle
   .. automethod:: hough_circle_peaks
   .. automethod:: hough_ellipse
   .. automethod:: hough_line
   .. automethod:: hough_line_peaks
   .. automethod:: hsv2rgb
   .. automethod:: ids
   .. automethod:: ifrt2
   .. automethod:: img_as_bool
   .. automethod:: img_as_float
   .. automethod:: img_as_float32
   .. automethod:: img_as_float64
   .. automethod:: img_as_int
   .. automethod:: img_as_ubyte
   .. automethod:: img_as_uint
   .. automethod:: imread
   .. automethod:: imread_collection
   .. automethod:: imread_collection_wrapper
   .. automethod:: imsave
   .. automethod:: imshow
   .. automethod:: imshow_collection
   .. automethod:: inertia_tensor
   .. automethod:: inertia_tensor_eigvals
   .. automethod:: inpaint_biharmonic
   .. automethod:: integral_image
   .. automethod:: integrate
   .. automethod:: intersection_coeff
   .. automethod:: inverse_gaussian_gradient
   .. automethod:: invert
   .. automethod:: iradon
   .. automethod:: iradon_sart
   .. automethod:: is_low_contrast
   .. automethod:: iscontiguous
   .. automethod:: isotropic_closing
   .. automethod:: isotropic_dilation
   .. automethod:: isotropic_erosion
   .. automethod:: isotropic_opening
   .. automethod:: item
   .. automethod:: items
   .. automethod:: iterate_structure
   .. automethod:: join_segmentations
   .. automethod:: keys
   .. automethod:: lab2lch
   .. automethod:: lab2rgb
   .. automethod:: lab2xyz
   .. automethod:: label
   .. automethod:: label2rgb
   .. automethod:: label_points
   .. automethod:: labeled_comprehension
   .. automethod:: laplace
   .. automethod:: lch2lab
   .. automethod:: learn_gmm
   .. automethod:: level_image
   .. automethod:: load_sift
   .. automethod:: load_surf
   .. automethod:: local_binary_pattern
   .. automethod:: local_maxima
   .. automethod:: local_minima
   .. automethod:: lookfor
   .. automethod:: luv2rgb
   .. automethod:: luv2xyz
   .. automethod:: majority
   .. automethod:: manders_coloc_coeff
   .. automethod:: manders_overlap_coeff
   .. automethod:: map_array
   .. automethod:: map_coordinates
   .. automethod:: marching_cubes
   .. automethod:: mark_boundaries
   .. automethod:: match_descriptors
   .. automethod:: match_histograms
   .. automethod:: match_template
   .. automethod:: matrix_transform
   .. automethod:: max
   .. automethod:: max_tree
   .. automethod:: max_tree_local_maxima
   .. automethod:: maximum
   .. automethod:: maximum_filter
   .. automethod:: maximum_filter1d
   .. automethod:: maximum_position
   .. automethod:: mean
   .. automethod:: mean_bilateral
   .. automethod:: mean_percentile
   .. automethod:: medial_axis
   .. automethod:: median
   .. automethod:: median_filter
   .. automethod:: meijering
   .. automethod:: merge_hierarchical
   .. automethod:: mesh_surface_area
   .. automethod:: min
   .. automethod:: minimum
   .. automethod:: minimum_filter
   .. automethod:: minimum_filter1d
   .. automethod:: minimum_position
   .. automethod:: mirror_footprint
   .. automethod:: modal
   .. automethod:: moments
   .. automethod:: moments_central
   .. automethod:: moments_coords
   .. automethod:: moments_coords_central
   .. automethod:: moments_hu
   .. automethod:: moments_normalized
   .. automethod:: montage
   .. automethod:: morphological_chan_vese
   .. automethod:: morphological_geodesic_active_contour
   .. automethod:: morphological_gradient
   .. automethod:: morphological_laplace
   .. automethod:: multiblock_lbp
   .. automethod:: multiscale_basic_features
   .. automethod:: noise_filter
   .. automethod:: nonzero
   .. automethod:: normalise
   .. automethod:: ocr_metadata
   .. automethod:: octagon
   .. automethod:: octahedron
   .. automethod:: opening
   .. automethod:: order_angles_golden_ratio
   .. automethod:: otsu
   .. automethod:: pad_footprint
   .. automethod:: partition
   .. automethod:: peak_local_max
   .. automethod:: pearson_corr_coeff
   .. automethod:: percentile
   .. automethod:: percentile_filter
   .. automethod:: perimeter
   .. automethod:: perimeter_crofton
   .. automethod:: pixel_graph
   .. automethod:: plot_histogram
   .. automethod:: plot_matched_features
   .. automethod:: plugin_info
   .. automethod:: plugin_order
   .. automethod:: points_in_poly
   .. automethod:: pop
   .. automethod:: pop_bilateral
   .. automethod:: pop_percentile
   .. automethod:: popitem
   .. automethod:: prewitt
   .. automethod:: prewitt_h
   .. automethod:: prewitt_v
   .. automethod:: probabilistic_hough_line
   .. automethod:: prod
   .. automethod:: product
   .. automethod:: profile_line
   .. automethod:: ptp
   .. automethod:: push
   .. automethod:: put
   .. automethod:: pyramid_expand
   .. automethod:: pyramid_gaussian
   .. automethod:: pyramid_laplacian
   .. automethod:: pyramid_reduce
   .. automethod:: quantize
   .. automethod:: quickshift
   .. automethod:: radial_coordinates
   .. automethod:: radial_profile
   .. automethod:: radon
   .. automethod:: rag_boundary
   .. automethod:: rag_mean_color
   .. automethod:: random_noise
   .. automethod:: random_walker
   .. automethod:: rank_filter
   .. automethod:: rank_order
   .. automethod:: ransac
   .. automethod:: ravel
   .. automethod:: reconstruction
   .. automethod:: rectangle
   .. automethod:: reduce_metadata
   .. automethod:: regionprops
   .. automethod:: regionprops_table
   .. automethod:: regular_grid
   .. automethod:: regular_seeds
   .. automethod:: relabel_sequential
   .. automethod:: remove_objects_by_distance
   .. automethod:: remove_outliers
   .. automethod:: remove_small_holes
   .. automethod:: remove_small_objects
   .. automethod:: repeat
   .. automethod:: rescale
   .. automethod:: rescale_intensity
   .. automethod:: reset_plugins
   .. automethod:: reshape
   .. automethod:: resize
   .. automethod:: resize_local_mean
   .. automethod:: rgb2gray
   .. automethod:: rgb2hed
   .. automethod:: rgb2hsv
   .. automethod:: rgb2lab
   .. automethod:: rgb2luv
   .. automethod:: rgb2rgbcie
   .. automethod:: rgb2xyz
   .. automethod:: rgb2ycbcr
   .. automethod:: rgb2ydbdr
   .. automethod:: rgb2yiq
   .. automethod:: rgb2ypbpr
   .. automethod:: rgb2yuv
   .. automethod:: rgba2rgb
   .. automethod:: rgbcie2rgb
   .. automethod:: richardson_lucy
   .. automethod:: roberts
   .. automethod:: roberts_neg_diag
   .. automethod:: roberts_pos_diag
   .. automethod:: rolling_ball
   .. automethod:: rotate
   .. automethod:: round
   .. automethod:: route_through_array
   .. automethod:: sato
   .. automethod:: save
   .. automethod:: save_npy
   .. automethod:: save_png
   .. automethod:: save_tiff
   .. automethod:: scharr
   .. automethod:: scharr_h
   .. automethod:: scharr_v
   .. automethod:: scipy__ndimage___filters__convolve
   .. automethod:: scipy__ndimage___filters__convolve1d
   .. automethod:: scipy__ndimage___filters__correlate
   .. automethod:: scipy__ndimage___filters__correlate1d
   .. automethod:: scipy__ndimage___filters__gaussian_filter
   .. automethod:: scipy__ndimage___filters__gaussian_filter1d
   .. automethod:: scipy__ndimage___filters__gaussian_gradient_magnitude
   .. automethod:: scipy__ndimage___filters__gaussian_laplace
   .. automethod:: scipy__ndimage___filters__generic_filter
   .. automethod:: scipy__ndimage___filters__generic_filter1d
   .. automethod:: scipy__ndimage___filters__generic_gradient_magnitude
   .. automethod:: scipy__ndimage___filters__generic_laplace
   .. automethod:: scipy__ndimage___filters__laplace
   .. automethod:: scipy__ndimage___filters__maximum_filter
   .. automethod:: scipy__ndimage___filters__maximum_filter1d
   .. automethod:: scipy__ndimage___filters__median_filter
   .. automethod:: scipy__ndimage___filters__minimum_filter
   .. automethod:: scipy__ndimage___filters__minimum_filter1d
   .. automethod:: scipy__ndimage___filters__percentile_filter
   .. automethod:: scipy__ndimage___filters__prewitt
   .. automethod:: scipy__ndimage___filters__rank_filter
   .. automethod:: scipy__ndimage___filters__sobel
   .. automethod:: scipy__ndimage___filters__uniform_filter
   .. automethod:: scipy__ndimage___filters__uniform_filter1d
   .. automethod:: scipy__ndimage___fourier__fourier_ellipsoid
   .. automethod:: scipy__ndimage___fourier__fourier_gaussian
   .. automethod:: scipy__ndimage___fourier__fourier_shift
   .. automethod:: scipy__ndimage___fourier__fourier_uniform
   .. automethod:: scipy__ndimage___interpolation__affine_transform
   .. automethod:: scipy__ndimage___interpolation__geometric_transform
   .. automethod:: scipy__ndimage___interpolation__map_coordinates
   .. automethod:: scipy__ndimage___interpolation__rotate
   .. automethod:: scipy__ndimage___interpolation__shift
   .. automethod:: scipy__ndimage___interpolation__spline_filter
   .. automethod:: scipy__ndimage___interpolation__spline_filter1d
   .. automethod:: scipy__ndimage___interpolation__zoom
   .. automethod:: scipy__ndimage___measurements__center_of_mass
   .. automethod:: scipy__ndimage___measurements__extrema
   .. automethod:: scipy__ndimage___measurements__find_objects
   .. automethod:: scipy__ndimage___measurements__histogram
   .. automethod:: scipy__ndimage___measurements__label
   .. automethod:: scipy__ndimage___measurements__labeled_comprehension
   .. automethod:: scipy__ndimage___measurements__maximum
   .. automethod:: scipy__ndimage___measurements__maximum_position
   .. automethod:: scipy__ndimage___measurements__mean
   .. automethod:: scipy__ndimage___measurements__median
   .. automethod:: scipy__ndimage___measurements__minimum
   .. automethod:: scipy__ndimage___measurements__minimum_position
   .. automethod:: scipy__ndimage___measurements__standard_deviation
   .. automethod:: scipy__ndimage___measurements__sum
   .. automethod:: scipy__ndimage___measurements__sum_labels
   .. automethod:: scipy__ndimage___measurements__value_indices
   .. automethod:: scipy__ndimage___measurements__variance
   .. automethod:: scipy__ndimage___measurements__watershed_ift
   .. automethod:: scipy__ndimage___morphology__binary_closing
   .. automethod:: scipy__ndimage___morphology__binary_dilation
   .. automethod:: scipy__ndimage___morphology__binary_erosion
   .. automethod:: scipy__ndimage___morphology__binary_fill_holes
   .. automethod:: scipy__ndimage___morphology__binary_hit_or_miss
   .. automethod:: scipy__ndimage___morphology__binary_opening
   .. automethod:: scipy__ndimage___morphology__binary_propagation
   .. automethod:: scipy__ndimage___morphology__black_tophat
   .. automethod:: scipy__ndimage___morphology__distance_transform_bf
   .. automethod:: scipy__ndimage___morphology__distance_transform_cdt
   .. automethod:: scipy__ndimage___morphology__distance_transform_edt
   .. automethod:: scipy__ndimage___morphology__generate_binary_structure
   .. automethod:: scipy__ndimage___morphology__grey_closing
   .. automethod:: scipy__ndimage___morphology__grey_dilation
   .. automethod:: scipy__ndimage___morphology__grey_erosion
   .. automethod:: scipy__ndimage___morphology__grey_opening
   .. automethod:: scipy__ndimage___morphology__iterate_structure
   .. automethod:: scipy__ndimage___morphology__morphological_gradient
   .. automethod:: scipy__ndimage___morphology__morphological_laplace
   .. automethod:: scipy__ndimage___morphology__white_tophat
   .. automethod:: searchsorted
   .. automethod:: separate_stains
   .. automethod:: set_fill_value
   .. automethod:: setdefault
   .. automethod:: setfield
   .. automethod:: setflags
   .. automethod:: sgolay2d
   .. automethod:: shannon_entropy
   .. automethod:: shape_index
   .. automethod:: shift
   .. automethod:: shortest_path
   .. automethod:: show
   .. automethod:: show_rag
   .. automethod:: shrink_mask
   .. automethod:: skeletonize
   .. automethod:: skimage___shared__filters__gaussian
   .. automethod:: skimage__color__colorconv__combine_stains
   .. automethod:: skimage__color__colorconv__convert_colorspace
   .. automethod:: skimage__color__colorconv__gray2rgb
   .. automethod:: skimage__color__colorconv__gray2rgba
   .. automethod:: skimage__color__colorconv__hed2rgb
   .. automethod:: skimage__color__colorconv__hsv2rgb
   .. automethod:: skimage__color__colorconv__lab2lch
   .. automethod:: skimage__color__colorconv__lab2rgb
   .. automethod:: skimage__color__colorconv__lab2xyz
   .. automethod:: skimage__color__colorconv__lch2lab
   .. automethod:: skimage__color__colorconv__luv2rgb
   .. automethod:: skimage__color__colorconv__luv2xyz
   .. automethod:: skimage__color__colorconv__rgb2gray
   .. automethod:: skimage__color__colorconv__rgb2hed
   .. automethod:: skimage__color__colorconv__rgb2hsv
   .. automethod:: skimage__color__colorconv__rgb2lab
   .. automethod:: skimage__color__colorconv__rgb2luv
   .. automethod:: skimage__color__colorconv__rgb2rgbcie
   .. automethod:: skimage__color__colorconv__rgb2xyz
   .. automethod:: skimage__color__colorconv__rgb2ycbcr
   .. automethod:: skimage__color__colorconv__rgb2ydbdr
   .. automethod:: skimage__color__colorconv__rgb2yiq
   .. automethod:: skimage__color__colorconv__rgb2ypbpr
   .. automethod:: skimage__color__colorconv__rgb2yuv
   .. automethod:: skimage__color__colorconv__rgba2rgb
   .. automethod:: skimage__color__colorconv__rgbcie2rgb
   .. automethod:: skimage__color__colorconv__separate_stains
   .. automethod:: skimage__color__colorconv__xyz2lab
   .. automethod:: skimage__color__colorconv__xyz2luv
   .. automethod:: skimage__color__colorconv__xyz2rgb
   .. automethod:: skimage__color__colorconv__xyz_tristimulus_values
   .. automethod:: skimage__color__colorconv__ycbcr2rgb
   .. automethod:: skimage__color__colorconv__ydbdr2rgb
   .. automethod:: skimage__color__colorconv__yiq2rgb
   .. automethod:: skimage__color__colorconv__ypbpr2rgb
   .. automethod:: skimage__color__colorconv__yuv2rgb
   .. automethod:: skimage__color__colorlabel__label2rgb
   .. automethod:: skimage__color__delta_e__deltaE_cie76
   .. automethod:: skimage__color__delta_e__deltaE_ciede2000
   .. automethod:: skimage__color__delta_e__deltaE_ciede94
   .. automethod:: skimage__color__delta_e__deltaE_cmc
   .. automethod:: skimage__exposure___adapthist__equalize_adapthist
   .. automethod:: skimage__exposure__exposure__adjust_gamma
   .. automethod:: skimage__exposure__exposure__adjust_log
   .. automethod:: skimage__exposure__exposure__adjust_sigmoid
   .. automethod:: skimage__exposure__exposure__cumulative_distribution
   .. automethod:: skimage__exposure__exposure__equalize_hist
   .. automethod:: skimage__exposure__exposure__histogram
   .. automethod:: skimage__exposure__exposure__is_low_contrast
   .. automethod:: skimage__exposure__exposure__rescale_intensity
   .. automethod:: skimage__exposure__histogram_matching__match_histograms
   .. automethod:: skimage__feature___basic_features__multiscale_basic_features
   .. automethod:: skimage__feature___canny__canny
   .. automethod:: skimage__feature___cascade__Cascade
   .. automethod:: skimage__feature___daisy__daisy
   .. automethod:: skimage__feature___fisher_vector__fisher_vector
   .. automethod:: skimage__feature___fisher_vector__learn_gmm
   .. automethod:: skimage__feature___hog__hog
   .. automethod:: skimage__feature__blob__blob_dog
   .. automethod:: skimage__feature__blob__blob_doh
   .. automethod:: skimage__feature__blob__blob_log
   .. automethod:: skimage__feature__brief__BRIEF
   .. automethod:: skimage__feature__censure__CENSURE
   .. automethod:: skimage__feature__corner__corner_fast
   .. automethod:: skimage__feature__corner__corner_foerstner
   .. automethod:: skimage__feature__corner__corner_harris
   .. automethod:: skimage__feature__corner__corner_kitchen_rosenfeld
   .. automethod:: skimage__feature__corner__corner_moravec
   .. automethod:: skimage__feature__corner__corner_orientations
   .. automethod:: skimage__feature__corner__corner_peaks
   .. automethod:: skimage__feature__corner__corner_shi_tomasi
   .. automethod:: skimage__feature__corner__corner_subpix
   .. automethod:: skimage__feature__corner__hessian_matrix
   .. automethod:: skimage__feature__corner__hessian_matrix_det
   .. automethod:: skimage__feature__corner__hessian_matrix_eigvals
   .. automethod:: skimage__feature__corner__shape_index
   .. automethod:: skimage__feature__corner__structure_tensor
   .. automethod:: skimage__feature__corner__structure_tensor_eigenvalues
   .. automethod:: skimage__feature__haar__draw_haar_like_feature
   .. automethod:: skimage__feature__haar__haar_like_feature
   .. automethod:: skimage__feature__haar__haar_like_feature_coord
   .. automethod:: skimage__feature__match__match_descriptors
   .. automethod:: skimage__feature__orb__ORB
   .. automethod:: skimage__feature__peak__peak_local_max
   .. automethod:: skimage__feature__sift__SIFT
   .. automethod:: skimage__feature__template__match_template
   .. automethod:: skimage__feature__texture__draw_multiblock_lbp
   .. automethod:: skimage__feature__texture__graycomatrix
   .. automethod:: skimage__feature__texture__graycoprops
   .. automethod:: skimage__feature__texture__local_binary_pattern
   .. automethod:: skimage__feature__texture__multiblock_lbp
   .. automethod:: skimage__feature__util__plot_matched_features
   .. automethod:: skimage__filters___fft_based__butterworth
   .. automethod:: skimage__filters___gabor__gabor
   .. automethod:: skimage__filters___gabor__gabor_kernel
   .. automethod:: skimage__filters___gaussian__difference_of_gaussians
   .. automethod:: skimage__filters___median__median
   .. automethod:: skimage__filters___rank_order__rank_order
   .. automethod:: skimage__filters___sparse__correlate_sparse
   .. automethod:: skimage__filters___unsharp_mask__unsharp_mask
   .. automethod:: skimage__filters___window__window
   .. automethod:: skimage__filters__edges__farid
   .. automethod:: skimage__filters__edges__farid_h
   .. automethod:: skimage__filters__edges__farid_v
   .. automethod:: skimage__filters__edges__laplace
   .. automethod:: skimage__filters__edges__prewitt
   .. automethod:: skimage__filters__edges__prewitt_h
   .. automethod:: skimage__filters__edges__prewitt_v
   .. automethod:: skimage__filters__edges__roberts
   .. automethod:: skimage__filters__edges__roberts_neg_diag
   .. automethod:: skimage__filters__edges__roberts_pos_diag
   .. automethod:: skimage__filters__edges__scharr
   .. automethod:: skimage__filters__edges__scharr_h
   .. automethod:: skimage__filters__edges__scharr_v
   .. automethod:: skimage__filters__edges__sobel
   .. automethod:: skimage__filters__edges__sobel_h
   .. automethod:: skimage__filters__edges__sobel_v
   .. automethod:: skimage__filters__lpi_filter__LPIFilter2D
   .. automethod:: skimage__filters__lpi_filter__filter_forward
   .. automethod:: skimage__filters__lpi_filter__filter_inverse
   .. automethod:: skimage__filters__lpi_filter__wiener
   .. automethod:: skimage__filters__rank___percentile__autolevel_percentile
   .. automethod:: skimage__filters__rank___percentile__enhance_contrast_percentile
   .. automethod:: skimage__filters__rank___percentile__gradient_percentile
   .. automethod:: skimage__filters__rank___percentile__mean_percentile
   .. automethod:: skimage__filters__rank___percentile__percentile
   .. automethod:: skimage__filters__rank___percentile__pop_percentile
   .. automethod:: skimage__filters__rank___percentile__subtract_mean_percentile
   .. automethod:: skimage__filters__rank___percentile__sum_percentile
   .. automethod:: skimage__filters__rank___percentile__threshold_percentile
   .. automethod:: skimage__filters__rank__bilateral__mean_bilateral
   .. automethod:: skimage__filters__rank__bilateral__pop_bilateral
   .. automethod:: skimage__filters__rank__bilateral__sum_bilateral
   .. automethod:: skimage__filters__rank__generic__autolevel
   .. automethod:: skimage__filters__rank__generic__enhance_contrast
   .. automethod:: skimage__filters__rank__generic__entropy
   .. automethod:: skimage__filters__rank__generic__equalize
   .. automethod:: skimage__filters__rank__generic__geometric_mean
   .. automethod:: skimage__filters__rank__generic__gradient
   .. automethod:: skimage__filters__rank__generic__majority
   .. automethod:: skimage__filters__rank__generic__maximum
   .. automethod:: skimage__filters__rank__generic__mean
   .. automethod:: skimage__filters__rank__generic__median
   .. automethod:: skimage__filters__rank__generic__minimum
   .. automethod:: skimage__filters__rank__generic__modal
   .. automethod:: skimage__filters__rank__generic__noise_filter
   .. automethod:: skimage__filters__rank__generic__otsu
   .. automethod:: skimage__filters__rank__generic__pop
   .. automethod:: skimage__filters__rank__generic__subtract_mean
   .. automethod:: skimage__filters__rank__generic__sum
   .. automethod:: skimage__filters__rank__generic__threshold
   .. automethod:: skimage__filters__rank__generic__windowed_histogram
   .. automethod:: skimage__filters__ridges__frangi
   .. automethod:: skimage__filters__ridges__hessian
   .. automethod:: skimage__filters__ridges__meijering
   .. automethod:: skimage__filters__ridges__sato
   .. automethod:: skimage__filters__thresholding__apply_hysteresis_threshold
   .. automethod:: skimage__filters__thresholding__threshold_isodata
   .. automethod:: skimage__filters__thresholding__threshold_li
   .. automethod:: skimage__filters__thresholding__threshold_local
   .. automethod:: skimage__filters__thresholding__threshold_mean
   .. automethod:: skimage__filters__thresholding__threshold_minimum
   .. automethod:: skimage__filters__thresholding__threshold_multiotsu
   .. automethod:: skimage__filters__thresholding__threshold_niblack
   .. automethod:: skimage__filters__thresholding__threshold_otsu
   .. automethod:: skimage__filters__thresholding__threshold_sauvola
   .. automethod:: skimage__filters__thresholding__threshold_triangle
   .. automethod:: skimage__filters__thresholding__threshold_yen
   .. automethod:: skimage__filters__thresholding__try_all_threshold
   .. automethod:: skimage__graph___graph__central_pixel
   .. automethod:: skimage__graph___graph__pixel_graph
   .. automethod:: skimage__graph___graph_cut__cut_normalized
   .. automethod:: skimage__graph___graph_cut__cut_threshold
   .. automethod:: skimage__graph___graph_merge__merge_hierarchical
   .. automethod:: skimage__graph___mcp__MCP
   .. automethod:: skimage__graph___mcp__MCP_Connect
   .. automethod:: skimage__graph___mcp__MCP_Flexible
   .. automethod:: skimage__graph___mcp__MCP_Geometric
   .. automethod:: skimage__graph___rag__RAG
   .. automethod:: skimage__graph___rag__rag_boundary
   .. automethod:: skimage__graph___rag__rag_mean_color
   .. automethod:: skimage__graph___rag__show_rag
   .. automethod:: skimage__graph__mcp__route_through_array
   .. automethod:: skimage__graph__spath__shortest_path
   .. automethod:: skimage__io___image_stack__pop
   .. automethod:: skimage__io___image_stack__push
   .. automethod:: skimage__io___io__imread
   .. automethod:: skimage__io___io__imread_collection
   .. automethod:: skimage__io___io__imsave
   .. automethod:: skimage__io___io__imshow
   .. automethod:: skimage__io___io__imshow_collection
   .. automethod:: skimage__io___io__show
   .. automethod:: skimage__io__collection__ImageCollection
   .. automethod:: skimage__io__collection__MultiImage
   .. automethod:: skimage__io__collection__concatenate_images
   .. automethod:: skimage__io__collection__imread_collection_wrapper
   .. automethod:: skimage__io__manage_plugins__call_plugin
   .. automethod:: skimage__io__manage_plugins__find_available_plugins
   .. automethod:: skimage__io__manage_plugins__plugin_info
   .. automethod:: skimage__io__manage_plugins__plugin_order
   .. automethod:: skimage__io__manage_plugins__reset_plugins
   .. automethod:: skimage__io__manage_plugins__use_plugin
   .. automethod:: skimage__io__sift__load_sift
   .. automethod:: skimage__io__sift__load_surf
   .. automethod:: skimage__measure___blur_effect__blur_effect
   .. automethod:: skimage__measure___colocalization__intersection_coeff
   .. automethod:: skimage__measure___colocalization__manders_coloc_coeff
   .. automethod:: skimage__measure___colocalization__manders_overlap_coeff
   .. automethod:: skimage__measure___colocalization__pearson_corr_coeff
   .. automethod:: skimage__measure___find_contours__find_contours
   .. automethod:: skimage__measure___label__label
   .. automethod:: skimage__measure___marching_cubes_lewiner__marching_cubes
   .. automethod:: skimage__measure___marching_cubes_lewiner__mesh_surface_area
   .. automethod:: skimage__measure___moments__centroid
   .. automethod:: skimage__measure___moments__inertia_tensor
   .. automethod:: skimage__measure___moments__inertia_tensor_eigvals
   .. automethod:: skimage__measure___moments__moments
   .. automethod:: skimage__measure___moments__moments_central
   .. automethod:: skimage__measure___moments__moments_coords
   .. automethod:: skimage__measure___moments__moments_coords_central
   .. automethod:: skimage__measure___moments__moments_hu
   .. automethod:: skimage__measure___moments__moments_normalized
   .. automethod:: skimage__measure___polygon__approximate_polygon
   .. automethod:: skimage__measure___polygon__subdivide_polygon
   .. automethod:: skimage__measure___regionprops__regionprops
   .. automethod:: skimage__measure___regionprops__regionprops_table
   .. automethod:: skimage__measure___regionprops_utils__euler_number
   .. automethod:: skimage__measure___regionprops_utils__perimeter
   .. automethod:: skimage__measure___regionprops_utils__perimeter_crofton
   .. automethod:: skimage__measure__block__block_reduce
   .. automethod:: skimage__measure__entropy__shannon_entropy
   .. automethod:: skimage__measure__fit__CircleModel
   .. automethod:: skimage__measure__fit__EllipseModel
   .. automethod:: skimage__measure__fit__LineModelND
   .. automethod:: skimage__measure__fit__ransac
   .. automethod:: skimage__measure__pnpoly__grid_points_in_poly
   .. automethod:: skimage__measure__pnpoly__points_in_poly
   .. automethod:: skimage__measure__profile__profile_line
   .. automethod:: skimage__morphology___flood_fill__flood
   .. automethod:: skimage__morphology___flood_fill__flood_fill
   .. automethod:: skimage__morphology___skeletonize__medial_axis
   .. automethod:: skimage__morphology___skeletonize__skeletonize
   .. automethod:: skimage__morphology___skeletonize__thin
   .. automethod:: skimage__morphology__binary__binary_closing
   .. automethod:: skimage__morphology__binary__binary_dilation
   .. automethod:: skimage__morphology__binary__binary_erosion
   .. automethod:: skimage__morphology__binary__binary_opening
   .. automethod:: skimage__morphology__convex_hull__convex_hull_image
   .. automethod:: skimage__morphology__convex_hull__convex_hull_object
   .. automethod:: skimage__morphology__extrema__h_maxima
   .. automethod:: skimage__morphology__extrema__h_minima
   .. automethod:: skimage__morphology__extrema__local_maxima
   .. automethod:: skimage__morphology__extrema__local_minima
   .. automethod:: skimage__morphology__footprints__ball
   .. automethod:: skimage__morphology__footprints__cube
   .. automethod:: skimage__morphology__footprints__diamond
   .. automethod:: skimage__morphology__footprints__disk
   .. automethod:: skimage__morphology__footprints__ellipse
   .. automethod:: skimage__morphology__footprints__footprint_from_sequence
   .. automethod:: skimage__morphology__footprints__footprint_rectangle
   .. automethod:: skimage__morphology__footprints__mirror_footprint
   .. automethod:: skimage__morphology__footprints__octagon
   .. automethod:: skimage__morphology__footprints__octahedron
   .. automethod:: skimage__morphology__footprints__pad_footprint
   .. automethod:: skimage__morphology__footprints__rectangle
   .. automethod:: skimage__morphology__footprints__square
   .. automethod:: skimage__morphology__footprints__star
   .. automethod:: skimage__morphology__gray__black_tophat
   .. automethod:: skimage__morphology__gray__closing
   .. automethod:: skimage__morphology__gray__dilation
   .. automethod:: skimage__morphology__gray__erosion
   .. automethod:: skimage__morphology__gray__opening
   .. automethod:: skimage__morphology__gray__white_tophat
   .. automethod:: skimage__morphology__grayreconstruct__reconstruction
   .. automethod:: skimage__morphology__isotropic__isotropic_closing
   .. automethod:: skimage__morphology__isotropic__isotropic_dilation
   .. automethod:: skimage__morphology__isotropic__isotropic_erosion
   .. automethod:: skimage__morphology__isotropic__isotropic_opening
   .. automethod:: skimage__morphology__max_tree__area_closing
   .. automethod:: skimage__morphology__max_tree__area_opening
   .. automethod:: skimage__morphology__max_tree__diameter_closing
   .. automethod:: skimage__morphology__max_tree__diameter_opening
   .. automethod:: skimage__morphology__max_tree__max_tree
   .. automethod:: skimage__morphology__max_tree__max_tree_local_maxima
   .. automethod:: skimage__morphology__misc__remove_objects_by_distance
   .. automethod:: skimage__morphology__misc__remove_small_holes
   .. automethod:: skimage__morphology__misc__remove_small_objects
   .. automethod:: skimage__restoration___cycle_spin__cycle_spin
   .. automethod:: skimage__restoration___denoise__denoise_bilateral
   .. automethod:: skimage__restoration___denoise__denoise_tv_bregman
   .. automethod:: skimage__restoration___denoise__denoise_tv_chambolle
   .. automethod:: skimage__restoration___denoise__denoise_wavelet
   .. automethod:: skimage__restoration___denoise__estimate_sigma
   .. automethod:: skimage__restoration___rolling_ball__ball_kernel
   .. automethod:: skimage__restoration___rolling_ball__ellipsoid_kernel
   .. automethod:: skimage__restoration___rolling_ball__rolling_ball
   .. automethod:: skimage__restoration__deconvolution__richardson_lucy
   .. automethod:: skimage__restoration__deconvolution__unsupervised_wiener
   .. automethod:: skimage__restoration__deconvolution__wiener
   .. automethod:: skimage__restoration__inpaint__inpaint_biharmonic
   .. automethod:: skimage__restoration__j_invariant__calibrate_denoiser
   .. automethod:: skimage__restoration__j_invariant__denoise_invariant
   .. automethod:: skimage__restoration__non_local_means__denoise_nl_means
   .. automethod:: skimage__restoration__unwrap__unwrap_phase
   .. automethod:: skimage__segmentation___chan_vese__chan_vese
   .. automethod:: skimage__segmentation___clear_border__clear_border
   .. automethod:: skimage__segmentation___expand_labels__expand_labels
   .. automethod:: skimage__segmentation___felzenszwalb__felzenszwalb
   .. automethod:: skimage__segmentation___join__join_segmentations
   .. automethod:: skimage__segmentation___join__relabel_sequential
   .. automethod:: skimage__segmentation___quickshift__quickshift
   .. automethod:: skimage__segmentation___watershed__watershed
   .. automethod:: skimage__segmentation__active_contour_model__active_contour
   .. automethod:: skimage__segmentation__boundaries__find_boundaries
   .. automethod:: skimage__segmentation__boundaries__mark_boundaries
   .. automethod:: skimage__segmentation__morphsnakes__checkerboard_level_set
   .. automethod:: skimage__segmentation__morphsnakes__disk_level_set
   .. automethod:: skimage__segmentation__morphsnakes__inverse_gaussian_gradient
   .. automethod:: skimage__segmentation__morphsnakes__morphological_chan_vese
   .. automethod:: skimage__segmentation__morphsnakes__morphological_geodesic_active_contour
   .. automethod:: skimage__segmentation__random_walker_segmentation__random_walker
   .. automethod:: skimage__segmentation__slic_superpixels__slic
   .. automethod:: skimage__transform___geometric__AffineTransform
   .. automethod:: skimage__transform___geometric__EssentialMatrixTransform
   .. automethod:: skimage__transform___geometric__EuclideanTransform
   .. automethod:: skimage__transform___geometric__FundamentalMatrixTransform
   .. automethod:: skimage__transform___geometric__PiecewiseAffineTransform
   .. automethod:: skimage__transform___geometric__PolynomialTransform
   .. automethod:: skimage__transform___geometric__ProjectiveTransform
   .. automethod:: skimage__transform___geometric__SimilarityTransform
   .. automethod:: skimage__transform___geometric__estimate_transform
   .. automethod:: skimage__transform___geometric__matrix_transform
   .. automethod:: skimage__transform___thin_plate_splines__ThinPlateSplineTransform
   .. automethod:: skimage__transform___warps__downscale_local_mean
   .. automethod:: skimage__transform___warps__rescale
   .. automethod:: skimage__transform___warps__resize
   .. automethod:: skimage__transform___warps__resize_local_mean
   .. automethod:: skimage__transform___warps__rotate
   .. automethod:: skimage__transform___warps__swirl
   .. automethod:: skimage__transform___warps__warp
   .. automethod:: skimage__transform___warps__warp_coords
   .. automethod:: skimage__transform___warps__warp_polar
   .. automethod:: skimage__transform__finite_radon_transform__frt2
   .. automethod:: skimage__transform__finite_radon_transform__ifrt2
   .. automethod:: skimage__transform__hough_transform__hough_circle
   .. automethod:: skimage__transform__hough_transform__hough_circle_peaks
   .. automethod:: skimage__transform__hough_transform__hough_ellipse
   .. automethod:: skimage__transform__hough_transform__hough_line
   .. automethod:: skimage__transform__hough_transform__hough_line_peaks
   .. automethod:: skimage__transform__hough_transform__probabilistic_hough_line
   .. automethod:: skimage__transform__integral__integral_image
   .. automethod:: skimage__transform__integral__integrate
   .. automethod:: skimage__transform__pyramids__pyramid_expand
   .. automethod:: skimage__transform__pyramids__pyramid_gaussian
   .. automethod:: skimage__transform__pyramids__pyramid_laplacian
   .. automethod:: skimage__transform__pyramids__pyramid_reduce
   .. automethod:: skimage__transform__radon_transform__iradon
   .. automethod:: skimage__transform__radon_transform__iradon_sart
   .. automethod:: skimage__transform__radon_transform__order_angles_golden_ratio
   .. automethod:: skimage__transform__radon_transform__radon
   .. automethod:: skimage__util___invert__invert
   .. automethod:: skimage__util___label__label_points
   .. automethod:: skimage__util___map_array__map_array
   .. automethod:: skimage__util___montage__montage
   .. automethod:: skimage__util___regular_grid__regular_grid
   .. automethod:: skimage__util___regular_grid__regular_seeds
   .. automethod:: skimage__util___slice_along_axes__slice_along_axes
   .. automethod:: skimage__util__apply_parallel__apply_parallel
   .. automethod:: skimage__util__arraycrop__crop
   .. automethod:: skimage__util__compare__compare_images
   .. automethod:: skimage__util__dtype__dtype_limits
   .. automethod:: skimage__util__dtype__img_as_bool
   .. automethod:: skimage__util__dtype__img_as_float
   .. automethod:: skimage__util__dtype__img_as_float32
   .. automethod:: skimage__util__dtype__img_as_float64
   .. automethod:: skimage__util__dtype__img_as_int
   .. automethod:: skimage__util__dtype__img_as_ubyte
   .. automethod:: skimage__util__dtype__img_as_uint
   .. automethod:: skimage__util__lookfor__lookfor
   .. automethod:: skimage__util__noise__random_noise
   .. automethod:: skimage__util__shape__view_as_blocks
   .. automethod:: skimage__util__shape__view_as_windows
   .. automethod:: skimage__util__unique__unique_rows
   .. automethod:: slic
   .. automethod:: slice_along_axes
   .. automethod:: sobel
   .. automethod:: sobel_h
   .. automethod:: sobel_v
   .. automethod:: soften_mask
   .. automethod:: sort
   .. automethod:: span
   .. automethod:: spline_filter
   .. automethod:: spline_filter1d
   .. automethod:: square
   .. automethod:: squeeze
   .. automethod:: standard_deviation
   .. automethod:: star
   .. automethod:: std
   .. automethod:: structure_tensor
   .. automethod:: structure_tensor_eigenvalues
   .. automethod:: subdivide_polygon
   .. automethod:: subtract_image
   .. automethod:: subtract_mean
   .. automethod:: subtract_mean_percentile
   .. automethod:: sum
   .. automethod:: sum_bilateral
   .. automethod:: sum_labels
   .. automethod:: sum_percentile
   .. automethod:: swapaxes
   .. automethod:: swirl
   .. automethod:: take
   .. automethod:: thin
   .. automethod:: threshold
   .. automethod:: threshold_isodata
   .. automethod:: threshold_li
   .. automethod:: threshold_local
   .. automethod:: threshold_mean
   .. automethod:: threshold_minimum
   .. automethod:: threshold_minmax
   .. automethod:: threshold_multiotsu
   .. automethod:: threshold_niblack
   .. automethod:: threshold_otsu
   .. automethod:: threshold_percentile
   .. automethod:: threshold_sauvola
   .. automethod:: threshold_triangle
   .. automethod:: threshold_yen
   .. automethod:: to_device
   .. automethod:: tobytes
   .. automethod:: tofile
   .. automethod:: toflex
   .. automethod:: tolist
   .. automethod:: torecords
   .. automethod:: tostring
   .. automethod:: trace
   .. automethod:: translate
   .. automethod:: translate_limits
   .. automethod:: transpose
   .. automethod:: try_all_threshold
   .. automethod:: uniform_filter
   .. automethod:: uniform_filter1d
   .. automethod:: unique_rows
   .. automethod:: unshare_mask
   .. automethod:: unsharp_mask
   .. automethod:: unsupervised_wiener
   .. automethod:: unwrap_phase
   .. automethod:: update
   .. automethod:: use_plugin
   .. automethod:: value_indices
   .. automethod:: values
   .. automethod:: var
   .. automethod:: variance
   .. automethod:: view
   .. automethod:: view_as_blocks
   .. automethod:: view_as_windows
   .. automethod:: warp
   .. automethod:: warp_coords
   .. automethod:: warp_polar
   .. automethod:: watershed
   .. automethod:: watershed_ift
   .. automethod:: white_tophat
   .. automethod:: wiener
   .. automethod:: window
   .. automethod:: windowed_histogram
   .. automethod:: xyz2lab
   .. automethod:: xyz2luv
   .. automethod:: xyz2rgb
   .. automethod:: xyz_tristimulus_values
   .. automethod:: ycbcr2rgb
   .. automethod:: ydbdr2rgb
   .. automethod:: yiq2rgb
   .. automethod:: ypbpr2rgb
   .. automethod:: yuv2rgb
   .. automethod:: zoom
