...
COLUMNS_FOR_FOREIGN_SWAPS = COLUMN_NAMES_MENU2205_PREPROCESSED

def get_foreign_swaps(date_ref=None):
    dfs = get_grouped_dfs_of_menu2205(col='자산', date_ref=date_ref)
    foreign_swaps = dfs['외화스왑'][COLUMNS_FOR_FOREIGN_SWAPS]
    return foreign_swaps

def get_foreign_swaps_by_fund(fund_code, date_ref=None):
    dfs = get_grouped_dfs_of_menu2205_by_fund(fund_code=fund_code, col='자산', date_ref=date_ref)
    foreign_swaps = dfs['외화스왑'][COLUMNS_FOR_FOREIGN_SWAPS]
    return foreign_swaps

def search_funds_having_foreign_swaps(date_ref=None, option_exist_data=True):
    fund_codes = get_all_fund_codes()
    dct_dfs = {}
    for fund_code in tqdm(fund_codes):
        try:
            dct_dfs[fund_code] = get_foreign_swaps_by_fund(fund_code=fund_code, date_ref=date_ref)
        except:
            dct_dfs[fund_code] = None
    dct_foreign_swaps = {fund_code: df for fund_code, df in dct_dfs.items() if df is not None} if option_exist_data else dct_dfs
    return dct_foreign_swaps
