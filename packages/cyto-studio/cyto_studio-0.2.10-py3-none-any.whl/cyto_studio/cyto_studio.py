...
#             tifffile.imsave(file_name, volume)
    ...
    def Load3D(self, text):
        ...
        result = self.data.Load3D(self.viewer, self.image_folder, self.comboBoxPath.currentText(), self.selected_channels, self.default_contrast_limits, self.thresholdN, self.channels_start, self.axio, self.old_method, self.overall_brightness, self.scroll, self.scroll_overall_brightness, self.pixel_size.text(), self.m_slice_spacing.text(), self.start_slice.text(), self.end_slice.text(), self.crop, self.crop_start_x, self.crop_end_x, self.crop_start_y, self.crop_end_y, self.origin_x, self.origin_y, use_mask)       
    ...
    def main(self):
        ...
        self.viewer.window._qt_window.setWindowTitle("cyto-studio (0.2.10)")
        ...
        hbox = QtWidgets.QHBoxLayout()
        self.cb_use_mask = QtWidgets.QCheckBox('Use mask')
