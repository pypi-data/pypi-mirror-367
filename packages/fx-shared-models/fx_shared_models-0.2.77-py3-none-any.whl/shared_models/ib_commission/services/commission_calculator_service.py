        ...
        with transaction.atomic():
            ...
            if processing_status == 'PENDING':
                ...
                for member in agreement_members:
                    member_customer_id_str = str(member.customer_id)
                    
                    # For the direct IB, only use the client mapping agreement
                    if member_customer_id_str == str(client_mapping.direct_ib_customer_id):
                        # Only add if it matches the client mapping agreement
                        if client_mapping.agreement_id and member.agreement_id == client_mapping.agreement_id:
                            ib_agreements[member_customer_id_str] = member.agreement
                        # Skip other agreements for the direct IB
                    else:
                        # For parent IBs in the hierarchy, use their agreements (current behavior)
                        ib_agreements[member_customer_id_str] = member.agreement
