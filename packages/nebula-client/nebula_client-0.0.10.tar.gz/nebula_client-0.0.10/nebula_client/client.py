...
class NebulaClient:
    ...
    def get_cluster_memories(
        ...
        """
        Get all memories in a cluster by filtering documents
    ...
    def store(
        ...
        Args:
            ...
            cluster_id: Optional cluster ID to store the memory in
    ...
    def delete(self, memory_id: str) -> bool:
        ...
        Note: This method uses content_hash filtering since individual document deletion
        by ID is not supported by the backend API.
        Returns True if successful, raises exception otherwise.
        """
        try:
            # First try direct deletion by ID
            self._make_request("DELETE", f"/v3/documents/{memory_id}")
            return True
        except Exception as e:
            # If direct deletion fails, try to get the memory first to extract content_hash
            if "404" in str(e) or "not found" in str(e).lower():
                try:
                    # Try to get the memory to extract content_hash
                    memory = self.get(memory_id)
                    if memory and memory.metadata and "content_hash" in memory.metadata:
                        content_hash = memory.metadata["content_hash"]
                        # Use filter-based deletion with content_hash
                        return self.delete_by_filter({"content_hash": {"$eq": content_hash}})
                    else:
                        raise NebulaException(
                            f"Could not find content_hash for memory {memory_id}. Use delete_by_filter() with appropriate filters.",
                            404,
                            {"error": "No content_hash available for deletion"}
                        )
                except Exception as get_error:
                    # If we can't get the memory either, suggest using filter-based deletion
                    raise NebulaException(
                        f"Individual document deletion not supported. Use delete_by_filter() with appropriate filters.",
                        404,
                        {"error": "Individual document deletion endpoint not available"}
                    )
            raise
    ...
    def delete_by_filter(self, filters: Dict[str, Any]) -> bool:
        ...
        Args:
            filters: Filter criteria using operators like $eq, $neq, $gt, $gte, $lt, $lte, $like, $ilike, $in, $nin
            
        Returns:
            True if successful
            
        Example:
            client.delete_by_filter({"document_type": {"$eq": "pdf"}})
        """
        try:
            self._make_request("DELETE", "/v3/documents/by-filter", json_data={"filters": filters})
            return True
        except Exception as e:
            raise NebulaException(
                f"Failed to delete documents by filter: {e}",
                500,
                {"error": "Filter deletion failed"}
            )
    ...
    def list_memories(
        ...
        """
        Get all memories, optionally filtered by cluster
        
        Args:
            limit: Maximum number of memories to return
            offset: Number of memories to skip
            cluster_id: Optional cluster ID to filter memories by
        ...
        if cluster_id:
            params["collection_ids"] = cluster_id
    ...
    def search(
        ...
        """
        Search across all memories in the system with configurable retrieval options
        ...
        # Add retrieval type if not simple
        if retrieval_type != RetrievalType.SIMPLE:
        ...
        # Add cluster filter using proper $overlap syntax
        if cluster_id:
            if "filters" not in search_settings:
                search_settings["filters"] = {}
            search_settings["filters"]["collection_ids"] = {"$overlap": [cluster_id]}
