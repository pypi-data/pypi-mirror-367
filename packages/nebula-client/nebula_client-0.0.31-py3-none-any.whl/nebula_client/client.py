...
class NebulaClient:
    ...
    def get(self, memory_id: str) -> Memory:
        """
        Get a specific memory by ID (via documents endpoint and chunks endpoint)
        """
        # Get document metadata
        doc_response = self._make_request("GET", f"/v3/documents/{memory_id}")
        
        # Unwrap 'results' if present
        if isinstance(doc_response, dict) and "results" in doc_response:
            doc_response = doc_response["results"]
        
        # Get document content from chunks
        content = "No content available"
        try:
            chunks_response = self._make_request("GET", f"/v3/documents/{memory_id}/chunks")
            if isinstance(chunks_response, dict) and "results" in chunks_response:
            ...
            # If chunks endpoint fails, fall back to document metadata
            pass
        
        memory_data = {
            "id": doc_response.get("id"),
            "content": content,
            "metadata": doc_response.get("metadata", {}),
            "collection_ids": doc_response.get("collection_ids", [])
        }
        return Memory.from_dict(memory_data)
    ...
    def list_memories(
        ...
        memories = []
        for doc in documents:
            # Get content from chunks for each document
            content = "No content available"
            try:
                doc_id = doc.get("id")
                if doc_id:
                    chunks_response = self._make_request("GET", f"/v3/documents/{doc_id}/chunks")
                    if isinstance(chunks_response, dict) and "results" in chunks_response:
                        chunks = chunks_response["results"]
                        if chunks and len(chunks) > 0:
                            # Combine all chunk texts
    ...
    def search(
        ...
        search_settings = {
            "limit": limit,
            "graph_settings": {
                "enabled": False  # Enable graph search by default
            }
        }
        ...
        if filters:
            # Convert collection_ids array to $overlap format if needed
            if "collection_ids" in filters and isinstance(filters["collection_ids"], list):
                search_filters["collection_ids"] = {"$overlap": filters["collection_ids"]}
                # Remove the original to avoid duplication
                filters_copy = filters.copy()
                del filters_copy["collection_ids"]
                search_filters.update(filters_copy)
            else:
                search_filters.update(filters)
        
        # Add cluster filter if provided
        if cluster_id:
            search_filters["collection_ids"] = {"$overlap": [cluster_id]}
        
        # Add filters to search settings if we have any
        if search_filters:
