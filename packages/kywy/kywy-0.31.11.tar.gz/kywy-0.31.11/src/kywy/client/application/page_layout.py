from .widget import SimpleChart, Table, ScatterChart, TextWidget, IndicatorChart
...
class ApplicationPage:
    ...
    def __init__(self, kawa_client, data_model, name: str):
        self._name = name
    ...
    def sync(self, application_id):
        ...
        self._dashboard_id = created_component[0]['componentConfiguration']['entityId']
        self._k.commands.run_command(
            ...
            command_parameters={
                ...
                "id": str(self._dashboard_id)
            }
        )

        for section in self._sections:
            section.sync(application_id)

        self.set_layout()

    def set_layout(self):
        dashboard = load_dashboard(self._k, self._dashboard_id)
        current_position = 0
...
class ApplicationPageSection:
    ...
    def __init__(self, kawa, page, data_model, title, num_columns):
        self._k = kawa
        self._title = title
        self._page = page
        self._data_model = data_model
...
class ApplicationPageColumn:
    ...
    def __init__(self, kawa, position, width, section, data_model):
        self._k = kawa
