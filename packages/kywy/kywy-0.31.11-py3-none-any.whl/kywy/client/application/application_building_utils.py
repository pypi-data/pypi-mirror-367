...
def init_sheet(kawa, sheet_name, primary_datasource_id):
    existing_sheets = kawa.entities.sheets().list_entities_by_name(sheet_name)
    if existing_sheets:
        sheet_id = str(existing_sheets[0]['id'])
        kawa.commands.run_command('deleteSheet', {'sheetId': sheet_id})

    created_sheet_id = kawa.commands.run_command(
        ...
        command_parameters={
            ...
            "sheet": {
                ...
                "dataSourceLinks": [{"targetDataSourceId": str(primary_datasource_id)}],
                "shared": False,
                "generalAccess": "RESTRICTED"
            }
        }
    )['id']
    info(f'🗒️ Sheet {sheet_name} was created (id={created_sheet_id})')
