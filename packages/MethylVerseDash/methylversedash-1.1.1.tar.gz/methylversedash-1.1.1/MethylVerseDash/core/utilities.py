...
def position_to_cpg(iframe: IntervalFrame,
    ...
    if genome_version == "hg38":
        ...
        anno_file = "/opt/data/hg38_anno.parquet"
    else:
        #anno_file = get_data_file("hg19_anno.parquet")
        anno_file = "/opt/data/hg19_anno.parquet"
...
def get_probe_names():
    ...
    iframe_cn = IntervalFrame.read_parquet("/opt/data/hg19_anno.parquet")
    probes = iframe_cn.df.loc[:,"cpg_name"].values
...
def convert_v2_to_v1(cpgs: pd.DataFrame) -> pd.DataFrame:
    ...
    v2_to_v1 = pd.read_parquet("/opt/data/EPIC2_to_EPIC.parquet")
...
def cpg_to_position(cpgs,
    ...
    # Read annotation file
    if genome_version == "hg38":
        iframe_cn = IntervalFrame.read_parquet("/opt/data/hg38_anno.parquet")
        iframe_cn = IntervalFrame.read_parquet("/opt/data/hg38_anno.parquet")
    else:
        iframe_cn = IntervalFrame.read_parquet("/opt/data/hg19_anno.parquet")
    probes = iframe_cn.df.loc[:,"cpg_name"].values
