...
class QueryMajorInfo(BaseTool):
    ...
    description: str = (
        "从本地数据库中查询某个专业的信息，返回信息包括：类别、代码、男女比例、文理比例、修业年限、授予学位、选科建议、介绍、开设课程、考研方向、社会名人、就业情况（就业率、薪酬、就业行业分布、就业岗位分布、就业地区分布"
    )
    parameters: dict = generate_param_schema(QueryMajorInfoParameters)

    def execute(self, return_format: str = "json", **kwargs: dict) -> ToolResult:
        try:
            ...
            if return_format == "markdown":
                ...
                raise ValueError(
                    f'Invalid "return_format": "{return_format}". Must be "markdown" or "json".'
                )
    ...
    def get_major_info_markdown(self, major_id: str) -> str:
        ...
        if job_rate_data:
            for single in job_rate_data:
                job_rate_md += (
                    f"- {single.get('year', 'N/A')}: {single.get('rate', 'N/A')}\n"
                )
        else:
            job_rate_md += "暂无数据"
        # --- 薪酬概览 ---
        ...
        if industry_distribution_data:
            for single in industry_distribution_data:
                industry_distribution_md += (
                    f"- {single.get('name', 'N/A')}: {single.get('rate', 'N/A')}%\n"
                )
        else:
        ...
        if position_distribution_data:
            for single in position_distribution_data:
                ...
                position_distribution_md += (
                    f"    - 具体职业: {single.get('detail_job', 'N/A')}\n"
                )
                position_distribution_md += (
                    f"    - 所在行业: {single.get('name', 'N/A')}\n"
                )
        else:
        ...
        if area_distribution_data:
            for single in area_distribution_data:
                area_distribution_md += (
                    f"- {single.get('area', 'N/A')}: {single.get('rate', 'N/A')}%\n"
                )
        else:
