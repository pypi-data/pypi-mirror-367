...
class BestTopDownStrategyMaker(StrategyMaker):
    ...
    def process_pair(self,symbol,pair_config):
        ...
        try:
            ...
            enter_eth_duration = top_down_strategy.get('enter_eth_duration', 4)
            ...
            if not self.skip_step.get(symbol, False):
                # 初始化HTF趋势相关变量
                ...
                # 条件满足后，设置skip_step
                self.skip_step[symbol]= True
                self.order_sides[symbol] = atf_side

            else:
                self.logger.info(f"{symbol} : HTF&ATF 条件满足跳过，直接监测下单。")
            ...
            if self.skip_step.get(symbol, False):
                atf_side = self.order_sides.get(symbol, None)
                if atf_side is None:
                    self.logger.warning(f"{symbol} : {step}. order_sides 未记录ATF订单方向.")
                    return
