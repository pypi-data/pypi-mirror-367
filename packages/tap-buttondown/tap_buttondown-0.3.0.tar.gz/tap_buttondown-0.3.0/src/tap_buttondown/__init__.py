...
class ButtondownAPI:
    ...
    def get_emails(self, page: int = 1, page_size: int = 100) -> Dict[str, Any]:
        """Fetch emails from Buttondown API"""
...
def load_schemas() -> Dict[str, Schema]:
    ...
    # Define email schema based on Buttondown API documentation
    email_schema = {
        "type": "object",
        "properties": {
            "id": {"type": "string"},
            ...
            "publish_date": {"type": ["null", "string"], "format": "date-time"},
            "subject": {"type": "string"},
            "body": {"type": "string"},
            "status": {"type": "string"},
            "slug": {"type": "string"},
            ...
            "metadata": {"type": "object"},
            "source": {"type": "string"},
            "secondary_id": {"type": ["null", "integer"]},
            "commenting_mode": {"type": "string"},
            "absolute_url": {"type": "string"},
            "filters": {"type": "object"},
            "analytics": {"type": ["null", "object"]},
            "template": {"type": ["null", "object"]},
            "attachments": {"type": "array", "items": {"type": "object"}},
            "related_email_ids": {"type": "array", "items": {"type": "string"}},
            "featured": {"type": "boolean"},
            "should_trigger_pay_per_email_billing": {"type": "boolean"}
        }
    }
    
    schemas["subscribers"] = Schema.from_dict(subscriber_schema)
    schemas["emails"] = Schema.from_dict(email_schema)
...
def discover() -> Catalog:
    ...
    for stream_id, schema in schemas.items():
        ...
        # Set replication key based on stream type
        if stream_id == "emails":
            replication_key = "modification_date"  # Use modification_date for emails
        else:
            replication_key = "creation_date"  # Use creation_date for subscribers
...
def sync_emails(config: Dict[str, Any], state: Dict[str, Any], catalog: Catalog) -> None:
    """Sync emails stream"""
    api = ButtondownAPI(config["api_key"])
    
    # Get the emails stream from catalog
    emails_catalog = catalog.get_stream("emails")
    if not emails_catalog:
        LOGGER.error("Emails stream not found in catalog")
        return
    
    bookmark_column = emails_catalog.replication_key
    
    start = get_bookmark(state, "emails", bookmark_column, config.get("start_date"))
    if start:
        start = strptime_to_utc(start)
    
    write_schema("emails", emails_catalog.schema.to_dict(), key_properties=["id"])
    ...
    while True:
        try:
            response = api.get_emails(page=page, page_size=100)
    ...
    LOGGER.info(f"Synced {total_records} email records")
...
def sync(config: Dict[str, Any], state: Dict[str, Any], catalog: Catalog) -> None:
    ...
    for stream in catalog.streams:
        ...
        elif stream.tap_stream_id == "emails":
            sync_emails(config, state, catalog)
