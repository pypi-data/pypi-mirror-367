...
class ChainlitConfig:
    ...
    def load_translation(self, language: str):
        ...
        elif builtin_translation_file_path.is_file():
            logger.info(
                f"Using built-in Chainlit translation for {language}."
            )
            translation = json.loads(
                builtin_translation_file_path.read_text(encoding="utf-8")
            )
        elif builtin_parent_language_file_path.is_file():
            logger.warning(
                f"Translation file for {language} not found. Using built-in parent translation {parent_language}."
            )
            translation = json.loads(
                builtin_parent_language_file_path.read_text(encoding="utf-8")
            )
        elif builtin_default_translation_file_path.is_file():
            logger.warning(
                f"Translation file for {language} not found. Using built-in default translation {default_language}."
            )
            translation = json.loads(
