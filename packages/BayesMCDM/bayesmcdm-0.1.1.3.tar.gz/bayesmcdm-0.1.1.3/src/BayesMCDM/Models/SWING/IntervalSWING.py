...
class IntervalSWING(StandardSWING, MCDMProblem):
    _basic_model = """
        data { 
            int<lower=2> CNo;
            int<lower=1> DmNo;
            array[DmNo] vector[CNo] Prf_l;
            array[DmNo] vector[CNo] Prf_h;
        ...
        parameters { 
            ...
            array[DmNo] vector<lower=0, upper=1>[CNo] Prf_trnf; // [0,1] intervals
        } 
        ...
        transformed parameters {
            ...
            for(i in 1:DmNo) {
                Prf[i] = Prf_l[i] + (Prf_h[i] - Prf_l[i]) .* Prf_trnf[i]; // moving [0,1] intervals to the desired intervals

                W[i] = Prf[i] ./ sum(Prf[i]); // Normalizing for a better fit
            }
