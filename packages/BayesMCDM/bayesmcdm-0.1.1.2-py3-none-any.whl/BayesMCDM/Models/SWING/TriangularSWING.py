...
class TriangularSWING(StandardSWING, MCDMProblem):
    _basic_model = """
        data { 
            int<lower=2> CNo;
            int<lower=1> DmNo;  
        ...
        parameters { 
            ...
            array[DmNo]vector<lower=0, upper=1>[CNo] Prf_trnf;            
        }

        transformed parameters {
            array[DmNo] vector<lower=0>[CNo] Prf; 
            array[DmNo] vector<lower=0>[CNo] W; 

            
            for(i in 1:DmNo) {
                Prf[i] = Prf_trnf[i] + Prf_md[i];
        ...
        model {
            ...
            for (i in 1:DmNo) {
                ...
                target += log1m(abs( Prf[i] - Prf_md[i] ));
            }
        } 
    """
    ...
    def __init__(self, Prf_md, alternatives = None, dm_cluster_number=-1, alt_sort_number=-1, num_chain=3, num_samples=1000, opt={}):
    ...
    def _check_input_data(self):
        ...
        assert self.Prf_MD.shape[0] >=1, "No input"
        assert self.Prf_MD.shape[1] >=2, "The number of criteria must be more than 2!"
