...
from funimpute.simple_cli import main as simple_cli_main
from funimpute.models import ImputationSuggestion
...
5,,55000,B"""
        ...
        metadata_content = """column_name,data_type,min_value,max_value,max_length,unique_flag,dependent_column,dependency_rule,business_rule,description
id,integer,1,999999,,TRUE,,,,"User identifier"
age,integer,18,100,,FALSE,,,,"User age"
income,float,0,,,FALSE,age,Higher with age,,"Annual income"
category,categorical,,,10,FALSE,,,,"User category"""
    ...
    def test_cli_help(self):
        ...
        result = self.runner.invoke(simple_cli_main, ['--help'])
    ...
    def test_cli_missing_required_args(self):
        ...
        # Missing both metadata and data
        result = self.runner.invoke(simple_cli_main, [])
        assert result.exit_code != 0
        
        # Missing data argument
        result = self.runner.invoke(simple_cli_main, ['-m', 'metadata.csv'])
        assert result.exit_code != 0
        
        # Missing metadata argument
        result = self.runner.invoke(simple_cli_main, ['-d', 'data.csv'])
        assert result.exit_code != 0
...
outlier_percentage_threshold: 0.1"""
    ...
    def test_cli_file_not_found_error(self):
        ...
        result = self.runner.invoke(simple_cli_main, [
            '-m', 'nonexistent_metadata.csv',
            '-d', 'nonexistent_data.csv'
        ])
    ...
    def test_cli_output_format(self):
        ...
        try:
            with self.runner.isolated_filesystem():
                ...
                summary_lines = [line for line in lines if 'Columns analyzed:' in line or 
                               'Total missing values:' in line or 'Average confidence:' in line]
                
                assert len(summary_lines) >= 3
                
                # Check that method distribution is shown
                method_lines = [line for line in lines if ':' in line and 
                              any(method in line for method in ['Mean', 'Median', 'Mode', 'Business Rule', 'No action needed'])]
...
id,integer"""
    ...
    @patch('funimpute.simple_cli.analyze_imputation_requirements')
    ...
    def test_cli_short_and_long_options(self):
        ...
        try:
            with self.runner.isolated_filesystem():
                # Test short options
                result_short = self.runner.invoke(simple_cli_main, [
                    '-m', metadata_path,
                    '-d', data_path,
                    '-o', 'short_output.csv',
                    '-v'
                ...
                # Test long options
                result_long = self.runner.invoke(simple_cli_main, [
                    '--metadata', metadata_path,
                    '--data', data_path,
                    '--output', 'long_output.csv',
                    '--verbose'
                ])
    ...
    def test_cli_examples_in_help(self):
        ...
        assert 'funimputer -m' in result.output
...
class TestCLIIntegration:
    ...
    @patch('funimpute.simple_cli.analyze_imputation_requirements')
    @patch('funimpute.simple_cli.save_suggestions')
    ...
    def test_cli_confidence_score_division_by_zero_fix(self):
        ...
        with patch('funimpute.simple_cli.analyze_imputation_requirements') as mock_analyze:
            # Mock empty suggestions (edge case that caused division by zero)
            mock_analyze.return_value = []
            
            with patch('funimpute.simple_cli.save_suggestions'):
                result = self.runner.invoke(simple_cli_main, [
                    '-m', 'metadata.csv',
                    '-d', 'data.csv'
                ])
                ...
                assert result.exit_code == 0
                assert 'Average confidence: 0.000' in result.output  # Should show 0.000 for empty list
    
    def test_cli_method_distribution_display(self):
        """Test that CLI correctly displays method distribution."""
        mock_suggestions = [
            ImputationSuggestion(column_name='col1', proposed_method='Mean', rationale='Test'),
            ImputationSuggestion(column_name='col2', proposed_method='Mean', rationale='Test'),
            ImputationSuggestion(column_name='col3', proposed_method='Mode', rationale='Test'),
            ImputationSuggestion(column_name='col4', proposed_method='Business Rule', rationale='Test'),
        ]
