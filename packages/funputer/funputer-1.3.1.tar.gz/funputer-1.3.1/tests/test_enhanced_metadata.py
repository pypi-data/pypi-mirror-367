...
from funputer.metadata_inference import MetadataInferenceEngine, infer_metadata_from_dataframe
...
class TestEnhancedMetadata:
    ...
    def test_role_inference(self):
        ...
        assert engine._infer_role('created_date', 'datetime', False, 1, 5) == 'time_index'
        
        # Test group by inference
        assert engine._infer_role('customer_segment', 'categorical', False, 1, 5) == 'group_by'
        ...
        assert engine._infer_role('any_column', 'float', False, 4, 5) == 'target'  # Last position
    ...
    def test_sentinel_values_inference(self):
        ...
        string_series = pd.Series(['A', 'B', 'UNKNOWN', 'C', 'UNKNOWN', 'D'])  # UNKNOWN appears 33%
    ...
    def test_time_index_inference(self):
        ...
        assert engine._infer_time_index('birth_date', 'datetime') == True  # Actually detects 'date'
    ...
    def test_group_by_inference(self):
        ...
        assert engine._infer_group_by('customer_segment', 'categorical', pd.Series()) == False  # Empty series
        
        # Test with non-empty series for name-based inference
        dummy_series = pd.Series(['A', 'B', 'A'])  # Non-empty for name-based check
        assert engine._infer_group_by('customer_segment', 'categorical', dummy_series) == True
        ...
        low_cardinality = pd.Series(['A'] * 50 + ['B'] * 40 + ['C'] * 10)  # 3 values in 100 rows
        assert engine._infer_group_by('some_column', 'categorical', low_cardinality) == True
        
        # Test high cardinality should not be group-by
        high_cardinality = pd.Series(range(100))  # 100 unique values in 100 rows
        assert engine._infer_group_by('some_column', 'categorical', high_cardinality) == False
    ...
    def test_complete_enhanced_inference(self):
        ...
        df = pd.DataFrame({
            ...
            'signup_timestamp': pd.to_datetime(['2024-01-01', '2024-01-02', '2024-01-03', '2024-01-04']),
        ...
        assert metadata_dict['account_balance'].sentinel_values == '-999'  # Integer sentinel, not float
...
class TestEnhancedMetadataErrorHandling:
    ...
    def test_invalid_role_still_works(self):
        ...
        assert result == 'feature'  # Single column defaults to feature (not target since total_cols <= 3)
        
        result = engine._infer_role('very_long_column_name_with_no_patterns', 'string', False, 2, 10)
