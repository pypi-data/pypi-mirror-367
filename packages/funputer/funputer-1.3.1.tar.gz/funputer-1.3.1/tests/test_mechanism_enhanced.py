...
from funimpute.mechanism import (
    analyze_missingness_mechanism,
    find_related_columns,
    chi_square_test,
    point_biserial_test
)
from funimpute.models import MissingnessAnalysis, MissingnessType, ColumnMetadata, AnalysisConfig
...
class TestMechanismEnhanced:
    ...
    def create_test_dataframe(self, missing_pattern="random"):
        ...
        if missing_pattern == "random":
            ...
            missing_mask = df["age"] > 50  # Older people more likely to have missing income
    ...
    def test_analyze_missingness_mechanism_basic(self):
        ...
        metadata_dict = {
            ...
            "education": ColumnMetadata(column_name="education", data_type="categorical"),
    ...
    def test_mechanism_analysis_with_age_dependent(self):
        ...
        assert isinstance(result, MissingnessAnalysis)
        assert result.mechanism in [MissingnessType.MAR, MissingnessType.MCAR, MissingnessType.UNKNOWN]
    ...
    def test_mechanism_detection_with_invalid_column(self):
        ...
        metadata_dict = {"income": ColumnMetadata(column_name="income", data_type="float")}
        config = AnalysisConfig()
        with pytest.raises(KeyError):
            analyze_missingness_mechanism("nonexistent_column", df, metadata_dict, config)

    # Test edge cases and error handling
    def test_mechanism_detection_with_constant_column(self):
        """Test mechanism detection with constant values."""
        df = pd.DataFrame({
            "constant": [1, 1, 1, 1, np.nan],
            "variable": [1, 2, 3, 4, 5]
        })
    ...
    def test_large_dataset_performance(self):
        ...
        metadata_dict = {
            ...
            "predictor2": ColumnMetadata(column_name="predictor2", data_type="categorical")
        }
        config = AnalysisConfig()
