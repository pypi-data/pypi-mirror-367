...
from funputer.models import (
    ...
    ImputationMethod, MissingnessType
)
...
class TestSimpleImputationAnalyzer:
    ...
    def test_init_with_config(self):
        ...
        config = AnalysisConfig(iqr_multiplier=2.0, outlier_threshold=0.1)
    ...
    def test_analyze_dataframe_basic(self):
        ...
        suggestions = self.analyzer.analyze_dataframe(self.sample_data, self.sample_metadata)
    ...
    def test_analyze_dataframe_no_metadata(self):
        """Test dataframe analysis without metadata."""
        suggestions = self.analyzer.analyze_dataframe(self.sample_data)
    ...
    def test_analyze_column_numeric_with_missing(self):
        ...
        suggestion = self.analyzer.analyze_column(series, metadata)
    ...
    def test_analyze_column_categorical_with_missing(self):
        ...
        series = pd.Series(['A', 'B', None, 'A', 'B'], name='cat_col')
    ...
    def test_analyze_column_no_missing_values(self):
        ...
        series = pd.Series([1, 2, 3, 4, 5], name='complete_col')
    ...
    def test_analyze_column_all_missing(self):
        ...
        assert suggestion.missing_percentage == 100.0
        assert 'all values missing' in suggestion.rationale.lower()
    
    def test_analyze_column_unique_identifier(self):
        """Test analysis of unique identifier column."""
        series = pd.Series([1, 2, None, 4, 5], name='id_col')
        metadata = ColumnMetadata(column_name='id_col', data_type='integer', unique_flag=True)
        
        suggestion = self.analyzer.analyze_column(series, metadata)
        
        assert suggestion.column_name == 'id_col'
        assert 'unique' in suggestion.rationale.lower() or 'identifier' in suggestion.rationale.lower()
    
    def test_analyze_column_with_outliers(self):
        """Test analysis of column with outliers."""
        # Create data with clear outliers
        series = pd.Series([1, 2, 3, 100, 5, None], name='outlier_col')
    ...
    def test_analyze_with_enhanced_metadata_fields(self):
        ...
        series = pd.Series([1, 2, None, 4, 5], name='enhanced_col')
        ...
        # Should respect do_not_impute flag
        if metadata.do_not_impute:
            assert 'not impute' in suggestion.rationale.lower() or suggestion.proposed_method == 'Manual Backfill'
    ...
    def test_analyze_empty_dataframe(self):
        ...
        with pytest.raises(Exception):
            self.analyzer.analyze_dataframe(empty_df, [])
    ...
    def test_analyze_with_high_missing_threshold(self):
        ...
        assert suggestion.missing_percentage > 50
...
class TestAnalyzeImputationRequirements:
    ...
    def test_analyze_invalid_metadata_path(self):
        ...
        with pytest.raises(Exception):
            analyze_imputation_requirements(
                data_path=self.test_csv,
                metadata_path='/nonexistent/metadata.csv'
            )
...
class TestEdgeCases:
    ...
    def test_analyze_with_special_characters(self):
        ...
        data = pd.DataFrame({
            'special_col': ['café', 'naïve', None, 'résumé', '中文']
        })
        
        analyzer = SimpleImputationAnalyzer()
        suggestions = analyzer.analyze_dataframe(data)
    ...
    def test_analyze_with_very_large_values(self):
        ...
        data = pd.DataFrame({
            'large_col': [1e10, 1e11, None, 1e12, 1e13]
        })
    ...
    def test_analyze_single_row_dataframe(self):
        ...
        assert len(suggestions) == 1
        assert suggestions[0].missing_percentage == 100.0
