...
class SimpleImputationAnalyzer:
        ...
        if metadata_path.lower().endswith('.json'):
            try:
                ...
                try:
                    ...
                    enterprise_metadata = load_enterprise_metadata(metadata_path, source_type="file", validate=False)
                    loader = get_metadata_loader()
                    legacy_metadata = loader.convert_to_legacy_format(enterprise_metadata)
                    logger.info(f"Loaded enterprise JSON metadata: {len(legacy_metadata)} columns")
                    ...
                    logger.info(f"Converted simple JSON metadata: {len(legacy_metadata)} columns")
                    return legacy_metadata
    ...
    def _convert_simple_json_to_legacy(self, json_data: dict) -> List[ColumnMetadata]:
        ...
        # Handle flat format where top level keys are column names
            ...
            columns_data = [{"name": k, **v} for k, v in json_data.items() if isinstance(v, dict)]
        ...
        logger.info(f"Converted {len(legacy_columns)} columns from JSON to legacy format")
