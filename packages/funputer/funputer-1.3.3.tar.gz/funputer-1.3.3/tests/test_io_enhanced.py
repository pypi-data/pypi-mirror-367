...
class TestIOEnhanced:
    ...
    def create_test_metadata_csv(self, filename: str = "metadata.csv") -> str:
        ...
        content = """column_name,data_type,min_value,max_value,max_length,unique_flag,nullable,allowed_values,dependent_column,dependency_rule,business_rule,description
user_id,integer,,,50,true,false,,,,,Unique user identifier
age,integer,0,120,,false,false,,,,,User age in years
name,string,,,100,false,true,,,,,User full name
status,categorical,,,10,false,false,"active,inactive",,,,"Account status"
"""
        filepath = os.path.join(self.temp_dir, filename)
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
...
"""
    ...
    def test_load_configuration_json_success(self):
        ...
        assert config.missing_threshold == 0.25
    ...
    def test_save_suggestions_permission_error(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.raises((PermissionError, OSError)):
                save_suggestions(suggestions, os.path.join(protected_path, "suggestions.csv"))
