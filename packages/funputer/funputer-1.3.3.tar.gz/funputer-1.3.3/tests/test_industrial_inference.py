...
class TestIndustrialDataInference:
    ...
    def setup_class(cls):
        ...
        data_path = Path(__file__).parent.parent / "data" / "industrial_equipment_data.csv"
        ...
        # Run inference once
        cls.metadata_list = infer_metadata_from_dataframe(cls.industrial_df, warn_user=False)
    ...
    def test_dataset_completeness(self):
        ...
        assert len(data_types) >= 4, f"Dataset should have varied data types, got: {data_types}"
        
        # Should have missing data for inference testing
        missing_columns = df.columns[df.isnull().any()].tolist()
        assert len(missing_columns) >= 5, f"Dataset should have missing data in multiple columns for testing"
    ...
    def test_identifier_role_inference(self):
        ...
        assert equipment_meta.role == 'identifier', f"equipment_id should be identifier, got: {equipment_meta.role}"
        ...
        print(f"✅ Identifier inference: equipment_id correctly identified as {equipment_meta.role}")
    ...
    def test_time_index_role_inference(self):
        ...
        assert timestamp_meta.role == 'time_index', f"timestamp should be time_index, got: {timestamp_meta.role}"
        assert timestamp_meta.data_type == 'datetime', f"timestamp should be datetime, got: {timestamp_meta.data_type}"
        ...
        print(f"✅ Time index inference: timestamp correctly identified as {timestamp_meta.role}")
    ...
    def test_group_by_role_inference(self):
        ...
        for col_name, expected_role in expected_roles.items():
            if col_name in self.metadata_dict:
                ...
                assert meta.role == expected_role, f"{col_name} should be {expected_role}, got: {meta.role}"
                ...
                print(f"✅ Role inference: {col_name} correctly identified as {meta.role}")
    ...
    def test_target_role_inference(self):
        ...
        if 'failure_prediction_score' in self.metadata_dict:
            ...
            assert failure_meta.role in ['target', 'feature'], f"failure_prediction_score should be target or feature, got: {failure_meta.role}"
            
            if failure_meta.role == 'target':
                assert failure_meta.do_not_impute == True, "Target variables should not be imputed"
    ...
    def test_ignore_role_inference(self):
        ...
        for col_name in ignore_candidates:
            if col_name in self.metadata_dict:
                ...
                print(f"📋 Debug/system column {col_name}: role={meta.role}, do_not_impute={meta.do_not_impute}")
    ...
    def test_sensor_data_inference(self):
        ...
        for col_name, expected_role in expected_roles.items():
            if col_name in self.metadata_dict:
                ...
                assert meta.do_not_impute != expected_imputable, f"{col_name} imputation expectation mismatch"
                ...
                print(f"✅ Sensor inference: {col_name} correctly classified as {meta.role} ({meta.data_type})")
    ...
    def test_sentinel_value_detection(self):
        ...
        if temp_backup_meta:
            ...
            if temp_backup_meta.sentinel_values:
                assert '-999' in temp_backup_meta.sentinel_values, f"Should detect -999 sentinel, got: {temp_backup_meta.sentinel_values}"
                print(f"✅ Sentinel detection: temp_backup_reading detected sentinels: {temp_backup_meta.sentinel_values}")
    
    def test_nullable_inference(self):
        """Test that nullable columns are correctly identified."""
        # Check columns with NULL values
        null_columns = self.industrial_df.columns[self.industrial_df.isnull().any()].tolist()
        ...
        for col_name in null_columns[:5]:  # Check first 5 null columns
            if col_name in self.metadata_dict:
                ...
                print(f"📊 Nullable column {col_name}: nullable={meta.nullable}, missing_count={self.industrial_df[col_name].isnull().sum()}")
    ...
    def test_unique_flag_detection(self):
        ...
        for col_name, meta in list(self.metadata_dict.items())[:10]:  # Check first 10 columns
            actual_unique_ratio = self.industrial_df[col_name].nunique() / len(self.industrial_df)
            ...
            if col_name == 'equipment_id':
                assert meta.unique_flag == True, "equipment_id should be flagged as unique"
    ...
    def test_industrial_domain_knowledge(self):
        ...
        if equipment_meta:
            assert equipment_meta.role == 'identifier', "Industrial equipment IDs should be identifiers"
            
        # Operational status should be categorical (updated expectation)
        status_meta = self.metadata_dict.get('operational_status')
        if status_meta:
            assert status_meta.role == 'feature', "Operational status classified as feature (improved algorithm)"
            assert status_meta.data_type == 'categorical', "Should be categorical data type"
        ...
        for sensor_col, expected_role in sensor_classifications.items():
            if sensor_col in self.metadata_dict:
                ...
                assert meta.role == expected_role, f"Sensor column {sensor_col} should be {expected_role}, got {meta.role}"
    ...
    def test_production_readiness(self):
        ...
        assert len(self.metadata_list) == len(self.industrial_df.columns), "Should have metadata for all columns"
...
class TestIndustrialInferenceEngine:
    ...
    def test_complex_industrial_scenarios(self):
        ...
        complex_data = pd.DataFrame({
            'plant_id': [f'PLANT_{i:03d}' for i in range(1, 51)],  # 50 unique plants
            'equipment_serial': [f'EQ{i:06d}' for i in range(100000, 100050)],  # Serial numbers
            ...
            'maintenance_code': np.random.choice(['A', 'B', 'C', 'D'], 50),  # Categorical
            'critical_threshold_exceeded': np.random.choice([True, False], 50),  # Boolean
            'performance_index': np.random.uniform(0.5, 1.0, 50),  # Performance metric
            'predicted_failure_days': np.random.randint(1, 365, 50),  # Target variable
        })
        ...
        assert metadata_dict['equipment_serial'].role in ['identifier', 'feature']  # Could be either
        assert metadata_dict['installation_date'].role == 'time_index'
        assert metadata_dict['maintenance_code'].role in ['group_by', 'feature']  # Could be either based on cardinality
        assert metadata_dict['predicted_failure_days'].role == 'target'  # Should be target
        
        print(f"✅ Complex industrial scenario: {len(metadata_list)} columns properly classified")
