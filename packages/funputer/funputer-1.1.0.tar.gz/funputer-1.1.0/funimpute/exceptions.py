...
def check_allowed_values_violation(
    ...
    if metadata.allowed_values is not None and len(data_series.dropna()) > 0:
        ...
        if allowed_values:
            non_null_data = data_series.dropna()
            invalid_values = non_null_data[~non_null_data.astype(str).isin(allowed_values)]
            
            if len(invalid_values) > 0:
                unique_invalid = invalid_values.unique()[:5]  # Limit to first 5 for readability
                return ImputationException(
                    method=ImputationMethod.ERROR_INVALID_METADATA,
...
def check_max_length_violation(
    ...
    """
    if metadata.max_length is not None and metadata.data_type in ['string', 'categorical']:
