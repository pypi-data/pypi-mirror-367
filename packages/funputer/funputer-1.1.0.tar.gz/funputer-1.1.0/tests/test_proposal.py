...
from funimpute.proposal import (
    propose_imputation_method, calculate_confidence_score
)
from funimpute.models import (
    ColumnMetadata, AnalysisConfig, ImputationMethod, MissingnessType,
    MissingnessAnalysis, OutlierAnalysis, OutlierHandling, ImputationProposal
)
...
class TestConfidenceScoreCalculation:
    ...
    def create_mock_analyses(self, missing_pct=0.1, mechanism=MissingnessType.MCAR, 
                           outlier_pct=0.02, data_size=100):
        ...
        missingness_analysis.p_value = 0.5 if mechanism == MissingnessType.MCAR else 0.01
    ...
    def test_confidence_score_low_missing(self):
        ...
        missingness_analysis, outlier_analysis = self.create_mock_analyses(missing_pct=0.02)
    ...
    def test_confidence_score_high_missing(self):
        ...
        missingness_analysis, outlier_analysis = self.create_mock_analyses(missing_pct=0.6)
    ...
    def test_confidence_score_with_business_rule(self):
        ...
        assert confidence == base_confidence  # Business rule doesn't affect confidence score directly
...
class TestImputationMethodProposal:
    ...
    def create_test_scenario(self, data_type='integer', missing_count=10, 
                           mechanism=MissingnessType.MCAR, unique_flag=False,
                           business_rule=None, dependent_column=None):
        """Helper to create test scenario."""
        data_series = pd.Series([1, 2, None, 4, 5, None, 7, 8, 9, 10] * 2)
        metadata = ColumnMetadata(
            'test_col', data_type, unique_flag=unique_flag,
            business_rule=business_rule, dependent_column=dependent_column
        )
        ...
        missingness_analysis.mechanism = mechanism
        missingness_analysis.related_columns = ['other_col'] if mechanism == MissingnessType.MAR else []
        missingness_analysis.p_value = 0.05 if mechanism == MissingnessType.MAR else None
        missingness_analysis.test_statistic = 2.5 if mechanism == MissingnessType.MAR else None
    ...
    def test_no_missing_values_exception(self):
        ...
        data_series, metadata, missingness_analysis, outlier_analysis, config = self.create_test_scenario()
...
class TestProposalIntegration:
    ...
    def test_adaptive_threshold_integration(self):
        ...
        proposal = propose_imputation_method(
            'test_col', data_series, metadata, missingness_analysis, 
            outlier_analysis, config, full_data, metadata_dict
        )
