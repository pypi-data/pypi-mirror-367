...
def calculate_confidence_score(
    ...
    if len(non_null_data) > 0:
        ...
        if metadata.data_type in ['string', 'categorical'] and metadata.max_length is not None:
    ...
    if metadata.data_type == 'categorical':
        ...
        if allowed_values and len(allowed_values) > 0:
            ...
            if mechanism == MissingnessMechanism.MCAR:
                ...
                confidence = _adjust_confidence_for_constraints(confidence, metadata, data_series)
        ...
        if metadata.max_length is not None:
            ...
            if mechanism == MissingnessMechanism.MCAR:
                return ImputationProposal(
                    ...
                    parameters={"strategy": "most_frequent", "max_length": metadata.max_length},
                    confidence_score=confidence
                )
            else:  # MAR
                return ImputationProposal(
