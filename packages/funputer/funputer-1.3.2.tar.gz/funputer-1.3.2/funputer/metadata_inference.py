...
class MetadataInferenceEngine:
    ...
    def _infer_column_metadata(self, df: pd.DataFrame, column_name: str) -> ColumnMetadata:
        ...
        role = self._infer_role(column_name, data_type, unique_flag, df.columns.get_loc(column_name), len(df.columns))
    ...
    def _infer_role(self, column_name: str, data_type: str, unique_flag: bool, position: int, total_cols: int) -> str:
        """Infer the role of a column."""
        name_lower = column_name.lower()
        
        # Identifier patterns - first priority
        if unique_flag and ('id' in name_lower or 'key' in name_lower or 'uuid' in name_lower):
            return 'identifier'
        
        # Time index patterns
        if data_type == 'datetime' and any(x in name_lower for x in ['time', 'date', 'timestamp']):
            return 'time_index'
        
        # Target patterns (common ML target names or last column)
        target_indicators = ['target', 'label', 'outcome', 'prediction', 'y_', 'class', 'response']
        if any(x in name_lower for x in target_indicators) or (position == total_cols - 1 and total_cols > 3):
            return 'target'
        
        # Group-by patterns
        group_indicators = ['segment', 'group', 'category', 'type', 'class', 'cohort', 'bucket']
    ...
    def _infer_sentinel_values(self, series: pd.Series, data_type: str) -> Optional[str]:
        ...
        if data_type in ['integer', 'float']:
            ...
            for sentinel in [-999, -99, -1, 999999, 0]:
                if sentinel in value_counts and value_counts[sentinel] / total_count > 0.05:
                    return str(sentinel)
    ...
    def _infer_time_index(self, column_name: str, data_type: str) -> bool:
        ...
        name_lower = column_name.lower()
        time_indicators = ['timestamp', 'time', 'date', 'created', 'updated', 'occurred', 'recorded']
        
        return any(indicator in name_lower for indicator in time_indicators)
    
    def _infer_group_by(self, column_name: str, data_type: str, series: pd.Series) -> bool:
        ...
        group_indicators = ['segment', 'group', 'category', 'type', 'class', 'cohort', 'bucket', 'region', 'zone']
...
STANDARD_CSV_LAYOUT = [
    "Column", "Data_Type", "Role", "Do_Not_Impute", "Time_Index", "Group_By",
...
STATISTICS_FIELDS = [
    "Missing_Count", "Missing_Pct", "Unique_Count", "Unique_Pct", "Sample_Values"
]
