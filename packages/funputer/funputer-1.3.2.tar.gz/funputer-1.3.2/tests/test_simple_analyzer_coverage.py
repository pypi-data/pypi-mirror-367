...
from funputer.simple_analyzer import (
    SimpleImputationAnalyzer, analyze_imputation_requirements, analyze_dataframe
)
...
class TestSimpleAnalyzerMissingCoverage:
    ...
    def test_analyze_enterprise_metadata_conversion(self):
        ...
        with patch('funimpute.simple_analyzer.load_metadata') as mock_load_meta, \
             patch('funimpute.simple_analyzer.load_data') as mock_load_data, \
             patch('funimpute.io.convert_enterprise_to_legacy') as mock_convert:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as data_file, \
                 tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as meta_file:
    ...
    def test_analyze_column_not_found_warning(self):
        ...
        with patch('funputer.simple_analyzer.logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_with("Column missing_col not found in data - skipping")
    ...
    def test_analyze_skip_column_configuration(self):
        ...
        with patch('funputer.simple_analyzer.logger') as mock_logger:
            ...
            skip_log_calls = [call for call in mock_logger.info.call_args_list if 'Skipping column skip_col per configuration' in str(call)]
            assert len(skip_log_calls) > 0, f"Expected skip log not found. Actual calls: {mock_logger.info.call_args_list}"
    ...
    def test_analyze_dataframe_missing_column_warning_path(self):
        ...
        with patch('funputer.simple_analyzer.logger') as mock_logger:
            ...
            # Should log warning for missing column
            mock_logger.warning.assert_called_with("Column missing not found in data - skipping")
            assert len(suggestions) == 1
    ...
    def test_analyze_dataframe_skip_column_info_path(self):
        ...
        with patch('funputer.simple_analyzer.logger') as mock_logger:
            ...
            # Should log info for skipped column
            skip_log_calls = [call for call in mock_logger.info.call_args_list if 'Skipping column skip per configuration' in str(call)]
            assert len(skip_log_calls) > 0, f"Expected skip log not found. Actual calls: {mock_logger.info.call_args_list}"
            assert len(suggestions) == 1
    ...
    def test_analyze_imputation_requirements_auto_inference_path(self):
        ...
        try:
            ...
            with patch('funimpute.metadata_inference.infer_metadata_from_dataframe') as mock_infer:
                mock_metadata = [
    ...
    def test_both_analyze_methods_timing_logs(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as data_file, \
            ...
            try:
                with patch('funimpute.simple_analyzer.load_metadata') as mock_load_meta, \
                     patch('funimpute.simple_analyzer.load_data') as mock_load_data, \
                     patch('funimpute.simple_analyzer.logger') as mock_logger:
...
class TestEdgeCasesForFullCoverage:
    ...
    def test_analyze_with_exception_in_processing(self):
        ...
        with patch('funimpute.simple_analyzer.analyze_outliers') as mock_outliers:
            # Let it work normally - just testing that we can handle processing
            mock_outliers.side_effect = lambda *args, **kwargs: MagicMock(
                outlier_count=0, outlier_percentage=0.0, handling_strategy=MagicMock(value="Leave as is"),
