...
from funimpute.models import (
    ColumnMetadata, MissingnessAnalysis, OutlierAnalysis, 
    ImputationMethod, MissingnessMechanism, AnalysisConfig
)
from funimpute.exceptions import (
    check_nullable_violation, check_allowed_values_violation, 
    check_max_length_violation
)
from funimpute.proposal import (
    propose_imputation_method, calculate_confidence_score,
    _get_allowed_values_list, _adjust_confidence_for_constraints
)
...
class TestMetadataIntegration:
    ...
    def test_confidence_adjustment_for_constraints(self):
        ...
        result = _adjust_confidence_for_constraints(base_confidence, metadata, data_series)
        ...
        result = _adjust_confidence_for_constraints(base_confidence, metadata, data_series_no_missing)
        assert result > base_confidence
    ...
    def test_proposal_with_allowed_values(self):
        ...
        proposal = propose_imputation_method(
            "color", data_series, metadata, missingness_analysis, 
    ...
    def test_proposal_with_max_length(self):
        ...
        proposal = propose_imputation_method(
            "name", data_series, metadata, missingness_analysis, 
            outlier_analysis, config
        )
    ...
    def test_nullable_false_with_missing_values(self):
        ...
        from funimpute.exceptions import apply_exception_handling
        result = apply_exception_handling(
            "test", data_series, metadata, missingness_analysis, 
            outlier_analysis, config
        )
    ...
    def test_description_field_usage(self):
        ...
        proposal = propose_imputation_method(
            "status", data_series, metadata, missingness_analysis, 
            outlier_analysis, config
        )
        ...
        assert proposal.method == ImputationMethod.MODE
        assert "allowed values constraint" in proposal.rationale
        assert proposal.parameters["allowed_values"] == ["active", "inactive", "suspended"]
