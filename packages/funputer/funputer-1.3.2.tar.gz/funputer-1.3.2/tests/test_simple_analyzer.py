...
class TestSimpleImputationAnalyzer:
    ...
    def test_analyze_dataframe_all_missing_values(self):
        ...
        all_missing_suggestion = next(s for s in suggestions if s.column_name == 'all_missing')
    ...
    def test_analyze_dataframe_business_rule(self):
        ...
        data = pd.DataFrame({
            'age': [25, 30, 35, 40, 45],
            'income': [50000, None, 70000, None, 90000]
        })
        metadata = [
    ...
    def test_analyze_with_file_paths(self, temp_csv_files):
        ...
        suggestions = analyzer.analyze(
            temp_csv_files['metadata_path'], 
            temp_csv_files['data_path']
        )
...
class TestAnalysisResults:
    ...
    def test_categorical_data_analysis(self):
        ...
        data = pd.DataFrame({
            'category': ['A', 'B', None, 'A', 'C', None, 'B', 'A']
        })
    ...
    def test_numeric_data_analysis_normal_distribution(self):
        ...
        assert suggestion.proposed_method == "Mean"  # Should choose mean for normal data
    ...
    def test_numeric_data_analysis_skewed_distribution(self):
        ...
        suggestion = suggestions[0]
        assert suggestion.column_name == 'values'
        assert suggestion.proposed_method == "Median"  # Should choose median for skewed data
        assert suggestion.missing_count == 5
    
    def test_boolean_data_analysis(self):
        """Test analysis of boolean data."""
        data = pd.DataFrame({
            'is_active': [True, False, None, True, None, False, True]
        })
    ...
    def test_high_missing_percentage_analysis(self, high_missing_data):
        ...
        mostly_missing_suggestion = next(s for s in suggestions if s.column_name == 'mostly_missing')
    ...
    def test_outlier_detection_integration(self, outlier_data):
        ...
        assert values_suggestion.outlier_handling in ["Cap to bounds", "Leave as is", "Convert to NaN"]