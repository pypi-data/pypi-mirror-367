    ...
    try:
        ...
        for metadata in inferred_metadata:
            ...
            template_row = {
                ...
                'allowed_values': getattr(metadata, 'allowed_values', '') or '',  # Auto-inferred for categorical/boolean
                'dependent_column': getattr(metadata, 'dependent_column', '') or '',  # Auto-inferred from correlations
...
@click.option('--sample-rows',
              default=2000,
              help='Maximum rows to sample (default: 2000)')
@click.option('--max-sniff-bytes',
              default=65536,
              help='Maximum bytes to read for format detection (default: 65536)')
@click.option('--delimiter',
              help='CSV delimiter hint (auto-detected if not specified)')
@click.option('--encoding',
              help='File encoding hint (auto-detected if not specified)')
@click.option('--no-header',
              is_flag=True,
              help='Indicate CSV has no header row')
@click.option('--json-out',
              help='Write JSON report to specified file')
def preflight(data, sample_rows, max_sniff_bytes, delimiter, encoding, no_header, json_out):
    """
    ...
    funimputer preflight -d data.csv
    
    # With custom sample size and hints
    funimputer preflight -d data.csv --sample-rows 5000 --encoding latin-1
    
    # Output JSON report
    funimputer preflight -d data.csv --json-out preflight.json
    """
...
def _run_advisory_preflight(data_path: str):
    ...
    try:
        ...
        if (report["status"] == "hard_error" and 
            report.get("file", {}).get("error")):
        ...
        if report["recommendation"] == "generate_metadata":
            ...
            click.echo("   Use: funimputer init -d {} -o metadata.csv".format(data_path))
            click.echo()
