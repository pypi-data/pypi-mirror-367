...
class HopConfig:
    ...
    def __pydantic_extra__(self) -> Dict[str, Union[int, InferredType]]:
        ...
        del extra['default']
        extra.pop('__pydantic_initialised__', None)
        return extra

    # NOTE: when this method is called, __pydantic_extra__ can contain values
    # of type str, which need to be converted to InferredType
    def __post_init__(self):
        for key, value in self.__pydantic_extra__.items():
            ...
            if value == 'inferred':
                setattr(self, key, InferredType.VALUE)
                continue
...
class TrainingTableGenerationPlan(PlanMixin):
    ...
    train_start_offset: Union[NonNegativeInt, InferredType,
    ...
    train_end_offset: Union[NonNegativeInt, InferredType,
                            MissingType] = compat_field(
                                default_factory=lambda: MissingType.VALUE,
                                metadata=Metadata(
                                    valid_query_types=[QueryType.TEMPORAL]),
    ...
    timeframe_step: Union[PositiveInt, InferredType,
                          MissingType] = compat_field(
                              default_factory=lambda: MissingType.VALUE,
                              metadata=Metadata(
                                  valid_query_types=[QueryType.TEMPORAL]),
...
class TrainingJobPlan(PlanMixin):
    ...
    num_experiments: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
    )
...
class OptimizationPlan(PlanMixin):
    ...
    max_epochs: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
    )

    min_steps_per_epoch: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
        le=4000,
    ...
    max_steps_per_epoch: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
    )

    max_val_steps: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
        le=8000,
    ...
    max_test_steps: Union[int, MissingType] = compat_field(
        default_factory=lambda: MissingType.VALUE,
        metadata=Metadata(),
        ge=1,
        le=8000,
