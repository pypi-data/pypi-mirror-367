...
class APIKeyManager:
    ...
    def _fetch_client_keys_from_api(self) -> Dict[str, str]:
        ...
        try:
            ...
            base_url = os.getenv("CONCEPTUAL_BASE_URL", "https://api.conceptualhq.com/api")
            ...
            with httpx.Client(timeout=httpx.Timeout(30.0)) as client:
                ...
                if response.status_code >= 400:
                    print(f"Failed to fetch client keys: HTTP {response.status_code}")
                    return {}
                ...
                if keys:
                    print(f"Successfully fetched keys for {len(keys)} clients from admin API")
                
                return keys
                
        except Exception as e:
            print(f"Error fetching client keys from admin API: {str(e)}")
            return {}