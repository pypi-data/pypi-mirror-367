...
def add_stim_misc(markers, stim=[str(val) for val in range(1, 200)], pre=False):
    for idx, marker in enumerate(markers):
        val = get_val_in_tag(marker, "marker")
        if val in stim:
            new_tag = "stim"
...
def add_tag_to_markers(
    ...
    **kwargs,
    ...
    for idx, marker in enumerate(markers):
        ...
        new_tag = default_tag
        for tag_name, val_search in kwargs.items():
            if val in val_search:
                new_tag = tag_name
...
def split_trials(
    markers, trial=[str(val) for val in range(201, 300)], init_trial_num=1, pre=False
):
