...
def make_n_hole_flange_rsolid(
    ...
    try:
        ...
        print(f"    法兰外圆柱创建成功，直径: {flange_outer_diameter}mm, 厚度: {flange_thickness}mm")
    ...
    try:
        ...
        print(f"    凸起圆环外圆柱创建成功，直径: {boss_outer_diameter}mm，高度: {boss_height}mm")
    ...
    try:
        ...
        print(f"    连接孔参数：孔径{hole_diameter}mm，分布圆直径{hole_circle_diameter}mm，孔数{hole_count}")
    ...
    try:
        ...
        # 尝试对主要边进行倒角
        try:
            chamfered_flange = chamfer_rsolid(flange_with_holes, all_edges, chamfer_size) 
...
def make_naca_propeller_blade_rsolid(
    ...
    def generate_naca_0016_points(chord_length=1.0, num_points=50):
        ...
        def naca_0016_thickness(x_c):
            ...
            if x_c < 0 or x_c > 1:
                return 0.0
            return t_c * (0.2969 * math.sqrt(x_c) - 0.1260 * x_c - 0.3516 * x_c**2 + 0.2843 * x_c**3 - 0.1015 * x_c**4)
    ...
    try:
        ...
        if len(section_wires) < 2:
            raise ValueError(f"截面数量不足：需要至少2个截面，当前有{len(section_wires)}个")
...
def make_threaded_rod_rsolid(
    ...
    print(f"  螺纹: 长度={thread_length}mm, 螺距={thread_pitch}mm, 起始位置={thread_start_position}mm")
    ...
    if thread_start_position < 0 or thread_start_position + thread_length > total_length:
    ...
    try:
        ...
        print(f"    螺杆主体创建成功，直径: {thread_diameter}mm，长度: {total_length}mm")
    ...
    try:
        ...
        thread_points = [
            ...
            (float(thread_radius - thread_depth), float(thread_pitch * 0.5), 0.0),  # 螺纹根部，轴向偏移pitch/2
            (float(thread_radius), float(thread_pitch), 0.0)  # 螺纹大径处，轴向偏移pitch
        ]
        ...
        # 调整螺旋切割体位置到螺纹起始位置
        translated_helical_cut = translate_shape(helical_cut_solid, (0, 0, -thread_length-thread_start_position))
        helical_cut_solid = cast(Solid, translated_helical_cut)
        print(f"    螺旋切割体创建成功，螺距: {thread_pitch}mm，起始位置: {thread_start_position}mm")
