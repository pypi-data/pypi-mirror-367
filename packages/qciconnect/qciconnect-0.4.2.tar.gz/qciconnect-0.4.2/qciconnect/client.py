...
from .token_managers import DummyTokenManager, KeycloakTokenManager
...
TAC_STATUS_ENDPOINT = "/api/terms_and_conditions/status"
PRIVACY_NOTICE_ENDPOINT = "/api/privacy_notice/status"
...
class QciConnectClient:
    ...
    def server_address(self, value: str):
        ...
        if self._username is None and self._password is None:
            ...
            self._keycloak_token_manager = KeycloakTokenManager(
        ...
        with httpx.Client(follow_redirects=False, verify=True) as client:
            try:
                post_response = client.post(
                    f"{self._server_address}{JOB_SUBMIT_ENDPOINT}", json=request, headers=headers
                )
    ...
    def get_tac_status(self) -> bool:
        ...
        endpoint_url = f"{self._server_address}{TAC_STATUS_ENDPOINT}"
        return self._get(endpoint_url)

    def get_privacy_notice_status(self) -> bool:
        """Retrieves status of privacy notice acceptance."""
        endpoint_url = f"{self._server_address}{PRIVACY_NOTICE_ENDPOINT}"
        return self._get(endpoint_url)
