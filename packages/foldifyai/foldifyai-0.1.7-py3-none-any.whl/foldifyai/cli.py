...
def fold(args):
    ...
    if args.output == '': output_dir = f"foldify_{folder}/"
    ...
    for c,p in enumerate(pbar):
        ...
        print(path)
        if os.path.exists(f"{path}boltz2_prediction_0.pdb"):
        ...
        try: 
            ...
            url = f"{args.host}/fold?only_return_zip=True&seq={encoded}&{args.args}&gpu={args.gpu}"
            ...
            while True:
                ...
                #print(f"[Foldify]{Style.RESET_ALL} {p} -  {num_tokens} - {len(result)/1000}KB")
...
def main() -> None:  # pragma: no cover
    ...
    parser.add_argument('-gpu', '-g', type=str, int=0, help='GPU')
