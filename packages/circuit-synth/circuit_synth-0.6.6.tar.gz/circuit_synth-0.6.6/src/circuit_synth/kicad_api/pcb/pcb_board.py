...
class PCBBoard:
            ...
            if "model" in library_data:
                for model_data in library_data["model"]:
                    ...
                    if model_info:
                        ...
                        footprint.model_offset = model_info.get(
                            "offset", (0.0, 0.0, 0.0)
                        )
                        footprint.model_scale = model_info.get("scale", (1.0, 1.0, 1.0))
                        footprint.model_rotate = model_info.get(
                            "rotate", (0.0, 0.0, 0.0)
                        )
                        logger.debug(
                            f"Added 3D model {model_info['path']} to footprint {reference}"
                        )
    ...
    def _parse_library_model(
        self, model_data: Union[List, Dict[str, Any]]
                        ...
                        if isinstance(item[1], list) and len(item[1]) >= 4:
                            ...
                            if str(xyz_data[0]) == "xyz" and len(xyz_data) >= 4:
                                try:
                                    ...
                                    logger.debug(
                                        f"Failed to parse {param_type} values from {xyz_data}"
                                    )
