...
def import_sessions(ops: dict[str, Any]) -> MultiDayData:
    ...
    for session_id, session_path in zip(session_ids, sessions):
        ...
        if ops["mesoscope_stripe_borders"]:
            ...
            for cell in selected_cells:
                # Checks if the cell is far enough from all borders
...
def export_masks_and_images(
    ...
    for session in data.sessions:
        ...
        output_folder = Path(ops["multiday_save_path"]).joinpath(
            ops["multiday_save_folder"], session.session_id, "multi_day"
        )
        ...
        np.save(
            ...
            session.template_cell_masks,  # type: ignore
        )

        # Template cell masks in the multi-day registered (deformed) visual space
        np.save(output_folder.joinpath("template_cell_masks.npy"), data.template_cell_masks)  # type: ignore

        # Reference images modified by deformation (multi-day registration) offsets
        np.save(output_folder.joinpath("transformed_images.npy"), session.transformed_images)  # type: ignore

        # Original (single-day) reference images
        np.save(output_folder.joinpath("original_images.npy"), session.reference_images)  # type: ignore

        # Multi-day processing parameters.
        np.save(output_folder.joinpath("ops.npy"), ops)  # type: ignore
