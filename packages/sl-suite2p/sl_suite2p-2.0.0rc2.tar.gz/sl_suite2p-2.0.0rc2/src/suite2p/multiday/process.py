...
across multiple sessions and generate deconvolved cell spike traces. Specifically, this module works with the output
of the multi-day registration (step 1) pipeline to re-extract the data from each processed session, using the
'multi-day' (template) cell masks."""
...
def extract_session_traces(ops: dict[str, Any], session_folder: Path, session_id: str) -> None:
    ...
    multiday_folder = Path(ops["multiday_save_path"]).joinpath(ops["multiday_save_folder"], session_id, "multi_day")
    ...
    cell_masks = roi_stats(cell_masks, ops["Ly"], ops["Lx"], ops["aspect"], ops["diameter"])  # type: ignore
    ...
    with BinaryFileCombined(
        ...
        plane_y_coordinate,
        plane_x_coordinate,
        registered_data_path,  # type: ignore
    ) as file:
        cell_fluorescence, neuropil_fluorescence = extraction.extract.extract_traces(  # type: ignore
            f_in=file,
    ...
    np.save(multiday_folder.joinpath("ops.npy"), ops)  # type: ignore
