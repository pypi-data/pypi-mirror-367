"""This module provides tools for importing data stored inside .tif or .tiff files. It also contains the tools for
working with .tiff stacks acquired via the 2-Photon Random Access Mesoscope, which requires additional processing,
compared to the 'regular' .tiff stacks."""
...
def generate_tiff_filename(
    ...
    if channel == 0:  # Channel 0.
        if functional_channel == alignment_channel:
            ...
            channel_index = 1
    else:  # Channel 1. This is NOT designed to work for more than two channels.
        if functional_channel == alignment_channel:
            tiff_root = path.joinpath("channel_2_tiffs")
            channel_index = 1
        else:
...
def _read_tiff(tiff: TiffFile, start_index: int, batch_size: int) -> NDArray[np.int16] | None:
    ...
    This function loads the requested subset of data into memory. Configure the batching parameters based on your system
    resources to avoid excessive memory usage.
    ...
    if tiff_length == 1:
        frames = tiff.asarray()  # If the .tiff file stores a single frame, reads it as a NumPy array
    else:
        # If the file is a stack with multiple frames, slices the file to read the frames from the start index onward.
        frames = tiff.asarray(key=range(start_index, start_index + frames_to_read))
    ...
    if frames.dtype.type == np.uint16:
        frames = (frames // 2).astype(np.int16)
    elif frames.dtype.type == np.int32:
        frames = (frames // 2).astype(np.int16)
...
def tiff_to_binary(ops: dict[str, Any]) -> dict[str, Any]:
    ...
    for file_index, file in enumerate(files):
        ...
        while True:
            ...
            # Updates progress bar with number of frames processed in this batch.
            pbar.update(nframes)
    ...
    for ops in plane_ops:
        ...
        # subdirectory.
        np.save(ops["ops_path"], ops)  # type: ignore
...
def mesoscan_to_binary(ops: dict[str, Any]) -> dict[str, Any]:
    ...
    # Extracts the total number of planes inside the input data to reduce code complexity below.
