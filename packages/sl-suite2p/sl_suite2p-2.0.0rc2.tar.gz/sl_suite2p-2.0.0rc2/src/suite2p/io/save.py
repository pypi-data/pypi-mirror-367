...
dataset. It includes functions for combining data from multiple plane directories, handling displacement corrections,
and saving the data as a MATLAB-compatible (.mat) file or a set of combined suite2p files."""
...
def compute_dydx(plane_ops: list[dict[str, Any]]) -> tuple[NDArray[np.int64], NDArray[np.int64]]:
    ...
    # Otherwise, uses "dy" and "dx" values directly.
        ...
        if roi_number < plane_number:
            ...
            plane_number = plane_number // roi_number
...
def combined(
    ...
    Returns:
        A tuple of ten elements. The first element is a NumPy array that stores the ROI statistics. The second element
        is the 'ops' dictionary that combines the data for all processed planes. The third, fourth, and fifth elements
        are the NumPy arrays storing the cell fluorescence, neuropil fluorescence, and deconvolved spike data. The
        sixth and seventh elements are NumPy arrays that store the boolean cell classification and likelihood data
        (in this order) for the first channel. The eighth and ninth elements are NumPy arrays that store the boolean
        cell classification and likelihood data for the second channel. The tenth element is a boolean flag that
        indicates whether the processed recording contained the second channel.
    """
    ...
    for plane_index, ops in enumerate(plane_ops):
        ...
        if plane_index == 0:
            cell_fluorescence, neuropil_fluorescence, spikes, roi_statistics, is_cell, red_cell = (
    ...
    if save:
        ...
        if ops.get("save_mat"):
            save_matlab(ops, roi_statistics, cell_fluorescence, neuropil_fluorescence, spikes, is_cell, red_cell)
