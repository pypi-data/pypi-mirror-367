...
It includes functions for searching and retrieving .tif and .tiff files from specified directories, optionally
searching through subdirectories and sorting them naturally. The module also includes functions for constructing
plane-specific 'ops' dictionaries for multi-plane imaging datasets, preparing memory-mapped binary files,
and ensuring the creation of necessary directories used in the data input and output processes."""
...
def _search_files_by_extension(
    ...
    Originally, this worker function was used by multiple higher-level functions to discover specific data files. In the
    current version of sl-suite2p, support for all container types other than .tif / .tiff has been deprecated.
    Therefore, the function is currently only used to discover .tiff files, despite being container-agnostic.
...
def _get_tiff_list(ops: dict[str, Any]) -> tuple[list[Path], dict[str, Any]]:
    ...
    # Otherwise, searches for .tif and .tiff files in directories or subdirectories.
        ...
        if len(file_paths) == 0:
            message = f"Could not find any TIF/TIFF files to process."
            console.error(message=message, error=FileNotFoundError)
...
def find_files_open_binaries(
    ...
    for ops in plane_ops:
        ...
        if "keep_movie_raw" in ops and ops["keep_movie_raw"]:
            ...
            channel_1_binary_files.append(open(ops["raw_file"], "wb"))
            # If there is a second channel, opens the raw binary and appends it to 'channel_2_binary_files'.
            if channel_number > 1:
                channel_2_binary_files.append(open(ops["raw_file_chan2"], "wb"))
        else:
            # Opens the registered binary file and appends it to 'channel_1_binary_files'.
            channel_1_binary_files.append(open(ops["reg_file"], "wb"))
            # If there is a second channel, opens the registered binary and appends it to 'channel_2_binary_files'.
            if channel_number > 1:
                channel_2_binary_files.append(open(ops["reg_file_chan2"], "wb"))

    # Determines the input format based on the first plane's 'ops' dictionary.
    if "input_format" in plane_ops[0].keys():
        input_format = plane_ops[0]["input_format"]

    # If no "input_format" key is specified in the plane-specific 'ops' dictionary, defaults to "tiff".
    else:
        input_format = "tiff"
...
def initialize_plane_ops(ops: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for plane_index in range(plane_number):
        ...
        if len(ops["save_folder"]) > 0:
            ops["save_path"] = ops["save_path0"].joinpath(ops["save_folder"], "plane%d" % plane_index)
        else:
            ops["save_path"] = ops["save_path0"].joinpath("suite2p", "plane%d" % plane_index)

        # Resolves the plane-specific working directory path.
        fast_disk = ops["fast_disk"].joinpath("suite2p", "plane%d" % plane_index)
