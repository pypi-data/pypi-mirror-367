...
class MultiDayS2PConfiguration(YamlConfig):
    ...
    Note:
        During runtime, the parameters from this class are merged with the parameters from the SingleDayS2PConfiguration
        class. The two classes are designed to be mutually compatible and to not reuse any top-level attributes and
        keys.
    """
    ...
    def from_ops(cls, ops_dict: dict[str, Any]) -> "MultiDayS2PConfiguration":
        """Creates a MultiDayS2PConfiguration instance from an ops.npy dictionary.

        This method reconstructs a configuration instance from the flattened ops dictionary format used by suite2p. It
        handles missing parameters by using default values and ignores extra parameters not recognized by the
        configuration schema.

        Notes:
            Primarily, this method is used internally to ensure that each runtime output folder contains both an
            'ops.npy' file and a 'configuration.yaml' file. Primarily, this is important to work around inherently
            unsafe 'ops.npy' pickling behavior which may make it impossible to unpickle the data from 'ops.npy' under
            certain circumstances.

        Args:
            ops_dict: The dictionary typically loaded from ops.npy file containing suite2p parameters.

        Returns:
            MultiDayS2PConfiguration instance with parameters from ops_dict, using configuration defaults for any
            missing parameters.
        """
...
def generate_default_multiday_ops(as_dict: bool = True) -> dict[str, Any] | MultiDayS2PConfiguration:
    ...
    Args:
        as_dict: If True, the function converts the class to dictionary format, which is the format used by the suite2p
            under-the-hood. Otherwise, returns the class as the 'MultiDayS2PConfiguration' dataclass instance, which
            is used by all high-level API functions.
    """
