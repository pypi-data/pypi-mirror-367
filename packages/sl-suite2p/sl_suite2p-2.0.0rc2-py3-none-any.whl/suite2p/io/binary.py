...
The 'BinaryFile' class exposes a NumPy-array-like interface for working with the binary data of a single
imaging plane, and the 'BinaryFileCombined' class extends this functionality to multi-plane datasets.
Both BinaryFile classes are designed to directly memory-map the data stored inside the files to reduce memory usage.
"""
...
class BinaryFile:
    ...
    def __init__(self, height: int, width: int, file_path: str | Path, frame_number: int = 0, dtype: str = "int16"):
    ...
    def convert_numpy_file_to_suite2p_binary(source_file_name: Path, destination_file_name: Path) -> None:
        ...
        Specifically, create a new BinaryFile instance using the data stored in the target NumPy file.
    ...
    def shape(self) -> tuple[int, int, int]:
        ...
        The first element is the number of frames. The second element is the height of each frame, and the third
    ...
    def close(self) -> None:
        ...
        self.file._mmap.close()  # type: ignore
    ...
    def __exit__(self, execution_type: Any, execution_value: Any, execution_traceback: Any) -> None:
...
class BinaryFileCombined:
    ...
    This class allows working with multiple image planes, each stored inside a separate Suite2p BinaryFile.
    ...
    def __init__(
        ...
        self.files = [
            ...
            for (plane_heights, plane_widths, file_name) in zip(self.plane_heights, self.plane_widths, self.file_paths)
        ]
    ...
    def __getitem__(self, indices: slice | int | tuple[int, ...] | NDArray[Any]) -> NDArray[np.int16]:
        ...
        """
        # Initializes an empty NumPy array to store the combined data. Note, the array uses the height and width
        # obtained from combining all managed planes. The number of frames is inferred from class properties.
        data = np.zeros((self.frame_number, self.height, self.width), dtype="int16")

        # Iterates through each file and copies the relevant data slice(s) into the combined array.
        for file_index, file in enumerate(self.files):
            file_data = file[indices]  # Reads the requested data slice from each file
