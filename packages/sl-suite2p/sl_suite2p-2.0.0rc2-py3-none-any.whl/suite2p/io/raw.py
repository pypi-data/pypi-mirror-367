...
class _RawFile:
    ...
    def __init__(self, directory_path: Path):
        ...
        with open(file=self._xml_file_path, mode="r", encoding="utf-8") as xml_file:
...
def raw_to_binary(ops: dict[str, Any], override_ops_parameters: bool = True) -> dict[str, Any]:
    ...
    raw_files = [_RawFile(path) for path in ops["data_path"]]
    ...
    ops_paths = _initialize_destination_files(
        ops=ops, raw_files=raw_files, override_ops_parameters=override_ops_parameters
    )
...
def _initialize_destination_files(
    ...
    if any(configuration != configurations[0] for configuration in configurations):
        message = (
            f"Unable to convert the input list of Thorlabs .raw files to Suite2P plane BinaryFiles. The recording "
            f"configurations of the input .raw files do not match for at least two file instances, indicating that "
            f"the files belong to separate recordings."
        )
...
def _single_raw_to_binary(plane_ops: list[dict[str, Any]], raw_file: _RawFile) -> None:
    ...
    with open(raw_file.path, "rb") as file:
        ...
        while frame_chunk:
            ...
            for z_plane_index in range(raw_file.z_planes):
                ...
                if raw_file.channel > 1:
                    ...
                    with open(ops["reg_file"], "ab") as channel_1_binary_file:
                    ...
                    with open(ops["reg_file_chan2"], "ab") as channel_2_binary_file:
