"""This module stores various helper functions used by all other modules of the 'multiday' sl-suite2p package. Any
function used by multiple multi-day processing modules is stored in this module."""
...
def create_mask_image(
    ...
    for index, mask in enumerate(cell_masks):
        ...
        if not contours:
            ...
            contours_ind = np.vstack(find_contours(temporary_image)).astype(int)  # type: ignore
...
def create_cropped_deform_field(
    ...
    image_size = deform[0].shape
    crop_size_array = np.asarray(crop_size)

    # Adjust origin if the crop exceeds image bounds
        ...
        deformed.append(
            {
                ...
                "lam": lam_values,
                "radius": regionprops(warped_lam.astype(np.uint8))[0].minor_axis_length,  # type: ignore
            }
        )
...
def _find_overlapping_pixels(all_pixels: NDArray[np.int32]) -> NDArray[np.int32]:
    ...
    duplicates = np.unique(sorted_pixels[is_duplicate])
...
def extract_unique_components(paths: list[Path] | tuple[Path, ...]) -> tuple[str, ...]:
    ...
    Notes:
        ...
        somewhere along the path hierarchy. If this assumption is violated, the function will raise a RuntimeError.
