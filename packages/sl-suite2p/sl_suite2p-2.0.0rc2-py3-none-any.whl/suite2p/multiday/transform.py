"""This module provides functions for registering, transforming, clustering, and averaging cell masks across multiple
imaging sessions. It uses diffeomorphic demons registration and clustering based on Jaccard distance to identify
putative cells across sessions, and supports forward and backward transformations of masks. All methods from this module
are used during the first step of the multi-day suite2p processing pipeline to discover and track a population of cells
...
def register_sessions(ops: dict[str, Any], data: MultiDayData) -> MultiDayData:
    """Registers session images to the same visual space using DiffeomorphicDemonsRegistration.

    This function generates the deformation offsets used to align all session visual fields across days and applies
    them to all reference images for each session. Then, the function also applies the deformations to all cell masks
    of each session to register them to the same visual space.

    Args:
        ops: The dictionary that stores the multi-day registration parameters.
        data: A MultiDayData instance that stores the data from all sessions being registered.
    ...
    with ProcessPoolExecutor(max_workers=parallel_workers) as executor:
        ...
        # Collects results with the progress bar
        results = []
        for future in tqdm(
...
def generate_template_masks(ops: dict[str, Any], data: MultiDayData) -> MultiDayData:
    ...
    Clustering the cells across sessions reveals cells that can be reliably tracked across days. Generating the template
    mask for each such cell cluster allows sampling the cell activity at each day by transforming the template mask back
    to the original visual space of each session.

    Notes:
        This function does not use parallelization. Primarily, this is because of the clustering algorithm's handling
        of cells on the borders of each spatial grid bin, which favors sequential processing. Additionally, the run
        time of this function is already low, so there is no exceptional need to parallelize this function. That said,
        this function may be a good target for future optimization efforts.
    ...
    for grid_pos in tqdm(
        ...
        for i, cell_1, cell_2 in pair_indices:
            # noinspection PyTypeChecker
            ...
            # appear in roughly the same spatial position across motion-corrected session images. In turn, the cell would
            # then strongly overlap with itself across sessions.
            ...
            if union_size == 0:
                ...
                jaccard_distance = 1 - (num_both / union_size)
...
def backward_transform_masks(ops: dict[str, Any], data: MultiDayData) -> MultiDayData:
    ...
    This is the prerequisite for extracting the fluorescence of multi-day-tracked cells from all sessions.
    The function can work in parallel mode to process multiple sessions simultaneously.

    Args:
        ops: The dictionary that stores the multi-day registration parameters, including parallelization settings.
...
def _backward_transform_session(template_masks: tuple[dict[str, Any], ...], session: Session) -> Session:
    ...
    session.template_cell_masks = deform_masks(
        ...
        deform=session.deform.as_backward_inverse(),  # type: ignore
    )
