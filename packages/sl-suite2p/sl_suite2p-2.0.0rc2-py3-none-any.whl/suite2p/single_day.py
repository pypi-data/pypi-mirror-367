"""This module contains the high-level API for the single-day (original) suite2p processing pipeline. Primarily, it
exposes two ways of running the single-day pipeline on data. The first way is to use the 'run_s2p' function, which
executes the entire pipeline on the input data. The second way is to call the 'resolve_binaries', 'process_plane', and
'combine_planes' functions to execute specific pipeline steps. The latter stepwise execution is helpful when running
suite2p on remote compute clusters to efficiently parallelize processing steps. Regardless of the runtime mode, all
functions require the 'ops_path' argument, which is generated by running the 'resolve_ops' function.
"""
...
def _register_plane(
    ...
    """Registers (motion-corrects) the frames for the input plane.
    ...
    Args:
        ...
        raw_frames_path: Same as 'raw_frames_path', but the data in this file is not overwritten during processing,
            keeping it 'raw'.
        frames_channel_2_path: Same as 'frames_path', but for the second functional channel, if planed ata contains
    ...
    with (
        ...
        reference_image = ops["refImg"] if "refImg" in ops and ops.get("force_refImg", False) else None
...
def _compute_registration_metrics(ops: dict[str, Any], plane_number: int, frames_path: str) -> dict[str, Any]:
    """Computes registration quality metrics for the target plane.

    In the original suite2p, this function was executed serially as part of the 'registration' step. However, since this
    step can take a significant portion of the overall runtime and is primarily executed on a single core, in the
    sl-suite2p it runs in parallel with processing plane ROIs.
    ...
    Notes:
        ...
        result in a significant processing time reduction for some movies.

    Args:
        ops: The dictionary that stores the plane registration metric computation parameters.
        plane_number: The number (index) of the processed plane.
    ...
    with (
        ...
        n_samples = min(2000 if n_frames < 5000 or height > 700 or width > 700 else 5000, n_frames)
...
def _process_rois(
    ...
    Specifically, carries out ROI discovery, trace extraction, and spike deconvolution for the target plane. This step
    depends on the registration step to be performed first but can run in parallel with computing the registration
    metrics.
    ...
    with (
        ...
        if len(roi_statistics) > 0:
            ...
            if ops.get("save_mat"):
                ...
                redcell = np.load(fpath.joinpath("redcell.npy")) if ops["nchannels"] == 2 else np.zeros_like(iscell)
                io.save_matlab(
                    ops,
                    roi_statistics,
                    cell_fluorescence,
...
def _resolve_ops_json(data_path: Path) -> dict[str, Any]:
    """Uses the data stored inside the ops.json file located under the directory specified by the 'data_path' to
    selectively override some configuration parameters as part of the resolve_ops() function runtime.

    This worker function is primarily used when processing mesoscope-acquired data. All mesoscope-acquired data is
    expected to contain an 'ops.json' file generated by a mesoscope data helper script. The file contains some important
    parameters for single-day and multi-day suite2p runtimes, which cannot be resolved automatically and have to be
    added to the configuration dictionary before it is saved as ops.npy file.

    Args:
        data_path: The path to the data folder containing the data files to be processed. This path should be resolved
            by the resolve_ops function, typically from the 'data_path' field of the 'ops' or 'db' dictionary.

    Returns:
        A dictionary that contains the configuration parameters to be overwritten and their new values.
    """

    # Loads the ops.json file if it exists under the input path
    ops_path = data_path.joinpath("ops.json")
    if ops_path.exists():
        with open(ops_path, "r") as f:
            precursor: dict[str, Any] = json.load(f)

        # Loads the mesoscope-specific processing parameters and builds a 'db' dictionary.
        db = {
            "fs": precursor["fs"],
        ...
        # Otherwise, returns an empty dictionary to indicate that no mesoscope overrides were discovered
        db = {}
...
def resolve_ops(ops: dict[str, Any], db: dict[str, Any]) -> Path:
    ...
    Notes:
        ...
        If both 'ops' and 'db' do not contain some of the expected parameters, they will be automatically filled using
        the 'default' dictionary generated using the SingleDayS2PConfiguration configuration class. Therefore,
        both 'ops' and 'db' only need to override the parameters that need to be different from default values.

        As part of its runtime, this function also creates a 'single_day_s2p_configuration.yaml' instance that stores
        the same parameters as the created ops.npy file. This file has a noticeable advantage of not relying on pickling
        to be loaded.

    Args:
        ops: A dictionary that contains suite2p runtime parameters. See SingleDayS2PConfiguration class for available
            configuration parameters, as it is used to generate the 'ops' dictionary during most runtimes.
    ...
    ops = {**generate_default_ops(as_dict=True), **ops, **db}  # type: ignore
    ...
    # Attempts to locate and parse 'ops.json' file. This file is generated by various helper scripts for
    # mesoscope-acquired data and contains important processing parameters. The parameters loaded from this file always
    # override the values already stored in the 'ops' dictionary by the time it reaches this point.
    for data in ops["data_path"]:
        overrides = _resolve_ops_json(data_path=Path(data))

        # While there may be multiple data folders, it is expected that each processed dataset only contains a single
        # 'ops.json' file. Therefore, once a file is discovered and parsed (which is indicated by the returned
        # dictionary not being empty), the process is aborted early.
    ...
    np.save(ops_path, ops, allow_pickle=True)  # type: ignore

    # Also generates a 'single_day_s2p_configuration.yaml' file that stores the same configuration data as the
    ...
    config.to_config(output_directory=save_folder)
...
def run_s2p(ops_path: Path) -> None:
    """Executes the single-day suite2p processing pipeline using the parameters inside the target ops.npy file.

    This function is intended to run the suite2p pipeline on a local machine. It sequentially calls all steps of the
    suite2p single-day processing pipeline, converting raw data frames into extracted cell fluorescence data.

    Notes:
        This function works as a high-level wrapper over the three main pipeline functions: 'resolve_binaries',
        'process_plane', and 'combine_planes'. If you want to run suite2p on a remote compute server in the most
        efficient way, implement a call hierarchy similar to the one showcased in the example notebook,
        which additionally parallelizes all individual processing steps across datasets and planes.
    ...
    if not ops_path.exists() or not ops_path.is_file() or ops_path.suffix != ".npy":
        message = (
            ...
            f"path {ops_path}. Use 'resolve_ops' function or 'ss2p-config' CLI command to generate the 'ops.npy' file "
            f"before calling single-day pipeline."
        )
    ...
    console.echo(message=f"Initializing single-day suite2p runtime...", level=LogLevel.INFO)
...
def resolve_binaries(ops_path: Path) -> None:
    """Ensures that the data.bin files and settings ops.npy dictionaries exist for all planes in the processed dataset.

    This function ensures that each imaging plane in the input dataset has a 'plane' directory that contains the
    data.bin file and ops.npy file. If necessary, it converts the input data stored in other formats to the
    binary data format. Overall, its purpose is to ensure that all input data is formatted in a standardized way,
    allowing the rest of the single-day suite2p pipeline to remain consistent for all supported data formats.

    Notes:
        All processing steps executed by this function are inherently single-core and have low RAM footprint due to
        efficient batching.
    ...
    if len(plane_folders) > 0 and ("input_format" in ops and ops["input_format"] == "binary"):
        ...
        for i, (folder, ops_file) in enumerate(zip(plane_folders, ops_paths)):
            ...
            ops["nframes"] = os.path.getsize(ops["bin_file"]) // n_bytes_read
    ...
    # not be available for all planes.
        ...
        ops_found_flag = all([ops_path.is_file() for ops_path in ops_paths])
    ...
    if files_found_flag:
        ...
        message = f"Removing previous cell detection and fluorescence extraction files, if present."
        console.echo(message=message, level=LogLevel.INFO)
    ...
    # Otherwise, regenerates all binaries from the source data.
        ...
        # Defaults to 'tif' input format if the explicit format is not provided.
            ...
            message = (
                f"No explicit input data format specified via the 'input_format' field of the 'ops' dictionary. "
        ...
        if ops["input_format"] in convert_functions:
            ops0 = convert_functions[ops["input_format"]](ops.copy())  # type: ignore
...
def process_plane(ops_path: Path, plane_index: int) -> None:
    """Runs the single-day suite2p pipeline on the target plane's data.

    This function acts as the entry point for all suite2p processing steps that follow the creation of the binary
    plane dataset. It can be called sequentially or in-parallel to optimize pre-plane processing speed, and it
    effectively replaces the 'pipeline' function from the original suite2p implementation.

    Notes:
        Many processing steps executed by this function are parallelized by numba. Depending on the execution context,
        this function may use up to 2 x plane binary size of RAM. It is advised to profile the memory requirement of
        this function before executing large-scale processing of data. As part of its runtime, it calls
        'set_num_threads()' numba function to set the number of parallel workers according to the plane processing
        parameters. Therefore, calling this function in parallel effectively results in recruiting multiple batches of
        CPU cores.
    ...
    Args:
        ...
        plane_index: The integer index of the processed plane, relative to the overall list of available planes.
    """
    # Guards against invalid inputs.
    if not ops_path.exists() or not ops_path.is_file() or ops_path.suffix != ".npy":
        message = (
    ...
    for key in generate_default_ops(as_dict=True).keys():  # type: ignore
        if key not in [
            "data_path",
            "save_path0",
            "fast_disk",
    ...
    ops = {
        **generate_default_ops(  # type: ignore
            as_dict=True
        ),
        **plane_ops,
    ...
    numba.set_num_threads(ops["parallel_workers"])  # type: ignore
    ...
    if ops["nframes"] < 200:
        message = (
            f"The number of frames for plane {plane_index} is below 200, unpredictable behaviors may occur during "
            f"processing."
        )
        console.echo(message=message, level=LogLevel.WARNING)
    ...
    raw_file_available = ops.get("keep_movie_raw") and "raw_file" in ops and os.path.isfile(ops["raw_file"])
    ...
    # constructed the BinaryFile instances. Now, each processing step resolves the necessary binary files on its own.
    # In turn, this design is helpful for processing parallelization.
...
def combine_planes(ops_path: Path) -> None:
    """Assembles all plane data processed as part of the single-day suite2p pipeline into a 'combined' suite2p folder or
    an .nwb file.

    This function combines all processed plane data into a unified dataset. Critically, assembling all data into the
    'combined' folder is required to run the multi-day processing pipeline on the single-day processed data.

    Notes:
        The primary reason this function exists as a standalone entity is to support processing multiple planes
