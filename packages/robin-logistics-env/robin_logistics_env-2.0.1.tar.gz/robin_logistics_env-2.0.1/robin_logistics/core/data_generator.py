...
def generate_scenario_from_config(base_config, nodes_df_raw, custom_config):
    ...
    if not inventory_split_equal:
        for wh_idx in range(len(warehouses)):
            warehouse_ratios.append(custom_config.get(f'wh_ratio_{wh_idx}', 1.0/len(warehouses)))
        total_ratio = sum(warehouse_ratios)
        if abs(total_ratio - 1.0) > 1e-6 and total_ratio > 0:
            warehouse_ratios = [r/total_ratio for r in warehouse_ratios]
    ...
    for wh_idx, wh in enumerate(warehouses):
        for sku in skus:
            ...
            if inventory_split_equal:
                ...
                ratio = warehouse_ratios[wh_idx] if wh_idx < len(warehouse_ratios) else 1.0/len(warehouses)
