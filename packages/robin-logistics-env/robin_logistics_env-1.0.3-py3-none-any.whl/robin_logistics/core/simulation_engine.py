"""
This module defines the SimulationEngine, which executes a proposed plan
step-by-step, updating the state of the environment dynamically.
"""
import copy
...
class SimulationEngine:
    ...
    def run_simulation(self):
        ...
        for vehicle_id, route in self.solution.get("routes", {}).items():
    ...
    def _plan_pickups(self, orders):
        ...
        for sku, total_qty in required_items.items():
            ...
            for wh in self.env.warehouses.values():
                if wh.inventory.get(sku, 0) >= total_qty:
                    dist = self.env.get_distance(self.env.warehouses['WH-1'].location.id, wh.location.id)
                    if dist < min_dist:
