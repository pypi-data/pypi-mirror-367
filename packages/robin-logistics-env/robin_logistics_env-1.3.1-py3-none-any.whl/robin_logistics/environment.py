...
class LogisticsEnvironment:
    ...
    def __init__(self, problem_config: Optional[Dict] = None, scenario: str = None):
        """
        Initialize the logistics environment.
        
        Args:
            problem_config: Optional configuration override. If None, uses default config.
            scenario: Scenario key to use ('beginner', 'intermediate', 'advanced'). If None, uses intermediate.
        """
        self._config = problem_config or config
        self._scenario = scenario
        self._current_scenario = None
        if scenario:
            self._load_problem_instance(scenario)
        else:
            self._env = None
            self._last_solution = None
        
    def _load_problem_instance(self, scenario_key: str = "intermediate"):
        """Load and initialize the problem instance with specified scenario."""
        data_dir = os.path.join(os.path.dirname(__file__), 'data')
        nodes_df = pd.read_csv(os.path.join(data_dir, 'nodes.csv'))
        edges_df = pd.read_csv(os.path.join(data_dir, 'edges.csv'))
        
        problem_data = generate_problem_instance(self._config, nodes_df, scenario_key)
    ...
    def get_available_scenarios(self) -> Dict[str, Dict]:
        """Get all available scenarios with their descriptions."""
        return {
            key: {
                "name": scenario["name"],
            ...
            for key, scenario in self._config.SCENARIOS.items()
        }
    
    def load_scenario(self, scenario_key: str):
        """Load a specific scenario."""
        if scenario_key not in self._config.SCENARIOS:
            raise ValueError(f"Invalid scenario: {scenario_key}. Available: {list(self._config.SCENARIOS.keys())}")
        
        self._load_problem_instance(scenario_key)
        return self.get_scenario_info()
    
    def get_scenario_info(self) -> Dict:
        """Get information about the currently loaded scenario."""
        if not self._current_scenario:
            return {"scenario": "None", "loaded": False}
        
        scenario = self._config.SCENARIOS[self._current_scenario]
        return {
            ...
            "distance_ratio": scenario["distance_ratio"]
        }
    ...
    def run_optimization(self, solver_function: Callable[['LogisticsEnvironment'], Dict]) -> Dict:
        ...
        if not self._env:
            return {
                'is_valid': False,
                'error_message': "No scenario loaded. Please load a scenario first.",
                'cost': 0.0,
                'vehicles_used': 0,
                'orders_fulfilled': 0,
                'solution': {}
    ...
    def launch_dashboard(self, solver_function: Callable[['LogisticsEnvironment'], Dict] = None):
        """
        Launch the interactive dashboard with scenario selection.
        
        Args:
            solver_function: Optional solver function. If provided, will run optimization first.
        """
        create_dashboard(self, solver_function)
