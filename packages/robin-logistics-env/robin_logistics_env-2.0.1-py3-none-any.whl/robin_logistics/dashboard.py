...
def main():
    ...
    with col2:
        ...
        if not inventory_split_equal:
            ...
            for i in range(num_warehouses):
                ...
                warehouse_ratios.append(ratio)
                total_ratio += ratio
            if abs(total_ratio - 1.0) > 0.01:
                st.warning(f"Sum of warehouse ratios is {total_ratio:.2f}. It should be 1.0.")
    ...
    # Store warehouse ratios in session state if not equal split
    if not inventory_split_equal:
        for i, ratio in enumerate(warehouse_ratios):
        ...
        if st.button("🚀 Run Simulation", type="primary", use_container_width=True):
            with st.spinner("Running solver and simulation..."):
                ...
                try:
                    ...
                    # Add warehouse ratios if not equal split
                    if not st.session_state.get('inventory_split_equal', True):
                        for i in range(st.session_state.get('num_warehouses', 2)):
                            custom_config[f'wh_ratio_{i}'] = st.session_state.get(f'wh_ratio_{i}', 1.0/st.session_state.get('num_warehouses', 2))
