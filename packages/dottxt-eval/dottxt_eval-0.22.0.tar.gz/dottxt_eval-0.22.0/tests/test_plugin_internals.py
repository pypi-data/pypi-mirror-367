...
from doteval.plugin import pytest_configure, pytest_sessionfinish
...
def test_pytest_configure_basic():
    ...
    assert hasattr(config, "_doteval_items")
    assert config._evaluation_results == {}
    assert config._doteval_items == []
...
def test_pytest_sessionfinish_with_concurrent_items(MockRunner):
    ...
    config._doteval_items = [mock_item]
...
def test_pytest_sessionfinish_no_doteval_items():
    """Test session finish with no doteval items."""
