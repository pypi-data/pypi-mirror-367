...
from doteval.runners import Runner, _is_async_evaluation
...
class TestRunner:
    ...
    async def test_run_sync_evaluation(self, runner, results_dict):
        ...
        def sync_eval(
            evaluation_name, experiment_name, samples, progress_manager, **kwargs
        ):
        ...
        with patch("doteval.fixtures.FixtureManager", mock_fm):
            # Run evaluation
...
class TestRunnerEdgeCases:
    ...
    async def test_runner_progress_manager(self):
        ...
        for i in range(3):
            ...
            item.name = f"test_{i}"
            item.fixtures = {}
            item.function = lambda **kwargs: {"id": kwargs.get("evaluation_name")}
            items.append(item)
