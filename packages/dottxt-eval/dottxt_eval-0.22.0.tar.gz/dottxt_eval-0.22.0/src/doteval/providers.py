"""Model providers for doteval - manage model clients, connections, etc.
...
def discover_model_providers() -> dict[str, str]:
    ...
    Model providers are registered via entry points in the "doteval.model_providers"
    ...
    try:
        ...
        entry_points = importlib.metadata.entry_points(group="doteval.model_providers")
    except TypeError:
        # Fall back to older API
        all_entry_points = importlib.metadata.entry_points()
        if isinstance(all_entry_points, dict):
            # Old API returns dict mapping group names to lists of entry points
            entry_points = all_entry_points.get("doteval.model_providers", [])  # type: ignore[arg-type]
        else:
            entry_points = []
