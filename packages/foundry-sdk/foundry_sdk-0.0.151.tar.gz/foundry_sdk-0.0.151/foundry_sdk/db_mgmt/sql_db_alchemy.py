    ...
    def _fetch_ids_chunks(
        ...
        with self.get_session() as session:
            for i, offset in enumerate(row_iter):
                ...
                if not key_rows:
                    continue
                # Get the actual primary key column name from the model (once per chunk)
                ...
                if use_temp:
                    ...
                    for col in p_unique_key_columns:
                        # Find the column in the model's __table__
                    ...
                    # Use named parameters and a list of dicts for SQLAlchemy compatibility
                    col_names_sql = ", ".join([f'"{col}"' for col in p_unique_key_columns])
                    param_names = [f":{col}" for col in p_unique_key_columns]
                    # Use outer single quotes to allow double quotes inside f-string (Python <3.12 compatibility)
