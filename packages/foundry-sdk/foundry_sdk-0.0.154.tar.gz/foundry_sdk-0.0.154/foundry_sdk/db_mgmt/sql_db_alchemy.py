            ...
            with self.get_session() as session:
                ...
                for i, offset in enumerate(row_iter):
                    ...
                    if use_temp:
                        ...
                        # Insert keys with quoted column names and parameterized values
                        insert_stmt = (
                            f'INSERT INTO "{temp_table_name}" ({col_names_sql}) VALUES ({", ".join(param_names)})'
                        )
                        param_dicts = [dict(zip(p_unique_key_columns, row, strict=True)) for row in key_rows]
                        for param in param_dicts:
                            session.execute(sa_text(insert_stmt), param)
