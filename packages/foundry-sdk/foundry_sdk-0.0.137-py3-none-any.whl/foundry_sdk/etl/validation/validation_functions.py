...
class DataValidator:
    ...
    def check_store_region_map(self, store_region_map: pl.DataFrame) -> None:
        ...
        optional_columns = store_region_map_schema.optional_columns()
        region_column = next((col for col in store_region_map.columns if col in optional_columns), None)

        if region_column:
            region_df = store_region_map.select([region_column, "country"]).unique()
            region_values = region_df.rows()

            # Query database regions with this type, joining with countries to get abbreviations
            ...
            db_regions = self.session.execute(
                select(Regions.abbreviation, country_region.abbreviation.label("country_abbrev"))
                .join(country_region, Regions.country == country_region.id)
                .where(Regions.type == region_column)
                .where(country_region.type == "country")
            ).all()

            db_region_abbrevs = {abbrev for abbrev, _ in db_regions}
            db_valid_pairs = set(db_regions)

            # Check if region values are known
            region_column_values = region_df.select(region_column).unique().to_series().to_list()
            missing_region_values = sorted(set(region_column_values) - db_region_abbrevs)

            # Check (region, country) pairs that are wrong
            mismatched_pairs = [
                (region, country)
                for region, country in region_values
                if region in db_region_abbrevs and (region, country) not in db_valid_pairs
            ]

            messages = []
            if missing_region_values:
                messages.append(f"Invalid values in '{region_column}' (not found in database): {missing_region_values}")

            if mismatched_pairs:
                formatted = "\n".join(f"- {r} / {c}" for r, c in mismatched_pairs)
                messages.append(f"Invalid (region, country) combinations for '{region_column}':\n{formatted}")
