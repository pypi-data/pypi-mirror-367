...
class SQLAlchemyDatabase:
    ...
    def _write_chunks(
        ...
        progress_bar = (
            tqdm(
                row_iter,
                total=total_rows,
                desc=f"Writing to table: {table_name}",
                unit="rows",
                dynamic_ncols=True,
                mininterval=0.1,
            )
        ...
        with self.get_session() as session:
            for i, offset in enumerate(row_iter):
                ...
                if show_progress:
                    # Calculate how many rows have been processed so far
                    processed = min((i + 1) * chunk_size, total_rows)
                    progress_bar.n = processed
                    progress_bar.last_print_n = processed
    ...
    def _fetch_ids_chunks(
        ...
        p_unique_key_columns: list[str],
        table_name: str,
        use_temp_table: int = 1000,
        profile: bool = False,
    ) -> pl.DataFrame:
        """Helper to fetch IDs in chunks after writing data. Uses temp table for large chunks."""

        def func(model_class, data, chunk_size, p_unique_key_columns, table_name, use_temp_table):
            ...
            progress_bar = (
                tqdm(
                    row_iter,
                    total=total_keys,
                    desc=f"Fetching IDs from table: {table_name}",
                    unit="rows",
                    dynamic_ncols=True,
                    mininterval=0.01,
                )
            ...
            with self.get_session() as session:
                # Precompute invariants for temp table approach
                if use_temp:
                    # Get column types from model (once)
                    col_defs = []
                    ...
                    col_names_sql = ", ".join([f'"{col}"' for col in p_unique_key_columns])
                    param_names = [f":{col}" for col in p_unique_key_columns]
                # Get the actual primary key column name from the model (once)
                pk_col = next(iter(model_class.__table__.primary_key)).name
                # Precompute join condition and select columns for temp table join
                if use_temp:
                ...
                for i, offset in enumerate(row_iter):
                    ...
                    if not key_rows:
                        continue
                    if use_temp:
                        # Use temp table approach
                        temp_table_name = f"temp_keys_{i}"
                        # Drop if exists (safety)
                    session.execute(text(f'DROP TABLE IF EXISTS "{temp_table_name}"'))
                    # Use UNLOGGED for faster temp table creation
                    session.execute(text(f'CREATE TEMP TABLE "{temp_table_name}" ({col_defs_sql}) ON COMMIT DROP'))
                    # Bulk insert all rows at once
                    insert_stmt = f'INSERT INTO "{temp_table_name}" ({col_names_sql}) VALUES ({", ".join(param_names)})'
                    param_dicts = [dict(zip(p_unique_key_columns, row, strict=True)) for row in key_rows]
                    if param_dicts:
                        session.execute(sa_text(insert_stmt), param_dicts)
                        # Join with main table using quoted column names
                        join_sql = f'SELECT {select_cols} FROM "{table_name}" AS main JOIN "{temp_table_name}" AS temp ON {join_conds}'
                        results = session.execute(text(join_sql)).fetchall()
                        # Always use "id" as the output key for the PK
                        for row in results:
                            row_dict = dict(zip([*p_unique_key_columns, pk_col], row, strict=True))
                            row_dict["id"] = row_dict.pop(pk_col)
                            ids.append(row_dict)
                        # Drop temp table (should be auto-dropped, but explicit for safety)
                        session.execute(text(f'DROP TABLE IF EXISTS "{temp_table_name}"'))
                        # ...handled above for temp table join...
                    else:
                        # Use IN clause approach
                        key_tuples = [tuple(row) for row in key_rows]
                        # Get the actual PK column name and attribute
                        pk_col = next(iter(model_class.__table__.primary_key)).name
                        pk_attr = getattr(model_class, pk_col)
                    ...
                    # Manually update progress bar to show correct number of rows processed
                    if show_progress:
                        processed = min((i + 1) * chunk_size, total_keys)
                        progress_bar.n = processed
                        progress_bar.last_print_n = processed
                        progress_bar.update(0)
        ...
        if profile:
            ...
            result = profiled_func(model_class, data, chunk_size, p_unique_key_columns, table_name, use_temp_table)
            profiler.print_stats()
        else:
            result = func(model_class, data, chunk_size, p_unique_key_columns, table_name, use_temp_table)
        return result
