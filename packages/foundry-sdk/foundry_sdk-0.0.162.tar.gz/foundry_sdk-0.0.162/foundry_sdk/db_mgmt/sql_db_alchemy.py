...
class SQLAlchemyDatabase:
    ...
    def _fetch_ids_chunks(
        ...
        if use_temp:
            self._fetch_ids_chunks_temp_table(
                model_class,
                key_df,
                chunk_size,
                p_unique_key_columns,
            ...
            self._fetch_ids_chunks_in_clause(
                model_class,
                key_df,
                chunk_size,
                p_unique_key_columns,
        ...
        with self.get_session() as session:
            ...
            for i, offset in enumerate(progress_bar):
                ...
                if param_dicts:
                    ...
                    # Use SQLAlchemy's select and join to avoid SQL injection
                    main_table = model_class.__table__
                    temp_table = Table(temp_table_name, metadata, autoload_with=session.bind)
                    join_condition = [
                        getattr(main_table.c, col) == getattr(temp_table.c, col) for col in p_unique_key_columns
                    ]
                    stmt = (
                        main_table.join(temp_table, *join_condition)
                        .select()
                        .with_only_columns(
                            *(getattr(main_table.c, col) for col in p_unique_key_columns), getattr(main_table.c, pk_col)
                        )
                    )
        ...
        *,
        ...
        batch_iter = (
            tqdm(
                ...
                total=total_batches,
                desc=f"Loading in batches: {table}",
                unit="batch",
                dynamic_ncols=True,
                mininterval=0.5,
            )
            if show_progress
            else range(0, data.height, chunk_size)
        )

        all_id_maps: list[dict[str, t.Any]] = []

        # Process each batch
        for offset in batch_iter:
            ...
            # 1) Create temp table
            cursor.execute(f'CREATE TEMP TABLE "{temp_table}" (LIKE "{table}" INCLUDING ALL) ON COMMIT DROP')

            # 2) COPY data into staging
            import io

            buf = io.StringIO()
            batch.write_csv(buf, has_header=False)
            buf.seek(0)
            cursor.copy_expert(f'COPY "{temp_table}" ({col_list}) FROM STDIN WITH CSV', buf)

            # 3) Build ON CONFLICT clause based on mode
            ...
            # 4) INSERT FROM staging into real table
            insert_sql = f'INSERT INTO "{table}" ({col_list}) SELECT {col_list} FROM "{temp_table}" ' + on_conflict
            cursor.execute(insert_sql)

            # 5) Optionally retrieve IDs by joining via COPY + Polars
            if returning_id:
                # Stream the join result as CSV and read back into a Polars DataFrame
                import io
                ...
                copy_sql = (
                    f'COPY (SELECT {select_list} FROM "{table}" AS r '
                    f'JOIN "{temp_table}" AS t USING ({conflict_cols})) TO STDOUT WITH CSV HEADER'
                )
                ...
                for row in df_ids.iter_rows(named=True):
                    mapping = {key: row[key] for key in cols_out}
                    all_id_maps.append(mapping)
        ...
        if returning_id:
            return pl.DataFrame(all_id_maps)
        return None
