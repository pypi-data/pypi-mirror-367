...
class SQLAlchemyDatabase:
    ...
    def handle_insertion_multi_line(
        ...
        mode: t.Literal["RAISE", "IGNORE", "UPDATE"],
        *,
        threshold_for_copy: int = 100,  # temporarily do not use copy
        returning_id: bool = False,
    ) -> pl.DataFrame | None:
        """
        ...
        if len(data) >= threshold_for_copy:
            print("Using COPY for bulk insert")
    ...
    def handle_insertion_multi_line_copy(
        ...
        *,
        ...
        for offset in batch_iter:
            ...
            batch.write_csv(buf)
            buf.seek(0)
            cursor.copy_expert(f'COPY "{temp_table}" ({col_list}) FROM STDIN WITH CSV HEADER', buf)
            ...
            insert_sql = f'INSERT INTO "{table}" ({col_list}) SELECT {col_list} FROM "{temp_table}" ' + on_conflict  # noqa: S608
