    ...
    def handle_insertion_multi_line_copy(
        ...
        *,
        ...
        try:
            ...
            # Process each batch
            for batch_idx, offset in enumerate(range(0, data.height, chunk_size)):
                batch = data.slice(offset, chunk_size)
                if batch.height == 0:
                    continue

                temp_table = f"temp_{table}_{batch_idx}"

                # 1) Create temp table (just structure, no constraints/indexes, exclude auto-generated ID)
                # Get non-ID columns for temp table
                non_id_cols = [c for c in cols if c.upper() != "ID"]
                non_id_col_list = ", ".join(f'"{c}"' for c in non_id_cols)

                cursor.execute(
                    f'CREATE TEMP TABLE "{temp_table}" AS SELECT {non_id_col_list} FROM "{table}" WHERE FALSE'
                )

                # 2) COPY data into staging (input data shouldn't have ID column)
                buf = io.StringIO()
                batch.write_csv(buf)
                buf.seek(0)
                cursor.copy_expert(f'COPY "{temp_table}" ({non_id_col_list}) FROM STDIN WITH CSV HEADER', buf)

                # 3) INSERT FROM staging into real table (only non-ID columns)
                insert_sql = (
