...
class SQLAlchemyDatabase:
    ...
    def _fetch_ids_chunks(
        ...
        """Helper to fetch IDs in chunks after writing data."""
        key_df = data.select(*p_unique_key_columns)
        total_keys = key_df.height
        key_total_chunks = (total_keys + chunk_size - 1) // chunk_size
        key_row_iter = range(0, total_keys, chunk_size)
        key_show_progress = key_total_chunks > 1
        key_progress_bar = (
            tqdm(
                key_row_iter,
                total=total_keys,
            ...
            if key_show_progress
            else key_row_iter
        )
        unique_key_attrs = [self.get_column_attr_by_db_name(model_class, col) for col in p_unique_key_columns]
        ids = []
        with self.get_session() as session:
            for i, offset in enumerate(key_row_iter):
                chunk_keys = key_df.slice(offset, chunk_size)
                key_tuples = [tuple(row) for row in chunk_keys.rows()]
                if not key_tuples:
                    continue
                query = session.query(*unique_key_attrs, model_class.id)
                ...
                results = query.all()
                ids.extend([dict(zip([*p_unique_key_columns, "id"], row, strict=True)) for row in results])
                # Manually update progress bar to show correct number of rows processed
                if key_show_progress:
                    processed = min((i + 1) * chunk_size, total_keys)
                    key_progress_bar.n = processed
                    key_progress_bar.last_print_n = processed
                    key_progress_bar.update(0)
