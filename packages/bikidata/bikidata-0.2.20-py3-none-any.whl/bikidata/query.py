...
def total():
    DB = duckdb.connect(DB_PATH, read_only=True)
    db_cursor = DB.cursor()
    total = db_cursor.execute("select count(s) from triples").fetchone()[0]
    return total


def count_by_property(property):
    SQL = "select I.value, count(s) from triples T join iris I on T.o = I.hash join iris II on T.p = II.hash where II.value = ? group by I.value"
    DB = duckdb.connect(DB_PATH, read_only=True)
    db_cursor = DB.cursor()
...
def spo(*args, **kwargs):
    ...
    SQL = (
        "select U.value, UU.value, UUU.value, L.value from triples T join iris U on T.s = U.hash join iris UU on T.p = UU.hash left join iris UUU on T.o = UUU.hash left join literals L on T.o = L.hash where "
        + "and ".join(conditions)
        + f" limit {size}"
    )
    DB = duckdb.connect(DB_PATH, read_only=True)
