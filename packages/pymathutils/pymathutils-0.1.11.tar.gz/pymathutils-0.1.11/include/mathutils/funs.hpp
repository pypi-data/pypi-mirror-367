...
// inline double spherical_Plm_l_minus_m_even(int l, int m, double theta) {
//   // this assumes  l-|m| is even
//   int k_max = (l - abs_m) / 2;
//   double cos_theta = std::cos(theta);
//   double sin_theta = std::sin(theta);
//   double abs_cos_theta = std::abs(cos_theta);
//   double abs_sin_theta = sin_theta; // std::abs(sin_theta);
//   // check for zero
//   if (sin_theta == 0.0 && abs_sin_theta == 0.0) {
//     return 0.0;
//   }
...
//   for (int k{0}; k <= (l - abs_m) / 2; k++) {
//     int ImLog_over_pi_Xklm =
//         k + (l - abs_m) * arg_over_pi_cos_theta + abs_m *
//         arg_over_pi_sin_theta;
//     double ReLog_Xklm = -0.5 * std::log(4 * M_PI) - abs_m * std::log(2) +
//                         0.5 * std::log(2 * l + 1) +
//                         0.5 * log_factorial(l + abs_m) +
//                         0.5 * log_factorial(l - abs_m) - 2 * k * std::log(2)
