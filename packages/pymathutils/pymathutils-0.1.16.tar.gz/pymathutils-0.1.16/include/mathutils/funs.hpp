...
init_Plm_recursion_three_term_upward_ell(int m, double theta) {
  double P_m_m =
      (theta == 0.0)
          ? (m == 0) / std::sqrt(4 * M_PI)
          : (1 - 2 * (m & 1)) *
                std::exp(m * std::log(std::sin(theta)) -
                         0.5 * std::log(4 * M_PI) - m * std::log(2) +
                         0.5 * std::lgamma(2 * m + 2) - std::lgamma(m + 1));
  double P_m_plus_one_m = std::sqrt(2 * m + 3) * std::cos(theta) * P_m_m;
  return {P_m_m, P_m_plus_one_m};
}

/**
 * @brief Generate the associated Legendre polynomial with spherical harmonics
 * normalization \f$P_{\ell m}(\theta) = e^{-im\phi}Y_{\ell m}(\theta,
 * \phi)\f$ for \f$\ell = \ell_{start}, \ell_{start} + 1, \ldots, \ell_{end}\f$
 * and \f$m=\ell_{start}\f$.
 */
mathutils::SimpleGenerator<double>
generate_spherical_Plm_recursion_three_term_upward_ell(int l_start, int l_end,
                                                       double theta) {
  if (l_start < 0 || l_end < l_start) {
    // throw std::out_of_range("l_start and l_end must be non-negative and l_end
    // " "must be greater than or equal to l_start");
    co_return;
  }
  if (theta < 0.0 || theta > M_PI_2) {
    throw std::out_of_range("theta must be in the range [0, pi/2]");
  }
  int m = l_start;
...
inline double reduced_spherical_Plm(int l, int m, double theta) {
  ...
  double P_ell_m;
  for (auto P :
       generate_spherical_Plm_recursion_three_term_upward_ell(l, l, theta)) {
    P_ell_m = P;
  }
  return P_ell_m;
}
