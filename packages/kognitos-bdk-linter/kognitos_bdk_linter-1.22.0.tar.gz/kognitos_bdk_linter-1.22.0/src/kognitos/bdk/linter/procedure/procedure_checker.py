...
def check_discover_rule(checker: ProcedureChecker, node: nodes.FunctionDef | nodes.AsyncFunctionDef) -> None:
    ...
    if has_discoverables:
        # Check for mandatory arguments: search: Optional[str], limit: Optional[float], offset: Optional[float]
        # Using float for pagination until we fix the issue with number casting
        required_args = {
            "search": ("Optional[str]", lambda ann: ann is not None and util.is_optional_type(ann) and util.is_str_type(ann)),
            "limit": ("Optional[float]", lambda ann: ann is not None and util.is_optional_type(ann) and util.is_float_type(ann)),
            "offset": ("Optional[float]", lambda ann: ann is not None and util.is_optional_type(ann) and util.is_float_type(ann)),
        }
