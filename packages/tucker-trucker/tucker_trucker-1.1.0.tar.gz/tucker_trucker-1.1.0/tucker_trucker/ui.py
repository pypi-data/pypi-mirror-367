...
class UI:
    ...
    def draw_player(self, player):
        ...
        if player.image:
            # Draw the Tucker image
            image_rect = player.image.get_rect()
            image_rect.center = (player_x, player_y)
            self.screen.blit(player.image, image_rect)
            
            # Add a subtle outline to make Tucker stand out
            pygame.draw.circle(self.screen, BLACK, (player_x, player_y), player.size + 2, 2)
        else:
            # Fallback to the original drawing method
            # Body (main circle)
            pygame.draw.circle(
