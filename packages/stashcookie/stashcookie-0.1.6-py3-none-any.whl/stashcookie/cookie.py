...
def upload_file(file_path: str, projectname: str, root_dir: str = ".", storage_class: str = "DEEP_ARCHIVE"):
    """Uploads a single file to AWS S3, preserving relative path from root_dir."""
    ...
    try:
        rel_path = file.resolve().relative_to(root)
    except ValueError:
        print(f"⚠️ Skipping: {file} (Not under root dir {root})")
        return
    
    s3_uri = f's3://{projectname}/{rel_path.as_posix()}'
