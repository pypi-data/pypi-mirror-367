    ...
    def __send_heartbeat(self):
        while self.is_sending_heartbeat and self.is_running:
            try:
                ...
                import traceback
                ErrorContext.publish(exceptions.SDKError(f"Heartbeat send error: {traceback.format_exc()}"))
            ...
            try:
                ...
                if udp_header.MessageType == 258:
                    ...
                    if hb_resp.NextBeat == 401:
                        ...
                        ErrorContext.publish(exceptions.SDKError(f"401,心跳"))
                        self.sign_in()
                    ...
                    if self.on_recv_invite is not None:
                        ErrorContext.publish(exceptions.SDKError(f"收到邀请，加入session: {invite_req}"))
                ...
                import traceback
                ErrorContext.publish(exceptions.SDKError(f"Receive message exception: {traceback.format_exc()}"))
