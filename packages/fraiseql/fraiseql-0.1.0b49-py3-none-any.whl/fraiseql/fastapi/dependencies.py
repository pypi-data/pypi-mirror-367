...
async def build_graphql_context(
    ...
    if config:
        # Enable JSON passthrough based on config and environment
        if hasattr(config, "json_passthrough_enabled") and config.json_passthrough_enabled:
            # Check if we should auto-enable in production
            if mode == "production" and getattr(config, "json_passthrough_in_production", True):
