...
async def try_raw_json_execution(
    ...
    if db:
        ...
        logger.info(
            f"Database methods: find_raw_json={has_find_raw_json}, available_methods={[m for m in dir(db) if 'find' in m.lower()]}"
        )

    if not db or not hasattr(db, "find_raw_json"):
        logger.info(
            f"Raw JSON execution skipped: db={db is not None}, has_find_raw_json={hasattr(db, 'find_raw_json') if db else False}"
        )
        return None
    ...
    while hasattr(original_fn, "__wrapped__") and unwrap_count < 10:
        ...
        unwrap_count += 1
        logger.info(
            f"Unwrapped {unwrap_count} time(s), current function: {original_fn.__name__ if hasattr(original_fn, '__name__') else 'unknown'}"
        )
...
def _extract_view_name(fn) -> Optional[str]:
    ...
    try:
        ...
        patterns = [
            ...
            r'return\s+await\s+db\.find(?:_one)?\s*\(\s*["\']([^"\']+)["\']',  # return await pattern
        ]
