...
def _import_module(ref: _ObjectRef) -> ModuleType:
    ...
    try:
        ...
        raise RuntimeError(f"The job's code contains errors.\n\n{_get_traceback(ref)}") from e
...
def _get_traceback(ref: _ObjectRef, with_colors: bool = True) -> str:
    """Return a formatted traceback string with colors and indentation."""
    lines = traceback.format_exc().rstrip().splitlines()
    skipping = True
    sb = ""

    def color(text: str, code: str) -> str:
        return f"\033[{code}m{text}\033[0m" if with_colors else text
    ...
    for line in lines[:-1]:
        if skipping and ref.module in line:
            skipping = False
        if not skipping:
            sb += bright_black(f"  {line}\n")
