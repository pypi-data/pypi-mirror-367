...
class SettingsWindow(QMainWindow):
    def __init__(self):
        ...
        # Maximizar ventana
        self.showMaximized()
        self.setWindowTitle("VacancyFinder-SiMAF   0.3.6.9")
    ...
    def load_dump(self, dump_path):
        self.progress.setValue(0)
        # Importar y generar superficie
        pipeline = import_file(dump_path)
        pipeline.modifiers.append(
            ConstructSurfaceModifier(radius=self.spin_radius.value())
        )
        ...
        # Malla de superficie (primer mesh)
        if hasattr(data, 'meshes') and len(data.meshes) > 0:
            mesh = data.meshes[0]
            verts = mesh.vertices
            faces = mesh.cells  # array Nx3
            # Prepend el conteo de vértices por cara
            faces = np.hstack([
                np.full((faces.shape[0], 1), 3),
                faces
            ]).astype(np.int64)
            pv_mesh = pv.PolyData(verts, faces)
            self.plotter.add_mesh(pv_mesh, opacity=0.5)
        self.plotter.reset_camera()

        # --- Visualización 2D (proyección XY) ---
