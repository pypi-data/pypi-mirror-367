...
class AtomicGraphGenerator:
    def __init__(self, json_params_path: str = None):
        # --- Leer configuración de input_params.json ---
        ...
        # Cargo pipeline OVITO de tu dump relajado (múltiples frames)
        self.pipeline = import_file(self.input_path, multiple_frames=True)

        # Guardo todos los registros en JSON para entrenamiento
        self.records = []

        # Preparo CSV y escribo solo el header que quieres
        self.csv_path = "outputs/csv/finger_data.csv"
        os.makedirs(os.path.dirname(self.csv_path), exist_ok=True)
        header = ["vacancys", "N", "mean", "surface_area", "filled_volume"]
    ...
    def run(self):
        ...
        for _ in range(self.iterations):
            for length in range(2, self.max_nodes + 1):
                ...
                # 4) Escribo solo las 5 columnas en el CSV
                row = [
                    len(ids),        # vacancys
                    stats['N'],      # N
                    stats['mean'],   # mean
        ...
        # Finalmente guardo el JSON completo en outputs/json
    ...
    def _export_and_dump(self, expr: str):
        # Aplico cadena de modificadores en OVITO
