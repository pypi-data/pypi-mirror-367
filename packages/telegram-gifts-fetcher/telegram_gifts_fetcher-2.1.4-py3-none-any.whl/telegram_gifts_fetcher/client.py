...
class TelegramGiftsClient:
    ...
    def __init__(self, api_id: Optional[int] = None, api_hash: Optional[str] = None, session_name: Optional[str] = None, strict: bool = True):
        ...
        try:
            self.config = get_config(api_id=api_id, api_hash=api_hash, session_name=session_name, strict=strict)
        except ValueError as e:
            if not strict:
                # In non-strict mode, create config without validation
                self.config = get_config(api_id=api_id, api_hash=api_hash, session_name=session_name, strict=False)
            else:
                raise e
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not self.config.is_valid:
                print(f"{Fore.RED}❌ Invalid configuration: API credentials are missing{Style.RESET_ALL}")
