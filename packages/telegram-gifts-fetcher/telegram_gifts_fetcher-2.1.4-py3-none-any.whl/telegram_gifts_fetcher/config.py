...
class Config:
    ...
    def __init__(self, api_id: Optional[int] = None, api_hash: Optional[str] = None, session_name: Optional[str] = None, strict: bool = True):
        ...
        # If credentials are not found and strict mode is enabled, show error
        if strict and (not self.api_id or not self.api_hash):
    ...
    def _get_env_str(self, key: str) -> Optional[str]:
        """Get string value from environment variable, removing quotes."""
        value = os.getenv(key)
        if value:
...
def get_config(api_id: Optional[int] = None, api_hash: Optional[str] = None, session_name: Optional[str] = None, strict: bool = True) -> Config:
    """Get or create configuration instance."""
    global config
    if config is None or api_id or api_hash or session_name:
        config = Config(api_id=api_id, api_hash=api_hash, session_name=session_name, strict=strict)
    return config