...
class ProjectStateManager:
    ...
    def _write_debug_state(self):
        ...
        try:
            ...
            if len(debug_data) > 0:
                logger.debug("Debug state updated: %d projects", len(debug_data))
    ...
    async def initialize_project_state(self, client_session_id: str, project_folder_path: str) -> ProjectState:
        ...
        # Check if there's an existing project state for the same folder from a different client session
        # This handles reconnection scenarios where the client gets a new session ID
        existing_session_id = None
        for session_id, state in self.projects.items():
            if state.project_folder_path == project_folder_path:
                existing_session_id = session_id
                logger.info("Found existing project state for folder %s under old session %s, migrating to new session %s", 
                          project_folder_path, session_id, client_session_id)
                break
        
        if existing_session_id:
            # Migrate the existing project state to the new client session ID
            existing_state = self.projects[existing_session_id]
            existing_git_manager = self.git_managers[existing_session_id]
            
            # Update the client session ID in the project state
            existing_state.client_session_id = client_session_id
            
            # Move to new key
            self.projects[client_session_id] = existing_state
            self.git_managers[client_session_id] = existing_git_manager
            
            # Clean up old key
            del self.projects[existing_session_id]
            del self.git_managers[existing_session_id]
            
            logger.info("Successfully migrated project state from %s to %s", existing_session_id, client_session_id)
            
            # Update debug state to reflect the migration
