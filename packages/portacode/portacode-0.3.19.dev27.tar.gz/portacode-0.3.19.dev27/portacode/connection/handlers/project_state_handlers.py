...
class GitManager:
    ...
    def get_file_status(self, file_path: str) -> Dict[str, Any]:
        """Get Git status for a specific file or directory with detailed staging information using GitPython API."""
        if not self.is_git_repo or not self.repo:
        ...
        try:
            ...
            # Use GitPython API to check staging status
            file_staged = False
            file_unstaged = False
            is_tracked = False
            ...
            if is_tracked:
                ...
                for diff_item in staged_diff:
                    if (diff_item.a_path == rel_path or diff_item.b_path == rel_path):
                        file_staged = True
                        break
                ...
                # File is not tracked, check if it's staged (newly added)
                staged_diff = self.repo.index.diff("HEAD")
                for diff_item in staged_diff:
                    if diff_item.b_path == rel_path and diff_item.new_file:
                        file_staged = True
                        is_tracked = True  # Will be tracked after commit
                        break
            
            # For directories, aggregate status from contents
            if os.path.isdir(file_path):
                if not is_tracked and not file_staged:
                    # Check if directory contains any tracked or staged files
                    try:
                        tracked_files = self.repo.git.ls_files(rel_path)
                        if tracked_files.strip():
                            is_tracked = True
                            # Check if any files in directory are staged/unstaged
                            all_staged = self.repo.index.diff("HEAD")
                            all_unstaged = self.repo.index.diff(None)
                            
                            for diff_item in all_staged:
                                if (diff_item.a_path and diff_item.a_path.startswith(rel_path + '/')) or \
                                   (diff_item.b_path and diff_item.b_path.startswith(rel_path + '/')):
                                    file_staged = True
                                    break
                            
                            for diff_item in all_unstaged:
                                if (diff_item.a_path and diff_item.a_path.startswith(rel_path + '/')) or \
                                   (diff_item.b_path and diff_item.b_path.startswith(rel_path + '/')):
                                    file_unstaged = True
                                    break
            ...
            # Determine status
            if file_staged and file_unstaged:
                file_status = "staged+modified"
            elif file_staged:
                file_status = "staged"
            elif file_unstaged:
