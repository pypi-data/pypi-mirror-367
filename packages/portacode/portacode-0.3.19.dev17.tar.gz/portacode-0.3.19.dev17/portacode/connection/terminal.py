...
class ClientSessionManager:
    ...
    def update_sessions(self, sessions: List[Dict]) -> List[str]:
        ...
        # Periodically clean up orphaned project states
        # Only do this if we have new sessions to avoid excessive cleanup
        if newly_added_sessions:
            # Schedule cleanup to run asynchronously since this method is not async
            import asyncio
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # Schedule the cleanup to run soon
                    loop.create_task(self._cleanup_orphaned_project_states())
                else:
                    # If no loop is running, create a new task when possible
                    logger.debug("No event loop running, skipping orphaned project state cleanup")
            except Exception as e:
                logger.debug("Could not schedule orphaned project state cleanup: %s", e)
