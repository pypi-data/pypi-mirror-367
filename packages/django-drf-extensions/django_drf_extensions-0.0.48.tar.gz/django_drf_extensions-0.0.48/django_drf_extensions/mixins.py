...
class OperationsMixin:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        """
        print(f"=== DEBUG: PATCH method called ===")
        print(f"DEBUG: request.query_params = {request.query_params}")
        print(f"DEBUG: request.data = {request.data}")
        print(f"DEBUG: isinstance(request.data, list) = {isinstance(request.data, list)}")
        
        unique_fields_param = request.query_params.get("unique_fields")
        print(f"DEBUG: unique_fields_param = {unique_fields_param}")
        
        if unique_fields_param and isinstance(request.data, list):
            print(f"DEBUG: Calling _sync_upsert")
            return self._sync_upsert(request, unique_fields_param)

        # If no unique_fields or not array data, this is invalid
        print(f"DEBUG: Invalid PATCH request - no unique_fields or not array data")
        return Response(
            {
    ...
    def _sync_upsert(self, request, unique_fields_param):
        ...
        print(f"=== DEBUG: Starting _sync_upsert ===")
        print(f"DEBUG: unique_fields_param = {unique_fields_param}")
        print(f"DEBUG: request.data = {request.data}")
        print(f"DEBUG: request.query_params = {request.query_params}")
        ...
        print(f"DEBUG: parsed unique_fields = {unique_fields}")
        ...
        print(f"DEBUG: partial_success = {partial_success}")

        data_list = request.data
        if not isinstance(data_list, list):
            print(f"DEBUG: data_list is not a list: {type(data_list)}")
            return Response(
        ...
        if len(data_list) > max_sync_items:
            print(f"DEBUG: Too many items: {len(data_list)} > {max_sync_items}")
            return Response(
                {
                    "error": f"Too many items for sync processing. Use /bulk/ endpoint for >{max_sync_items} items.",
        ...
        if not unique_fields:
            print(f"DEBUG: No unique fields provided")
            return Response(
        ...
        if not update_fields:
            ...
            print(f"DEBUG: Auto-inferred update_fields = {update_fields}")

        # Perform sync upsert
        try:
            print(f"DEBUG: Calling _perform_sync_upsert")
            result = self._perform_sync_upsert(
                data_list, unique_fields, update_fields, partial_success, request
            )
            print(f"DEBUG: _perform_sync_upsert returned: {result}")
            return result
        except Exception as e:
            print(f"DEBUG: Exception in _sync_upsert: {e}")
            return Response(
    ...
    def _perform_sync_upsert(
        ...
        print(f"=== DEBUG: Starting _perform_sync_upsert ===")
        ...
        print(f"DEBUG: serializer_class = {serializer_class}")
        ...
        for index, item_data in enumerate(data_list):
            print(f"DEBUG: Processing item {index}: {item_data}")
            try:
                ...
                for field in unique_fields:
                    if field in item_data:
                        ...
                        print(f"DEBUG: Found unique field '{field}' = {item_data[field]}")
                    else:
                        missing_fields.append(field)
                        print(f"DEBUG: Missing unique field '{field}'")
                ...
                if missing_fields:
                    ...
                    print(f"DEBUG: Validation error for item {index}: {validation_error}")
                    continue
        ...
        if not partial_success and validation_errors:
            print(f"DEBUG: Failing due to validation errors: {validation_errors}")
            return Response(
                {
        ...
        for index, item_data in enumerate(data_list):
            print(f"DEBUG: Second pass - Processing item {index}: {item_data}")
            try:
                ...
                if failed_validation:
                    print(f"DEBUG: Item {index} failed validation, skipping")
                    if partial_success:
                        error_to_add = next(
                            error
                ...
                print(f"DEBUG: Serializer fields: {list(serializer_fields.keys())}")

                for field in unique_fields:
                    print(f"DEBUG: Processing unique field '{field}'")
                    # Check if this field is a SlugRelatedField in the serializer
                    serializer_field = serializer_fields.get(field)
                    print(f"DEBUG: Serializer field for '{field}': {serializer_field}")
                    print(f"DEBUG: Field type: {type(serializer_field)}")
                    
                    if serializer_field and isinstance(
                        serializer_field, serializers.SlugRelatedField
                    ):
                        print(f"DEBUG: '{field}' is a SlugRelatedField")
                        ...
                        try:
                            ...
                            print(f"DEBUG: Looking up {field} with value: {item_data[field]}")
                            print(f"DEBUG: SlugRelatedField queryset: {serializer_field.queryset}")
                            print(f"DEBUG: SlugRelatedField slug_field: {serializer_field.slug_field}")
                            
                            related_obj = serializer_field.queryset.get(
                                **{serializer_field.slug_field: item_data[field]}
                            )
                            print(f"DEBUG: Found related object: {related_obj}")
                            lookup_filter[f"{field}_id"] = related_obj.id
                            print(f"DEBUG: Added to lookup_filter: {field}_id = {related_obj.id}")
                        except Exception as e:
                            print(f"DEBUG: Error looking up {field}: {e}")
                            # If we can't convert, skip this field for now
                            continue
                    else:
                        print(f"DEBUG: '{field}' is not a SlugRelatedField")
        ...
        print(f"DEBUG: To create: {len(to_create)} items")
        print(f"DEBUG: To update: {len(to_update)} items")

        # Process creates using bulk_create
        if to_create:
            print(f"DEBUG: Processing {len(to_create)} items to create")
            try:
                ...
                for index, item_data in to_create:
                    print(f"DEBUG: Validating create data for index {index}: {item_data}")
                    serializer = serializer_class(data=item_data)
                    if serializer.is_valid():
                        validated_data = serializer.validated_data
                        print(f"DEBUG: Validated data for index {index}: {validated_data}")
                        ...
                        print(f"DEBUG: Added to create_objects: {instance}")
                    else:
                        error_info = {
                            "index": index,
                        ...
                        print(f"DEBUG: Validation failed for create index {index}: {serializer.errors}")
                        errors.append(error_info)
                ...
                if create_objects:
                    print(f"DEBUG: Performing bulk_create with {len(create_objects)} objects")
                    created_instances = model_class.objects.bulk_create(
                        create_objects,
                        batch_size=1000,  # Adjust batch size as needed
                        ignore_conflicts=False,
                    )
                    print(f"DEBUG: Created {len(created_instances)} instances")
                    ...
                    for i, instance in enumerate(created_instances):
                        ...
                        print(f"DEBUG: Created instance {instance.id} for index {index}")
                ...
                print(f"DEBUG: Exception during bulk_create: {e}")
                if not partial_success:
                    return Response(
                        {
        ...
        if to_update:
            print(f"DEBUG: Processing {len(to_update)} items to update")
            try:
                ...
                for index, item_data, existing_instance in to_update:
                    print(f"DEBUG: Validating update data for index {index}: {item_data}")
                    ...
                    if serializer.is_valid():
                        validated_data = serializer.validated_data
                        print(f"DEBUG: Validated update data for index {index}: {validated_data}")
                        ...
                        print(f"DEBUG: Update data for index {index}: {update_data}")

                        # Apply updates to the existing instance
                        for field, value in update_data.items():
                            setattr(existing_instance, field, value)
                            print(f"DEBUG: Set {field} = {value} on instance {existing_instance.id}")

                        update_objects.append(existing_instance)
                        update_serializers.append((index, serializer))
                        update_indices.append(index)
                        print(f"DEBUG: Added to update_objects: {existing_instance}")
                    else:
                        ...
                        print(f"DEBUG: Validation failed for update index {index}: {serializer.errors}")
                ...
                if update_objects:
                    print(f"DEBUG: Performing bulk_update with {len(update_objects)} objects")
                    ...
                    print(f"DEBUG: Fields to update: {fields_to_update}")
                    ...
                    print(f"DEBUG: Bulk update completed")
                    ...
                    for i, instance in enumerate(update_objects):
                        ...
                        instances.append(instance)
                        print(f"DEBUG: Updated instance {instance.id} for index {index}")
                ...
                print(f"DEBUG: Exception during bulk_update: {e}")
                if not partial_success:
                    return Response(
                        {
        ...
        print(f"DEBUG: Final results - created_ids: {created_ids}, updated_ids: {updated_ids}")
        print(f"DEBUG: Final results - errors: {errors}")
        print(f"DEBUG: Final results - success_data: {len(success_data)} items")
