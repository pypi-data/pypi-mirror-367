                ...
                for index, item_data, existing_instance in to_update:
                    ...
                    if serializer.is_valid():
                        ...
                        for field, value in update_data.items():
                            # Handle foreign key fields properly for bulk_update
                            if hasattr(existing_instance._meta.get_field(field), 'related_model') and existing_instance._meta.get_field(field).related_model:
                                # This is a foreign key field, we need to set the ID, not the object
                                if hasattr(value, 'id'):
                                    # Value is an object, use its ID
                                    setattr(existing_instance, field, value.id)
                                    print(f"DEBUG: Set {field} = {value.id} (from object {value}) on instance {existing_instance.id}", file=sys.stderr)
                                else:
