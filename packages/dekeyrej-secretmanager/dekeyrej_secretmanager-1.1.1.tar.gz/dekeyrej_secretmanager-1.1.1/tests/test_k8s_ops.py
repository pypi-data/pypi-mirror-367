...
@patch("secretmanager._k8s_ops.config.incluster_config.load_incluster_config", side_effect = config.ConfigException("Not in cluster"))
...
def test_get_k8s_service_account_token_failure():
    mock_client = MagicMock()
    mock_client.create_namespaced_service_account_token.side_effect = ApiException("API error")
...
def test_read_k8s_secret_single_key(mock_decode):
    ...
    manager = SecretManager()
    manager.k8s_client = mock_k8s_client
    mock_k8s_client.read_namespaced_secret.return_value.data = {"config.json": "ZW5jb2RlZA=="}
    secret_def = {
        "namespace": "default",
        "secret_name": "my-secret",
        "read_type": "SECRET",
...
@patch("secretmanager._k8s_ops.client.CoreV1Api.read_namespaced_secret", side_effect=client.exceptions.ApiException("API failure"))
...
@patch("secretmanager._k8s_ops.client.CoreV1Api.read_namespaced_config_map", side_effect=Exception("Unexpected error"))
