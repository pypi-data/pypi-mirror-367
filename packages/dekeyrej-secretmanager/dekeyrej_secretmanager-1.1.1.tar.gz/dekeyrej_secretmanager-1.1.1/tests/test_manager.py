...
def test_manager_accepts_valid_source(monkeypatch):
    ...
    monkeypatch.setattr("secretmanager.manager.SecretManager.execute", lambda self, backend, verb, *args, **kwargs: f"{backend}:{verb}")
...
def test_manager_reconfigures_source(monkeypatch, caplog):
    monkeypatch.setattr("secretmanager.manager.SecretManager.execute", lambda self, backend, verb, *args, **kwargs: f"{backend}:{verb}")
    monkeypatch.setattr("secretmanager.verbregistry.VerbRegistry.perform", lambda self, backend, verb, *args, **kwargs: f"{backend}:{verb}")
