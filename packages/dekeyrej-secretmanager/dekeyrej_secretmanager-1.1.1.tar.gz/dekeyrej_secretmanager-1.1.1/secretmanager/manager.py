...
class SecretManager:
    ...
    def __init__(self, config: dict = None, log_level=logging.INFO):
        ...
        self.config = None
        if not config:
            logger.info("No configuration provided. Call configure_secret_type with a valid secret_type.")
        else:
            self.configure_secret_type(config)

    def configure_secret_type(self, config: dict):
        if config.get("SOURCE") not in self.SOURCES:
            raise ValueError(f"Invalid configuration source. Must be one of: {', '.join(self.SOURCES)}.")
        if not self.config:
            logger.info(f"SecretManager initializing with source: {config.get('SOURCE')}")
        elif self.config and self.config.get("SOURCE") != config.get("SOURCE"):
            logger.info(f"Changing configuration source from {self.config.get('SOURCE')} to {config.get('SOURCE')}")
            self.execute(self.config.get("SOURCE"), "LOGOUT")
        elif self.config and self.config.get("SOURCE") == config.get("SOURCE"):
            logger.info(f"SecretManager already configured with source: {self.config.get('SOURCE')}")
            return
