...
def init_kubevault(manager: SecretManager):
    ...
    hvac_client = connect_to_vault(manager.config.get("vault_url"), manager.config.get("ca_cert", True))
    if not hvac_client.is_authenticated():
        jwt = _get_k8s_service_account_token(k8s_client, manager.config.get("service_account"), manager.config.get("namespace"))['data']
        if not jwt:
            logger.error(f"Failed to retrieve service account token for '{manager.config.get('service_account')}' in namespace '{manager.config.get('namespace')}'.")
            return {"status": "failure", "verb": "INIT", "source": "KUBEVAULT"}
        hvac_client.token = _authenticate_vault_via_kubernetes(hvac_client, manager.config.get("role"), jwt)
        if not hvac_client.is_authenticated():
...
def reauthenticate_vault_via_kubernetes(manager: SecretManager) -> bool:
    ...
    jwt = _get_k8s_service_account_token(manager.k8s_client, manager.config.get("service_account"), manager.config.get("namespace"))['data']
    if not jwt:
        logger.error(f"Failed to retrieve service account token for '{manager.config.get('service_account')}' in namespace '{manager.config.get('namespace')}'.")
        return False
    manager.hvac_client.token = _authenticate_vault_via_kubernetes(manager.hvac_client, manager.config.get("role"), jwt)
...
def read_encrypted_secrets(manager: SecretManager, secret_def: dict) -> dict:
    ...
    k8s_enc_secret = read_k8s_secret(manager, secret_def)['data'] #[secret_def['read_key']]
    ...
    if not manager.hvac_client.is_authenticated():
        ...
        if not status:
            ...
            return {"status": "failure", "verb": "READ", "source": "KUBEVAULT"}
    decrypted_data = decrypt_data_with_vault(manager.hvac_client, transit_key, k8s_enc_secret)
    ...
    if decrypted_data is None:
        ...
        return {"status": "failure", "verb": "READ", "source": "KUBEVAULT"}
    return {"status": "success", "verb": "READ", "source": "KUBEVAULT", "data": load_json_secrets(decrypted_data)}
