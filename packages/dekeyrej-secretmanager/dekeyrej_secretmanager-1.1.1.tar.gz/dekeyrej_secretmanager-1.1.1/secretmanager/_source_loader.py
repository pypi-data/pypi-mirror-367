...
def read_secrets_from_env(secret_def:dict) -> dict:
    ...
    try:
        ...
        return {"status": "success", "verb": "READ", "source": "ENVIRONMENT", "data": secrets}
    except KeyError as e:
        logger.error(f"Missing key in secret definition: {e}")
        return {"status": "failure", "verb": "READ", "source": "ENVIRONMENT", "error": str(e)}
...
def create_secrets_file(secret_def: dict, data: dict) -> dict:
    ...
    if secret_def['file_type'] == 'JSON':
        ...
        return {"status": "failure", "verb": "CREATE", "source": "FILE", "error": f"Unknown secret type '{secret_def['file_type']}'."}
    ...
    try:
        ...
        return {"status": "failure", "verb": "CREATE", "source": "FILE", "error": str(e)}
    else:
