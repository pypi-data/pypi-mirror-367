...
# the following methods handle Kubernetes service account token retrieval for KubeVault.
...
def create_k8s_secret(manager: SecretManager, secret_def: dict, data) -> dict:
    ...
    try:
        manager.k8s_client.create_namespaced_secret(namespace=secret_def['namespace'], body=secret)
        logger.info(f"Secret '{secret_def['secret_name']}' created in namespace '{secret_def['namespace']}'.")
        return {"status": "success", "verb": "CREATE", "source": "KUBERNETES"}
    except client.ApiException as e:
        if e.status == 409:  # Conflict, secret already exists
            manager.k8s_client.replace_namespaced_secret(name=secret_def['secret_name'], namespace=secret_def['namespace'], body=secret)
            logger.warning(f"Secret '{secret_def['secret_name']}' updated in namespace '{secret_def['namespace']}'.")
            return {"status": "success", "verb": "CREATE", "source": "KUBERNETES"}
        else:
...
def read_k8s_secret(manager: SecretManager, secret_def: dict):
    ...
    try:
        if secret_def['read_type'] == "SECRET":
            api_response = manager.k8s_client.read_namespaced_secret(secret_def['secret_name'], secret_def['namespace'])
            ...
            if secret_def.get('read_key'):
                ...
                logger.debug(f"{secret_def['read_key']}: {crypto_utils.decode_data(api_response.data[secret_def['read_key']])}")
                secrets = crypto_utils.decode_data(api_response.data[secret_def['read_key']])
                return {"status": "success", "verb": "READ", "source": "KUBERNETES", "data": secrets}  # return raw secret
            else:
                secrets = {}
                for data_name in api_response.data:
                    logger.debug(f"{data_name}: {crypto_utils.decode_data(api_response.data[data_name])}")
                    secrets[data_name] = crypto_utils.decode_data(api_response.data[data_name])
                return {"status": "success", "verb": "READ", "source": "KUBERNETES", "data": secrets} # returns dict of all secrets in the secret
        elif secret_def['read_type'] == "CONFIG_MAP":
            api_response = manager.k8s_client.read_namespaced_config_map(secret_def['secret_name'], secret_def['namespace'])
            ...
            return {"status": "success", "verb": "READ", "source": "KUBERNETES", "data": secrets}  # returns dict of all secrets in the configmap
    except client.exceptions.ApiException as apiex:
        logger.debug(apiex)
        return {"status": "failure", "verb": "READ", "source": "KUBERNETES", "error": str(apiex)}
    except KeyError as ke:
        logger.error(f"Key error: {ke}")
        return {"status": "failure", "verb": "READ", "source": "KUBERNETES", "error": str(ke)}
    except Exception as e:
        logger.error(f"An error occurred while reading the secret: {e}")
        return {"status": "failure", "verb": "READ", "source": "KUBERNETES", "error": str(e)}
