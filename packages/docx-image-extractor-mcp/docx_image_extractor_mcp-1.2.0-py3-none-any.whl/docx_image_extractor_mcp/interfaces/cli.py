...
def extract_command(args) -> None:
    ...
    for docx_path in args.files:
        ...
        result = extract_images(docx_path, base_dir)
        results.append({
            'file': docx_path,
            'result': result
        })
...
示例用法:
  ...
  # 提取多个文件的图片到指定目录
  python cli.py extract -o images/ doc1.docx doc2.docx
  
  # 预览文档结构
  python cli.py preview document.docx
  
  # 转换文件名为ASCII
  python cli.py convert "测试文档.docx" "另一个文档.docx"
  ...
  python cli.py config show
    ...
    # 提取命令
    extract_parser = subparsers.add_parser("extract", help="提取DOCX文件中的图片")
    extract_parser.add_argument("files", nargs="+", help="DOCX文件路径")
    ...
    # 预览命令
    preview_parser = subparsers.add_parser("preview", help="预览DOCX文件结构")
    preview_parser.add_argument("files", nargs="+", help="DOCX文件路径")
