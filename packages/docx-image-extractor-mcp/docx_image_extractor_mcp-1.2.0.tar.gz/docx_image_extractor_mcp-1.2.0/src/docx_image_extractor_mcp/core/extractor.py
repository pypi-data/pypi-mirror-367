...
def extract_images(docx_path: str, base_image_dir: str = 'images') -> Dict[str, Union[bool, str, int]]:
    """
    提取 docx 所有图片到 base_image_dir/文档英文名/ 下
    
    Args:
        docx_path: Word文档的完整路径
    ...
    if not os.path.exists(docx_path):
        error_msg = f'文档不存在: {docx_path}'
        logger.error(error_msg)
        return {'success': False, 'msg': error_msg, 'output_dir': '', 'count': 0}
    
    if not docx_path.lower().endswith('.docx'):
        error_msg = '只支持.docx文件'
        logger.error(error_msg)
        return {'success': False, 'msg': error_msg, 'output_dir': '', 'count': 0}
    
    # 创建输出目录
    doc_dir = to_ascii_dirname(docx_path)
