...
class ReverseRelatedManager(BaseRelatedManager):
    ...
    def get_queryset(self, instance: "Model") -> QuerySet:
        ...
        return self.related_model_class.objects().filter(
            **{self.foreign_key_field.name: pk_value}
        )
...
class Model(metaclass=ModelMeta):
    ...
    def __setattr__(self, name, value):
        ...
        Handles special logic for relationship managers and ForeignKey fields.
        ...
        if hasattr(self, "_relationships") and self._relationships:
            ...
            if manager:
                ...
                if hasattr(self, "_database") and self._database:
                    # Check if value is a model instance or a simple value (like an int for ID)
                    if value is not None and hasattr(value, "_meta"):
                        ...
                        fk_field = self._fields.get(manager.foreign_key_field.name)
                        if fk_field:
                            fk_value = fk_field.validate(value)
                        else:
                            fk_value = value
                    # Set the actual foreign key field's value in _data
                    self._data[manager.foreign_key_field.name] = fk_value
                    return
    ...
    async def save(self):
        ...
        Performs an INSERT if the instance is new, or an UPDATE if it already exists.
    ...
    async def _fast_insert(self):
        """
        Fast insert with minimal overhead and correct placeholder handling.
        Properly handles auto-increment fields by excluding them when their value is None.
        """
        ...
        # First pass: Identify fields to insert
        for field_name, field in self._fields.items():
            field_value = self._data.get(field_name)

            # Special handling for auto-increment primary keys
            if field.primary_key and field.auto_increment:
                # If the value is None, exclude it from INSERT (let DB auto-generate)
                # If the value is explicitly set (not None), include it
                if field_value is not None:
                    columns.append(field_name)
                    values.append(field_value)
                # else: skip this field, it will be auto-generated
            else:
                # For all other fields, include them if they have a value or if they are nullable
                # This also includes non-PK fields with None value if they are nullable
                if field_value is not None:
                    columns.append(field_name)
                    values.append(field_value)
                elif field.nullable:
                    # Explicitly insert NULL for nullable fields with None value
                    columns.append(field_name)
                    values.append(None)
                # Note: Non-nullable fields with None value are intentionally skipped
                #       This should ideally be caught by field.validate() earlier

        # Generate query with correct placeholders
        if columns:
            # Correctly generate placeholders based on database type
            ...
            # No fields to insert, insert default values
            query = f"INSERT INTO {table_name} DEFAULT VALUES"
            lastrowid = await connection.execute(query)

        # Handle auto-increment field value assignment after insert
        for field_name, field in self._fields.items():
            if (
                field.primary_key
                and field.auto_increment
                and self._data.get(field_name) is None
            ):
                # Assign the auto-generated ID to the model instance
    ...
    async def _fast_update(self):
        ...
        if hasattr(connection, "get_placeholder") and callable(
            ...
            if connection.get_placeholder() == "$":
                ...
                where_placeholder = connection.get_placeholder()
        else:
            where_placeholder = "?"
        query = f"UPDATE {table_name} SET {', '.join(set_clauses)} WHERE {pk_field} = {where_placeholder}"
    ...
    async def bulk_create(
        cls, objects: List[Dict] | List["Model"], batch_size: int = 1000
    ) -> List["Model"]:
        """
    ...
    async def _bulk_create_batch(
        cls, objects: List[Dict] | List["Model"]
    ) -> List["Model"]:
    ...
    async def _bulk_insert_batch(cls, data_list: List[Dict]) -> List["Model"]:
        ...
        Properly handles auto-increment fields.
        """
        if not data_list:
            return []
...
class Database:
    ...
    async def transaction(self):
        ...
        Yields:
            The database instance for use within the transaction.

        Example:
            async with db.transaction():
                await User.create(username="test")
                await Post.create(title="Test Post", author_id=1)
        """
    ...
    async def create_tables(self):
        ...
        This method also sets up relationships before creating tables.
        """
        if not self.models:
            return
    ...
    async def create_table(self, model_class: Type["Model"]):
        ...
        for field_name, field in model_class._fields.items():
            ...
            if (
                ...
                # Use IDENTITY for Postgres 10+ (or SERIAL for older PG versions)
                fields_sql_parts.append(
                    f"{field_name} INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
                )
                continue
        ...
        for field_name, field in model_class._fields.items():
            if isinstance(field, ForeignKeyField):
                ...
                fields_sql_parts.append(
                    f"CONSTRAINT {constraint_name} FOREIGN KEY ({field_name}) REFERENCES {ref_table}(id)"
                )
...
class PostgreSQLConnection(DatabaseConnection):
    ...
    async def execute(self, query: str, params: tuple = None) -> Any:
        if not self.pool:
            raise DatabaseError("Database not connected")
        async with self.pool.acquire() as conn:
            # اصلاح: پارامترها را به درستی به asyncpg پاس می‌دهیم
            if params:
