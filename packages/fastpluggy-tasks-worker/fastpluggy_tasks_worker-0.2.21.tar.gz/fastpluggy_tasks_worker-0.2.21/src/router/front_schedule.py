...
def scheduled_task_monitoring(
    ...
    try:
        ...
        max_reports_per_task = min(max(1, max_reports_per_task), 50)  # Limit between 1 and 50
        page = max(1, page)  # Ensure page is at least 1
        page_size = min(max(1, page_size), 20)  # Limit between 1 and 20

        # Create FilterCriteria object from individual parameters
        filter_criteria = FilterCriteria(
            task_name=task_name,
            start_time=start_time,
            end_time=end_time,
            max_reports_per_task=max_reports_per_task
        )
        ...
        # Create a default FilterCriteria for error state
        filter_criteria = FilterCriteria(
            task_name=task_name,
            start_time=start_time,
            end_time=end_time,
