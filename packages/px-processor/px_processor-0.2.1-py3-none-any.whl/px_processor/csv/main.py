...
class CSVValidator(BaseValidator):
    ...
    def __init__(
        ...
        *,
        ...
        self.column_comparators: list[CSVColumnComparatorConstraint] = (
            column_comparators or []
        )
        ...
        if not self._csv_paths:
            ...
            if self.mandatory_columns and not set(self.mandatory_columns).issubset(
                ...
                missing_columns: list[str] = list(
                    set(self.mandatory_columns) - set(self._column_names)
                )
                self.errors.append(
                    MandatoryColumnError(missing_columns=missing_columns)
                )
            ...
            self._columns_with_no_missing_data: list[str] = (
                columns_with_no_missing_data or []
            )
            ...
            self._category_columns: list[str] = [
                column for column in self._valid_column_values
            ]
            self._add_category_columns_in_dtype_mapping()
            self._read()
            if self._engine == "pandas":
    ...
    def _auto_detect_engine() -> str:
        ...
        if _HAS_PANDAS:
            ...
            return "pandas"
        error_message: str = (
            "You must install either 'pandas' or 'polars' to use CSVValidator."
        )
    ...
    def _add_category_columns_in_dtype_mapping(self) -> None:
    ...
    def _read_single_file(self, csv_path: str):  # noqa: ANN202
        ...
        error_message: str = (
            f"Invalid pandas engine: {self.pandas_config.pandas_engine}"
        )
        raise ValueError(error_message)
    ...
    def _check_date_format_polars(self) -> None:
        ...
        for column in self._date_columns:
            ...
            if (
                column in self._columns_with_no_missing_data
                or missing_dataframe.height == 0
            ):
                try:
                    self.lazy_data.with_columns(
                        self.pl.col(column).str.to_date()
                    ).collect()
                except (
                    ...
                    self.lazy_data = self.lazy_data.with_columns(
                        self.pl.col(column).str.to_date()
                    )
    ...
    def _validate_single_column(
        ...
        if not self.data[column].isin(values=valid_values).all():
            self.errors.append(
                FixedValueColumnError(column=column, valid_values=valid_values)
            )
        ...
        if self.column_value_limits:
            for column_value_limit in self.column_value_limits:
                if column_value_limit.column_name not in self.data_columns:
                    self.errors.append(
                        MissingColumnError(column=column_value_limit.column_name)
                    )
                    continue
                            ...
                            .is_between(
                                lower_bound=(
                                    ...
                                    if column_value_limit.min_value is not None
                                        ...
                                        column_value_limit.column_name
                                    ).min()
                                ),
                                upper_bound=(
    ...
    def _check_column_comparisons_polars(self) -> None:
        ...
        columns_to_check: set[str] = {
            col
            for comp in self.column_comparators
            for col in [comp.left_column, comp.right_column]
        }
        ...
        comparison_failures: list[ComparisonFailure] = (
            self._extract_comparison_failures_polars(result)
        )
    ...
    def _build_comparison_expressions_polars(self) -> list["Expr"]:
        ...
        for index, comparator in enumerate(self.column_comparators):
            ...
            null_check: Expr = (left.is_null() | right.is_null()).alias(
                f"has_null_{index}"
            )
            comparison: Expr = self._comparison_operators[comparator.operator](
                left, right
            )
        ...
        expressions.extend(
            [
                self.pl.col(col).is_null().alias(f"null_{col}")
    ...
    def _extract_comparison_failures_polars(
        ...
        comparison_failures: list[ComparisonFailure] = []
        for index, comparator in enumerate(self.column_comparators):
            failed_rows = result.filter(self.pl.col(f"comp_fail_{index}"))[
                "row_idx"
            ].to_list()
            if failed_rows:
                comparison_failures.append(
