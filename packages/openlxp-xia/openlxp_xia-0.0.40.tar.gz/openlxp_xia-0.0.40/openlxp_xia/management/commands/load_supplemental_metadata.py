        ...
        # POSTing data to XIS
            try:
                ...
                if xis_response.status_code == 201:
                    trans_status.target_metadata_transmission_status = 'Successful'
                    trans_status.target_metadata_transmission_date = timezone.now()
...
def load_supplemental_metadata_to_xis(xis):
    ...
    if xis:
        ...
        data = SupplementalLedger.objects.filter(
            ...
            supplementaltransmissionstatus__target_metadata_transmission_status_code=400
        ).values(
    ...
    def handle(self, *args, **options):
        ...
        if 'config' in options:
            ...
            try:
                ...
                logger.error(
                    f'XIS Configuration with ID {options["config_id"]} does not exist')
                return
        if not xis:
