...
def get_timestamp_entropy(salt: bytes = b"muerte32") -> bytes:
    """Creates a time-based entropy seed using current timestamp and process randomness."""
...
def serpent_encrypt_raw(key: bytes, plaintext: bytes) -> bytes:
    """
    Encrypt plaintext bytes with Serpent (ECB mode) and PKCS7 padding.
    Args:
        key (bytes): Serpent key (16, 24, or 32 bytes)
...
def serpent_decrypt_raw(key: bytes, ciphertext: bytes) -> bytes:
    """
    Decrypt Serpent ciphertext bytes (ECB mode) and remove PKCS7 padding.
    Args:
        key (bytes): Serpent key (same length as used for encryption)
