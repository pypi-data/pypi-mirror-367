...
<script lang="ts">
    ...
    function handleContextMenu(e) {
		...
		if (!file) return;          // rien à télécharger
	...
	function downloadFile() {
		...
		segments[segments.length - 1] = encodeURIComponent(file.orig_name);
	...
	// Fonction pour gérer le clic en dehors du menu contextuel
			...
			<div class="button-row">
				{#if Array.isArray(value) && value.length > 1}
					<button class="left-arrow" on:click={previous_file} title={previous_file_title}>
						<svg xmlns="http://www.w3.org/2000/svg" width="80%" height="80%" viewBox="0 0 24 24">
							<path d="M15 6L8 12L15 18" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
						</svg>
					</button>
				{/if}
				{#if Array.isArray(value) && value.length > 0}
				...
				{#if Array.isArray(value) && value.length > 1}
					<button class="right-arrow" on:click={next_file} title={next_file_title}>
						<svg xmlns="http://www.w3.org/2000/svg" width="80%" height="80%" viewBox="0 0 24 24">
							<path d="M9 6L16 12L9 18" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
						</svg>
					</button>
				{/if}
			</div>
