...
class EfficientLLMCLI:
    ...
    def __init__(self):
        # Use package resources to find data files
        try:
            import importlib.resources as resources
            # Modern approach using importlib.resources
            try:
                with resources.path('efficientllm', '__init__.py') as pkg_path:
                    # Go up one level to get the package root where data files are
                    self.base_dir = pkg_path.parent.parent
            except:
                # Fallback to pkg_resources (deprecated but widely available)
                import pkg_resources
                try:
                    # Get the installation directory of this specific package
                    dist = pkg_resources.get_distribution('efficientllm-toolkit')
                    self.base_dir = Path(dist.location)
                except:
                    # Another fallback
                    import efficientllm
                    self.base_dir = Path(efficientllm.__file__).parent.parent
        except ImportError:
            # If neither available, use current directory
