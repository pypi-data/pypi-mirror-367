...
class EfficientLLMCLI:
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                resource_dir = pkg_resources.resource_filename('efficientllm-toolkit', '')
                if Path(resource_dir).exists():
                    self.base_dir = Path(resource_dir)
                else:
                    # Fallback to site-packages location
                    import site
                    for site_dir in site.getsitepackages():
                        candidate = Path(site_dir) / "efficientllm-toolkit"
                        if candidate.exists():
                ...
                import efficientllm
                package_root = Path(efficientllm.__file__).parent.parent.parent
                self.base_dir = package_root
        except ImportError:
            # If pkg_resources not available, use current directory
            self.base_dir = Path.cwd()
