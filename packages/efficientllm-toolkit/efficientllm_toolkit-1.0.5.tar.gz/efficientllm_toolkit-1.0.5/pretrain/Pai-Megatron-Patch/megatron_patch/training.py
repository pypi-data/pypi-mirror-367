...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if mpu.get_data_parallel_rank() == 0:
        total_params = sum([sum([p.nelement() for p in model_module.parameters()])
                           for model_module in model])
        ...
        trainable_params = sum([sum([p.nelement() for p in model_module.parameters() if p.requires_grad == True])
                               for model_module in model])
        print(' > number of trainable parameters on (tensor, pipeline) '
              'model parallel rank ({}, {}): {}'.format(
            mpu.get_tensor_model_parallel_rank(),
            mpu.get_pipeline_model_parallel_rank(),
            trainable_params), flush=True)
        
        # Initialize EfficientLLM metrics with total model parameters
        try:
            initialize_efficientllm_metrics(total_params)
        except Exception as e:
            print(f"Warning: Failed to initialize EfficientLLM metrics: {e}")
            print("Continuing without EfficientLLM metrics collection")
...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        # EfficientLLM: Log metrics to tensorboard
        try:
            if should_log_metrics(iteration):
                metrics_collector = get_metrics_collector()
                # Calculate current loss for metrics logging
                current_loss = 0.0
                for key in loss_dict:
                    if 'loss' in key.lower():
                        current_loss = loss_dict[key].item()
                        break
                metrics_collector.log_metrics(iteration, current_loss, writer)
        except Exception:
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        # EfficientLLM: Record iteration start
        try:
            metrics_collector = get_metrics_collector()
        ...
        # EfficientLLM: Record iteration end and calculate tokens/samples processed
        try:
            metrics_collector = get_metrics_collector()
            # Calculate tokens processed in this iteration
            tokens_processed = args.seq_length * args.micro_batch_size * get_num_microbatches()
            samples_processed = args.micro_batch_size * get_num_microbatches()
    ...
    # EfficientLLM: Finalize metrics collection
    try:
        finalize_efficientllm_metrics()
    except Exception:
