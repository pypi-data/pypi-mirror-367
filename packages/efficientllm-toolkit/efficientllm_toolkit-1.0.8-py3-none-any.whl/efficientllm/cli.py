...
class EfficientLLMCLI:
    ...
    def __init__(self):
        # Try multiple methods to find the data files
        self.base_dir = None
        
        # Method 1: Check current working directory first
        current_dir = Path.cwd()
        if ((current_dir / "quantization").exists() and 
            (current_dir / "fine-tune").exists() and 
            (current_dir / "pretrain").exists()):
            self.base_dir = current_dir
        
        # Method 2: Check if user has cloned the toolkit to home directory
        if not self.base_dir:
            home_toolkit_dir = Path.home() / "efficientllm-toolkit"
            if (home_toolkit_dir / "quantization").exists():
                self.base_dir = home_toolkit_dir
        
        # Method 3: Try to find installed data files
        if not self.base_dir:
            try:
                import sysconfig
                data_path = sysconfig.get_path('data')
                # Data files might be installed under data/quantization, data/fine-tune, etc.
                for candidate in [data_path, os.path.join(data_path, 'share')]:
                    if os.path.exists(os.path.join(candidate, 'quantization')):
                        self.base_dir = Path(candidate)
                        break
            except:
                pass
        
        # Method 4: Try to find in site-packages directory structure
        if not self.base_dir:
            try:
                import pkg_resources
                dist = pkg_resources.get_distribution('efficientllm-toolkit')
                # Check if data files are installed alongside site-packages
                site_packages = Path(dist.location)
                # Look in parent directories for our data
                for level in range(3):  # Go up 3 levels max
                    check_path = site_packages
                    for _ in range(level):
                        check_path = check_path.parent
                    for subdir in ['quantization', 'fine-tune', 'pretrain']:
                        if (check_path / subdir).exists():
        ...
        # Method 5: Search system-wide for our data files
        if not self.base_dir:
            import subprocess
            try:
                result = subprocess.run(['find', '/', '-name', 'batch_eval.py', '-path', '*/quantization/lm-evaluation-harness-main/batch_eval.py', '2>/dev/null'], 
                                      capture_output=True, text=True, timeout=10)
                if result.stdout.strip():
                    batch_eval_path = result.stdout.strip().split('\n')[0]
                    # Extract base directory (should be 3 levels up from batch_eval.py)
                    self.base_dir = Path(batch_eval_path).parent.parent.parent
            except:
                pass
        ...
        if hasattr(self, '_debug'):
            ...
            print(f"Quantization dir exists: {self.quantization_dir.exists()}")
            print(f"Fine-tune dir exists: {self.finetune_dir.exists()}")
            print(f"Pretrain dir exists: {self.pretrain_dir.exists()}")
...
Examples aligned with paper experiments:
  ...
  # Download EfficientLLM pre-trained checkpoints
  efficientllm --down
  ...
  efficientllm --reproduce_paper --experiment quantization
        ...
        parser.add_argument("--model", 
                          choices=["qwen2.5", "mamba", "pythia", "rwkv", 
                                 "DeepSeek-R1-Distill-Qwen-1.5B", "DeepSeek-R1-Distill-Llama-8B", "DeepSeek-R1-Distill-Qwen-14B",
                                 "Qwen2.5-7B", "Qwen2.5-14B", "Qwen2.5-32B", "Phi-4", "Phi-3.5-mini", "Yi-34B"],
        ...
        parser.add_argument("--debug-paths", action="store_true",
                          help="Show debug information about file paths")
    ...
    def ensure_project_files(self) -> bool:
        """Ensure project files are available by auto-cloning if needed"""
        # Check if we already have the necessary directories
        if ((self.base_dir / "quantization").exists() and 
            (self.base_dir / "fine-tune").exists() and 
            (self.base_dir / "pretrain").exists()):
            return True
        
        # Need to download the project
        print("⚠️  EfficientLLM project files not found.")
        print("📥 Auto-downloading from GitHub...")
        
        import subprocess
        import shutil
        
        repo_url = "https://github.com/EfficientLLM-tookit/EfficientLLM.git"
        
        # Determine download location - use home directory if not in a project dir
        current_dir = Path.cwd()
        if current_dir == self.base_dir and self.base_dir == Path.cwd():
            # User is in current working directory, clone to home
            download_dir = Path.home() / "efficientllm-toolkit"
        else:
            download_dir = self.base_dir
        
        # Remove existing directory if it exists but is incomplete
        if download_dir.exists() and not (download_dir / "quantization").exists():
            print(f"Cleaning incomplete directory: {download_dir}")
            shutil.rmtree(download_dir)
        
        try:
            # Clone the repository
            print(f"Cloning to: {download_dir}")
            result = subprocess.run([
                "git", "clone", repo_url, str(download_dir)
    ...
    def run_debug_paths(self, args) -> int:
        """Debug path information"""
        print("=== EfficientLLM Path Debug Information ===")
        print(f"Base directory: {self.base_dir}")
        print(f"Base directory exists: {self.base_dir.exists()}")
        print(f"Quantization dir: {self.quantization_dir}")
        print(f"Quantization dir exists: {self.quantization_dir.exists()}")
        print(f"Fine-tune dir: {self.finetune_dir}")
        print(f"Fine-tune dir exists: {self.finetune_dir.exists()}")
        print(f"Pretrain dir: {self.pretrain_dir}")
        print(f"Pretrain dir exists: {self.pretrain_dir.exists()}")
        
        # Check for batch_eval.py specifically
        batch_eval_path = self.quantization_dir / "lm-evaluation-harness-main" / "batch_eval.py"
        print(f"batch_eval.py path: {batch_eval_path}")
        print(f"batch_eval.py exists: {batch_eval_path.exists()}")
        
        # Show system paths
        import sysconfig
        print(f"\nSystem paths:")
        print(f"Data path: {sysconfig.get_path('data')}")
        ...
        try:
            import subprocess
            result = subprocess.run(['find', str(Path.home()), '-name', 'batch_eval.py', '-type', 'f'], 
                                  capture_output=True, text=True, timeout=30)
            if result.stdout.strip():
                print("Found batch_eval.py files:")
                for path in result.stdout.strip().split('\n'):
                    print(f"  {path}")
            else:
                print("No batch_eval.py files found")
        except Exception as e:
    ...
    def reproduce_paper(self, args) -> int:
        ...
        if experiment in ["quantization", "all"]:
            ...
            # Use correct model names that match batch_eval.py
...
def main():
    ...
    try:
        if args.debug_paths:
            return cli.run_debug_paths(args)
        elif args.down:
            # For --down command, ensure project files exist first
            if not cli.ensure_project_files():
                print("❌ Failed to download project files. Cannot proceed.")
                return 1
            return cli.run_download(args)
        elif args.reproduce_paper:
            # Ensure project files exist before reproducing experiments
            if not cli.ensure_project_files():
                print("❌ Failed to download project files. Cannot proceed.")
                return 1
            return cli.reproduce_paper(args)
        elif args.stage:
            # Ensure project files exist before running any stage
            if not cli.ensure_project_files():
                print("❌ Failed to download project files. Cannot proceed.")
                return 1
