...
class MultiStepReasoner:
    ...
    async def _get_tool_args_from_llm(self, tool_name: str, tool_def: Dict, conversation: List[Dict], generate_func, model_cfg: Dict) -> Dict:
        ...
        """
        self.config.reasoning_trace(f"<content>Asking LLM to generate arguments for tool: {tool_name}</content>")
...
"""
        ...
        self.config.reasoning_trace(f"<content>LLM generated arguments: {args_text}</content>")
    ...
    async def execute_reasoning_loop(self, question: str, guidelines: str, initial_plan: str,
        ...
        for i in range(self.config.max_iterations):
            ...
            try:
                ...
                if not final_answer:
                    self.config.reasoning_trace(f"<content>{assistant_text}</content>")
                ...
                if tool_calls:
                    for tc_data in tool_calls:
                        self.config.reasoning_trace(f"<content>Processing tool call: {tc_data.get('name', 'unknown')}</content>")
                        ...
                        if result and 'content' in result:
                            self.config.reasoning_trace(f"<content>Tool call output: {result['content']}</content>")
                ...
                if code_blocks and self.config.enable_code_execution:
                    ...
                    for code in code_blocks:
                        self.config.reasoning_trace(f"<content>Executing code: {code}\n</content>")
                        output = python_interpreter(code, self.python_context)
                        code_outputs.append(output)
                        self.config.reasoning_trace(f"<content>Code Output: {output}\n...</content>")
                ...
                if not code_blocks and not tool_calls:
                    no_code_output_msg = f"""<content>No code execution or tool calls detected</content>"""
