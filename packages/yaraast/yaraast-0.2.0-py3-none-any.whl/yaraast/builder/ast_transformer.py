...
class CloneTransformer:
    ...
    def clone_rule(rule: Rule) -> Rule:
        ...
        return Rule(
            ...
            condition=CloneTransformer.clone(rule.condition)
            if rule.condition
            else None,
        )
    ...
    def clone_yara_file(yara_file: YaraFile) -> YaraFile:
        ...
        return YaraFile(
            imports=[
                Import(module=imp.module, alias=imp.alias) for imp in yara_file.imports
            ],
            ...
            extern_rules=[
                CloneTransformer.clone(rule) for rule in yara_file.extern_rules
            ],
            extern_imports=[
                CloneTransformer.clone(imp) for imp in yara_file.extern_imports
            ],
...
class YaraFileTransformer:
    ...
    def remove_import(self, module: str) -> YaraFileTransformer:
        ...
        self.yara_file.imports = [
            imp for imp in self.yara_file.imports if imp.module != module
        ]
        return self
    ...
    def remove_include(self, path: str) -> YaraFileTransformer:
        ...
        self.yara_file.includes = [
            inc for inc in self.yara_file.includes if inc.path != path
        ]
        return self
    ...
    def remove_rule(self, rule_name: str) -> YaraFileTransformer:
        ...
        self.yara_file.rules = [
            rule for rule in self.yara_file.rules if rule.name != rule_name
        ]
        return self
