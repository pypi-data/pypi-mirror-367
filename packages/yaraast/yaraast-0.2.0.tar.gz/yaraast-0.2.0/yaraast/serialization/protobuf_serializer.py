...
class ProtobufSerializer(ASTVisitor[Any]):
    ...
    def serialize_text(
        self, ast: YaraFile, output_path: str | Path | None = None
    ) -> str:
    ...
    def _convert_expression_to_protobuf(self, expr, pb_expr) -> None:
        ...
        if isinstance(expr, Identifier):
            ...
            self._convert_expression_to_protobuf(
                expr.left, pb_expr.binary_expression.left
            )
            self._convert_expression_to_protobuf(
                expr.right, pb_expr.binary_expression.right
            )
        elif isinstance(expr, UnaryExpression):
            pb_expr.unary_expression.operator = expr.operator
            self._convert_expression_to_protobuf(
                expr.operand, pb_expr.unary_expression.operand
            )
