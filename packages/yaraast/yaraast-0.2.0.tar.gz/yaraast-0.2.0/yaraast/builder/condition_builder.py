...
class ConditionBuilder:
    ...
    def in_range(
        self, start: int | ConditionBuilder, end: int | ConditionBuilder
    ) -> Self:
        """String in range."""
        if not self._expression or not isinstance(self._expression, StringIdentifier):
    ...
    def for_any(
        ...
        return ConditionBuilder(
            ForExpression(
                quantifier="any", variable=var, iterable=iter_expr, body=cond_expr
            )
        )
    ...
    def for_all(
        ...
        return ConditionBuilder(
            ForExpression(
                quantifier="all", variable=var, iterable=iter_expr, body=cond_expr
            )
        )
    ...
    # Helper methods
    def _binary_op(
        self, op: str, other: ConditionBuilder | Expression | int | str
    ) -> Self:
    ...
    def _to_expression(
        self, value: ConditionBuilder | Expression | int | str
    ) -> Expression:
