        ...
        if self.imports:
            with dot.subgraph(name="cluster_imports") as imports_cluster:
                imports_cluster.attr(
                    label="Imports", style="filled", fillcolor="lightcyan"
                )
        ...
        if self.includes:
            with dot.subgraph(name="cluster_includes") as includes_cluster:
                includes_cluster.attr(
                    label="Includes", style="filled", fillcolor="lightyellow"
                )
        ...
        if self.rules:
            with dot.subgraph(name="cluster_rules") as rules_cluster:
                ...
                for rule_name, rule_info in self.rules.items():
                    ...
                    color = (
                        "lightgreen" if rule_info["string_count"] > 0 else "lightcoral"
                    )
    ...
    def get_dependency_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rules_with_strings": sum(
                1 for r in self.rules.values() if r["string_count"] > 0
            ),
                ...
                [
                    (
                        ...
                        len(
                            [
                                refs
                                for refs in self.module_references.values()
                                if mod in refs
                            ]
                        ),
                    )
            ...
            "average_strings_per_rule": sum(
                r["string_count"] for r in self.rules.values()
            )
...
def build_dependency_graph(ast: YaraFile) -> DependencyGraph:
    ...
    class DependencyFinder(ASTVisitor[None]):
        ...
        def visit_yara_file(self, node) -> None: pass
        def visit_import(self, node) -> None: pass
        def visit_include(self, node) -> None: pass
        def visit_rule(self, node) -> None: pass
        def visit_tag(self, node) -> None: pass
        def visit_string_definition(self, node) -> None: pass
        def visit_plain_string(self, node) -> None: pass
        def visit_hex_string(self, node) -> None: pass
        def visit_regex_string(self, node) -> None: pass
        def visit_string_modifier(self, node) -> None: pass
        def visit_hex_token(self, node) -> None: pass
        def visit_hex_byte(self, node) -> None: pass
        def visit_hex_wildcard(self, node) -> None: pass
        def visit_hex_jump(self, node) -> None: pass
        def visit_hex_alternative(self, node) -> None: pass
        def visit_hex_nibble(self, node) -> None: pass
        def visit_expression(self, node) -> None: pass
        def visit_string_identifier(self, node) -> None: pass
        def visit_string_count(self, node) -> None: pass
        def visit_string_offset(self, node) -> None: pass
        def visit_string_length(self, node) -> None: pass
        def visit_integer_literal(self, node) -> None: pass
        def visit_double_literal(self, node) -> None: pass
        def visit_string_literal(self, node) -> None: pass
        def visit_regex_literal(self, node) -> None: pass
        def visit_boolean_literal(self, node) -> None: pass
        def visit_binary_expression(self, node) -> None:
            self.visit(node.left)
            self.visit(node.right)
        def visit_unary_expression(self, node) -> None:
        ...
        def visit_of_expression(self, node) -> None:
            ...
            if hasattr(node.string_set, "accept"):
                self.visit(node.string_set)
        def visit_meta(self, node) -> None: pass
        ...
        def visit_string_operator_expression(self, node) -> None:
            self.visit(node.left)
            self.visit(node.right)
        def visit_extern_import(self, node) -> None: pass
        def visit_extern_namespace(self, node) -> None: pass
        def visit_extern_rule(self, node) -> None: pass
        def visit_extern_rule_reference(self, node) -> None: pass
        def visit_in_rule_pragma(self, node) -> None: pass
        def visit_pragma(self, node) -> None: pass
        def visit_pragma_block(self, node) -> None: pass
...
def export_dependency_graph(graph: DependencyGraph, output_path: str | Path, format: str = "json") -> None:
