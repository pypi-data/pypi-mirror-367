...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file for metrics report"
)
@click.option(
    "--format",
    "-f",
...
@click.option(
    "--quality-gate", type=int, default=70, help="Quality gate threshold (0-100)"
)
...
@click.option(
    "--interactive", is_flag=True, help="Generate interactive HTML with search"
)
...
def tree(
    yara_file: str, output: str | None, interactive: bool, title: str, no_metadata: bool, collapsible: bool
):
...
@click.argument("yara_file", type=click.Path(exists=True, dir_okay=False))
@click.option(
    "--output-dir", "-d", type=click.Path(), help="Output directory for all reports"
)
@click.option(
    "--format",
    "-f",
...
def report(yara_file: str, output_dir: str | None, format: str):
    ...
    dep_generator.generate_rule_graph(
        ast, str(output_path / f"{base_name}_rules.{format}"), format
    )
    ...
    tree_generator.generate_interactive_html(
        ast, str(output_path / f"{base_name}_tree.html")
    )
    ...
    if metrics.module_usage:
        lines.extend(
            [
                "📦 Module Usage:",
                *[
                    f"  {module}: {usage} times"
                    for module, usage in metrics.module_usage.items()
                ],
                "",
            ]
        )
