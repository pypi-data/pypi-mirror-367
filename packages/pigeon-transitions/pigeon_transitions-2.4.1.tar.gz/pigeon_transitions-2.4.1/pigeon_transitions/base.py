...
PIGEON_TRANSITIONS_MODULE = Path(__file__)
TRANSITIONS_MODULE = Path(transitions.__file__)
        ...
        with self._zipkin_tracer.zipkin_span(
            ...
            try:
                ...
                for frame in traceback.extract_tb(e.__traceback__)[::-1]:
                    module_path = Path(frame.filename).parent
                    if module_path in PIGEON_TRANSITIONS_MODULE.parents or module_path in TRANSITIONS_MODULE.parents:
                        break
                    tb.insert(0, frame)
