...
class AuthenticateRequest(BaseModel):
    email: str = Field(description="Email address for BRAIN platform account")
    password: str = Field(description="Password for BRAIN platform account")
...
class CNHKMCPServer:
    """MCP Server for WorldQuant BRAIN platform."""

    def __init__(self):
        self.client = ApiClient()
        self.server = Server("brain-mcp")
        self._setup_tools()
    ...
    def _setup_tools(self) -> None:
        ...
        async def authenticate(arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                return [{
                    "type": "text",
                    "text": f"Successfully authenticated with BRAIN platform. User ID: {result.get('user', {}).get('id', 'unknown')}"
                }]
        ...
        async def create_multi_simulation(arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
            """Submit multiple alpha expressions for batch simulation."""
            ...
            try:
                ...
                return [{
                    "type": "text",
                    "text": f"Multi-simulation created with {len(req.simulations)} alphas. Batch ID: {result.get('batch_id', 'unknown')}"
                }, {
                    "type": "text",
                    "text": f"Simulation details: {result}"
                }]
        ...
        async def get_alpha_details(arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
            """Get comprehensive alpha metadata and simulation results."""
            ...
            try:
                ...
                return [{
                    "type": "text",
                    "text": f"Alpha {alpha_id} details retrieved successfully"
                }, {
                    "type": "text",
                    "text": f"Details: {result}"
                }]
            except Exception as e:
                logger.error(f"Failed to get alpha details: {e}")
                return [{
                    "type": "text",
                    "text": f"Failed to get alpha details: {str(e)}"
                }]

        @self.server.call_tool()
        async def get_user_alphas(arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
            """Retrieve user's IS/OS alpha list with pagination."""
            ...
            try:
                ...
                return [{
                    "type": "text",
                    "text": f"Retrieved {len(result.get('results', []))} {stage} alphas"
                }, {
                    "type": "text",
                    "text": f"Alpha list: {result}"
                }]
            except Exception as e:
                logger.error(f"Failed to get user alphas: {e}")
                return [{
                    "type": "text",
                    "text": f"Failed to get user alphas: {str(e)}"
                }]
        ...
        async def combine_pnl_data(arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                return [{
                    "type": "text",
                    "text": f"Combined PnL data from {combined['summary']['total_alphas']} alphas"
                }, {
                    "type": "text",
        ...
        tools = [
            Tool(
                name="authenticate",
                description="🔐 REQUIRED FIRST: Authenticate with BRAIN platform to start any research workflow",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "email": {"type": "string", "description": "Email address for BRAIN platform account"},
                        "password": {"type": "string", "description": "Password for BRAIN platform account"}
                    },
            ...
            Tool(
                ...
                description="🚀 CORE RESEARCH: Submit alpha expressions for simulation",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            Tool(
                ...
                description="🚀 BATCH RESEARCH: Submit multiple alpha expressions for simultaneous simulation",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "simulations": {
            ...
            Tool(
                ...
                description="📊 RESULT EXTRACTION: Get alpha metadata and simulation results",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "alphaId": {"type": "string"}
            ...
            Tool(
                ...
                description="📋 ALPHA MANAGEMENT: Retrieve user's IS/OS alpha list with pagination",
                inputSchema={
                    "type": "object",
                    "properties": {
            ...
            Tool(
                ...
                description="📈 PORTFOLIO ANALYSIS: Aggregate PnL data from multiple alphas",
                inputSchema={
                    "type": "object",
                    "properties": {
