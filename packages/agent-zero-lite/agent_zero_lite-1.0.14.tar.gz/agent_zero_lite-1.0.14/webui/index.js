...
import * as api from "/js/api.js?v=1.0.3";
...
export async function sendMessage() {
  try {
    // Prevent double-sending by disabling send button
    if (sendButton.disabled) {
      console.debug("Send button already disabled, preventing duplicate send");
      return;
    }
    ...
    if (message || hasAttachments) {
      ...
      if (hasAttachments) {
        // Don't create a user message here - let the backend log it to avoid duplicates
        // The backend will properly log the message with attachments

        // sleep one frame before upload starts - better UX
        sleep(0);
...
let lastLogGuid = "";
let displayedLogNumbers = new Set(); // Track which log numbers have been displayed to prevent duplicates
let displayedMessageIds = new Set(); // Track which message IDs have been displayed to prevent duplicates
let lastSpokenNo = 0;
...
async function poll() {
  ...
  try {
    ...
    if (lastLogGuid != response.log_guid) {
      ...
      displayedLogNumbers.clear(); // Clear the tracking set when chat resets
      displayedMessageIds.clear(); // Clear the message ID tracking set when chat resets
      await poll();
      return;
    }
    ...
    if (lastLogVersion != response.log_version) {
      ...
      for (const log of response.logs) {
        ...
        // Skip logs that have already been displayed to prevent duplicates
        // Check both log.no and messageId to handle cases where same content has different IDs
        if (displayedLogNumbers.has(log.no) || displayedMessageIds.has(messageId)) {
          continue;
        }
        
        // Also check if DOM element already exists to prevent duplicates
        ...
        // Mark this log as displayed using both tracking methods
        displayedLogNumbers.add(log.no);
...
export const setContext = function (id) {
  ...
  lastLogVersion = 0;
  lastSpokenNo = 0;
  displayedLogNumbers.clear(); // Clear displayed log tracking when switching contexts
  displayedMessageIds.clear(); // Clear displayed message ID tracking when switching contexts
...
function setupTabs() {
  ...
  if (chatsTab && tasksTab) {
    ...
    console.log("Tab elements found and event listeners attached");
  } else {
    // Tab elements are commented out in HTML, so don't spam console with retries
    // Just log once that tabs are not available
    if (!setupTabs.loggedOnce) {
      console.log("Tab elements not found - tabs feature appears to be disabled");
      setupTabs.loggedOnce = true;
    }
    // Don't keep retrying if elements don't exist
    return;
  }
}
...
function activateTab(tabName) {
  ...
  // Reset all tabs and sections (with null checks)
