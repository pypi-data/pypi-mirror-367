...
from agent_zero_lite import models
from agent_zero_lite.python.helpers import runtime, whisper, defer, git
from . import files, dotenv
from agent_zero_lite.python.helpers.print_style import PrintStyle
from agent_zero_lite.python.helpers.providers import get_providers
            ...
            for field in section["fields"]:
                if field["value"] != PASSWORD_PLACEHOLDER:
                    if field["id"].endswith("_kwargs"):
                        ...
                        # Extract provider name from field ID (e.g., "api_key_openrouter" -> "openrouter")
                        provider_name = field["id"][8:]  # Remove "api_key_" prefix
                        current["api_keys"][provider_name] = field["value"]
                    else:
...
def get_default_settings() -> Settings:
    return Settings(
        ...
        browser_model_ctx_length=100000,
...
def _apply_settings(previous: Settings | None):
    ...
    if _settings:
        ...
        from agent_zero_lite.initialize import initialize_agent
        ...
        if not previous or (
            ...
            from agent_zero_lite.python.helpers.memory import reload as memory_reload

            memory_reload()

        # update mcp settings if necessary
        if not previous or _settings["mcp_servers"] != previous["mcp_servers"]:
            from agent_zero_lite.python.helpers.mcp_handler import MCPConfig
        ...
        if not previous or current_token != previous["mcp_server_token"]:
            ...
            async def update_mcp_token(token: str):
                from agent_zero_lite.python.helpers.mcp_server import DynamicMcpProxy
        ...
        if not previous or current_token != previous["mcp_server_token"]:
            ...
            async def update_a2a_token(token: str):
                from agent_zero_lite.python.helpers.fasta2a_server import DynamicA2AProxy
