...
from agent_zero_lite.python.helpers.strings import truncate_text_by_ratio
import copy
...
class Log:
    ...
    def _update_item(
        ...
        **kwargs,
        ...
        # Only add to updates if this is a real update (not just appending duplicate)
        # This prevents the same log item from appearing multiple times in the output
        if not self.updates or self.updates[-1] != item.no:
            self.updates += [item.no]
        self._update_progress_from_item(item)
