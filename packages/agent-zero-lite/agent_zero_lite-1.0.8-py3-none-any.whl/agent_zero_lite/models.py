...
from agent_zero_lite.python.helpers import dotenv
from agent_zero_lite.python.helpers.dotenv import load_dotenv
from agent_zero_lite.python.helpers.providers import get_provider_config
from agent_zero_lite.python.helpers.rate_limiter import RateLimiter
from agent_zero_lite.python.helpers.tokens import approximate_tokens
...
def get_api_key(service: str) -> str:
    ...
    if env_key and env_key != "None":
        return env_key
    
    # Then check settings system
    try:
        from agent_zero_lite.python.helpers import settings
        settings_data = settings.get_settings()
        api_keys = settings_data.get("api_keys", {})
        settings_key = api_keys.get(service.lower())
        if settings_key and settings_key != "None":
            return settings_key
    except Exception:
        pass  # Fallback if settings system not available
...
class LocalSentenceTransformerWrapper(Embeddings):
    ...
    def __init__(self, provider: str, model: str, model_config: Optional[ModelConfig] = None, **kwargs: Any):
        ...
        # Filter out API-related kwargs that SentenceTransformer doesn't accept
        st_kwargs = {k: v for k, v in kwargs.items() 
                    if k not in ['api_key', 'base_url', 'api_base', 'provider']}
