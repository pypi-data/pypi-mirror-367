...
class SessionBasedLLMLearningDashboard:
    ...
    def _create_session_tab(self, parent):
        ...
        ttk.Label(ref_frame, text="📖 Reference Code", 
        ...
        self.reference_text = scrolledtext.ScrolledText(
            ...
            padx=8, pady=8, state='disabled', height=20
        )
        ...
        ttk.Button(action_frame, text="💡 Hint", command=self._get_hint, 
                  style='Session.TButton').pack(side='left', fill='x', expand=True, padx=(0, 3))
        ttk.Button(action_frame, text="📋 Copy", command=self._copy_reference, 
    ...
    def _copy_reference(self):
        ...
        if current_session:
            ...
            self.status_label.config(
                text="📋 Reference code copied! But try typing it manually for better learning."
            )
...
def main():
    ...
    try:
        ...
        messagebox.showerror(
            "Dependency Error", 
            "Llama CPP library not found. Please install:\npip install llama-cpp-python"
        )
        return
    ...
    def on_setup_complete(model_path):
        if model_path:
            ...
            messagebox.showerror(
                "Setup Failed", 
                "AI model setup failed. The application will now close."
            )
            root.destroy()
