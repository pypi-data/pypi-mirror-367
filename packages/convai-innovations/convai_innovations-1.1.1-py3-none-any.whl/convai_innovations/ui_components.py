...
class CodeGenerationPanel(tk.Frame):
    """Panel for AI code generation with Argostranslate support"""
    ...
    def _create_widgets(self):
        """Create code generation widgets"""
        # Title
        ...
        ttk.Label(output_frame, text="Generated Code:", style='TLabel').pack(anchor='w')
        
        self.generated_text = scrolledtext.ScrolledText(
            output_frame, height=15, font=('Consolas', 10),
            bg='#2d3748', fg='#e2e8f0', insertbackground='white',
            wrap=tk.WORD, bd=1, relief='solid', state='disabled'
        )
    ...
    def _insert_generated(self):
        ...
        if self.generated_code:
            self.code_editor.insert_text(self.generated_code)
        else:
            messagebox.showinfo("No Code", "Please generate code first.")
    ...
    def _replace_with_generated(self):
        ...
        if self.generated_code:
            result = messagebox.askyesno(
                "Replace Code", 
                "This will replace all code in the editor. Continue?"
            )
            if result:
                self.code_editor.set_text(self.generated_code)
        else:
            messagebox.showinfo("No Code", "Please generate code first.")
...
class ModelDownloader:
    ...
    def _setup_worker(self):
        """Enhanced setup worker with Argostranslate"""
        # Phase 1: Check dependencies
        self.status_var.set("Phase 1/3: Checking dependencies...")
        ...
        try:
            ...
            self.details_var.set("Please install llama-cpp-python: pip install llama-cpp-python")
            time.sleep(3)
            self.setup_window.after(0, self._finalize, None)
            return
        
        # Phase 2: Check Argostranslate
        self.status_var.set("Phase 2/3: Initializing translation system...")
        ...
        try:
            ...
            self.details_var.set("✅ Argostranslate available - Offline translation ready!")
            time.sleep(1)
        except ImportError:
            self.details_var.set("⚠️ Argostranslate not found - Translation will use fallbacks")
            time.sleep(2)
        
        # Phase 3: Model setup
        self.status_var.set("Phase 3/3: Setting up AI mentor model...")
        ...
        if self.model_path.exists() and self.model_path.stat().st_size > 100_000_000:
            self.status_var.set("AI Mentor Sandra found! Initializing platform...")
            self.progress_bar['value'] = 100
            self.details_var.set("✅ Model loaded • ✅ Translation ready • ✅ TTS available")
            time.sleep(2)
            self.setup_window.after(0, self._finalize, str(self.model_path))
            return

        # Download model
        self.status_var.set("Downloading AI mentor model...")
        self.details_var.set("Enhanced with offline translation - No internet needed after setup!")
        ...
        try:
            with urllib.request.urlopen(self.model_url) as response, open(self.model_path, 'wb') as out_file:
                ...
                while True:
                    ...
                    self.status_var.set(f"Downloading AI mentor... {mb_downloaded:.1f} / {mb_total:.1f} MB")
                    self.details_var.set("🌍 Offline translation • 🔒 Private • 🚀 Fast neural models")
            
            self.status_var.set("Setup complete! Launching enhanced platform...")
            self.details_var.set("✅ AI mentor ready • ✅ Translation system active • ✅ Privacy focused")
            time.sleep(2)
            self.setup_window.after(0, self._finalize, str(self.model_path))
            
        except Exception as e:
            self.status_var.set("Download failed")
            self.details_var.set(f"Error: {str(e)}")
            time.sleep(3)
            self.setup_window.after(0, self._finalize, None)
