...
class SpectrumAnnotator:
    ...
    def plot(self, spectrum, peptidoform, charge, xmin=None, xmax=None, mask_isolation_width=None, ymax=None, write_files=None):
        ...
        figure_height = 6
        figure_width = 8
        spectrum_viewport = [0.02, 0.2, 1.01, 1.00]
        residuals_viewport = [0.02, -0.04, 1.01, 0.21]
        if include_third_plot:
            figure_height = 7.5
            spectrum_viewport = [0.02, 0.28, 1.01, 1.00]
            residuals_viewport = [0.02, 0.10, 1.01, 0.28]
            third_plot_viewport = [0.02, -0.03, 1.01, 0.15]
        if include_coverage_graphic:
            figure_width = 9
            spectrum_viewport = [0.02, 0.2, 0.9, 1.00]
            residuals_viewport = [0.02, -0.04, 0.9, 0.21]
            coverage_viewport = [0.83, 0.2, 1.02, 1.00]
        ...
        #### Shows the USI if available
        if show_usi is True:
            if 'usi' in spectrum.attributes and spectrum.attributes['usi'] is not None:
                usi_length = len('USI: ' + str(spectrum.attributes['usi']))
                usi_fontsize = int(usi_length / 9)
                plot1.text(xmin - 0.07 * (xmax-xmin), ymax * 1.0, 'USI: ' + str(spectrum.attributes['usi']), fontname=fontname, fontsize=usi_fontsize, ha='left', va='bottom')
        ...
        if include_coverage_graphic:
            ...
            for residue in peptidoform.residues:
                ...
                print(i_residue,base_residue)
                x_off = 1 + 0.5
                x_sz = 0.5
                y_off = 1.0 - (i_residue - 0.5) * y_coverage_scale
                y_sz = y_coverage_scale / 2.0
                plot4.text(x_off - 0.25, y_off - 0.05 * y_coverage_scale, base_residue, fontname=fontname, fontsize=9, ha='center', va='center')
                ...
                series_attributes = { 'b': { 'color': 'tab:blue', 'offset': -1, 'direction': 1 }, 'y': { 'color': 'tab:red', 'offset': 1, 'direction': -1 } }
                for series in [ 'b', 'y' ]:
                    x_off = 1 + series_attributes[series]['offset'] + 0.5
                    x_sz = 0.5
                    y_off = 1.0 - (i_residue - 0.5) * y_coverage_scale
                    if series_attributes[series]['direction'] == -1:
                        y_off = 1.0 - ( len(peptidoform.residues) -1 ) * y_coverage_scale + (i_residue - 0.5) * y_coverage_scale
                    color = series_attributes[series]['color']
                    if f"{series}{i_residue}" in annotations_dict:
                        rect = patches.Rectangle((x_off-x_sz, y_off-0.4*y_sz), 2*0.8*x_sz, 2*0.7*y_sz, linewidth=0.5, edgecolor=color, facecolor=color)
                        plot4.add_patch(rect)
                    if f"{series}{i_residue}-H2O" in annotations_dict:
                        rect = patches.Rectangle((x_off-x_sz, y_off-y_sz), 2*0.8*x_sz, 0.6*y_sz, linewidth=0.5, edgecolor=color, facecolor=color)
                        plot4.add_patch(rect)
                    if f"{series}{i_residue}-NH3" in annotations_dict:
                        rect = patches.Rectangle((x_off+0.6*x_sz, y_off-0.4*y_sz), 2*0.2*x_sz, 2*0.7*y_sz, linewidth=0.5, edgecolor=color, facecolor=color)
                        plot4.add_patch(rect)
                    if f"{series}{i_residue}-" in annotations_dict:
                        rect = patches.Rectangle((x_off+0.6*x_sz, y_off-y_sz), 2*0.2*x_sz, 0.6*y_sz, linewidth=0.5, edgecolor=color, facecolor=color)
                        plot4.add_patch(rect)
                    plot4.text(x_off - 0.1, y_off + 0.1 * y_coverage_scale, f"{series}{i_residue}", fontname=fontname, fontsize=8, ha='center', va='center')
                    plot4.plot([x_off-x_sz, x_off-x_sz, x_off+x_sz, x_off+x_sz, x_off-x_sz], [y_off+y_sz, y_off-y_sz, y_off-y_sz, y_off+y_sz, y_off+y_sz], color='k', linewidth=0.5)
                    plot4.plot([x_off-x_sz, x_off+x_sz], [y_off-0.4*y_sz, y_off-0.4*y_sz], color='k', linewidth=0.5)
