...
def main():
    ...
    parser.add_argument(
        "--profile",
        help="Path to yaml file with snakemake profile settings (default: None)",
        default=None,
    )
    parser.add_argument(
        "--make_json_reports",
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"eDentity Metabarcoding Pipeline: {pkg_version}",
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Dry run mode (equivalent to -n in snakemake)",
    )
