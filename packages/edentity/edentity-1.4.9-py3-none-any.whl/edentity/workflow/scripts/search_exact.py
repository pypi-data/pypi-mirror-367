...
try:
    ...
    sample_esv_df = pd.read_csv(snakemake.output.ESV_table_per_sample, sep="\t")
    ...
    if sample_esv_df.shape[0] > 0:  # check if there are ESVs in the sample
        for record in SeqIO.parse(snakemake.input.ESV_fasta, "fasta"):
            sample_esv_df.loc[sample_esv_df["#OTU ID"] == record.id, "sequence"] = str(
                record.seq
            )
    ...
    sample_esv_df.to_csv(snakemake.output.ESV_table_per_sample, sep="\t", index=False)
