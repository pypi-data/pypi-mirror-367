...
try:
    ...
    report_df = pd.read_csv(snakemake.input.denoise_report, sep="\t")
    report_df["chimeric"] = chimeric_match
    report_df["borderline"] = borderline_match  # also filtered out
    report_df.loc[report_df["Sample"] == sample_name, "ESVs"] = n_esv
    report_df.to_csv(snakemake.output.summary_report, sep="\t", index=False)
