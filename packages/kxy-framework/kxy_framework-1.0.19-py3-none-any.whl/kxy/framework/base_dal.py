...
from .context import user_id,user_info,tenant_id,dep_id
...
class BaseDal(Generic[T]):
    ...
    __isDelete_field = 'IsDelete'
    __createUser_field = 'CreateUser'
    __createDate_field = 'CreateDate'
    __lastModifiedUser_Field = 'LastModifiedUser'
    __lastModifiedDate_Field = 'LastModifiedDate'
    ...
    def _add_extend_filter(self, filters):
        ...
        # 检查模型类是否有 tenant_field 属性（由 TenantData 注解添加）
        if hasattr(self.modal, 'tenant_field'):
            # 获取租户字段名称
            tenant_field_name = self.modal.tenant_field
            # 检查模型实例是否有对应的租户字段
            if hasattr(self.modal, tenant_field_name):
                # 获取租户字段值（这里假设从某个地方获取租户ID，例如从上下文或会话中）
                # 你可能需要根据实际情况调整 tenant_id 的获取方式
                tenant_ids = tenant_id.get()
                if tenant_ids:
                    # 构造租户过滤条件
                    tenant_filter = getattr(self.modal, tenant_field_name).in_(tenant_ids)
                    filters_copy.append(tenant_filter)
        if hasattr(self.modal, 'dep_field'):
            dep_field_name = self.modal.dep_field
            if hasattr(self.modal, tenant_field_name):
                dep_ids = dep_id.get()
                if dep_ids:
                    # 构造部门过滤条件
                    dep_filter = getattr(self.modal, dep_field_name).in_(dep_ids)
    ...
    async def Insert(self, entity):
        setattr(entity,self.__createUser_field,self.UserId)
        setattr(entity,self.__createDate_field,datetime.now())
        setattr(entity,self.__lastModifiedUser_Field,self.UserId)
        setattr(entity,self.__lastModifiedDate_Field,datetime.now())
    ...
    async def Delete(self, id):
        ...
        setattr(entity,self.__isDelete_field,1)
        self.AddLogger('逻辑删除',f'删除id为{id}的数据')
        await self.Update(entity)
    async def DeleteBatch(self,ids,status=10):
        if len(ids):
            fil=[getattr(self.modal,self.__id_field).in_(status)]
            self.AddLogger('批量逻辑删除',f'删除id为{ids}的数据')
            await self.UpdateFields(fil,{self.__status_field: status})

    async def TrueDel(self, id):
        self.AddLogger('物理删除',f'删除id为{id}的数据')
        if self.AutoCommit:
            fil = [getattr(self.modal,self.__id_field) == str(id)]
    ...
    async def TrueDelBatch(self,ids):
        if len(ids):
            ...
            if self.AutoCommit:
                fil = [getattr(self.modal,self.__id_field).in_(ids)]
                fil = self._add_extend_filter(fil)
    ...
    async def GetExistByUser(self, id)->T:
        '''获取id和uid为当前用户数据'''
        exist = await self.session.execute(select(self.modal).filter(getattr(self.modal,self.__id_field) == str(id),getattr(self.modal,self.__uid_field) == self.UserId).limit(1))
        result = exist.scalar()
        await self.IfCommit()
        if result is None:
            raise FriendlyException('不存在' + str(id))
    ...
    async def GetByUser(self):
        return await self.QueryOne([getattr(self.modal,self.__uid_field)==self.UserId])
    ...
    def AddLogger(self, action, data:Union[TypeList[T],T,str]):
