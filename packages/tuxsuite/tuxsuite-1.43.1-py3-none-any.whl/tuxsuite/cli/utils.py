...
def load_build_definition(definition_file):
    def deprecated_warning():
        print(
            "⚠️ WARNING: Support for build definition files in JSON format is deprecated and will be removed in release v1.44.0 or not later than a release on 6th August, 2025. Please migrate to the YAML format."
        )
    ...
    try:
        ...
        if urlparse(str(definition_file)).scheme in ["http", "https"]:
            ...
            if is_json(Path(definition_file).read_text()):
                deprecated_warning()
