...
def spawn_adapter_server(
    ...
    if wait_for_server(
        AdapterServer.DEFAULT_ADAPTER_HOST, AdapterServer.DEFAULT_ADAPTER_PORT
    ):
        logger.info(
            f"Adapter server started on {AdapterServer.DEFAULT_ADAPTER_HOST}:{AdapterServer.DEFAULT_ADAPTER_PORT}"
        )
        return process

    logger.error(
        f"Adapter server failed to start on {AdapterServer.DEFAULT_ADAPTER_HOST}:{AdapterServer.DEFAULT_ADAPTER_PORT}"
    )
    process.terminate()
...
class AdapterServerProcess:
    ...
    def __enter__(self):
        # FIXME(tgrzegorzek): AdapterConfig should be always validated. This classmethod should be default
        # In an ideal world, adapter_config should be just another field we access. Probably should go within validation
        adapter_config: AdapterConfig | None = AdapterConfig.get_validated_config(
            self.evaluation.model_dump()
        )
        if not adapter_config:
            return
        self.evaluation.target.api_endpoint.url = f"http://{AdapterServer.DEFAULT_ADAPTER_HOST}:{AdapterServer.DEFAULT_ADAPTER_PORT}"
