...
# TODO(tgrzegorzek) improve typing
def _get_framework_evaluations(
    def_file: str,
) -> tuple[dict[str, dict[str, Evaluation]], dict[str, dict], dict[str, Evaluation]]:
    # TODO(martas) needs cleanup
    # TODO(tgrzegorzek): we should decide if this should raise at this point.
    ...
    logger.debug(f"Loading task definitions", filepath=def_file)
    (
        framework_name,
        framework_defaults,
...
def check_required_default_missing(run_config: dict):
    # TODO(tgrzegorzek): this should be refactored into something more modular
...
def get_evaluation(evaluation_config: EvaluationConfig, target_config: EvaluationTarget) -> Evaluation:  # type: ignore
    ...
    # FIXME(tgrzegorzek): there is a miszmasz here when it comes to serializing and deserializing dataclasses
    # (basically going back and forth between dump and object).
