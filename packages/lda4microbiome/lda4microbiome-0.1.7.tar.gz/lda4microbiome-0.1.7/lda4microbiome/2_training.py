...
class MALLETStyleMetricsCalculator:
    ...
    def calculate_mallet_style_perplexity(self, 
        ...
        perplexity = math.exp(-total_log_likelihood)
        return perplexity
    
    def _calculate_document_likelihood(self, 
                                     doc_topics: np.ndarray, 
                                     doc_length: int, 
                                     num_particles: int,
                                     word_topic_probs: Optional[np.ndarray] = None) -> float:
        """
        Calculate document likelihood using MALLET's left-to-right particle filtering.
        
        This implements a more accurate approximation of MALLET's particle filtering
        approach for estimating document likelihoods using left-to-right evaluation.
        
        Args:
            doc_topics: Document-topic distribution
            doc_length: Length of the document
            num_particles: Number of particles for sampling
            word_topic_probs: Optional word-topic probability matrix (V x K)
        
        Returns:
            Log likelihood of the document
        """
        particles_log_likelihood = []
        
        # If word_topic_probs not provided, use uniform approximation
        if word_topic_probs is None:
            vocab_size = 1000  # Approximate vocabulary size
            word_topic_probs = np.ones((vocab_size, len(doc_topics))) / vocab_size
        
        for particle_idx in range(num_particles):
            particle_ll = 0.0
            
            # Left-to-right evaluation: for each word position
            for pos in range(doc_length):
                # Calculate probability of current word given previous context
                # In MALLET's left-to-right, this involves marginalizing over topic assignments
                
                position_likelihood = 0.0
                
                # For each possible topic for current position
                for topic_k in range(len(doc_topics)):
                    # Probability of assigning topic k to current position
                    topic_prob = doc_topics[topic_k]
                    
                    # Simulated word probability given topic (using position-dependent sampling)
                    # This approximates the conditional probability P(w_i | w_1...w_{i-1}, topics)
                    word_idx = (pos + particle_idx) % word_topic_probs.shape[0]
                    word_given_topic_prob = word_topic_probs[word_idx, topic_k]
                    
                    # Add contribution of this topic to position likelihood
                    position_likelihood += topic_prob * word_given_topic_prob
                
                # Add log probability for this position
                particle_ll += math.log(max(position_likelihood, 1e-15))
            
            particles_log_likelihood.append(particle_ll)
        
        # Compute log of average likelihood across particles (more numerically stable)
        if not particles_log_likelihood:
            return -float('inf')
            
        max_ll = max(particles_log_likelihood)
        if max_ll == -float('inf'):
            return -float('inf')
            
        # Use log-sum-exp trick for numerical stability
        log_mean_likelihood = max_ll + math.log(
            np.mean([math.exp(ll - max_ll) for ll in particles_log_likelihood])
        )
