...
def get_quiz_files() -> Generator[Path, None, None]:
    ...
    """
    # Look for YAML files in 'yaml' subdirectory or at root
    yaml_dir = QUESTION_DATA_DIR / "yaml"
    if yaml_dir.is_dir():
        yield from yaml_dir.glob("*.yaml")
    else:
        yield from QUESTION_DATA_DIR.glob("*.yaml")

    # Look for JSON files in 'json' subdirectory or at root
    json_dir = QUESTION_DATA_DIR / "json"
    if json_dir.is_dir():
        yield from json_dir.glob("*.json")
    else:
        yield from QUESTION_DATA_DIR.glob("*.json")
...
def extract_links_from_file(file_path: Path) -> Set[str]:
    ...
    try:
        ...
        return set()
    # Extract all URLs anywhere in the file
