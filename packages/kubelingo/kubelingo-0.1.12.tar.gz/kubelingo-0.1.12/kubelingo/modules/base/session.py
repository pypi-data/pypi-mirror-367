...
class SessionManager:
    ...
    def _update_review_status(self, question_id: str, review: bool):
        ...
        # Parse module and question id
        try:
            module, qid = question_id.split('::', 1)
        except (ValueError, IndexError):
            self.logger.error(f"Invalid question ID format for review flagging: {question_id}")
            return
        # Locate the source YAML file
        ...
        if not os.path.exists(data_file):
            ...
            return
        # Load the YAML content (could be a dict with 'questions' or a list)
        try:
        ...
        if isinstance(content, dict) and 'questions' in content and isinstance(content['questions'], list):
            ...
            self.logger.error(f"Invalid format in {data_file}: expected a list or dict of questions.")
            return
        # Locate and update review flag
