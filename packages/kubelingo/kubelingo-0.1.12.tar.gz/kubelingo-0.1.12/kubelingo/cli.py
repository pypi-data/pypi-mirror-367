...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        # Handle --k8s shortcut
        if args.k8s_mode:
            # Shortcut for Kubernetes exercises
            # Non-interactive invocation: delegate to Rust bridge or fallback to Python loader
            if args.num and args.num > 0:
                # Delegate non-interactive quiz to Rust bridge or fallback
                try:
                    ...
                    if rust_bridge.is_available():
                        ...
                        if not success:
                            ...
                            # Fallback to Python loader
                            from kubelingo.modules.kubernetes.session import load_questions
                            load_questions(args.file)
                        return
                    else:
                        # Rust bridge not available, fallback to Python loader
                        from kubelingo.modules.kubernetes.session import load_questions
                        load_questions(args.file)
                        return
                except ImportError:
                    # Rust bridge import failed, fallback to Python loader
                    from kubelingo.modules.kubernetes.session import load_questions
                    load_questions(args.file)
                    return
            # Interactive menu for Kubernetes module
            args.module = 'kubernetes'
