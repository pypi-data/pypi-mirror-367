...
from typing import Any, Callable, Iterable, NamedTuple, Sequence, TypedDict, TypeVar, cast
        ...
        with self._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            try:
                ...
                if bot.lease_id is None:
                    ...
                    elif session_lease.state == LeaseState.COMPLETED.value:
                        LOGGER.debug("No lease in database, but session lease is completed. Skipping.", tags=log_tags)
                        valid = True

                    if valid:
                        # Attempt to proactively fetch a new job for the bot.
                        # None is returned if no job is available.
                        return self._fetch_job_for_bot(session, bot, log_tags)
                    else:
                ...
                if session_lease.state == LeaseState.COMPLETED.value and job.lease_state() in (
                    ...
                    if (
                        ...
                        self._complete_job(session, job, session_lease.status, bot_name, session_lease.result)

                    self._notify_job_updated(job.name, session)

                    if next_lease := self._fetch_job_for_bot(session, bot, log_tags):
                        return next_lease
                    return None
