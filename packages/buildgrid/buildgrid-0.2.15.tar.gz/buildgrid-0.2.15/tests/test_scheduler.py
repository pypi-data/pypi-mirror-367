...
def test_compete_job_with_locality_hint(controller, controller_postgres):
...
@pytest.mark.parametrize("controller", ["action-cache"], indirect=True)
def test_compete_job_with_locality_hints_limit_size(controller, controller_postgres):
    """Test that jobs with locality hint can be competed."""
...
def test_scheduling_match_by_locality(controller_postgres):
    ...
    lease1 = scheduler.synchronize_bot_lease(bot_name, bot_id, BotStatus.OK.value, None)
    assert lease1 is None
    lease2 = scheduler.synchronize_bot_lease(bot_name2, bot_id2, BotStatus.OK.value, None)
