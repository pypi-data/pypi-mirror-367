...
DEFAULT_IGNORED_FILES: Set[str] = {'.env', '.env.example'}
...
def setup_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=".",  # Sets the default value to the current directory
...
def load_config(root_path: str) -> Dict[str, Any]:
    """Loads configuration from a project-specific file, merging with defaults."""
    config = {
    ...
    if os.path.isfile(config_path):
        logging.info(
            f"Loading project-specific configuration from: {CONFIG_FILENAME}")
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                user_config = json.load(f)
            config["allowed_dirs"] = user_config.get(
                "allowed_dirs", DEFAULT_ALLOWED_DIRS)
            config["ignored_files"].update(
                user_config.get("ignored_files", []))
            config["ignored_extensions"].update(
                user_config.get("ignored_extensions", []))
            config["ignored_dirs"].update(user_config.get("ignored_dirs", []))
        except json.JSONDecodeError as e:
            logging.error(
                f"Syntax error in '{CONFIG_FILENAME}': {e}. Please check the JSON format.")
            raise  # Stop execution if config is malformed
        ...
        logging.info(
            f"No '{CONFIG_FILENAME}' file found. Using default ignore lists.")
...
def discover_files(root_path: str, config: Dict[str, Any], spec: Optional[pathspec.PathSpec]) -> List[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root_path, topdown=True):
        ...
        if spec:
            dirs_to_remove = {d for d in dirnames if spec.match_file(os.path.relpath(
                os.path.join(dirpath, d), root_path).replace(os.path.sep, '/'))}
...
def main():
    ...
    try:
        ...
        files_to_process = discover_files(
            root_directory, config, gitignore_spec)
        ...
        with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
            ...
            for full_path in tqdm(files_to_process, desc="Writing context file", unit="file"):
                try:
                    ...
                    relative_path = os.path.relpath(
                        full_path, root_directory).replace(os.path.sep, '/')
                    ...
                    logging.warning(
                        f"Could not read or process file {full_path}: {e}")
